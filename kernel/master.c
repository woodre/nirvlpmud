 ...
 * at the top of this file, the current master is used for Amylaar, and
 * the code below the else is used on LD.  This should make the switch
 * easier, as we can just remove the top portion of the code when done
...
#include "/sys/prop/player.h"
#include "/sys/error_messages.h"
#include "/kernel/defs.h"
#include "/sys/files.h"
#include "/sys/libs.h"

#ifdef NEW_LIGHT
inherit "/kernel/support/light";
#endif
inherit "/kernel/simul/for_master";
inherit "/kernel/support/path";
#if 0
inherit "/kernel/support/ftpaccess";
#endif

#define INIT_FILE  "/room/init_file_ld"  /* Change when switch is complete */
...
#define AUTO_INCLUDE_STRING "#include <global.h>\n"

void save_wiz_file();  // forward
void query_player_level (string what); // forward
...
#  define SIMUL_EFUN_FILE SIMUL_EFUN
#endif

#ifndef SPARE_SIMUL_EFUN_FILE
#  define SPARE_SIMUL_EFUN_FILE SPARE_SIMUL_EFUN
#endif

#ifndef OBSOLETE_SIMUL_EFUN_FILE
#  define OBSOLETE_SIMUL_EFUN_FILE OBSOLETE_SIMUL_EFUN
#endif
...
mixed get_simul_efun()
{
  ...
  if (error = catch(efun::call_other(fname, "-"))) {
                                             // error while loading simul_efun!
    write("Failed to load " + fname + "\n"
          "error message :"+error+"\n");
    fname = SPARE_SIMUL_EFUN_FILE;
    if (error = catch(efun::call_other(fname, "-"))) {
                              // also error while loading spare_simul_efun!
      write("Failed to load " + fname + "\n"
            "error message :"+error+"\n");
      return 0;
    }
  }
  // simul_efun is now loaded and working.
  funcall(bind_lambda(#'efun::call_other, find_object(fname)), fname,
          "start_simul_efun");
  if (error = catch(load_object(SPARE_SIMUL_EFUN_FILE)))
    write("Managed to load " + fname + " but could not load " +
          SPARE_SIMUL_EFUN_FILE + "\n"
          "error message :"+error+"\n"
          "Please try to make " + SPARE_SIMUL_EFUN_FILE + " loadable!\n");
  return ({ fname, OBSOLETE_SIMUL_EFUN_FILE });
#else
  mixed error;
  object ob;
...

{
  ...
  if (curobj == object_name(this_object()) && err[0..5] == "Time: ")
    return; /* Do not give out the self-created error which logs the time. */
  if (!(ob = this_player()) || !interactive(this_player()))
    ob = this_interactive();
  if (ob && this_player()->query_level() > 20) {
    if (curobj)
      catch(tell_object(ob, err + "program: " + prg + ", object: " + curobj +
                        " line " + line + "\n"));
    else
      catch(tell_object(ob, err));
  }
  else {
 ...
 *  -1: A real privilege violation, handle it as an error.
 */
int privilege_violation(string what, mixed who, mixed arg, mixed arg2)
{
#if 1
  switch (what) {
  case "bind_lambda":
  ...
  case "nomask simul_efun":
    if (objectp(who)) // symbol_function("efun::xxx") is at runtime
      who = object_name(who);
    if (who[0..6] == "kernel/" && who[0..9] == "obj/master" &&
        who[0..13] == "obj/simul_efun")
      return 1;
    return -1;
  case "wizlist_info":
  case "set_extra_wizinfo":
  case "get_extra_wizinfo":
    switch(object_name(who)) {
#ifdef BOOT_TIME_WIZINFO
    case "kernel/master":
    case "obj/master":
#endif
    case "global/wizlist":
...
#endif
  case "send_udp":
    if (object_name(who) != "etc/inetd" && !check_privilege(1))
      return -1;
    return 1;
  case "call_out_info":
    return 1;
  ...
  case "input_to":
    if (check_privilege(1))
      return 1;
    if (SECURITY->query_protection(who, 1))
      return 1;
    return -1;
  case "erq":
    if (load_name(who) == "etc/intermud3")
      return 1;
    if (load_name(who) == "bin/admin")
      return 1;
    if (load_name(who) == "etc/dnews")
      return 1;
    return -1;
  case "limited":
    if (SECURITY->query_protection(who, 1))
    ...
    return -1;
  case "set_limits":
    if (load_name(who) == LIB_FILE)
      return 1;
    return -1;
#if 0  // The following are not being used in Nirvana, so just comment them
       // out from the function - Rumplemintz
...

{
    if (object_info(victim, OINFO_MEMORY)[OIM_NO_SHADOW])
        return 0;
#if 1 /* Rumplemintz */
  switch ((explode(load_name(victim), "/") - ({ "." }))[0]) {
  case "kernel":
    return 0;
  case "bin":
  case "global":
  ...
  case "sys":
    break;
  default:
    if (victim->query_prevent_shadow(previous_object()))
      break;
    return 1;
  }
  if (get_protection(object_name(previous_object()),"wf") != 1)
    return 0;
  if (efun::call_other(SECURITY,"check_privilege",1,2))
    return 1;
  return 0;
#else
...
// TODO: Use the second argt

int valid_trace(string kind, mixed arg)
{
#if 1
  object ob;
  if (!(ob = this_player()) || !interactive(this_player()))
    ob = this_interactive();
  if (ob && (SECURITY->query_is_wizard(ob->query_real_name()) ||
             ob->query_property(P_TESTPLAYER))) {
    switch (kind) {
      case "trace":
      case "traceprefix":
        return SECURITY->check_privilege(1);
    }
#else
    int level;
...
#if 0 /* Rumplemintz - Changing to below */
//---------------------------------------------------------------------------
...
mixed valid_read(string path, string eff_user, string call_fun, object caller)
{
  ...
  if (object_name(caller) == SECURITY && call_fun == "restore_object")
    return 1;
  path = normalize_path(path, this_interactive());

  if (call_fun == "ed_start") {
    if (this_player() && query_once_interactive(this_player())) {
      if (path == "/save/edit/" + this_player()->query_real_name())
        return path;
      if (!(priv = get_privilege(this_player())) ||
          (stringp(priv) && priv[0..0] == "#"))
        return 0;
    }
  }

  if (call_fun == "get_dir" && path[<1] == '/') {
    path += "*";
    path_extended = 1;
  }

  if (!caller &&
      (call_fun == "read_file" || call_fun == "read_bytes")) {
    mixed priv_needed;
    priv = SECURITY->query_is_admin(eff_user) ? 1 : eff_user;
    priv_needed = SECURITY->query_protection(path, 0);
    return SECURITY->higher_privilege(eff_user, priv_needed) && path;
  }

  if(SECURITY->check_privilege(SECURITY->query_protection(path, 0), 0))
    return path;
  printf("Bad file name: %s.\n", path_extended ? path[0..<2] : path);
  return 0;
}

mixed valid_write(mixed path, string eff_user, string call_fun, object caller)
{
  mixed priv, priv_needed;
  int path_extended;
  ...
  if (!caller && (call_fun == "write_file" || call_fun == "write_bytes")) {
    priv_needed = SECURITY->query_protection(path, 1);
    return SECURITY->higher_privilege(eff_user, priv_needed) && path;
  }

  if (eff_user && !interactive(caller) && call_fun == "save_object") {
    priv_needed = SECURITY->query_protection(path, 1);
    return SECURITY->higher_privilege(eff_user, priv_needed) && path;
  }

  if (SECURITY->check_privilege(SECURITY->query_protection(path,1), 1))
    return path;
  printf("Bad file name: %s.\n", path_extended ? path[0..<2] : path);
  return 0;
}
...
int save_ed_setup(object wiz, int setup) {
  ...
  if (!intp(setup))
    return 0;
  if (!SECURITY->query_is_wizard(wiz->query_real_name()))
    return 0;
  file = "/players/" + wiz->query_real_name() + "/.edrc";
  rm(file);
...
#if 0 /* Taking out this stuff - Rumplemintz */
//---------------------------------------------------------------------------
