...
#if 0
  if(!str) return 0;
  log_file("LDBUG", sprintf("%s%s %-11s: %s\n%s", HIR, ctime(), capitalize((string)this_player()->query_real_name()), object_name(environment(this_player())), NORM));
  log_file("LDBUG", sprintf("%=-78s\n", str));
  write("Completed bug entry.  Thanks!\n");
  return 1;
#endif
