...
string msghome;
string description;
#if 0 /* Moved to obj/guild.c - Rumplemintz */
string guild_name;
#endif
string clan_name;
string clan_master;
...
string pwd;
status errormsgs;
#if 0 /* Moved go obj/guild.c - Rumplemintz */
...
reset(arg) {
  if (!arg) {
    ...
    if( !prestige_guild )
      prestige_guild = allocate( sizeof( VALID_PRES_GUILD ) );
      
    /** Resize arrays in case more are added later **/
    if( sizeof( prestige_skills ) < sizeof( VALID_PRES_SKILL ) ) 
      prestige_skills += allocate( sizeof( VALID_PRES_SKILL ) - sizeof( prestige_skills ) );
      
    if( sizeof( prestige_guild ) < sizeof( VALID_PRES_GUILD ) ) 
      prestige_guild += allocate( sizeof( VALID_PRES_GUILD ) - sizeof( prestige_guild ) );
...
heart_beat() {
  ...
  if (is_interactive) 
  {
    ...
    if( PRES_SKILL_DETOX > random( PRES_SKILL_DETOX + 4 ) ) {
      ...
      if (stuffed > 0) stuffed -= det_amt;
      if (soaked > 0)  soaked  -= det_amt;
      infuse = (infuse > 0 ? infuse - det_amt : 0);
...
add_exp(x) {
  ...
  if (this_player() && previous_object() && this_player() != this_object() && this_player()->query_level() > 1 && query_ip_number(this_player()) && level < 20 ) {
    ...
    if(tmpo != "obj/party" && object_name(previous_object()) != "obj/partymaster") {
  ...
  if (is_interactive) {
    ...
    if( is_testchar() ) {
      if( previous_object()&& previous_object()->is_npc() ) 
        set_big_kill( e, previous_object() );
    }
...
#if 1 /* Rumplemintz */
    if(num==4 && arg == -9) log_file("b_amulet","-9 added "+object_name(caller())+"\n");
#else
  if (num == 4 && arg == -9)
    log_file("b_amulet", "-9 added " + object_name(previous_object()) + "\n");
#endif
    phys_at[num] += arg;
    return 1;
}
...
set_prestige_guild( string attr, mixed value ) {
  ...
  prestige_guild[idx] = value;
}

clear_prestige_guild() {
  int i;

  for( i=0; i < sizeof(prestige_guild); i++ )
    prestige_guild[i] = 0;
}
...
restore_prestige_guild() {
  ...
  guild_file = PRES_GUILD_FILE;
  guild_name = PRES_GUILD_NAME;
  guild_rank = PRES_GUILD_RANK;
  guild_exp = PRES_GUILD_EXP;
  home = PRES_GUILD_HOME;
  
  /** Clone guild object **/
  gob = clone_object( query_guild_file() );
  if (gob) {
...
mixed *query_big_kill() { return big_kill; }

mixed *set_big_kill( int exp, object corpse ) {
  object rm;
  
  if( corpse )
    rm = environment( corpse );
    
  if( !big_kill || big_kill[0] < exp ) {
    tell_object( this_object(), RED+"You just beat your top kill!\n"+NORM);
    big_kill = ({ exp, corpse ? (string) corpse->query_name() : "Unknown", rm ? (string) rm->short() : "Unknown" });
  }
