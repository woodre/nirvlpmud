 ...
 *  3/02/2014: Gnar        - Added ability to use colors, fixed a bug that
 *                           occured when emotes had a newline in them.
 */
#pragma no_clone
#pragma strict_types
...
/* Added silently - Rumplemintz */
void  remove_channel(string channel_name, object player, int silently);
void  add_channel(string channel_name, object player, int silently);
...
query_channel(string channel_name, object player)
{
  ...
  /** Added in case no one is on the channel **/
  return 0;
...
varargs void /* Added vara4rgs and silently 20140324 - Rumplemintz */
add_channel(string channel_name, object player, int silently)
{
  object *players;
  int i;
...
/*         player->add_channel_name(channel_name); */
    ...
    else if (!silently) /* Rumplemintz - Added silently */
...
varargs void /* Added varargs and silently 20140324 - Rumplemintz */
remove_channel(string channel_name, object player, int silently)
{
  object *players;
  if(ChannelsMap == 0){ ChannelsMap = ([ ]); }
  ...
  if(players = ChannelsMap[channel_name])
  {
#if 1 /* Rumplemintz - rewrote this slightly */
    if (member(players, player) == -1) {
      if (!silently)
...
channel_message(string channel, string msg, string prefix)
{
  ...
  if( this_player() && !this_player()->setup_broadcast(channel))
    return 1;
...
build_msg(string channel, string msg, string prefix) {
  ...
  if( member( WIZ_CHANNELS, channel ) < 0 || !this_player()->query_invis() ) {
    ...
    msg = "(Invis) " + capitalize((string)this_player()->query_real_name()) + " " +
            channel + "s: " + msg + "\n";
  }
...
transmit_message( string channel, string msg, object *exclude_objs ) {
  ...
  msg = (string ) COLOR_PARSER->parse_pcodes(msg);
...
query_player_channels( object target ) {
  ...
  size = sizeof(ALL_CHANNELS);
  for( i=0; i < size; i++ ) {
    write("    " + pad(ALL_CHANNELS[i],8) + " -- " + (member( on_channel( ALL_CHANNELS[i] ), target ) > -1 ? "ON" : "OFF") + "\n" );
  }
  if( !this_player() || this_player()->query_level() < EXPLORE)
    return 1;
...
valid_channel(string channel) {
  if (member(ALL_CHANNELS, channel) != -1)
    return 1;
  if (member(WIZ_CHANNELS, channel) != -1)
    /* Only wizards have access to these channels. */
    return ( this_player() && this_player()->query_level() >= EXPLORE );
  if(channel == "admin") return (this_player() && this_player()->query_level() > SENIOR );
  return 0;
}
