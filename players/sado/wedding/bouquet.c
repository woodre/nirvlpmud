...
do_catch(p)
{
   ...
   j = sizeof(u);
   t = allocate(j);
   for(i=0; i<j; i++) if(u[i]->is_player() &&
                         u[i]->query_gender() == "female" &&
                         !present("wedding ring",u[i])) t[n++] = u[i];
   if(n == 0)
   {
      tell_room(env(p),"Doh!  There are no single women present!  The bouquet lands on the ground\n");
      move_object(this_object(),env(p));
      return 1;
   }
   i = random(n);
