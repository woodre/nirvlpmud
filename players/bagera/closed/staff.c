...
#define Call(XXX,YYY,ZZZ) call_other(XXX,YYY,ZZZ);
 ...
 short() {return "A newbie wiz staff";}
long() {write("All a new wiz's basic needs.\n");}
...
init(){
   ...
   add_action("echo","ec");
   add_action("echo_all","ea");
   add_action("trans","tr");
   add_action("move_into","Mv");
   add_action("Get","G");
   add_action("goin","goin");
}
staff_help(){
 cat("/open/newbie/staff_help");
   return 1;}
echo (str){
...
trans(arg){
   ...
   write("You trans "+arg+".\n");
 tell_object(plyr,"You feel ripped from where you once were to");
 tell_object(plyr,"someplace else.\n");
   write("Ok.\n");
   return 1;
}
move_into(str){
