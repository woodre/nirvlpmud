reset(arg) {
    ...
    query_auto_load() {
    return "/players/dragnar/magic.c:";
}

long() {
	write("This is the toy of dragnar.\n");
        return 1;
}
...
who = find_living(str);
	...
	tell_object(who,"Dragnar summons you to him.\n");
	move_object(who, environment(this_player()));
	say("Dragnar has summoned " + capitalize(str) + ".\n");
	command("look",who);
...
obj = present(what);
	...
	tell_object(plyr,capitalize(what) + " is sent to you by Dragnar.\n");
