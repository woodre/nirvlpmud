...
long() {
    write("This is Humble's Note Board.\n");
    write("Read a note with 'read num', and remove an old note with 'remove num'.\n");
    if (num_messages == 0) {
	write("It is empty.\n");
	return;
    }
...
reset(arg) {
    if (arg)
	return;
    restore_object("players/humble/notice");
}
...
get_body(str) {
	...
	new_body = 0;
	new_hd = 0;
      save_object("players/humble/notice");
	write("Ok.\n");
	who = 0;
	return;
    }
...
remove(str) {
  ...
  if(this_player()->query_level() < 21) {
    write_file("/players/humble/BOARD",
