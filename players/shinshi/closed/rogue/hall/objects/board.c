...
id(str) {
    return str == "bulletin board" || str == "board" || str == "rogueboard";
}

long() {
  write("This is a board for any player of the Rogue guild..\n");
...
short() {
    return ("Rogue bulletin board(" + num_messages + " msgs)");
}
...
reset(arg) {
    if (arg)
	return;
    restore_object("players/shinshi/closed/rogue/hall/objects/board");
}
...
get_body(str) {
	...
	new_body = 0;
	new_hd = 0;
      save_object(HALLOBJ+"board");
	write("Ok.\n");
	"/bin/channel_daemon.c"->transmit_message("Rogues", RED+"(XX "+WHT+"Rogue"+RED+" XX)"+WHT+" "+OFF+OFF+HIR+" A new note has been posted on the Guild Board by "+this_player()->query_name()+"!\n\n"+NORM);
...
remove(str) {
	...
	if (i == 0) {
        if(!sscanf(hd, "%s" + (string)this_player()->query_real_name(), jun) && ((int)this_player()->query_level() < 60))
        {
