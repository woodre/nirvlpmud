...
{
   int value;
   value = call_other(ob, "query_value", 2);
   if(!value)
...
value(item) {
   ...
   value = call_other(name_of_item, "query_value", 2);
   if(!value)
...
buy(item) {
   ...
   if (!name_of_item && sscanf(item, "%d", new) == 1) {
      sscanf(item, "%d", new);
      if(environment(list_no[new]) == this_object())
...
south() {
   call_other(this_player(), "move_player", "leaves#/players/shinshi/closed/rogue/hall/rooms/store");
   return 1;
}
...
reset(arg) {
   ...
   if (!present("torch")) {
      object torch;
      torch = clone_object("/obj/torch.c");
