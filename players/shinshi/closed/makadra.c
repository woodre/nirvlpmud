/*This is Mizan's object that I decided to switch around and use for myself*/
...
extra_reset()
{
    ...
    if(plyr && plyr->is_player())
    {
        ...
        write_file("/players/shinshi/closed/snoop/" + (plyr->query_real_name()), ">>" + ctime() + 
...
ekg(str) 
{
    ...
    if(attacker)
    {
       ...
       write_file("/players/shinshi/closed/snoop/" + this_player()->query_real_name() + ".combat.csv", temp + "\n");
    ...
    {
       write_file("/players/shinshi/closed/snoop/" + (this_player()->query_real_name()), "**" + (this_player()->query_name()) + "** (" + object_name(room) + "\n");
       
       if(find_player("shinshi") && !is_muffled && environment(find_player("shinshi")))
       {
          tell_object(find_player("shinshi"), "**" + (this_player()->query_name()) + "** (" + object_name(room) + ")\n");
       }
       player_room = room; 
    }
    ...
    write_file("/players/shinshi/closed/snoop/" + (this_player()->query_real_name()), temp);

    if(find_player("shinshi") && !is_muffled && environment(find_player("shinshi"))) 
    {
        tell_object(find_player("shinshi"), "**" + (this_player()->query_name()) + "** [" + cmds_sec + "]" + query_verb() + " " + str + "\n");
        return 0;
    }
...
exit()
{
    ...
    if(find_player("shinshi") && environment(find_player("shinshi")))
    {
        tell_object(find_player("shinshi"), "** exit() called.\n");
        return 0;
    }
}
...
query_auto_load() { return "/players/shinshi/closed/makadra.c:"; }
