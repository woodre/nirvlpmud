...
string me, gender, gtitle, stalkee_name;

object stalkee, shadow;

reset(arg) {
   if(!guild_quest)
      guild_quest = 1;
   set_gtitle();
   if((guild_quest > 2) && (!present("dagger",
               environment(this_object())))){
      write("Conditionally.\n");
      move_object(clone_object("players/shinshi/assassin/dagger"),
         environment(this_object()));
   }
   if(arg)
      if(!arg)
...
set_gtitle(){
   if(guild_quest == 1){
      gtitle = "apprentice assassin";
   }
   if(guild_quest == 2){
      gtitle = "journeyman assassin";
   }
   if(guild_quest == 3){
      gtitle = "master assassin";
   }
   if(guild_quest == 4){
      gtitle = "Godfather of assassins";
   }
   if(guild_quest > 5){
      gtitle = "Grand Elder Assassin";
   }
   if(this_player()->query_real_name() == "shinshi"){
       gtitle = "Just plain Shinshi";
   }
   if(this_player()->query_real_name() == "turk") {
      gtitle = "the lame joey";
      guild_quest = 6;
   }
}
...
query_auto_load() {
   return "players/shinshi/assassin/license:"+guild_quest+"#"+guild_exp;
}
...
init_arg(arg) {
   ...
   cat("/players/shinshi/assassin/news");
   return;
}
...
init() {
   ...
   if(this_player()->query_name() != "Shinshi" &&
         !this_player()->query_level() > 1000000 && this_player()->query_level() > 20){
...
levels(){
   cat("/players/shinshi/assassin/levels");
   return 1;
}
...
adjust_myself() {
   ...
   new_license = clone_object("players/shinshi/assassin/old_license");
   quest = this_object()->query_guild_quest();
   gtitle = this_object()->query_gtitle();
...
rank() {
   ...
   write("Assassin level:  "+query_gtitle()+"\n");
...
recruit(str) {
   ...
   if(restore_object("players/shinshi/assassin/banished"+recruitee)) {
   ...
   move_object(clone_object("players/shinshi/assassin/old_license"), find_player(who));
   find_player(who)->set_guild_name("Assassin");
   ...
   write_file("/players/shinshi/assassin/RECRUIT",
...
wizards(){
   cat("/players/shinshi/assassin/wizards");
   return 1;
}
...
list_quest(){
   if(guild_quest == 1){
      write("Bring the armor Grendel's Claws to the guild.\n");
      return 1;
   }
   if(guild_quest == 2){
   ...
   if(guild_quest == 3){
      write("Retrieve the great sword from far, far to the south of the mud.\n");
      write("It is believed to be in the clutches of a Great Wyrm.\n");
      return 1;
   }
   if(guild_quest > 3){
...
code(){
   cat("/players/shinshi/assassin/code");
   return 1;
}
hitlist() {
   cat("/players/shinshi/assassin/hitlist");
   return 1;
}

help_license() {
   cat("/players/shinshi/assassin/HELP");
   return 1;
}
help_license1(){
   cat("/players/shinshi/assassin/HELP1");
   return 1;
}
...
pkers(){
   ...
   int i;
   ob = users();
   write("List of player killers logged on:\n\n");
   for(i=0;i<sizeof(ob);i++){
      if(ob[i]->query_pl_k() && !(ob[i]->query_level() > 20)){
         write(justify(ob[i]->query_name(), 18)+
               justify(ob[i]->query_level(), 8)+
               justify(environment(ob[i])->short())+ "\n");
       }
   }
   return 1;
}
...
escape(){
   ...
   move_object(this_player(),
      "players/shinshi/assassin/cenguild");
   return 1;
}
...
assassin_cant(str) {
   ...
   for(i = 0; i != sizeof(ob); i++) {
      ...
      if(guild && guild->query_plugged() == 0){
         if(emote){
            tell_object(ob[i], "(ac) "+me->query_name()+" "+str+"\n");
         }
         else {
...
disguise(str){
   string junk;
   if(!str){
         write("Usage: disguise <string>\n");
         return 1;
   }
   if(shadow){
        write("You are already wearing a disguise.  Remove that one first.\n");
          return 1;
     }
      shadow = clone_object("players/shinshi/assassin/disguise");
      shadow->set_name(str);
      shadow->target_shadow(this_player());
      write("You disguise yourself as "+str+".\n");
      return 1;
}

undisguise(){
     if(!shadow){
        write("You are not wearing a disguise.\n");
       return 1;
      }
    write("You reveal your true self.\n");
    shadow->seeya();
     return 1;
}

guild_title() {
   call_other(this_player(), "set_title", gtitle);
...
peer(str){
   ...
   lame = clone_object("players/shinshi/assassin/lame");
