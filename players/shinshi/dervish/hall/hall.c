...
reset(arg){
 ...
 if(!arg){
  if(!present("tablet")) {
    move_object(clone_object(OBJECTPATH+"/sandstone.c"),this_object());
  }
  if(!present("mound")) {
    move_object(clone_object(OBJECTPATH+"/mound.c"),this_object());
  }
 set_light(1);
...
init() {
  ...
  if(present(GID,TP)) add_action("guild_leave","LeaveDervish");
  }
...
guild_leave(str) {
  ...
  CHATOB->resetchannel();
  rm(MEMPATH+"/"+TP->query_real_name()+".o");
  return 1; }
