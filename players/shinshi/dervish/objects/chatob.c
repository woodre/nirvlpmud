...
send_msg(string str){
  ...
  for(i=0,n=sizeof(listeners);i<n;i++){
   ...
   "/obj/user/chistory"->add_history("Dervish", str);
}
...
tell(string str){
  ...
  {
    write("/obj/user/chistory"->query_history("Dervish"));
    return 1;
  }
  msg = "";
  msg += RED+BOLD+"**DERVISH** ";
...
emote(string str){
  string msg;
    ...
    if(file_size(file="/bin/soul/_"+em+".c") > 0) {
      return (int)file->guildcast("dervish", arg);
    }
  msg = "";
  msg += RED+BOLD+"**DERVISH** "+OFF+OFF;
...
add_history(m) { 
  "/obj/user/chistory"->add_history("Dervish", m);
}

query_tag() {
  return RED+BOLD+"**DERVISH**"+OFF+OFF;
}
