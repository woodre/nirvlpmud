...
init() {
  ...
  add_action("set_mannparents","setnparents");
...
reset(arg) {
  ...
  nparents=allocate(CAP+1);
     ptoo = allocate(CAP+1);
   preg = allocate(CAP+1);
  restore_object("players/boltar/sprooms/daycare6");
  while (i<CAP+1) {
    inout[i] = 1;
    whoout[i] = 0;
    i += 1;
...
move_object(clone_object("obj/kid_soul"), kid);
       ...
       say(kid->query_name()+" runs out of the playroom to "+this_player()->query_name()+"\n");
        reset_count();
        save_object("players/boltar/sprooms/daycare6");
   return 1;
 }
leave(str) {
int par_too_flag;
...
if(str=="baby") { write("You must name your child.\n"); return 1; }
     ...
     if(call_other("players/boltar/sprooms/daycare1", "check_names", str)) yyu =1;
     ...
     if(call_other("players/boltar/sprooms/daycare5", "check_names", str)) yyu =1;
     if(yyu==1) {
...
south() {
	call_other(this_player(), "move_player", "south#players/boltar/sprooms/daycare5.c");
	return 1;
  }
reset_count(){
  int k;
