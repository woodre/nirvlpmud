...
ONE_EXIT("players/molasar/CASTLE/road2", "east",
	 "Durangas' Bizarre",
...
do_sell(ob) {
    ...
    call_other("players/molasar/CASTLE/store", "store", ob);
    return 1;
}
...
value(item) {
    ...
    if (!name_of_item) {
	if (call_other("players/molasar/CASTLE/store", "value", item))
	    return 1;
	write("No such item ("); write(item); write(") here\n");
...
buy(item) {
    if (!item)
	return 0;
    call_other("players/molasar/CASTLE/store", "buy", item);
    return 1;
}
...
north() {
    ...
    call_other(this_player(), "move_player", "north#players/molasar/CASTLE/store");
    return 1;
}

list(obj) {
    call_other("players/molasar/CASTLE/store", "inventory", obj);
    return 1;
}
