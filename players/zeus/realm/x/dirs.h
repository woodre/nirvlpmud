...
/* Removed (string) casting on file_name checks
 * Rumplemintz - Dec-055-2009
 */
make_monsy(){
	int x;
	object mon;
   ...
   do {
	   ...
	   else
       {
         if(!random(2))
           ...
           y = cow_hide(1+random(3));
       }
/* removing cast for testing - Rumplemintz */
/*  } while((string)file_name(environment(this_player())) == y);
*/
    } while(file_name(environment(this_player())) == y);
...
/* remove cast for testing - Rumplemintz */
/*
  while((string)file_name(environment(this_player())) == y);
*/
  while(file_name(environment(this_player())) == y);
