...
// 2005.08.20 -Forbin
// Changes:
//   raised charges to 1500 and switched weapon_hit to lazer_hit
//   and changed the hit_players to other|lazer
*/
...
reset(arg) {
  ...
  set_value(5932);
  set_class(2);
  set_params("other|laser",0,"lazer_hit");
...
int lazer_hit(object target){
   ...
   if(target->query_hp() > ((x*y)/2+2))
	  target->hit_player((x*y)/2+2, "other|laser");
   else target->hit_player(target->query_hp(), "other|laser");
   return (x*y);
}
