...
#define MONSTART "/players/zeus/museum/arena/ar2.c"
string *sucker;
...
"window that provides an excellent view of outer space.  Hanging on the\n"+
"west wall is a digital display board.  A soft light is eminating from\n"+
"the ceiling.  The hallway continues to the south.\n";
 set_light(1);
 items=({
	"board",
...
"on the museum arena type 'arena_help'",
 ...
 dest_dir=({  "/players/zeus/museum/hall3.c", "south", });
}
...
status start_cmd(string str){  /*  Start arena */
  ...
  if(opponent == 0)
    ...
    MO(CO("/players/zeus/museum/NPC/tol'kor.c"), MONSTART);
    o1 -= 1; }
  else if(opponent == 2){
    MO(CO("/players/zeus/museum/NPC/ninja.c"), MONSTART);
    o2 -= 1; }
  else if(opponent == 3){
    MO(CO("/players/zeus/museum/NPC/mage.c"), MONSTART);
    o3 -= 1; }
  else if(opponent == 4){
    MO(CO("/players/zeus/museum/NPC/mutant.c"), MONSTART);
    o4 -= 1; }
  else if(opponent == 5){
    MO(CO("/players/zeus/museum/NPC/sd.c"), MONSTART);
    o5 -= 1; }
  else if(opponent == 6){
    MO(CO("/players/zeus/museum/NPC/golem.c"), MONSTART);
    o6 -= 1; }
  else if(opponent == 7){
    MO(CO("/players/zeus/museum/NPC/bug_storm.c"), MONSTART);
    o7 -= 1; }
  else if(opponent == 8){
    MO(CO("/players/zeus/museum/NPC/light.c"), MONSTART);
    o8 -= 1; }
  else if(opponent == 9){
    MO(CO("/players/zeus/museum/NPC/ani.c"), MONSTART);
    o9 -= 1; }
  else if(opponent == 666){
    MO(CO("/players/zeus/museum/NPC/classic_golem.c"), MONSTART);
    o666 -= 1; }
  else
    write("You have not selected an opponent.\n");
  if(opponent)
