 ...
 */
   ...
   write_file(LOGPATH + "POLY_VIBE", temp);
   
   log_file("mizan.poly_vibes", temp);
...
polymorph_treehouse()
{
   ...
   if(present("treehouse", environment(this_player())))
   {
      write("There is already a treehouse here.\n");
      return 1;
   }
   ...
   if(gob->query_charge() < POLY_TREEHOUSE_COST) 
   {
      write("You are too drained to do this.\n");
      return 1;
   }
   ...
   tree = clone_object(POLY_TREEHOUSE_FORM);
   tree->set_owner(this_player()->query_real_name());
   move_object(tree, environment(this_player()));
...
polymorph_mimic()
{
   ...
   if(!uh) return 0;
   if(uh->query_charge() < POLY_MIMIC_COST) 
   {
      write("You are too drained to do this.\n");
      return 1;
   }
   ...
   frm = clone_object(POLY_MIMIC_FORM);
...
polymorph_spuckmobile()
{
   ...
   if(gob->query_charge() < POLY_SPUCKMOBILE_COST) 
   {
      write("You are too drained to do this.\n");
      return 1;
   }
   ...
   car = clone_object(POLY_SPUCKMOBILE_FORM);
   car->set_owner(this_player()->query_real_name());
   move_object(car, environment(this_player()));
...
polymorph_high_mimic()
{
   ...
   if(!uh) return 0;
   if(uh->query_charge() < POLY_HIGH_MIMIC_COST) 
   {
      write("You are too drained to do this.\n");
      return 1;
   }
   ...
   frm = clone_object(POLY_HIGH_MIMIC_FORM);
   frm->set_owner(this_player()->query_real_name());
   move_object(frm, environment(this_player()));
   return 1;
}
 ...
 */
do_polymorph(pid, cost, chan, str, arg, weapons, armor) 
{
   object morpho, gob;
   ...
   if(this_player()->query_level() > 19) /* UNCOMMENT THIS LINE TO MAKE LIVE */
   ...
   if(this_player()->query_level() > 19) /* UNCOMMENT THIS LINE TO MAKE LIVE */
   if((ltime / 60) < poly_timelapse[pid])
   {
...
check_overdamagecap(person, damage)
{
   ...
   if(ob->query_exp() < BASELINE_EXP)
      dam = MAGIC_DAMAGE_CAP;
   else
   {
      diff_dam = dam - MAGIC_DAMAGE_CAP;
            ...
            while(i < sizeof(weapon)) 
            {
               ...
               if(!weapon[i] && !a) 
               {
                  ...
                  else
                  {
                     ...
                     write("You strap " + lower_case(ob[j]->short()) + " as a " + get_type(i) + " weapon.\n");
...
dark_tetrapod_cloak()
{
   ...
   if(this_player()->query_attack())
   {
      write("You cannot do this while fighting!\n");
      return 1;
   }
