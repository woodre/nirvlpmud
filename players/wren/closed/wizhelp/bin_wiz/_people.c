...
cmd_people(string str) {
  object *list;
    ...
    if((int)this_player()->query_level() < EXPLORE) return 0;
    if(!str || str =="") str = "a";
      if(sscanf(str, "%s %d %d", option,ll,hl) == 3) str = option;
     if(hl < ll || !hl) {
    hl = 2147483647;
    ll = -2;
  }
...
if(hl != 2147483647) write(" Only those between level "+ll+" and level "+hl+" are listed\n");
...
if(str == "O") write(extract(file_name(list[i]), 10, 15));
