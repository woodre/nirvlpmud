cantrip(str)
{
	...
	if (str == "bauble") {
		if (spell(-50, 1, 3, this_object()) == -1)
			return 0;
		move_object(clone_object("/players/reflex/guilds/bards/bauble.c"),
					this_player());
		write
	...
	if (str == "bubble") {
		if (spell(-100, 3, 10, this_object()) == -1)
			return 0;
		move_object(clone_object("/players/reflex/guilds/bards/bubble.c"),
					this_player());
	...
	if (str == "strand") {
		if (spell(-149, 7, 5) == -1)
			return 0;
		move_object(clone_object("/players/reflex/guilds/bards/strand.c"),
					TP);
