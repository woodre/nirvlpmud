...
Modified by Reflex - For thematic reasons Bards should use this version
and not the regular say command.  I will attempt to replace say and ' with
this version.
*/
new_say(str)
{
	...
	if (!str || sscanf(str, "%s", message) != 1) {
		...
		write(HIR+"You yell \""+NORM+str+HIR+"\"\n"+NORM);
		say(HIR+tp+" yells \""+NORM+str+HIR+"\"\n"+NORM);
		return 1;
		} else if (sscanf(str, "%s!", message)) {
		write(RED+"You exclaim \""+NORM+str+RED+"\"\n"+NORM);
		say(RED+tp+" exclaims \""+NORM+str+RED+"\"\n"+NORM);
		return 1;
		} else if (sscanf(str, "%s???", message)) {
		write(HIB+"You wonder \""+NORM+str+HIB+"\"\n"+NORM);
		say(HIB+tp+" wonders \""+NORM+str+HIB+"\"\n"+NORM);
		return 1;
		} else if (sscanf(str, "%s?", message)) {
		write(BLU+"You ask \""+NORM+str+BLU+"\"\n"+NORM);
		say(BLU+tp+" asks \""+NORM+str+BLU+"\"\n"+NORM);
		return 1;
		} else if (sscanf(str, "%s", message)) {
		write(HIM+"You say \""+NORM+str+HIM+"\"\n"+NORM);
		say(HIM+tp+" says \""+NORM+str+HIM+"\"\n"+NORM);
		return 1;
		} else {
		write("ERROR - TELL REFLEX\n");
		return 1;
	}
}
...
juggle()
{
	...
	say(tp +
		...
		"to deftly juggle them between " + TP->query_possessive() +
	" hands.\n");
	return 1;
}
...
{
	write
	("You lazily juggle four balls of colored light with one hand.\n");
	say(tp +
	" lazily juggles four balls of colored light with one hand.\n");
	return 1;
}
...
blueli(str)
{
	...
	write("You point at " + CAP + ", surrounding " +
		ob->query_objective() +
	" with an aura of pulsating blue magic.\n");
	say(tp + " points at " + CAP + ", surrounding " +
		ob->query_objective() + " with an aura of pulsating blue magic.\n",
	ob);
	tell_object(ob,
		tp +
		" points at you, surrounding you with an aura of pulsating blue magic.\n"
	+ "You feel different.\n");
	return 1;
}
...
disappear(str)
{
	...
	if (!str) {
		write
		("Muttering a brief incantation, you momentarily cloak yourself in invisibility.\n");
	...
	say(tp + " incants sharply and points at " + CAP + ", causing " +
		ob->query_objective() + " to\n" +
...
strings()
{
	...
	say(tp + " lightly strums the strings of " + TP->query_possessive() +
		" " + TO->query_instrument() + ".\n" +
	"  The image of a melody dances through your mind.\n");
	return 1;
}
...
clearth()
{
	...
	say(tp + " clears " + TP->query_possessive() +
		" throat with a loud sound.\n" + "  You think " +
	TP->query_pronoun() + " must have something important to say.\n");
	return 1;
}
...
tune()
{
	...
	say(tp + " tunes " + TP->query_possessive() + " " +
	TO->query_instrument() + ".\n");
	return 1;
}
...
sparkle(str)
{
	...
	if (!str) {
		...
		say(tp + " looks up with a sparkle in " + TP->query_possessive() +
		" eyes.\n");
		return 1;
	}
	ob = present(str, ENV);
	...
	tell_object(ob,
		tp + " looks up at you with a sparkle in " +
...
handkiss(str)
{
	...
	say(tp + " bows slightly, takes the hand of " + CAP +
	" and kisses it lightly.\n", ob);
	tell_object(ob,
		tp +
	" bows slightly, takes your hand and kisses it lightly.\n");
	return 1;
}
