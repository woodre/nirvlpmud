...
reset(arg)
{
	mark = clone_object("/players/reflex/guilds/bards/bard_mark.c");
...
bye()
{
	...
	say(capitalize(this_object()->short()) + " looks over at " +
		capitalize(this_object()->query_master()) + " and nods.\n");
	say(PET + " flies off into the sky.\n");
	ob = present(this_object()->query_master(),
...
give(str)
{
	...
	if (!str) {
		notify_fail("Have " + this_object()->name() +
					" take what to who?\n");
		return 0;
	}
	if (sscanf(str, "%s to %s", what, who) < 2) {
		notify_fail("Have " + this_object()->name() + " deliver what?\n");
		return 0;
	}
	item =
		present(lower_case(what),
				find_living(this_object()->query_master()));
	...
	if (!item) {
		...
		if (!item) {
			notify_fail("Neither you or " + this_object()->name() +
						" has that item.\n");
			return 0;
		}
		flag = 1;
	}
	if (what == "instrument" || what == "base_obj" || what == "rock"
		|| what == "soul") {
		return 1;
	}
	if (flag) {
		...
		say(PET + " takes an item from " + this_object()->query_master() +
			" and flies off into the sky.\n\n" + PET +
			" returns and nods to " + this_object()->query_master() +
			".\n");
	}
	tell_room(environment(target),
			  PET + " flies in and gives a " + what + " to " + who +
...
dart_command(str)
{
	...
	case "guard":
		...
		break;
	default:
		notify_fail("Command " + PET + " to do what?\n");
		return 0;
		break;
	}
}
...
talk()
{
	...
	write("You look over at your raven and smile.\n");
	return 1;
}
...
silence()
{
	...
	write("You look over at your raven and shake your head.\n");
	return 1;
}
...
pet_follow()
{
	...
	if (!present(master, environment(this_object()))) {
		say("The raven flies from the room after its master.\n");
		move_object(this_object(), environment(there));
		say("A black raven flies into the room.\n");
	}
	call_out("pet_follow", 6);
	return 1;
}
...
hand(str)
{
	...
	if (!str) {
		notify_fail("Have the raven give you what?\n");
		return 0;
	}
	...
	if (!target || !living(target)) {
		notify_fail("Have the raven give the " + what + " to who?\n");
		return 0;
	}
	if (!item) {
		notify_fail("The raven does not have a " + what + ".\n");
		return 0;
	}
	if (what == "instrument" || what == "base_obj" || what == "rock"
		|| what == "soul") {
	...
	tell_room(environment(this_object()),
			  "The raven drops a " + what + " into " + capitalize(who_to) +
			  "'s hand.\n");
	move_object(item, target);
	return 1;
}
...
guard()
{
	...
	if (this_object()->query_attack()) {
		...
		tell_room(environment(this_object()),
				  ...
				  +
				  "\n       \"\This raven is under my protection.\"\n\n" +
	...
	write("Your raven is not under attack by a player or pet or kid.\n");
	return 1;
}
