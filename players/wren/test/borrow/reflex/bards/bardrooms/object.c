...
items=({
 ...
 "instruments","There are a LOT of musical instruments here",
  ...
  dest_dir=({
 "/players/reflex/guilds/bards/bardrooms/enterance.c","east",
           });
  }   }
init(){
 ::init();
...
if(str == "bards")  {
  ...
  ob->set_guild_name("none");
  write_file("/players/reflex/guilds/bards/logs/leave",tp+" left the bards->"+ctime(time())+"\n");
