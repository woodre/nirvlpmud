...
{
	if (!present("selsase")) {
		move_object(clone_object("/players/reflex/guilds/bards/selsase.c"),
					this_object());
	}
	if (!arg) {
		set_light(1);
		...
		items = ({
			...
			"circle",
			"A small circle of stones.  There's something funny about the\n"
			+ "way that they have been arranged", });

		dest_dir = ({ "/players/saber/ryllian/mountain2.c", "west", });
...
quest5()
{
	ROM(" \"What is my cosmic index number?\"\n");
	call_out("blast", 61);
	call_out("quest6", 60);
	return 1;
}
...
blast()
{
	...
	while (ob) {
		...
		say("You are blasted by a , temp) == 1) {
...
end_it2()
{
	string NAME, abil, teruct(find_living("selsase"));
