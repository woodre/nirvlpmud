...
if(str == "bauble")  {
if(spell(-50,1,3,this_object()) == -1)  return 0;
  move_object(clone_object("/players/reflex/guilds/bards/bauble.c"),this_player());
  write("You murmur strange syllables which flow softly over your tongue.\n"+
  "A solid bauble of light materializes in your hand.\n");
  say(tp+" murmurs strange syllables which flow softly over "+TP->query_possessive()+" tongue.\n"+
...
if(spell(-100,2,5,this_object()) == -1) return 0;
  ...
  "You feel drunk.\n");
  say(tp+" traces magical symbols in the air with "+TP->query_possessive()+" little finger.\n"+
...
if(str == "bubble")  {
if(spell(-100,3,10,this_object()) == -1)  return 0;
  move_object(clone_object("/players/reflex/guilds/bards/bubble.c"),this_player());
  ...
  say(tp+" utters a single sharp syllable.\n"+
    "A solid sphere of blue light appears in "+TP->query_possessive()+" hand.\n");
  call_other(TP, "add_spell_point", -10);
  return 1;
        }
if(str == "storm")  {
if(spell(-100,6,20,this_object()) == -1)  return 0;
move_object(clone_object("/players/saber/effects/storm2.c"),environment(TP));
  write("You whistle up a storm.\n");
  say(tp+" whistles up a strom.\n");
...
if(str == "strand")  {
if(spell(-149,7,5) ==-1) return 0;
move_object(clone_object("/players/reflex/guilds/bards/strand.c"),TP);
  write("You reach through reality and grasp a strand of chaos.\n");
  say(tp+" reaches through reality and pulls forth a strand of chaos.\n");
  return 1;
  call_other(TP,"add_spell_point",-5);
  return 1;
        }
