...
reset(arg)
{
	ob = clone_object("/players/reflex/guilds/bards/bardsword2.c");
	...
	if (!arg) {
		...
		set_race("human");
		set_long
			("Brian of the West Wall is a tall man with flowing black hair and dark\n"
			 +
		...
		set_spell_mess1("\nBrian punches his foe in the solar plexus.\n");
		set_spell_mess2("\nBrian punches you in the solar plexus!\n" +
						"Ouch\nOuch\Ouch\n");
...
north()
{
	...
	say("Brian steps aside to let " + this_player()->query_name() +
		" pass into the Bardic Guild.\n");
}

up()
{
	if (!present("bard_obj", this_player())) {
		say("Brian says \"\You must be a member of the Bards to go up\"\.\n");
		return 1;
	}
	say("Brian bows to " + this_player()->query_name() + " as " +
		this_player()->query_pronoun() + " climbs the stairs.\n");
}
...
pass(str)
{
	...
	if (kid->is_pet()) {
		...
		ob = clone_object("/players/reflex/guilds/bards/bard_mark.c");
		move_object(ob, kid);
		return 1;
	}
