...
#define ENVV environment(this_player())
	...
	if (name == "raven") {
		if (spell(-149, 2, 50) == -1)
			return 0;
		ob = clone_object("/players/reflex/guilds/bards/pet/raven.c");
	} 
	else if (name == "hawk") {
		if (spell(-149, 4, 50) == -1)
			return 0;
		ob = clone_object("/players/reflex/guilds/bards/pet/hawk.c");
	} 
	else if (name == "drake") {
		if (spell(-149, 6, 50) == -1)
			return 0;
		ob = clone_object("/players/reflex/guilds/bards/pet/drake.c");
	} 
	else if (name == "phoenix") {
		if (spell(-149, 8, 50) == -1)
			return 0;
		ob = clone_object("/players/reflex/guilds/bards/pet/phoenix.c");
	} 
	else if (name == "faerie" && TP->query_level() > 19) {
		if (spell(-149, 4, 50) == -1)
			return 0;
		ob = clone_object("/players/reflex/guilds/bards/pet/faerie.c");
	} 
	else if (name == "brownie" && TP->query_level() > 19) {
		if (spell(-149, 4, 50) == -1)
			return 0;
		ob = clone_object("/players/reflex/guilds/bards/pet/brownie.c");
	} 
	else {
