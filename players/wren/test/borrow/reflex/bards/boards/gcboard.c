...
#define ERROR_LOG "/players/reflex/guilds/bards/log/GCBOARD"
#define BOARD_NAME "players/reflex/guilds/bards/boards/gcnotes"
...
long()
{
	...
	write("This is the GC Board for the Bards.\n");
	...
	write("The GC board contains " + msg_num);
	if (msg_num == 1)
		write(" note:\n\n");
	else
		write(" notes:\n\n");
...
short()
{
	...
	if (msg_num == 1)
		msg = "1 message";
	if (msg_num > 1)
		msg = msg_num + " messages";
	return "The GC Board (" + msg + ")";
}

get()
{
	write("It is firmly secured to the wall.\n");
	return 0;
}
...
new_msg(msg_head)
{
	...
	say(capitalize(curr_writer->query_name()) +
		" starts writing a note on the board.\n");
	new_head = msg_head;
	new_text = "";
	input_to("get_msg");
...
get_msg(str)
{
	...
	if (str == "**") {
		...
		say(capitalize(curr_writer->query_name()) +
			" has completed a note: " + new_head + "\n");
		headers[msg_num] =
			new_head + "(" + capitalize(this_player()->query_real_name()) +
...
remove_msg(what_msg)
{
	...
	say(capitalize(this_player()->query_name()) +
		" removes a note titled '" + headers[note] + "'.\n");
	ind = note;
	while (ind < msg_num - 1) {
