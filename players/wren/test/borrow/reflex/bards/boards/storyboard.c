...
#define BOARD_NAME "players/reflex/guilds/bards/boards/storyboard2"
...
long() {
	int ind;
	write("On this wall a Bard may write down their greatest stories.\n");
	write("write <story title> \n");
   write("read <story number>\n");
   write("If you need to, please use more than one slot.\n");
 if(TP->query_level() > 19){
   write("As a wizard, you may remove <story number> and \n"+
         "store <story number> <filepath>.\n");
   }
	if (!msg_num) {
...
new_msg(msg_head) {
	...
	if (msg_num == 70) {
		write("This book is now full.  Please report this to Reflex.\n");
		return 1;
	}
