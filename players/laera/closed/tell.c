...
init() {
   ...
   if(this_player()->query_real_name() != "scathe") destruct(this_object());
...
wiz_tell(str) {
   ...
   tell_object(plyr,HIR+"**>> "+NORM+myname+" tells You: "+what+"\n");
   write("You tell "+capitalize(who)+": "+color+what+NORM+"\n");
    find_player(who)->add_tellhistory("From Scathe: "+what+".\n");
   return 1;
}
...
wiz_mote(str) {
   ...
   if(in_editor(find_player(who))) { write("Person is in edit...\n"); return 1; }
   tell_object(plyr,HIR+"**>> "+NORM+myname+" "+what+"\n");
