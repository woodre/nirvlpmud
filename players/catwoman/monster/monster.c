...
heart_beat()
{
    ...
    attack();
    if (attacker_ob && whimpy && hit_point < max_hp/5 && random(200)< cool)
	run_away();
...
set_level(l) {
    ...
    musc = random(3) + 2 + level/5;
    tough = random(3) + 2 + level/5;
    attack = 1;
    spd = random(10) + random(10) + 20 + level * 2;
    dex = random(10) + random(10) + 20 + level * 3;
    brain = random(10) + random(10) + 20 + level * 2;
    cool = random(10) + random(10) + 20 + level * 2;
    willp = random(10) + random(10) + 20 + level * 2;
    fellow = random(10) + random(10) + 20 + level * 2;
}

/* set_fear() {
 *   if (call_other(this_player(), "query_cool",) < random(100))
 *   call_other(this_player(), "run_away",) }
 */
...
init() {
	...
	set_heart_beat(1);
	if ((aggressive == 1) && (random(100) + aggressive) > call_other(this_pl
ayer(), "query_fellow"))
	    kill_ob = this_player();
    }
}
