...
reset(arg) {
	if(!ob1){
                ob1 = clone_object("players/catwoman/monster/celine");
		move_object(ob1, this_object());
		}
	if(!ob2){
		ob2 = clone_object("players/catwoman/armor/teddy2");
		move_object(ob2, ob1);
		}
     set_light(1);
     if (arg) return;
...
*/
east() {
  call_other(this_player(),"move_player", "east#players/catwoman/city/31");
  return 1;
}
/*
west() {
