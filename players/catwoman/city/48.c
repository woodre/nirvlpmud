...
reset(arg) {
	if(!ob1){
		ob1 = clone_object("players/catwoman/monster/pimp");
		move_object(ob1, this_object());
		}
	if(!ob2){
		ob2 = clone_object("players/catwoman/armor/phat");
		move_object(ob2, ob1);
		}
...
north() {
  call_other(this_player(),"move_player", "north#players/catwoman/city/47");
  return 1;
}
/*
south() {
...
long() {
 write("A dead end\n");
 write("To the north is North Ave\n");
