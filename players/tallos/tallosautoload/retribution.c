...
if(i == 0) {
	...
	tell_object(attacker, tpn+" deals you a stunning blow with "+
	            "the help of God's "+YELLOW+"Retribution"+END+
                    ".\n");
	say(tpn+" deals a stunning blow to "+attacker->query_name()+
            " with the help of God's "+YELLOW+"Retribution"+END+
            ".\n");
	return 9;
}
if(i == 1) {
...
return YELLOW+"God's spirit surrounds "+environment(this_object())->query_name()+END; 
}
return "";
}
