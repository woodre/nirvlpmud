/* 07/04/06 Earwax: Formatted this.  It looks like it was coded by
 * a 3rd grader.
 */
...
reset(arg) {
 ...
 ::reset(arg);
  ...
  set_long(
    ...
    "serrated, tear-like teeth.  Those who wear this shield\n"+
    "shall triumph over an enemy if their determination holds.\n");
  set_ac(1);
  set_type("shield");
  set_weight(1);
  set_value(2500);  
...
do_special(owner){
  if(random(100) < USER->query_attrib("wil") / 2 && ATT)  /* Feldegast 8/2000 */
  {
    if(ATT->is_npc())
    ...
    tell_room(environment(USER),
      USER->query_name()+" invokes "+USER->query_possessive()+" shield and a small crystal tear cuts into the heart of "+ATT->query_name()+"!\n", ({USER}));
    return 0;
  }
  return 0;
}	
