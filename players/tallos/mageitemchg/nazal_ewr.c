...
#define BONUS_TYPE "conjuration"
#define BONUS_TYPE2 "enchantment"
#define BONUS_TYPE3 "evocation"
#define BONUS_AMOUNT 2
#define BONUS_AMOUNT2 1
#define ROBES HIG+"Emer"+NORM+GRN+"ald "+HIG+"Wiz"+NORM+GRN+"ard "+HIG+"Ro"+NORM+GRN+"bes"+NORM

int Go, Count;

reset(arg) 
{
  set_name("emerald wizard robes");
  SetMultipleIds(({"robe","robes","wizard robe","wizard robes","emerald wizard robe"}));
  set_short(ROBES);
  set_ac(2);
...
add_guild_bonus()
{
  if(present(GUILDID, this_player()))
  {
	...
	present(GUILDID, this_player())->add_guild_bonus(BONUS_TYPE3, -BONUS_AMOUNT2);		
 }
...
void long()
{
  if(Go) 
    tell_object(this_player(),
      ...
      "is woven with a magnificent design: a wide emerald stripe runs the\n"+
    ...
    tell_object(this_player(),
      ...
      "back of the robes is woven with a magnificent design: a wide emerald\n"+
...
int do_special(object owner)
{
  ...
  {
    tell_object(owner,
      "Your "+ROBES+" "+HIG+"shimmers"+NORM+" brightly!\n");
    tell_room(environment(owner),
      (string)owner->query_name()+"'s "+ROBES+" "+HIG+"shimmers"+NORM+" brightly!\n", ({ owner }));
    if(random(3))
      Count += 5;
    return 5;
  }
  ...
  {
    tell_object(owner,
      HIG+"\nThe enchantm"+NORM+GRN+"ent surrou"+HIW+"nd your rob"+NORM+"es fades...\n\n");
    Go = 0;
  }
  return 4; 
}
