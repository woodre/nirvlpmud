...
#define BONUS_TYPE2 "necromancy"
#define BONUS_TYPE3 "enchantment"
#define BONUS_AMOUNT 2
#define BONUS_AMOUNT2 1
inherit "obj/armor";

#define ROBES HIW+"Op"+NORM+WHT+"al "+HIW+"Wiz"+NORM+WHT+"ard "+HIW+"Ro"+NORM+WHT+"bes"+NORM

int Go, Count;

reset(arg) 
{
  set_name("opal wizard robes");
  SetMultipleIds(({"robe","robes","wizard robe","wizard robes","opal wizard robe"}));
  set_short(ROBES);
  set_ac(2);
...
void long()
{
  if(Go) 
    tell_object(this_player(),
      ...
      "is woven with a magnificent design: a wide opal stripe runs the\n"+
    ...
    tell_object(this_player(),
      ...
      "back of the robes is woven with a magnificent design: a wide opal\n"+
...
int do_special(object owner)
{
  ...
  {
    tell_object(owner,
      "Your "+ROBES+" "+HIW+"shimmers"+NORM+" brightly!\n");
    tell_room(environment(owner),
      (string)owner->query_name()+"'s "+ROBES+" "+HIW+"shimmers"+NORM+" brightly!\n", ({ owner }));
    if(random(3))
      Count += 5;
    return 5;
  }
  ...
  {
    tell_object(owner,
      HIW+"\nThe enchantment surround your rob"+NORM+"es fades...\n\n");
    Go = 0;
  }
  return 4; 
}
