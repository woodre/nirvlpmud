...
//  Notes:        high AC, eventually wears off
//  Notable Changes:            
*/
...
#define BONUS_TYPE3 "illusion"
#define BONUS_AMOUNT 2
#define BONUS_AMOUNT2 1
#define ROBES HIR+"Crim"+NORM+RED+"son "+HIR+"Wiz"+NORM+RED+"ard "+HIR+"Ro"+NORM+RED+"bes"+NORM
...
void long()
{
  if(Go) 
    ...
    tell_object(this_player(),
      ...
      "back of the robes is woven with a magnificent design: a wide burgundy\n"+
...
int do_special(object owner)
{
  ...
  {
    tell_object(owner,
      "Your "+ROBES+" "+HIR+"shimmers"+NORM+" brightly!\n");
    tell_room(environment(owner),
      (string)owner->query_name()+"'s "+ROBES+" "+HIR+"shimmers"+NORM+" brightly!\n", ({ owner }));
    if(random(3))
      Count += 5;
    return 5;
  }
  ...
  {
    tell_object(owner,
      HIR+"\nThe enchantm"+NORM+RED+"ent surrou"+HIW+"nd your rob"+NORM+"es fades...\n\n");
    Go = 0;
  }
  return 4; 
}
