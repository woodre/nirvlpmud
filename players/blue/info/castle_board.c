...
#define BOARD_NAME "players/blue/info/castle_bd"
...
long() {
   ...
   write("If you have a castle approved and open, you may post a note on"+
      " this board with\nthe command 'cpost'.  Each wiz can have one note "+
      "at a time.  Try to keep\nthe note under 20 or 30 lines.\n");
  write("Usage : cpost <headline>, cread/cremove <message number>\n");
	write("        store <message number> <file name>\n");
   } else {
      write("This is the guild bulletin board.\n");
      write("Usage: 'cread <message number>'.\n");
   }
	if (!msg_num) {
...
short() {
   return "The Castle Bulletin Board ("+msg_num + " msg(s))";
}
...
init() {
  add_action("long", "msgs");
	add_action("read_msg", "cread");
   if(LEVEL > 39) {
          add_action("new_msg", "cpost");
   	add_action("remove_msg", "cremove");
   }
	if (!looked_at) {
	        int i;
		string arr;
