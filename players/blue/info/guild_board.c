...
#define ERROR_LOG "/players/blue/info/board_log"
#define BOARD_NAME "players/blue/info/guild_bd"
...
id(str) {
   return str == "guild board";
}

long() {
	int ind;
   write("This is the guild bulletin board.\n");
   if(LEVEL > 39) {
      write("Welcome wizard!\n");
      write("If you have a guild that is open and has been approved, you may "+
      "post _one_ note\nabout it.  Try to sum up the theme of the guild, and "+
      "mention a few of the powers.\nTry to keep it under 30 lines long.\n");
	write("Usage : gpost <headline>, gread/gremove <message number>\n");
	write("        gstore <message number> <file name>\n");
   } else {
      write("This is the guild bulletin board.\n");
      write("Usage: 'gread <message number>'.\n");
   }
	if (!msg_num) {
...
short() {
   return "The Guild Bulletin Board ("+msg_num + " msg(s))";
}
...
init() {
  add_action("long", "msgs");
	add_action("read_msg", "gread");
   if(LEVEL > 39) {
          add_action("new_msg", "gnote");
   	add_action("remove_msg", "gremove");
   }
	if (!looked_at) {
	        int i;
		string arr;
...
remove_msg(what_msg) {
   ...
   write_file("/players/blue/info/board_log",this_player()->query_real_name()+
       " removed a note from the guild board @ "+ctime(time())+"\n");
	messages[ind] = 0;
	headers[ind] = 0;
	msg_num -= 1;
	save_board();
