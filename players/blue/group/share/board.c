...
#define ERROR_LOG "/players/blue/group/share/board_error"
#define BOARD_NAME "players/blue/group/share/group5_board"
...
long() {
	int ind;
	write("This is the Group's bulletin board.\n");
   write("Usage : note <headline>, read <message number>\n");
...
short() {
   return "The GROUP V bulletin board(" + msg_num + " msg(s))";
}
...
remove_msg(what_msg) {
	...
	int note, ind;
   if(this_player()->query_real_name() != "blue") return;
	if (!sscanf(what_msg, "%d", note)) 
		if (!sscanf(what_msg, "note %d", note))
			return 0;
	if (note < 1 || note > msg_num) {
