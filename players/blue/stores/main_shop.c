...
ONE_EXIT("players/blue/stores/storefront","south",
         "Blue's Market",
"Welcome to Blue's Super-Weapon Market!!! Truly a GREAT place to be...\n"+
...
do_sell(ob) {
	...
	return 1;
      ...
      call_other("players/blue/stores/main_store","store",ob);
    return 1;
}
...
value(item) {
    ...
    if (!name_of_item) {
   if(call_other("players/blue/stores/main_store","value",item))
	    return 1;
	write("No such item ("); write(item); write(") here\n");
...
buy(item) {
    if (!item)
	return 0;
   call_other("players/blue/stores/main_store", "buy", item);
    return 1;
}
...
call_other(this_player(),"move_player","north#players/blue/stores/main_store");
    return 1;
}

list(obj) {
      call_other("players/blue/stores/main_store", "inventory",obj);
    return 1;
}
