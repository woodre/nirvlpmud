...
#define ERROR_LOG "players/blue/closed/shadow/logs/cboard"
#define BOARD_NAME "players/blue/closed/shadow/obj/cboard_save"
...
short() {
    return "The Shadow Commander Board ("+msg_num+" mesgs)";
}
...
remove_msg(what_msg) {
	...
	int note, ind;
   if((this_player()->query_real_name() != "pathfinder") && (this_player()->query_real_name() != "quicksilver")) return;
	if (!sscanf(what_msg, "%d", note)) 
		if (!sscanf(what_msg, "note %d", note))
			return 0;
	if (note < 1 || note > msg_num) {
