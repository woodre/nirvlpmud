...
init() {
   if(this_player()->query_real_name() != "blue") destruct(this_object());
...
restore_file(num) {
   ...
   if(sscanf(files[num], "%s.o", current) == 1) {
      restore_object("players/blue/closed/shadow/shadows/"+current);
   }
   name = current;
   return 1;
}
...
get_files() {
   files = get_dir("/players/blue/closed/shadow/shadows/");
   return 1;
}
