 /*###                          shadow3_0.c                       ###*/
 /*###                      shadow guild object                   ###*/
 /*###                  Version 3.0, Jan. 22 1994                 ###*/
       
#include "/players/blue/closed/lib/color.h"
#include "/players/blue/closed/shadow/define.h"
...
static string command_str, rep_tell, logo;

static object at_ob, servant, follow, pearl;



id(string str) { return str == "mark"         ||
                        str == "shadow-mark"  ||
                        str == "shadow3.0"    ||
                        str == "shadow3_0"    ||
                        str == "guild_object";
...
short() { 
   string short;
   short = ESC+"[1m"+"A Mark of the Shadows"+ESC+"[0m";  
   return short;
}
...
init_arg(str) {
   guild_echo("\n<"+BOLD + CN + NOSTYLE + "> has entered the game.\n");
   ok = 1;
   write(BORDER);
   write("<\t\t\t\t"+BOLD+"SHADOWS\t\t\t\t"+NOSTYLE+">\n");
   write("<\t\t\Type 'shnews' to see the shadow news!'\t\t\t>\n");
 /*
  write(BOLD+"New Guild object in effect!  type 'ditch_mark'\n" + NOSTYLE);
 */
   write(BORDER);
...
reset(arg) {
   ...
   if(!arg) {
      ...
      shlevel = 1;
      ok = 0;
   call_out("heal_shpoints", 1);
      logo = read_file("/players/blue/closed/shadow/logo");
...
heal_shpoints() {
/*NO FREE HEALS!!!! This function causes start up errors too -Bp */
  return 1;
   shpoints +=1;
   if(shpoints > max_shp) shpoints = max_shp;
   call_out("heal_shpoints", 5);
...
unjoin_me() {
   TP->add_exp(-(TP->query_exp() / 6));
   write("You have been banished from the shadow guild and have lost 1/6 of"+
         " your\nexperience points.  Mail Blue with any problems.\n");
   write_file(PATH + "shadows/GUILDNUKE", CN+" was guild nuked at "+
      ctime(time())+"\n");
   guild_status = -1;
   save_me();
   deguild_me();
...
enable_guild() {
   ...
   add_action("show_news", "shnews");
   add_action("guild_who","shw");
 /*
  add_action("remove_old_guild_mark","ditch_mark");
 */
   ...
   */
   ...
   /*# commander functions #*/
      ...
      if(shlevel > 12 || CN == "Pathfinder" || CN == "Quicksilver") {
     
      add_action("add_shadow_shxp","<>addshxp");
      add_action("show_logo", "<>logo");
      add_action("echo_logo", "<>elogo");
      add_action("suspend_mark","<>suspend");
      add_action("revoke_mark", "<>revoke");
   ...
   /*# spells #*/
   add_action("leave_shadow", "leave_shadows");

   add_action("make_shades", "shades");
...
remove_old_guild_mark() {
  object newmark;
  int newshxp, newshlev;
  if(!present("shadow-mark",this_player())) {
    tell_object(this_player(),"You are not a shadow.\n");
    return 1;
    }
  newshxp = shxp;
  newshxp = shxp * 2;
  if(newshxp > 1900000) { newshlev = 8; }
  else if(newshxp > 1200000) { newshlev = 7; }
  else if(newshxp > 500000) { newshlev = 6; }
  else if(newshxp > 250000) { newshlev = 5; }
  else if(newshxp > 100000) { newshlev = 4; }
  else if(newshxp > 40000) { newshlev = 3; }
  else if(newshxp > 20000) { newshlev = 2; }
  else if(newshxp > 0) { newshlev = 1; }
  newmark = clone_object("/players/pathfinder/closed/shadow/shadow");
  newmark->set_shxp(newshxp);
  newmark->set_shlev(newshlev);
  move_object(newmark,TP);
  newmark->save_me();
...
update_me(str) {
   object ob;
   if(str) return;
   ob=clone_object("/players/blue/closed/shadow/shadow");
   save_me();
...
suspend_me() {
   guild_status = 0;
   TEOWN("You have been suspended from the shadow guild\n");
   TEOWN("Mail Blue if you have any problems or questions.\n");
   de_guild(ENVTO);
   save_me();
   destruct(TO);
   return 1;
...
revoke_me() {
   guild_status = -1;
   TEOWN("You have been revoked from the shadow guild\n");
   TEOWN("Mail Blue if you have any problems or questions.\n");
   de_guild(ENVTO);
   save_me();
   destruct(TO);
...
de_guild(ob) {
   if(!ob) return 0;
   ob->add_guild_exp(-GUILD_EXP);
   ob->set_guild_name("0");
   ob->add_guild_rank(-GUILD_RANK);
   return 1;
}
...
shadow_monitor() {
   ...
   if(servant) {
      ...
      TEOWN("<serv: "+servant->query_hp()+"/130>");
...
less_pts(pts) {
   ...
   temp_spell = this_player()->query_spell_points();
   temp_shadow = shpoints;
   if(shlevel > 15) return 1;
   if(pts > (SP + shpoints)) {
...
tell_player(str) {
   ...
   rep_tell = player->query_real_name();
   str = implode((explode(str, " ") - ({explode(str, " ")[0]})), " ");
   str = format(str, 60);
   if(player->query_level() < 20)
      tell_object(player, BOLD + CN + " tells you: " + NOSTYLE + str + "\n");
   else tell_object(player, CN + " tells you: " + str + "\n");
   write("Ok.\n");
...
repeat_tell(str) {
   ...
   if(!find_player(rep_tell)) {
      if(rep_tell) write("Couldn't find "+ CAP(rep_tell) + " on the game.\n");
      else write("Couldn't find that player on the game.\n");
      return 1;
   }
   tell_player(rep_tell + " "+ str);
   return 1;
}
...
make_shades(str) {
   if(!str) {
      if(present("shadow-shades", TP)) {
         write("You already have a pair of shades.\n");
         return 1;
      }
      move_object(clone_object(SHADE_FILE), TP);
      write("You open your palm, and a blackness gathers.\n");
      write("The blackness comes together, and forms a pair of sunglasses.\n");
      say(CN+" holds out "+ query_possesive() + " palm, and a blackness"+
          " gathers.\n");
      say("The blackness comes together, and forms a pair of sunglasses.\n");
      return 1;
   }
}
...
other_who() {
   ...
   while(n < sizeof(pepl)) {
        if(!(pepl[n]->query_invis()&&pepl[n]->query_level() > LEVEL)&&
         ...
         if(!guild) guild = "None";
         if(guild == "Black Circle Mage") guild = "BC Mage";
...
show_news() {
   string news;
   news = read_file(NEWSFILE);
   if(news) write(news);
      else write("No news (is good news).\n");
   return 1;
}
...
drain_monster(str) {
   object att;
   if(TP->query_attack()) str = (TP->query_attack())->query_real_name();
   if(!str) str = 0;
   if(!time_check()) return 1;
  att=find_living(str);
   if(!att)  
   ...
   if(att&&!att->query_attack()) {
    TP->attack_object(att);
     att->attack_object(TP);
  }
   TP->spell_object(att,"drain",drain_damage,0);
   spell_time = time();
   return 1;
}
...
do_mark(per) {
   ...
   if(guild_status == -1) {
      write("That person has been revoked from the shadow guild and cannot "+
            " be given\na mark.  Contact blue if there are problems.\n");
      return 1;
   }
   if(guild_status == 0) {
      write("That person has been suspended from the shadow guild.\n");
      write("You must unsuspend him/her first.\n");
      write("Mail Blue if there are any problems.\n");
      return 1;
   }
...
suspend_mark(str) {
   object shadow, mark;
   if(!str) return 0;
   if(str == "pathfinder" || str == "quicksilver") {
      write("You fail to banish your leader.\n");
      write_file(PATH+"logs/suspend",CN+"attempted to suspend Blue at "+
           ctime(time())+"\n\n");
     return 1;
   }
  ...
  if(!present("shadow-mark",shadow)) {
   ...
   mark = present("shadow-mark",shadow);
   mark->suspend_me();
   write_file(PATH+"logs/suspend", CN+" suspended "+shadow->query_name()+" on "+
      ctime(time())+"\n\n");
   write("You suspendd "+CAP(shadow->query_name())+" from the shadow guild.\n");
   return 1;
}
...
revoke_mark(str) {
   object shadow, mark;
   if(!str) return 0;
   if(str == "quicksilver" || str == "pathfinder") {
      write("You fail to banish your leader.\n");
  ...
  if(!present("shadow-mark",shadow)) {
   ...
   mark = present("shadow-mark",shadow);
   mark->suspend_me();
   write_file(PATH+"logs/revoke", CN+" revoked "+shadow->query_name()+
   " "+  ctime(time())+"\n\n");
...
echo_logo() {
   ...
   size = sizeof(people);
   n = 0;
   while(n < size) tell_object(people[n++], BOLD+"\n"+logo+NOSTYLE+"\n");
   write("Done.\n");
   return 1;
}
/* COMMAND TO LEAVE GUILD -Bp */
leave_shadow() {
     int gxp,gra,exp;
     gxp=this_player()->query_guild_exp();
     gra=this_player()->query_guild_rank();
     exp=this_player()->query_experience();
     this_player()->add_guild_exp(-gxp);
     this_player()->add_guild_rank(-gra);
     this_player()->set_guild_name(0);
     this_player()->add_exp(-exp/6);
     write("You lose 1/6 of your experience for leaving a guild.\n");
     write("You are no longer in the shadows.\n");
     destruct(this_object());
     return 1;
}
