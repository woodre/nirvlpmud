...
init() {
  ...
  add_action("use", "use");
  add_action("use", "read");
  add_action("memorize", "learn");
  add_action("memorize", "memorize");
  add_action("stats", "stat");
  add_action("set_nasty", "setnasty");
#endif
}
...
*/
   if (random(100) < prob) {
        ...
        call_other(nasty, "set_alias", "nasty");
        call_other(nasty, "set_level",14+random(6));
        call_other(nasty, "set_hp", 120 + (14+random(6))*10);
...
*/
    ...
    if(!player->is_player() || !player->query_interactive()) {
      nasty(110);
      return 1;
     }
    if (arg=="heal") i=8;
