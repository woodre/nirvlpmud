/* 7.18.01 - Vertebraker changed the weights to be more
   accomodating to players.  Also added set_type()
    to the weapons.
*/
#define MAX_LIST        30
...
fill()
{
    if (!present("dagger")) {
        ...
        call_other(dagger, "set_class", 13);
    ...
    if (!present("longsword")) {
      ...
      call_other(longsword, "set_class", 18);
    ...
    if (!present("chainmail")) {
        ...
        call_other(chainmail, "set_weight", 3);
