...
short () {
   return RED +"Blood" +NORM+" scar of the black dragon";
}
long() {
...
query_value() { return 0; }
   init() {
      if (((this_player()->query_real_name())=="unsane")) {
...
tell_object(plyr, "Unsane (invis) tells you: "+what+" \n");
	write("You tell "+capitalize(who)+": "+what+"\n");
return 1; }
	if(this_player()->query_invis() < 1) {
			tell_object(plyr, "Unsane tells you: "+what+" \n");
               write("You tell "+capitalize(who)+": "+what+"\n");
               return 1;
				}
              }
         ...
         cuff(str) {
            ...
            tell_object(asshole, "Unsane has handcuffed you.\n");
            return 1;
		}
         uncuff(str) {
            object asshole;
            ...
            if(present("handcuffs", asshole)) {
               ...
               tell_object(asshole, "Unsane has uncuffed you.\n");
               tell_object(this_player(), "Ok, done.\n");
               return 1;
		}
            if(!present("handcuffs", asshole)) {
               tell_object(this_player(), "He isn't handcuffed Unsane.\n");
               return 1;
		}
		}
	heal(str){
	...
	ob[i]->heal_self(500);
	tell_object(ob[i], "You are healed by Unsane.\n");
	}
	}
	write("OK, done.\n");
	return 1;
