...
extract_args(string arg, object *ppl)
{
    ...
    i = sizeof(ppl);
    while (i--) {
	if ((victim = ppl[i]) && victim->id(name)) {
	    break;
	}
    }
    if (!victim)
...
int
do_cmd( string verb, string args, string chan, object *ppl )
{
    mixed *dat, *argv;
    string adverb, *msgs;
...
#ifdef NO_MAPPINGS
string *query_feelings() { return atmos[0]; }
#else
string *query_feelings() { return m_indices(atmos); }
#endif
