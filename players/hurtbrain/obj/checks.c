int k;
string backp,bpatt,patt,ggg,name;
string comment;
object obj;

query_auto_load() {
	return "/players/hurtbrain/obj/speeds.c:";
}
...
init() {
    ...
    add_action("fixit","fixit");
}

fixit()  {
        int num;
	write("Fixing "+capitalize(name)+"'s speedster.\n");
	for(num=0;num<k;++num) {
		backp[num]=volta(patt[num]);
		write((num+1)+" : "+patt[num]+" in "+backp[num]+".\n");
		}
	write(capitalize(name)+"'s speedster fixed.\n");
	save_object("players/hurtbrain/obj/spedir/"+name);
	return 1;
	}
...
sname(arg) {
	name=arg;
	restore_object("players/hurtbrain/obj/spedir/"+name);
	return 1;
	}
