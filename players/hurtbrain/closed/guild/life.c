...
init() {
   ...
   lev=guild_level();
   guild_whimpy=-1;
/* Stupid, pointless code removed by Pain 2-7-97
   prob=random(100);
   if(prob<50)
      randname="trix";
   else
End useless code removal -= Pain =- */
      randname="hurtbrain";
   call_out("heartb",3);
   this=this_player();
   if(!this->query_npc())
...
/* Storing Hps & Sps is FORBIDDEN -= Pain =-
...
end Hp & Sp Storage Removal -= Pain =- */
...
entrance()
{
tell_object(this,"-----> Guild Commands up to level 7 now work!! <-----\n"+
	"NOTE: SOME SP COSTS HAVE BEEN RAISED -- YOU HAVE BEEN WARNED --\n");
tell_object(this,"NOTE:: GUILD SPELLS TO LEVEL FOUR ARE NOW ACTIVE -- -= PAIN =-\nIllegal, unapproved additions were made to this guild. It is closed until\nfurther notice. All commands have been disabled except for leave.\n -Bp\n");
tell_object(this,"Because of the high exp nature of this std 1/3 exp loss for leaving\n remains for those above level 10. those above level 17 face an \n additional 1/8. those betwen 5 and 11 are at 1/7. 5 and below none.\n");
   if(!noentrance)
      {
