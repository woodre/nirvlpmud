...
#define FILENAME "players/hurtbrain/ball"
...
if(LEVEL >= ARCH) {
  ...
  write("hgo <string>   :  teleports you into <string> \n");
  ...
  write("trans         :  trans a player to you\n");
...
write("Clone (str)   :  clone something\n");
  write("patch\n");
...
trans( str ) {
   ...
   tell_object( who, "\nYou think your mind would explode! Hurtbrain summons you.\n" );
   tell_room(  env, "\nHurtbrain snaps his fingers and " +who->query_name() +
       " appears at his feet.\n");
        move_object( who, env );
...
out_clone(str)  {
  object obj;
  if(!str)  {
         notify_fail("Usage: Clone <str>.\n");
         return 0;
         }
   obj=clone_object(str);
 if(!obj)  {  
         notify_fail("No file with that name.\n");        
return 0;
}
