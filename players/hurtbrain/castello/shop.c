...
ONE_EXIT("/players/hurtbrain/castello/entrance", "east",
         "The shop",
"You are in Hurtbrain's shop. You can buy or sell things here.\n" +
"Commands are: 'buy item', 'sell item', 'sell all', 'list', 'list weapons'\n"+
"'list armors' and 'value item'.\n" +
"To the east you see a strange looking machine.\n", 1)
...
do_sell(ob) {
    ...
    call_other("/players/hurtbrain/castello/store", "store", ob);
    return 1;
}
...
value(item) {
    ...
    if (!name_of_item) {
        if (call_other("/players/hurtbrain/castello/store", "value", item))
            return 1;
        write("No such item ("); write(item); write(") here\n");
...
buy(item) {
    if (!item)
        return 0;
    call_other("/players/hurtbrain/castello/store", "buy", item);
    return 1;
}

north() {
    if (call_other(this_player(), "query_name", 0) != "Hurtbrain") {
    ...
    call_other(this_player(), "move_player","north#players/hurtbrain/castello/store");
    return 1;
}

list(obj) {
    call_other("/players/hurtbrain/castello/store", "inventory", obj);
    return 1;
}
