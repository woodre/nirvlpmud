...
south() {
   call_other(this_player(), "move_player", "leaves#/players/hurtbrain/castello/shop");
   return 1;
}
...
reset(arg) {
   ...
   if (!present("dagger")) {
      object dagger;
      dagger = clone_object("/players/hurtbrain/castello/dag/dagger");
      move_object(dagger, this_object());
   }
   if (!present("buckler")) {
      object buckler;
      buckler = clone_object("/players/hurtbrain/castello/dag/buckler");
      move_object(buckler, this_object());
   }
   if (!present("trident")) {
      object trident;
      trident = clone_object("/players/hurtbrain/castello/dag/trident");
      move_object(trident, this_object());
   }
   if (!present("helm")) {
      object helm;
      helm = clone_object("/players/hurtbrain/castello/dag/helm");
      move_object(helm, this_object());
   }
   if (!present("mace")) {
      object mace;
      mace = clone_object("/players/hurtbrain/castello/dag/gaul/mace");
      move_object(mace, this_object());
   }
   if (!present("tunic")) {
      object tunic;
      tunic = clone_object("/players/hurtbrain/castello/dag/tunic2");
      move_object(tunic, this_object());
   }
   if (!present("skin")) {
      object skin;
      skin = clone_object("/players/hurtbrain/castello/dag/gaul/skin");
      move_object(skin, this_object());
   }
   if (!present("tusks")) {
      object tusks;
      tusks = clone_object("/players/hurtbrain/castello/dag/gaul/tusks");
      move_object(tusks, this_object());
   }
   if (!present("necklace")) {
      object amulet;
      amulet = clone_object("/players/hurtbrain/castello/dag/necklace");
      move_object(amulet, this_object());
   }
   if (!present("boots")) {
      object boots;
      boots = clone_object("/players/hurtbrain/castello/dag/gaul/boots");
      move_object(boots, this_object());
   }
   if (!present("ring")) {
      object ring;
      ring = clone_object("/players/hurtbrain/castello/dag/ring");
