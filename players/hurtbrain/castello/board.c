...
id(str) {
    return str == "suggestion board" || str == "board" || str == "suggestionboard";
}

long() {
    write("This is Hurtbrain's castle board. If you have any idea for a\n");
    write("funny or useful object post it here. I'll see if i can make it.\n");
    ...
    write("The suggestion board contains " + num_messages);
    if (num_messages == 1)
        write(" note:\n\n");
    else
        write(" notes:\n\n");
    say(call_other(this_player(), "query_name") +
          " studies the suggestion board.\n");
    headers();
}

short() {
    return ("A suggestion board(" + num_messages + " msgs)");
}
...
reset(arg) {
    if (arg)
	return;
    restore_object("players/hurtbrain/castello/b_dir/sugg");
}
...
get_body(str) {
	...
	new_body = 0;
	new_hd = 0;
        save_object("players/hurtbrain/castello/b_dir/sugg");
	write("Ok.\n");
	who = 0;
	return;
    }
