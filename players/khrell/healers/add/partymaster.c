...
#define PARTYCHAN "[ "+GREEN+PartyName+OFF+" ] "
...
PartyAdd( string name, string member_name, string partyname ){
   ...
   if(member_name == this_player()->query_real_name()){
      ...
      if(!(members = Parties[name])){
         ...
         tell_object(member, PARTYCHAN+"You have started "+partyname+
            " with "+member->query_level()+" shares.\n");
         return 1;
      }
   ...
   if(!member->is_player() && !member->is_kid()){
      write(capitalize(member_name)+" can not be added to the party.\n");
      return 1;
   }
   ...
   if (member_array(member, members) < 0) {
      ...
      if(member->query_guild_name() == "healer") new_object->SPartyShares(0);
	  else new_object->SPartyShares(member->query_level());
...
ShareExp(int amount,string name,object killer){
   object *members;
   object *membershere;
   int *shareshere;
   int expshare;
   ...
   for(j=0;j<sizeof(membershere);j++){
     if(membershere->query_guild_name() != "healer") {
      membershere[j]->add_exp(expshare*shareshere[j]);
      tell_object(membershere[j],PARTYCHAN+RED+"You have gained "+(expshare*shareshere[j])+" experience."+OFF+"\n");
      present("party_object",membershere[j])->APartyExperience(expshare*shareshere[j]);
	 }
   }
}
...
PartyStatus(string name, string partyname){
   ...
   write("\n\n");
   for(i=0;i<sizeof(members);i++){
      write(justify(members[i]->query_name(),15));
...
PartyLeave(string name, object member){
   ...
   if(previous_object()->QPartyFollower()){
      for(i=0;i<sizeof(members);i++){
         PartyName = previous_object()->QPartyName();
         tell_object(members[i], PARTYCHAN+member->query_name()+" has left the Party.\n");
      }
      Parties[name] -= ({ member });
...
PartyShare(string name, string member_name, int number){
   ...
   if(member->query_guild_name() == "healer") {
      write("A healer can not gain any shares.\n");
   return 1; }
   level = member->query_level();
   if(number > level){ 
      write("You can not give "+capitalize(member_name)+" more than "+
         level+" shares.\n");
      return 1;
   }
   if(number < (level - 4)){
      write("You can not give "+capitalize(member_name)+" less than "+
         (level - 4)+" shares.\n");
      return 1;
   }
   if(number < 1){
      write("You can not give anyone less than 1 share.\n");
      return 1;
   }
