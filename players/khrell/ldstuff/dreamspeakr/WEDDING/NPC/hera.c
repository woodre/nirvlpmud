...
marry(str) {
  ...
  if(str=="help") {
    if(!efun::query_verb())
      say("\n");
    say(format(HIM+ "Hera says"+NORM+": I will marry you and your beloved for the " +
  ...
  if(str == "hera"){
    say (HIM+ "Hera falls on the floor laughing"+NORM+":  You fool, Zeus would KILL you!\n");
    return 1;
  }
  if(!present(str)) {
    say(format(HIM+ "Hera says"+NORM+": I'm sorry, but that person isn't here.\n"));
    return 1;
  }
  if(!find_player(str) ) {
  ...
  if(!ceremony_in_progress)
    ...
    say(format(HIM+ "Hera says"+NORM+": I am already performing a ceremony.\n"));
  return 1;
}
...
me->add_money(-1000);
  tell_room(environment(), HIM+ "Hera says"+NORM+":  Friends we are gathered here at this hour to\n");
...
ceremony_two() {
  tell_room(environment(),  HIM+ "Hera says"+NORM+":  We have come to join " + capitalize(mate) + " and " + capitalize(spouse) + "\n");
...
ceremony_three() {
  tell_room(environment(), HIM+ "Hera continues"+NORM+":  The essence of this commitment is not to be\n");
...
ceremony_four() {
  tell_room(environment(), HIM+ "Hera says"+NORM+":  The physical union of marriage is one of life's\n");
...
ceremony_five() {
  tell_room(environment(), HIM+ "Hera says"+NORM+":  The day-to-day companionship, the pleasure of \n");
...
ceremony_six() {
  tell_room(environment(), HIM+ "Hera continues"+NORM+":  Marriage symbolizes the intimate sharing\n");
...
ceremony_seven() {
  tell_room(environment(), HIM+ "Hera says"+NORM+":  We are here today to celebrate the love which \n");
...
ceremony_eight() {
  tell_room(environment(), "\n");
  tell_room(environment(), HIM+ "Hera says"+NORM+":  Do you, " + capitalize(mate) + ", offer " + capitalize(spouse) + " not only the\n");
...
ceremony_nine() {
  tell_room(environment(), "\n");
  tell_room(environment(), HIM+ "Hera says"+NORM+":  Do you, " + capitalize(spouse) + ", offer " + capitalize(mate) + " not only the\n");
  tell_room(environment(), "summer of your life but the autumn, brisk and vibrant?  Do you\n");
...
*/
  tell_room(environment(), "\n");
  tell_room(environment(), HIM+ "Hera says"+NORM+": Then by the power vested in me by Dreamspeakr the\n");
  tell_room(environment(), "Shaman and the gods of Nirvana, I join you in the bonds of \n");
  tell_room(environment(), "matrimony.\n");
  tell_room(environment(), "\n");
  tell_room(environment(), HIM+ "Hera says"+NORM+": Take these rings as symbols of your enduring\n");
...
stop_ceremony() {
  say("\n" +HIM+ "Hera says"+NORM+": Since you have not answered I will " +
   "end the ceremony.\n");
  say(prompt);
  mate=0; spouse=0; ceremony_in_progress=0;
  mate_ob=0; spouse_ob=0; mate_yes=0; spouse_yes=0;
