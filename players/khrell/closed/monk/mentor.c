...
if(!get_spouse()) {
   ...
   return 1;
     ...
     tell_object(ob,this_player()->query_real_name()+" healed you "+amt+" points.\n");
...
if(!present(str,this_player())) {
   ...
   gary = present(str,this_player());
   if(gary->query_drop() == 1) {
     write("You can not throw that object.\n");
     return 1;
   }
  write("You throw "+str+" to "+spouse+".\n");
  this_player()->add_spell_point(-5);
  tell_object(ob,this_player()->query_real_name()+" throws you a "+str+".\n");
move_object(gary,ob);
  return 1;
}
