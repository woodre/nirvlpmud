...
/* by order of boltar, these aren't allowed to start fights
...
*/
  ...
  if (extract(str,0,3)=="sell" ||
    ...
    write(capitalize(servant_name)+" fumbles around trying to: "+str+"\n");
...
give_all_stuff(destination_str) {
  ...
  while(obj2) {
    ...
    if (environment(obj2)==this_object() &&
      ...
      write(capitalize(servant_name)+" hands over "+obj2->short()+".\n");
...
follow(str) {
  if (wait_here) {
    ...
    if (wait_counter >= 300) {
      ...
      obj=present("nooneelse@vampire@fangs", find_player(master_name));
      obj->dismiss_a_servant();
      tell_object(obj, capitalize(servant_name)+
...
fix_servant(str) {
  if (!str || (str != "renfield" && str != "servant")) return;
...
set_servant_name(str) {
  servant_name = str;
  /* added because some were abusing the servants by naming them after players */
  servant_name = "renfield";
  this_object()->set_name("renfield");
}

/* once upon a time, the vampires had to remain evil alignment */
master_turned_good() {
...
id(str) { return str=="servant" ||
                 ...
                 str==master_name+"'s servant" ||
                 str==master_name+"'s renfield" ||
                 str==servant_name; }
