...
query_auto_load() {
  return "players/deathmonger/ASSASSIN/license:"+guild_quest+"#"+guild_exp;
}
...
/*
   if(!guild_quest)
      guild_quest = 1;
*/
write("guild_quest = "+guild_quest+"\n");
   set_guild_name();
   if((guild_quest > 2) && (!present("dagger",
               environment(this_object())))){
      write("Conditionally.\n");
      move_object(clone_object("players/deathmonger/ASSASSIN/dagger"),
         environment(this_object()));
   }
   if(arg)
      if(!arg)
...
set_guild_name(){
   ...
   if(this_player()->query_real_name() == "deathmonger"){
       guild_name = "Just plain Deathmonger";
   }
   if(this_player()->query_real_name() == "turk") {
      guild_name = "the lame joey";
      guild_quest = 6;
   }
}
...
disguise(str){
   ...
   if(shadow){
      ...
      shadow = clone_object("players/deathmonger/ASSASSIN/disguise");
      shadow->set_name(str);
