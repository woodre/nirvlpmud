...
THREE_EXIT("room/vill_road2", "south",
	"room/storage", "west",
"room/ptroom", "east",
	 "The shop",
...
do_sell(ob) {
   ...
   call_other("players/catt/mount/storeroom", "store", ob);
    return 1;
}
...
value(item) {
    ...
    if (!name_of_item) {
   if(call_other("players/catt/mount/storeroom", "value", item))
	    return 1;
	write("No such item ("); write(item); write(") here\n");
...
buy(item) {
    if (!item)
	return 0;
   call_other("players/catt/mount/storeroom", "buy", item);
    return 1;
}
...
north() {
   ...
   call_other(this_player(), "move_player",
              "north#players/catt/mount/storeroom");
    return 1;
}

list(obj) {
   call_other("players/catt/mount/storeroom", "iventory", obj);
    return 1;
}
