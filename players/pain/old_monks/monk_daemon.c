...
/*                                                                      */
/*  Version 2.1 Pain 4 OCT 2002                                         */
/*      * Changed the monk channel & removed some bs functions          */
/*                                                                      */
/*----------------------------------------------------------------------*/
...
register() {
    ...
    object ob, new_robe, oldrobe;
    int rank, exp, sh, stam, loan, wat, wmp, myac, muffled;
    string wir;
    ...
    new_robe->set_color(ROBE->query_color());
    ...
    destruct(oldrobe);
    if(ROBE->query_color() == 1)
        write(CYA+"Your robes have been mended."+END+"\n");
    else
...
mcolor(str){
	if(!str && ROBE->query_color() == 0){
		write("Color On.\n");  ROBE->set_color(1);  return 1; }
	if(!str && ROBE->query_color() == 1){
		write("Color off.\n");  ROBE->set_color(0);  return 1; }
	if(str == "on" || str == "1"){
		write("Color on.\n");  ROBE->set_color(1);  return 1; }
	if(str == "off" || str == "0"){
		write("Color off.\n");  ROBE->set_color(0);  return 1; }
...
monk_tell(str) {
   ...
   if(ROBE->query_muffled() == 1) { return 1;}
   if(str[0] == ':')  {
      message = CYA+"/*>"+END+BOLD+"monk"+END+CYA+"<*\\"+END+" "+BOLD+TPN+END+" "+str[1..strlen(str)];
      message2 = "/*>monk<*\\ "+TPN+" "+str[1..strlen(str)];
   }
   else {
      message = CYA+"/*>"+END+BOLD+TPN+END+CYA+"<*\\:"+END+" "+str;
      message2 = "/*>"+TPN+"<*\\: "+str;
   }
   "/obj/user/chistory.c"->add_history(channel,message2+"\n");
   ob = users();
   for( x = 0; x < sizeof(ob); x += 1) {
      if(present("mrobe", ob[x])){
         patch = present("mrobe", ob[x]);
...
list_users() {
   ...
   str = "Monk channel listeners : ";
   people = users();
   for (index = sizeof(people) - 1; index >= 0; index--)
   if(present("mrobe", people[index]) && present("mrobe",people[index])->query_muffled() == 0)
