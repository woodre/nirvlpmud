...
long() {
 write("This is the Monk Guild board where you can post notes\n"+
        "for other guild members to read. NOTE: There is another\n"+
        "board in the equipment room where Spell Ideas and such\n"+
        "should be posted (dont post them here).\n");
...
reset(arg) {
    if (arg)
	return;
    restore_object("players/hawkeye/closed/monk/notes");
}
...
capitalize(this_player()->query_name())+" drags the board off into\n"+
 "the corner to start writing a note.\n");
  move_object(this_player(), "players/hawkeye/closed/guild/note_room");
   move_object(this_object(), "players/hawkeye/closed/guild/note_room");
    new_hd = hd;
    input_to("get_body");
...
" : "+ctime()+
	...
	new_body = 0;
	new_hd = 0;
     save_object("players/hawkeye/closed/monk/notes");
	write("Ok.\n");
write("You pull the guild board back into the room so everyone\n"+
      "can admire your handy work.\n");
move_object(this_player(), "players/hawkeye/closed/guild/guild_room");
move_object(this_object(), "players/hawkeye/closed/guild/guild_room");
	who = 0;
	return;
    }
