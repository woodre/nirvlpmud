/* oks... This weapon has once again become a little out of proportion
   max wc 19? (its a guild weapon.. i.e. savable weapon)  does
   1 - 2 special attacks per round....each attack doing a random(max of 5)?
   i.e. max of wc 19 + return 8?
   i dont think so.
   i have set the max wc to be 17 so that max is wc 17 + return 8 savable
   - Mythos <6-19-96>
*/
...
if(me->query_guild_rank() > 5) ob->set_class(16);
if(me->query_guild_rank() > 7) ob->set_class(17);
