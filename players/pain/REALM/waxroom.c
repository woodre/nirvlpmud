 ...
 * ON_SEARCH
 *
 * Meant to be overridden if there's something to actually find.
 *
*/

int on_search(string arg) {

  if (!arg) {
    write("You search the area thoroughly, but don't turn up anything.\n");
    return 1;
  }

  write("You search the "+arg+" thoroughly, but don't turn up anything.\n");
  return 1;
}

int do_search(string arg) { return on_search(arg); }
...
void set_long(string arg) { long_desc = line_wrap(arg); }
...
string *query_items() { return m_indices(Descs); }
string *query_exits() { return Dirs; }
string *query_dests() { return Dests; }
...
void long() {
  ...
  if (long_desc) {
    if (this_player() && (int)this_player()->query_brief() != 2)
      write(short() + "\n");
