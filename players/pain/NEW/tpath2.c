...
reset(arg) {
    ...
    no_castle_flag = 0;
    long_desc =
        "You are well northwest of where the path splits.\n"
        ...
        + "To the far southeast there is a meeting of two paths.\n";

items = ({
	"path", "A narrow dirt path running through the forest",
	"cave", "A cave nestled in amoungst the trees to the northwest",
	"trees", "Tall trees line either side of the path",
});
    dest_dir =
        ({
        "players/pain/NEW/tpath3", "north",
	"players/pain/NEW/tpath1", "southeast",
        });
if(!present("footman"))
{
