...
id ( string str ) {
if(hidering != "5") return str == "ring" || str == "ND"  || str == "wedding ring";
return str == "ring" || str == "ND";
}
...
long() { 
  if(this_player()->query_gender() == "female" ) {
     write(format("You see a delicately wrought band of platinum set with" +
      " a large, blue-white diamond. It is engraved with "+spouse_name + "\n\n"));
  }
  else {
    write(format("You see a wide band of pure platinum, simple and " +
     "elegant. It is engraved with "+spouse_name + "\n\n"));
  }
...
short () { 
object bbb;
if(hidering == "5") return 0;
bbb=environment(this_object());
if(bbb)
  return "A wedding ring (worn)";
}
...
query_auto_load () {
  return "players/bastion/closed/ringz:" + spouse_name;
}
...
ring_chat ( string str ) {
   ...
   if( spouse && find_player(spouse) ) {
      call_other(find_player(spouse),"add_tellhistory",mess);
...
aux_ring_chat ( string str ) {
  if(spouse && find_player(spouse)){
    ...
    call_other(find_player(spouse),"add_tellhistory",format(str));
   }
}
...
restore ( string str ) {
   ...
   new = clone_object("/players/bastion/closed/ringz.c");
...
renew_my_ring () {
   object ob;
   ob = clone_object("/players/bastion/closed/ringz.c");
