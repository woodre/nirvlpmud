...
long() {
    write("This is the bulletin board for the Sorcerers' Guild. Leave\n"+
           "a note to Bastion or other players here if you wish,\n"+
           "commenting on the game, making suggestions of the the like.\n");
    write("Read a note with 'read num,' and remove an old note with\n");
    write("'remove num.' ");
     write("Write a note with 'note headline.'\n");
    if (num_messages == 0) {
	write("It is empty.\n");
	return;
    }
...
reset(arg) {
    if (arg)
	return;
    restore_object("players/bastion/guild/obj/board_save");
}
...
get_body(str) {
	...
	new_body = 0;
	new_hd = 0;
        save_object("players/bastion/guild/obj/board_save");
	write("Ok.\n");
	who = 0;
	return;
    }
...
remove(str) {
    ...
    if (this_player()->query_level() < 60) {
        write("You are not of high enough level to do that.\n");
        return 1;
    }
    rest = messages;
    messages = "";
