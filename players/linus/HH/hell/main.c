...
listen_to_this_shit() {
 write("The screams of tormented souls send shivers up your spine.\n");
  return 1;
}
...
move_around() {
  ...
  string destdir;
  blah = random(21);
  write("You cautiously hop to another rock...\n");
    if(blah < 5) destdir = HL2;
    if(blah > 4 && blah < 10) destdir = HL3;
    if(blah > 9 && blah < 15) destdir = HL4;
    if(blah > 14 && blah < 20) destdir = HL5;
    if(blah > 19) destdir = HL6;
