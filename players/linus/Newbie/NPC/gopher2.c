...
id(str) { return str=="fat_gopher" || str=="gopher"; }
 reset(arg) {
  ::reset(arg);
  if(arg) return;
set_name("gopher");
set_short("A fat gopher");
set_long("A fat gopher that lives in Rabbit's garden.  It glances\n"+
 "up at you as it casually munches on a carrot.\n");
set_race("rodent");
set_level(5);
set_wc(9);
set_ac(4);
