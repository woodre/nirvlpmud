...
Author: Feldegast
Date: 5/07/98
      2/16/01 Made additions/added comments
Description:
  A cloak with teleport capabilities.  Although it does not check
for a chance of failure, it is on a very difficult monster.  See
arien.c.
*/
#define VOID "/players/feldegast/realm/void.c"
#define DEST "/players/feldegast/realm/teleport.c"
...
reset(arg) {
  ...
  if(arg) return;
  set_name("cloak");
  set_alias("midnight cloak");
  set_type("misc");
  set_ac(1);
  set_short("The "+HIB+"Midnight"+NORM+" Cloak");
  set_weight(1);
  set_value(1200);
...
*/
  ...
  move_object(TP,VOID);
  call_out("move_again",4,this_player());
  return 1;
}

void move_again(object targ) {
  tell_room(environment(targ),(string)targ->query_name()+" disappears.\n",({ targ }));
  move_object(targ,DEST);
  command("look",targ);
  tell_room(environment(targ),(string)targ->query_name()+" arrives.\n",({ targ }));
}

do_special(owner) {
  if(owner->query_attrib("ste") > random(200)) {
    tell_object(this_player(),
      HIB+"Your weapon becomes entangled in "+owner->query_name()+"'s cloak!"+NORM);
    write(HIB+"The Midnight Cloak envelopes you, protecting you from harm!"+NORM);
    return 3;
  }
}
