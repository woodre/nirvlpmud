...
reset(arg) {
  ...
  venom = 0; web = 0;
  if(arg) return;
  move_object(clone_object(OBJ_PATH+"wand-spiders.c"), this_object());
  set_name("arachnia");
  set_alt_name("widow");
  set_alias("sorceress");
  ...
  load_chat("Arachnia says: I could just eat you up!\n");
  set_a_chat_chance(5);
  ...
  set_wc_bonus(8); /* For the spiders and the poison. */
  set_ac_bonus(3); /* For the webbing. */
}
