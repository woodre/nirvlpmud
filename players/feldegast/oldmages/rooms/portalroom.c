...
#define PATH "/players/wocket/mages/rooms/"
inherit ROOM;
...
int cmd_intone(string str) {
  ...
  say(TPN+" carefully intones: \""+str+"\".\n");
  while(remove_call_out("open_portal")!=-1) ;
  while(remove_call_out("change_portal")!=-1) ;
  if(!destination) {
    destination=new_dest;
    if(TP->query_sp() < 10) {
      write("You do not have magical energy.\n");
      return 1;
    }
    else TP->add_spell_point(-10);
...
cmd_enter(str) {
  ...
  if(!portal) return 0;
  say(TPN+" steps into the "+BOLD+CYN+"azure"+NORM+" portal.\n");
  write("You step through the "+BOLD+CYN+"azure"+NORM+" portal.\n");
  move_object(this_player(),destination);
  say(TPN+" steps into the room through an "+BOLD+CYN+"azure"+NORM+" portal.\n");
