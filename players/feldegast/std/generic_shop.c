#define STORAGE "/players/feldegast/std/storage.c"
...
void reset(int arg) {
  if(!storage) storage=clone_object(STORAGE);
}
...
value(item) {
    ...
    if (!name_of_item) {
	if (call_other(storage, "value", item))
	    return 1;
	write("No such item ("); write(item); write(") here\n");
...
buy(item) {
    if (!item)
	return 0;
    call_other(storage, "buy", item);
    return 1;
}

list(obj) {
    call_other(storage, "inventory", obj);
    return 1;
}
