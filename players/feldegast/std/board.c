...
get_body(str) {
	...
	new_hd = ctime(time())+" "+new_hd + "(" + capitalize(name) +
	    ")";
...
read(str) {
	...
	return 0;
    if (i > num_messages) {
	write("There aren't that many messages.\n");
	return 1;
    }
    rest = messages;
    while(rest != 0 && rest != "") {
