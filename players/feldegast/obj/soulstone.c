...
*/
/*  [changes] 2005.03.10 -Forbin
//  upped the sp heal from random(40) to 40+random(40) and made so if
//  amt is greater than remaining power, it sets amt to remaining power
*/
...
/* [added] 2005.03.10 -Forbin added so soulstone can start out charged when player finds it */
set_power(int num) { power = num; }
...
int cmd_heal(string str) {
  int amt;
  amt = 40+random(40); /*  [change] 2005.03.10 -Forbin random(40) to 40+random(40)  */
  if(amt > power && power > 0)
    amt = power; 
  else if(power == 0) {
    notify_fail(
      "You whisper to the stone, asking it for power.  Nothing happens.\n");
    return 0;
  }
  if(used) {
