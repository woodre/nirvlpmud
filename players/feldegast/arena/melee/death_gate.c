...
get() { return 1; }
init() {
  if(environment()) master=environment()->query_master();
...
cmd_enter(str) {
  ...
  for(i=0; i<sizeof(world);i++) {
    if(environment(world[i]) && environment(world[i])->query_master() == master &&
  ...
  if(is_open) {
    ...
    move_object(TP,ROOT+"arena/rooms/inner");
    command("look",TP);
    say(TPN+" steps out of the vortex.\n");
    master->remove_player(this_player());
