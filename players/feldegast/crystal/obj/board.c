...
#define FILE "players/feldegast/crystal/obj/board"
...
long() {
    write("A bulletin board containing notes and announcements is on the wall here.\n"+
          "You may: "+BOLD+"note <Title>"+NORM+" to post a new announcement, "+BOLD+"read #"+NORM+", or "+BOLD+"remove"+NORM+" an old note.\n");
	...
	write(" notes:\n\n");
    say(call_other(this_player(), "query_name") + " studies the book.\n");
    headers();
}
...
get() {
    write("It is secured to the wall.\n");
    return 0;
}
...
get_body(str) {
    if (str == "**") {
        new_hd = BOLD+"[" +NORM + ctime(time()) + BOLD + "]" + NORM + cutstr(new_hd) + " ( " + BLU + this_player()->query_name() + NORM + " )";
...
remove(str) {
	...
	if (i == 0) {
            if(sscanf(hd, "%sfeldegast", jun) == 1 && this_player()->query_real_name() !="feldegast") {
