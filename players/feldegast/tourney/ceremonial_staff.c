...
process_rooms(str, mixed x) {
  ...
  int i;
  for(i=0; i<10; i++) {
    rooms=({"m2","m2","m2b","m3","m4","m5","m6","m6b","m7","m8"});
    room=rooms[i];
...
"ground.  A few minutes later, he drops you somewhere else.\n");
      move_object(ob,PATH+room_list[random(sizeof(room_list))] );
