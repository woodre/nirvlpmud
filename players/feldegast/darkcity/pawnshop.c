...
reset(arg) {
  if(!present("pawnbroker")){
    object gene;
    gene=clone_object("/players/feldegast/mon/pawnbroker");
    move_object(gene,this_object());
  }
  if(arg)return;
  backroom=PATH+"pawnback.c";
  short_desc="Pawn Shop";
  long_desc=
...
"the marketplace.\n";
  items=({
     "sign",
"The sign says:\n"+
"\tWelcome to Gene Eric(tm) Pawn Shops Ltd.\n"+
