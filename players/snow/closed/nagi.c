...
stab2 = NI+TPN+" rip-slices into "+MEATN+"'s ";
  if(X == 0) {
    write(stab1+"head.\n");
    say(stab2+"head.\n");
   MEAT->hit_player(RAN(ARTR/5)+4);
return 1;
    }
  if(X == 1) {
    write(stab1+"gut.\n");
    say(stab2+"gut.\n");
   MEAT->hit_player(RAN(ARTR/5)+2);
return 1;
    }
  if(X == 2) {
    write(stab1+"chest.\n");
...
call_other(MEAT,"hit_player",-(RAN(ARTR/4)+1));
return 1;
}
...
stab2 = NI+TPN+" bashes the "+NAG+"'s staff into "+MEATN+"'s ";
  if(X == 0) {
    write(stab1+"head.\n");
    say(stab2+"head.\n");
  MEAT->hit_player(RAN(ARTR/7)+4);
return 1;
    }
  if(X == 1) {
    write(stab1+"gut.\n");
    say(stab2+"gut.\n");
  MEAT->hit_player(RAN(ARTR/7)+2);
return 1;
    }
  if(X == 2) {
    write(stab1+"chest.\n");
...
call_other(MEAT,"hit_player",-((ARTR/(RAN(3)+1))+RAN(3)));
return 1;
}
...
call_other(MEAT,"hit_player",-(RAN(ARTR)+RAN(ARTR/3)));
return 1;
}
...
NI+"The vortex surrounding "+TPN+" disappears.\n");
   MEAT->hit_player((ARTR*2)/(RAN(2)+1));
   MEAT->hit_player(RAN(ARTR*2));
return 1;
}
...
nag_multi_hit();
    TR(ETP,
MEATN+"'s battered body falls to the ground and twitches a little, then is still.\n");
   MEAT->hit_player(ARTR+1);
    write(
NI+"You raise your "+NAG+" in victory!\n");
    say(
