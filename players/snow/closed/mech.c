...
#define TON CAP(TO->query_name())
#define FINV first_inventory
#define NINV next_inventory
...
*/
  ...
  if(spec_attack_chance && ENV(attacker_ob) == ENV(TO) ) {
...
mech_kill(arg) {
 object meat;
meat = present(arg,(ENV(TP)));
  if(TP == master) {
   if(!arg) { write("Mkill what?\n"); return 1; }
   if(!meat) {
     write("There is no "+CAP(arg)+" present!\n");
   return 1;
   }
   if(!meat->query_npc()) {
  ...
  if(meat->is_pet()) {
    ...
    return 1; }
   if(meat == TO) {
     write("Your "+TON+" will not attack itself!\n");
   return 1;
   }
    if(killmsg) TR(ENV(TO),killmsg);
...
mech_stuff(arg) {
  ...
  return 1;
    ...
    TR(ENV(TO),CAP(master->query_name())+"'s "+TON+" beeps and hums.\n");
  if(arg == "guard") {
    if(mech_guarding == 0) {
      write("Your "+TON+" is in guard mode.\n");
      mech_guarding = 1;
    return 1;
    }
    if(mech_guarding == 1) {
      write("Your "+TON+" is no longer in guard mode.\n");
      mech_guarding = 0;
    return 1;
    }
  }
  ...
  if(arg == "stop") {
  if(!MEAT) { write("Your "+TON+" is not fighting!\n"); return 1; }
  write("Your "+TON+" stops fighting.\n");
    if(MEAT->query_attack() == TO) {
      MEAT->stop_fight();
      MEAT->stop_fight();
      MEAT->stop_hunter();
     }
  ...
  if(arg == "implode") {
    write("You command your "+TON+" to self-destruct...\n"+"\n");
    TR(ENV(TO),TPN+"'s "+TON+" implodes upon itself and is destroyed!\n");
    move_object(TO, "/players/snow/storage.c");
    destruct(TO);
  return 1;
  }
  if(arg == "repair") {
    write("\tYou link with your "+TON+" and enhance its self-repair\n"+
          "\t\tmechanisms with your own energy.\n");
    TP->add_spell_point(-30);
    TO->heal_self(random(5)+15);
    write("Your "+TON+" beeps and hums happily.\n");
  return 1;
  }
...
sacrifice(arg) {
  object meat;
  int dam;
dam = random(((TO->query_level())*5) + 10);
  if(TP == master) {
    if(!arg) {
      if(!TO->query_attack()) { write("Kami what?\n"); return 1; }
      arg = TO->query_attack();
    }
meat = present(arg,ENV(TO));
    if(!meat) { 
    ...
    if(dam < 20) {
      say(TON+" SMASHES into "+capitalize(arg)+"!\n");
    }
    if(dam > 19 && dam < 40) {
      say(TON+" EXPLODES into "+capitalize(arg)+"!\n");
    }
    if(dam > 39) {
      say(
    "~*~KAMIKAZI~*~\n"+TON+
...
jumpstart() {
  if(TP == master) {
    write("You put your hand on your "+TON+" and jolt it back to life!\n");
...
mech_take() {
    ...
    if(present("implants",TP)) {
      if(find_living("mech"+lower_case(TP->query_real_name()))) {
        if(!find_living("mech"+lower_case(TP->query_real_name())) == TO) {
      ...
      write("You reprogram the "+TON+" to be your mech.\n");
      set_master(TP);
    return 1;
    }
  }
...
amt = random(spec_dam);
  amt = amt/3;
  tell_room(ENV(TO),spec_msg);
    if(damagemsg == 1) {
      tell_room(ENV(TO),get_blunt_msg(amt));
    }
    if(damagemsg == 2) {
      tell_room(ENV(TO),get_sharp_msg(amt));
    }
    if(damagemsg == 3) {
      tell_room(ENV(TO),get_laser_msg(amt));
    }
  CO(MEAT,"hit_player",amt);
return 1;
}
 
mech_life() {
    if(!master || master->query_ghost()) {
     master = 0;
...
mech_follow() {
    if(!present(master, ENV(TO)) ) {
       ...
       if(follow_msg1) tell_room(ENV(TO),follow_msg1);
    ...
    if(attacked) {
     if(!MEAT == attacked) {
       if(attacked->query_npc()) {
         tell_room(ENV(TO),guard_msg);
         TO->attack_object(attacked);
       }
     }
    }
   }
...
mech_oprep() {
  ...
  if(ENV(attacker_ob) == ENV(TO) ) {  /* was  100 now is 10 - mythos  */
...
self_destruct() {
  ...
  if(destruction > 0) {
   ...
   if(ENV(TO))
    TR(ENV(TO), TON+" self destruct in T - "+destruction+" seconds.\n");
  ...
  if(destruction < 1) {
   /*needs to check if we have an environment!!!!!!! -Bp */
   if(ENV(TO))
    TR(ENV(TO), TON+" obliterates itself in a massive explosion!\n");
  move_object(TO, "/players/snow/storage.c");
  destruct(TO);
  return 1;
  }
