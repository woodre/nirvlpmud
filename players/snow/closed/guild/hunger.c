...
reset(arg) {
  ...
  switchs = 1;
  /*
  find_object("players/mythos/closed/fake_beater")->set_beat(this_object());
  */
  call_out("hb",3);
...
/*
  ...
  add_action("ddrink","sip");
     ...
     removed ban on drink and sip because of the 
     change with the intox check in the fake_hb()
    consenquence of taking intox is pretty harsh  - mythos <5-19-97> */
...
#else
  add_action("dark_tell", "=>", 3);
#endif
  took this out because boltar wishes no custon tells
  - mythos 
  */
 ...
 if(present("symbol",ep)) { destruct(present("symbol",ep));}
  ...
  if(ep->query_intoxination() > 0) { 
    if(ep->query_intoxination() > 5) {
       tell_object(ep,"\n\n\t\t Using Intox somehow!?!?!\n\n\n");
       ep->zap_object(ep); ep->zap_object(ep); ep->zap_object(ep);
       ep->zap_object(ep); ep->zap_object(ep); ep->zap_object(ep);
       ep->zap_object(ep); ep->zap_object(ep); ep->zap_object(ep);
        write_file("/players/mythos/closed/guild/INTOX",epn+" "+ctime(time())+"\n");
    }
...
hsch = nn = 0;
  ...
  if(ep) {
  if(environment(ep)) {
  if(creator(environment(ep)) == "boltar") {
  if(environment(ep)->short()) {
    hsch = environment(ep)->short();
    if(sscanf(hsch,"%s hotel room",nn) == 1) is_light = 0; 
  }  } 

  if(!present("mailread",ep)) { 
  
  if(present("dark_cloak",ep)) { is_light = -1; 
   if(set_light(0) < 0) is_light = set_light(0) - 1; }
  
  if(present("weather",environment(ep)) && !aa) { aa = 1; is_light = 0; }
  if(present("myst",environment(ep)) && !b) { b = 1; is_light = is_light - 1; }
  if(present("darkness",environment(ep)) && !c) { c = 1; is_light = is_light - 2; }
  ...
  if(is_light < -2 && ep->query_hp() < ep->query_mhp() ) { tell_object(ep,
                   "You feel a cool touch as the darkness heals you.\n"); }
    if(ep->query_hp() - is_light > 0) { 
    /* was heal_self but because heal self uses hitplayer things get 
       stopped here */
      if(is_light > 5) is_light = 5;
      if(is_light < -5) is_light = -5;
   if(is_light > 0) is_light = is_light + 1;
      ep->add_hit_point(-(is_light)); 
      ep->add_spell_point(-(is_light));}
    else { command("drop all",ep); 
           ep->second_life();  
           log_file("DEATHS",ctime(time())+" "+epn+" "+ep->query_exp()+" exp "+
                             ep->query_money()+" gold killed by the light.\n");
           ep->add_exp(-(ep->query_exp()/2));
           }
  } } }
  is_light = 0; }
call_out("light_dam",3);
...
dquit() {
   ...
   if(environment(tp) && file_name(environment(tp)) != "room/prison") {
...
save_dark() {
  ...
  if(ep->query_level() < 20) {
  if(environment(ep) && file_name(environment(ep)) != "room/prison") {
  if(!present("deathmark",ep) && !present("bold",ep)) {
   ep->set_home("/players/mythos/closed/guild/assembly.c"); }
   if(present("deathmark",ep) && !present("bold",ep)) {
   ep->set_home("/players/mythos/closed/guild/death/death.c"); }
   if(present("bold",ep)) {
   ep->set_home("/players/mythos/closed/guild/task/horlach/home.c"); }
  }}
return 1;}
...
update_me() {
  ...
  if(present("dgc",ep)) { destruct(present("dgc",ep)); }
...
if(d > 13) d = 13;
if(d < 0) d = 0;
if(ep->query_real_name() == "mythos") d = 13;
  glevel = d;
  vs = ep->query_guild_rank();
  ep->add_guild_rank(d-vs);
  if((d > -1 && d <12) || ep->query_real_name() == "mythos") {
  ...
  if(ep->query_real_name() == "mythos") { ctitle = ""; dtitle = "the Prophet"; }
