...
#define BOARD_NAME "players/snow/closed/cyber/boards/tourneyboard"
...
long() {
  int ind;
  write("This is the CyberNinja Tournament Board.\n");
...
short() {
  return "The CyberNinja Tournament Board ("+msg_num+" mesgs)";
}
...
if(impob->head_referee() || 
  ...
  write("1>>");
  return 1; }
  write("You cannot put a message on the Tournament board.\n");
  return 1;
}
...
remove_msg(what_msg) {
   ...
   if((this_player()->query_real_name() != "snow") 
       && (this_player()->query_real_name() != "dune")) {
      write("Information on this board can only be removed\n"+
            "by an Emperor.\n");
      return 1; }
   if (!sscanf(what_msg, "%d", note))
       if (!sscanf(what_msg, "note %d", note)) return 0;
