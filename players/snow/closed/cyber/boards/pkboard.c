...
#define BOARD_NAME "players/snow/closed/cyber/boards/pkboard"
...
id(str) {
  return str == "terminal" || str == "term" || str == "pkterminal";
}

long() {
  int ind;
  if(!present("either implants", this_player()) &&
     this_player()->query_guild() != "cyberninja") {
    tell_object(this_player(), "You are sucked out of the room!\n");
    move_object(this_player(), "/players/snow/ROOMS/tundra/tundra_ice.c");
   return 1; }

  write("Feel free to discuss any pk plots, etc.\n");
...
short() {
  return "The PK Terminal ("+msg_num+" entries)";
}

get() {
  write("It is embedded within the columns.\n");
  return 0;
}
