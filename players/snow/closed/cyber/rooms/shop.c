...
if(!present("pkterminal")) {
   move_object(clone_object("/players/snow/closed/cyber/boards/pkboard.c"),
   this_object());
   }

if(!present("merchant")) {
   move_object(clone_object("/players/snow/closed/cyber/rooms/MerchanT.c"),
   this_object());}
 if(!arg){
 set_light(1);
...
items=({
 "door","Only persons with proper security clearance are allowed north",
  ...
  dest_dir=({
 "/players/snow/closed/cyber/rooms/hallway2.c","south",
 "/players/snow/closed/cyber/rooms/weaponry.c","east",
/* Taken out due to Ninja inability to wear armor.
 "/players/snow/closed/cyber/rooms/armory.c","west",
*/
 "/players/snow/closed/cyber/rooms/mechshop.c","down",
      });
  }   }
...
sell(item) {
    object ob;
    if(!present("merchant")) {
      TE(TP, "There is no merchant here to run the shop.\n");
      return 1;}
    if (!item)
        return 0;
    if (item == "all") {
