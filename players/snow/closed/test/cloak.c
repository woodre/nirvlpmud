/* This cloak is gained by winning the Elemental Test. If it seems powerful, please try
...
#define AMULET RED+"~ "+OFF+BLUE+"Ele"+OFF+GREEN+"ment"+OFF+BOLD+"al C"+OFF+GREEN+"lo"+OFF+BLUE+"ak"+OFF+RED+" ~"+OFF

reset(arg) {
  ::reset(arg);
  set_name("elemental cloak");
  set_alias("cloak");
  set_short(AMULET);
  set_long("");
  set_type("cloak");
...
wear(str)
{
  ...
  if(str == "cloak" || str == "elemental cloak"){  
   ::wear(str);
   return 1; }
 }
}
...
void long() {
  ...
  write(
    "  You see before you a legendary cloak, molded from the very elements\n"+
    "themselves, forged by the gods of Nirvana for those mighty and worthy.\n"+
    "Woven from strands of blue and green Elementite, the cloak shimmers a\n"+
    "dull blue-green.  Legend tells that when struck, the strands tense,\n"+
    "helping to protect the wearer from injury.  The cloak sparkles with\n"+
    "thousands of tiny white dots.");
  if((a = this_player()) && (b = environment()) && (b == a)) {
    write("\n  You can sense a "+BLUE+"mys"+GREEN+"tic"+OFF+BOLD+"al "+OFF+GREEN+"ene"+BLUE+"rgy"+OFF+" coursing through the cloak.");
    if(present("elemental aura",environment(this_object())))
      write("  Worn by\n"+
            "those with both valor and might, this cloak will protect anyone so brave\n"+
