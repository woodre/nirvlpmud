...
#define SWORD RED+"~ "+OFF+BLUE+"Ele"+OFF+GREEN+"men"+OFF+BOLD+"tal P"+OFF+GREEN+"ik"+OFF+ BLUE+"e"+OFF+RED+" ~"+OFF

int smack;

reset(arg) {
  ::reset(arg);
  set_name("elemental pike");
  set_alias("pike");
  set_short(SWORD);
  set_long("");
  set_class(21);
...
extra_hit(object ob) {
  ...
  tell_room(environment(ob),
    environment(this_object())->query_name()+" slices into "+ob->query_name()+" with a "+
    "vicious attack!\n");
...
void long() {
  ...
  write(
    ...
    "The pike, made from a straight lengthy rod is smooth and unblemished.\n"+  
  ...
  if((a = this_player()) && (b = environment()) && (b == a)) {
    write("\n  You can sense a "+BLUE+"mys"+GREEN+"tic"+OFF+BOLD+"al "+OFF+GREEN+"ene"+BLUE+"rgy"+OFF+" coursing through the pike.");
