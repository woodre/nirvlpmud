...
#define SWORD RED+"~ "+OFF+BLUE+"Ele"+OFF+GREEN+"men"+OFF+BOLD+"tal War"+OFF+GREEN+"Dag"+OFF+BLUE+"ger"+OFF+RED+" ~"+OFF

int smack;

reset(arg) {
  ::reset(arg);
  set_name("elemental dagger");
  set_alias("wardagger");
  set_short(SWORD);
  set_long("");
  set_class(21);
...
extra_hit(object ob) {
  ...
  tell_room(environment(ob),
    environment(this_object())->query_name()+" stabs "+ob->query_name()+" with a "+
    "vicious thrust!\n");
...
void long() {
  ...
  write(
    ...
    "Barely a foot in length, this keen edged dagger is a beautiful work of\n"+
  ...
  if((a = this_player()) && (b = environment()) && (b == a)) {
    write("\n  You can sense a "+BLUE+"mys"+GREEN+"tic"+OFF+BOLD+"al "+OFF+GREEN+"ene"+BLUE+"rgy"+OFF+" coursing through the dagger.");
