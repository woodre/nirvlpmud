...
#define SWORD RED+"~ "+OFF+BLUE+"Ele"+OFF+GREEN+"men"+OFF+BOLD+"tal War"+OFF+GREEN+"sta"+OFF+BLUE+"ff"+OFF+RED+" ~"+OFF

int smack;

reset(arg) {
  ::reset(arg);
  set_name("elemental staff");
  set_alias("warstaff");
  set_short(SWORD);
  set_long("");
  set_class(21);
...
extra_hit(object ob) {
  ...
  tell_room(environment(ob),
    environment(this_object())->query_name()+" pummels "+ob->query_name()+" with a "+
    "vicious attack!\n");
...
void long() {
  ...
  write(
    ...
    "A well made staff, an inch in diameter and six feet in length, has been\n"+
  ...
  if((a = this_player()) && (b = environment()) && (b == a)) {
    write("\n  You can sense a "+BLUE+"mys"+GREEN+"tic"+OFF+BOLD+"al "+OFF+GREEN+"ene"+BLUE+"rgy"+OFF+" coursing through the staff.");
