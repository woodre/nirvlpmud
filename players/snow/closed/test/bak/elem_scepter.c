...
#define SWORD RED+"~ "+OFF+BLUE+"Ele"+OFF+GREEN+"men"+OFF+BOLD+"tal Sc"+OFF+GREEN+"ept"+OFF+ BLUE+"er"+OFF+RED+" ~"+OFF

int smack;

reset(arg) {
  ::reset(arg);
  set_name("elemental scepter");
  set_alias("scepter");
  set_short(SWORD);
  set_long("");
  set_class(21);
...
extra_hit(object ob) {
  ...
  tell_room(environment(ob),
    environment(this_object())->query_name()+" club "+ob->query_name()+" with a "+
    "vicious attack!\n");
...
void long() {
  ...
  write(
    ...
    "Just over two feet long, a teardrop shaped gem - swirling blue, green,\n"+
  ...
  if((a = this_player()) && (b = environment()) && (b == a)) {
    write("\n  You can sense a "+BLUE+"mys"+GREEN+"tic"+OFF+BOLD+"al "+OFF+GREEN+"ene"+BLUE+"rgy"+OFF+" coursing through the scepter.");
