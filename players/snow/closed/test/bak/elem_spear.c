...
#define SWORD RED+"~ "+OFF+BLUE+"Ele"+OFF+GREEN+"men"+OFF+BOLD+"tal Hal"+OFF+GREEN+"fSp"+OFF+ BLUE+"ear"+OFF+RED+" ~"+OFF

int smack;

reset(arg) {
  ::reset(arg);
  set_name("elemental spear");
  set_alias("spear");
  set_short(SWORD);
  set_long("");
  set_class(21);
...
id(str){ return (::id(str) || str == "snow_elem_wep" || str == "halfspear"); } /*added by Zeus 5/02*/
...
void long() {
  ...
  write(
    ...
    "Each over a foot from base to tip, two sharp arrow-shaped spearheads\n"+  
  ...
  if((a = this_player()) && (b = environment()) && (b == a)) {
    write("\n  You can sense a "+BLUE+"mys"+GREEN+"tic"+OFF+BOLD+"al "+OFF+GREEN+"ene"+BLUE+"rgy"+OFF+" coursing through the spear.");
