...
#define SWORD RED+"~ "+OFF+BLUE+"Ele"+OFF+GREEN+"men"+OFF+BOLD+"tal Spi"+OFF+GREEN+"ked"+OFF+BLUE+"whip"+OFF+RED+" ~"+OFF

int smack;

reset(arg) {
  ::reset(arg);
  set_name("elemental whip");
  set_alias("spikedwhip");
  set_short(SWORD);
  set_long("");
  set_class(21);
  set_type("exotic");
...
id(str){ return (::id(str) || str == "snow_elem_wep" || str == "whip"); }
...
extra_hit(object ob) {
  ...
  tell_room(environment(ob),
    environment(this_object())->query_name()+" whips "+ob->query_name()+" with a "+
    "vicious strike!\n");
...
void long() {
  ...
  write(
    ...
    "A metallic looking cord of six feet or so is attached to a foot long\n"+
  ...
  if((a = this_player()) && (b = environment()) && (b == a)) {
    write("\n  You can sense a "+BLUE+"mys"+GREEN+"tic"+OFF+BOLD+"al "+OFF+GREEN+"ene"+BLUE+"rgy"+OFF+" coursing through the whip.");
