...
#define SWORD RED+"~ "+OFF+BLUE+"Ele"+OFF+GREEN+"men"+OFF+BOLD+"tal War"+OFF+GREEN+"Scy"+OFF+ BLUE+"the"+OFF+RED+" ~"+OFF

int smack;

reset(arg) {
  ::reset(arg);
  set_name("elemental scythe");
  set_alias("scythe");
  set_short(SWORD);
  set_long("");
  set_class(21);
...
id(str){ return (::id(str) || str == "snow_elem_wep" || str == "warscythe"); } /*added by Zeus 5/02*/
...
extra_hit(object ob) {
  ...
  tell_room(environment(ob),
    environment(this_object())->query_name()+" gouges into "+ob->query_name()+" with a "+
    "vicious attack!\n");
...
void long() {
  ...
  write(
    ...
    "A long, wicked-looking blade is strapped to a gnarled woodend handle.\n"+
  ...
  if((a = this_player()) && (b = environment()) && (b == a)) {
    write("\n  You can sense a "+BLUE+"mys"+GREEN+"tic"+OFF+BOLD+"al "+OFF+GREEN+"ene"+BLUE+"rgy"+OFF+" coursing through the scythe.");
