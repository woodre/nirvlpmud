...
#define SWORD RED+"~ "+OFF+BLUE+"Ele"+OFF+GREEN+"men"+OFF+BOLD+"tal Bat"+OFF+GREEN+"tle"+OFF+BLUE+"axe"+OFF+RED+" ~"+OFF

int smack;

reset(arg) {
  ::reset(arg);
  set_name("elemental axe");
  set_alias("battleaxe");
  set_short(SWORD);
  set_long("");
  set_class(21);
...
extra_hit(object ob) {
  ...
  tell_room(environment(ob),
    environment(this_object())->query_name()+" hacks into "+ob->query_name()+" with a "+
    "vicious chop!\n");
...
void long() {
  ...
  write(
    ...
    "A suberbly crafted weapon, the twin blades of the axe are perfectly\n"+ 
  ...
  if((a = this_player()) && (b = environment()) && (b == a)) {
    write("\n  You can sense a "+BLUE+"mys"+GREEN+"tic"+OFF+BOLD+"al "+OFF+GREEN+"ene"+BLUE+"rgy"+OFF+" coursing through the axe.");
