...
#define AMULET RED+"~ "+OFF+BLUE+"Ele"+OFF+GREEN+"ment"+OFF+BOLD+"al R"+OFF+GREEN+"in"+OFF+BLUE+"g"+OFF+RED+" ~"+OFF
...
wear(str)
{
  ...
  if(str == "ring" || str == "elemental ring"){  
   ::wear(str);
   return 1; }
 }
}
...
void long() {
  ...
  write(
    "  You see before you a legendary ring, molded from the very\n"+ 
    ...
    "about the glimmering onyx stone placed in the middle. ");
  if((a = this_player()) && (b = environment()) && (b == a)) {
    write("\n  You can sense a "+BLUE+"mys"+GREEN+"tic"+OFF+BOLD+"al "+OFF+GREEN+"ene"+BLUE+"rgy"+OFF+" coursing through the ring.");
    if(present("elemental aura",environment(this_object())))
      write("  Worn by\n"+
            "those with both valor and might, this ring will protect anyone so brave\n"+
