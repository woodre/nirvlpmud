...
#define AMULET RED+"~ "+OFF+BLUE+"Ele"+OFF+GREEN+"ment"+OFF+BOLD+"al Am"+OFF+GREEN+"ul"+OFF+BLUE+"et"+OFF+RED+" ~"+OFF
...
reset(arg) {
  ...
  set_long("");
  set_type("amulet");
/* Changed to amulet slot, not sure why it was misc...  -Dar (Aug 10th 2002)  */
  set_ac(2);
  set_weight(1);
  set_value(10000);
...
wear(str)
{
  ...
  if(str == "amulet" || str == "elemental amulet"){ 
  ::wear(str);
  return 1;}
 }
}
...
void long() {
  ...
  write(
    "  You see before you a legendary amulet, molded from the very elements\n"+
    "themselves, forged by the gods of Nirvana for those mighty and worthy.\n"+
    "Hanging from a brilliant metal chain, the amulet has been carved from\n"+
    "a single piece of Elementite, and is beautifully indicative of the\n"+
    "elements blue and green color.  Thousands of white sparkles dance\n"+
    "about its surface.");
  if((a = this_player()) && (b = environment()) && (b == a)) {
    write("\n  You can sense a "+BLUE+"mys"+GREEN+"tic"+OFF+BOLD+"al "+OFF+GREEN+"ene"+BLUE+"rgy"+OFF+" coursing through the amulet.");
    if(present("elemental aura",environment(this_object())))
      write("  Worn by\n"+
            "those with both valor and might, this amulet will protect anyone so brave\n"+
