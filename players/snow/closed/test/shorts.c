...
#define AMULET RED+"~ "+OFF+BLUE+"Ele"+OFF+GREEN+"ment"+OFF+BOLD+"al Sh"+OFF+GREEN+"or"+OFF+BLUE+"ts"+OFF+RED+" ~"+OFF

reset(arg) {
  ::reset(arg);
  set_name("elemental ring");
  set_alias("shorts");
  set_short(AMULET);
  set_long("");
  set_type("underwear");
...
wear(str)
{
  ...
  if(str == "shorts" || str == "elemental shorts"){  
   ::wear(str);
   return 1; }
 }
}
...
void long() {
  ...
  write(
    "  You see before you underwear of legend, molded from the very\n"+ 
    "elements themselves, forged by the gods of Nirvana for those mighty\n"+ 
    "and worthy.  Crafted from soft leather, they have been reinforced\n"+
    "with plates of blue-green Elementite. Flickering white specks float\n"+
    "about the amazingly soft fabric. ");
  if((a = this_player()) && (b = environment()) && (b == a)) {
    write("\n  You can sense a "+BLUE+"mys"+GREEN+"tic"+OFF+BOLD+"al "+OFF+GREEN+"ene"+BLUE+"rgy"+OFF+" coursing through the shorts.");
    if(present("elemental aura",environment(this_object())))
      write("  Worn by\n"+
            "those with both valor and might, these shorts will protect anyone so brave\n"+
