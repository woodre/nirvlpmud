/* This shield is gained by winning the Elemental Test. If it seems powerful, please try
...
#define AMULET RED+"~ "+OFF+BLUE+"Ele"+OFF+GREEN+"ment"+OFF+BOLD+"al Sh"+OFF+GREEN+"ie"+OFF+BLUE+"ld"+OFF+RED+" ~"+OFF

reset(arg) {
  ::reset(arg);
  set_name("elemental shield");
  set_alias("shield");
  set_short(AMULET);
  set_long("");
  set_type("shield");
...
wear(str)
{
  ...
  if(str == "shield" || str == "elemental shield"){  
   ::wear(str);
   return 1; }
 }
}
...
void long() {
  ...
  write(
    "  You see before you a legendary shield, molded from the very elements\n"+
    "themselves, forged by the gods of Nirvana for those mighty and worthy.\n"+
    "Rectangular in shape and slightly curving along its horizontal plane,\n"+
    "the blue and green shield of Elementite bears carvings representing\n"+
    "the five elementals.  Sparkling white flecks dance about the shield.");
  if((a = this_player()) && (b = environment()) && (b == a)) {
    write("\n  You can sense a "+BLUE+"mys"+GREEN+"tic"+OFF+BOLD+"al "+OFF+GREEN+"ene"+BLUE+"rgy"+OFF+" coursing through the shield.");
    if(present("elemental aura",environment(this_object())))
      write("  Worn by\n"+
            "those with both valor and might, this shield will protect anyone so brave\n"+
