/* This neckguard is gained by winning the Elemental Test. If it seems powerful, please try
...
#define AMULET RED+"~ "+OFF+BLUE+"Ele"+OFF+GREEN+"ment"+OFF+BOLD+"al Neck"+OFF+GREEN+"gu"+OFF+BLUE+"ard"+OFF+RED+" ~"+OFF

reset(arg) {
  ::reset(arg);
  set_name("elemental neckguard");
  set_alias("neckguard");
  set_short(AMULET);
  set_long("");
  set_type("misc"); /* 11/22/05 Earwax: changed from 'chest' *boggle* */
  set_ac(2);
  set_weight(1);
  set_value(10000);
...
wear(str)
{
  ...
  if(str == "neckguard" || str == "elemental neckguard"){  
   ::wear(str);
   return 1; }
 }
}
...
void long() {
  ...
  write(
    "  You see before you a legendary neckguard, molded from the very\n"+ 
    "elements themselves, forged by the gods of Nirvana for those mighty\n"+
    "and worthy.  Large and angular, the neckguard fits over the shoulders\n"+
    "and sweeps back around the back of the head.  It looks heavy, yet well\n"+
    "engineered and built.  Its deep blue-green color is offset by two red\n"+
    "stripes that run around it's exterior and sparkling white flecks that\n"+
    "adorn it's surface.");
  if((a = this_player()) && (b = environment()) && (b == a)) {
    write("\n  You can sense a "+BLUE+"mys"+GREEN+"tic"+OFF+BOLD+"al "+OFF+GREEN+"ene"+BLUE+"rgy"+OFF+" coursing through the neckguard.");
    if(present("elemental aura",environment(this_object())))
      write("  Worn by\n"+
            "those with both valor and might, this necguard will protect anyone so brave\n"+
