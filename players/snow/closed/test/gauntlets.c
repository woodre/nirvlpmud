/* These gauntlets are gained by winning the Elemental Test. If it seems powerful, please try
...
#define AMULET RED+"~ "+OFF+BLUE+"Ele"+OFF+GREEN+"ment"+OFF+BOLD+"al Gau"+OFF+GREEN+"ntl"+OFF+BLUE+"ets"+OFF+RED+" ~"+OFF

reset(arg) {
  ::reset(arg);
  set_name("elemental gauntlets");
  set_alias("gauntlets");
  set_short(AMULET);
  set_long("");
  set_type("gloves"); /* 11/22/05 Earwax changed from ring type */
  set_ac(2);
  set_weight(1);
  set_value(10000);
...
wear(str)
{
  ...
  if(str == "gauntlets" || str == "elemental gauntlets" || str == "ring"){  
   ::wear(str);
   return 1; }
 }
}
...
void long() {
  ...
  write(
    "  You see before you a legendary pair of spiked gauntlets, molded from\n"+
    "the very elements themselves, forged by the gods of Nirvana for those\n"+
    "mighty and worthy.  Plates of blue and green Elementite form the outer\n"+
    "layer of the gauntlets, while tightly ringed chainmail forms further\n"+
    "protection underneath.  Several rows of spikes protrude outward from\n"+
    "the knuckles of the gauntlets.  Blue and green swirls cover both of\n"+
    "the guantlets, while white specks flutter all about them.");
  if((a = this_player()) && (b = environment()) && (b == a)) {
    write("\n  You can sense a "+BLUE+"mys"+GREEN+"tic"+OFF+BOLD+"al "+OFF+GREEN+"ene"+BLUE+"rgy"+OFF+" coursing through the gauntlets.");
    if(present("elemental aura",environment(this_object())))
      write("  Worn by\n"+
            "those with both valor and might, these gauntlets will protect anyone so brave\n"+
