...
#define AMULET RED+"~ "+OFF+BLUE+"Ele"+OFF+GREEN+"ment"+OFF+BOLD+"al B"+OFF+GREEN+"el"+OFF+BLUE+"t"+OFF+RED+" ~"+OFF

reset(arg) {
  ::reset(arg);
  set_name("elemental belt");
  set_alias("belt");
  set_short(AMULET);
  set_long("");
  set_type("belt");
...
wear(str)
{
  ...
  if(str == "belt" || str == "elemental belt"){  
   ::wear(str);
   return 1; }
 }
}
...
void long() {
  ...
  write(
    "  You see before you a legendary belt, molded from the very\n"+ 
    ...
    "with plates of blue-green Elementite, and wide leather hoops with\n"+
    "a strong metal buckle fastens it tightly around ones waist.  Flickering\n"+
    "white specks float about them. ");
  if((a = this_player()) && (b = environment()) && (b == a)) {
    write("\n  You can sense a "+BLUE+"mys"+GREEN+"tic"+OFF+BOLD+"al "+OFF+GREEN+"ene"+BLUE+"rgy"+OFF+" coursing through the belt.");
    if(present("elemental aura",environment(this_object())))
      write("  Worn by\n"+
            "those with both valor and might, this belt will protect anyone so brave\n"+
