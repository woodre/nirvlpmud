...
dinvis(string str) {
  ...
  if(this_player()->query_no_spell() || this_player()->checkNM() ||
     this_player()->query_sp() < 40) { write("You are unable to work the Sands at this time.\n"); return 1; }
  if(str) {
  ...
  if(ob == this_player()) {
    ...
    say(this_player()->query_name()+" covers "+ob->query_name()+" with a "+VEIL+" and "+ob->query_name()+" fades from normal sight.\n");
  }
  ob->set_invs_sp();
