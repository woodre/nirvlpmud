...
dervish_sharpen(str)
{
 ...
 if(this_player()->query_no_spell() || this_player()->checkNM() || this_player()
->query_sp() < 30) {
 ...
 write("You command the sands to glide over your "+capitalize(wep->query_name())+", sharpening it.\n");
 say(this_player()->query_name()+" commands the sands to glide over their "+capitalize(wep->query_name())+", sharpening it.\n");
 wep->set_misses(1);
 wep->set_hits(1);
