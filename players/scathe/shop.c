...
ONE_EXIT("players/scathe/cave/entry", "west",
   "The shop",
   "This is the shop from the hood. You can buy or sell things here.\n"+
...
do_sell(ob) {
   ...
   if (value > 6000)
      do_destroy = 1;
   if (value > 1000) {
      int er;  /* new value calc by verte */
      write("The shop is low on money...\n");
      /* illegal.    value = 600 + random(1000);       -verte */
     er = 1000 + random(100);
      if(value > er) value = er;
   }
   ...
   call_other("players/scathe/store", "store", ob);
   return 1;
}
...
value(item) {
   ...
   if (!name_of_item) {
      if (call_other("players/scathe/store", "value", item))
         return 1;
      write("No such item ("); write(item); write(") here\n");
...
buy(item) {
   if (!item)
      return 0;
   call_other("players/scathe/store", "buy", item);
   return 1;
}
...
north() {
   ...
   call_other(this_player(), "move_player", "north#players/scathe/store");
   return 1;
}

list(obj) {
   call_other("players/scathe/store","inventory",obj);
   return 1;
}
