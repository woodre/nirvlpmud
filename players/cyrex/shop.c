...
ONE_EXIT("players/cyrex/cargobay", "north",
	 "The shop",
"You are in a Far-Out shop.  You can buy or sell items here.\n" +
...
*/
  ...
  value = 800 + this_player()->query_level() + random(100);
    }
...
*/
    ...
    call_other("players/cyrex/store", "store", ob);
    return 1;
}
...
value(item) {
    ...
    if (!name_of_item) {
        if (call_other("players/cyrex/store", "value", item))
	    return 1;
	write("No such item ("); write(item); write(") here\n");
...
buy(item) {
    if (!item)
	return 0;
    call_other("players/cyrex/store", "buy", item);
    return 1;
}
...
south() {
    ...
    call_other(this_player(), "move_player", "south#players/cyrex/store");
    return 1;
}

list(obj) {
    call_other("players/cyrex/store", "inventory", obj);
    return 1;
}
