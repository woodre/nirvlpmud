...
move_object(clone_object("/players/daranath/mons/fungi4.c"),this_object());
 ...
 if(!arg){
  ...
  long_desc=
  "Winding its way unto the rear corner of the cavern, the dirt path ends\n"+
  "at a small, strange looking hut. The circular building rests against\n"+
  "the cavern wall, with a dim light eminating from a front door which is\n"+
  "slightly ajar. The path travels back to the west.\n";
...
items=({
 ...
 "darkness", "The darkness leaves you wondering what might be hidden in"+
             "every corner",
  "path", "The small path leads to the west, back to the farm",
  "hut", "The fungus hut is built of a strange fiber which keeps it\n"+
          "together. A soft glow is coming from under the doorway",
 });

  dest_dir=({
 "/players/daranath/underdark/level1/farm_hut.c","enter",
 "/players/daranath/underdark/level1/farm8.c","west",
           });
  }   }
init(){
 ::init();
