...
if(!str){
  ...
  else { write("Frost strike who?\n"); return 1; }
}
else{ att = present(str, environment(tp)); }
if(tp->query_spell_dam()) return 0;
if(tp->query_sp() < 7) {
write(GRN+"<< "+HIG+"Druid"+NORM+GRN+" >>"+NORM+" Not enough spell points to generate a Frost strike.\n");
return 1; }
if (!att) { write ("There is no "+str+" here to cast a frost strike at"); return 1; }
