/* Psion Guild Daemon for chat lines, adapted from my Necro chatter object */
...
id(str) { return str == "psion_chatter"; }
...
chat_line(string msg,string who,int emote) {
  int a;
    for(a=0;a<sizeof(guild_chat);a++) {
	  if(guild_chat[a]) {
       if(!emote) tell_object(guild_chat[a],"<"+HIW+"NECRO"+NORM+"> "+who+" intones: "+msg+"\n");
       else tell_object(guild_chat[a],"<"+HIW+"NECRO"+NORM+"> "+who+" "+msg+"\n");
	  } 
	}
}
