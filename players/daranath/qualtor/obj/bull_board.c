...
#define ERROR_LOG "/players/daranath/qualtor/docs/BOARD_ER"
#define BOARD_NAME "players/daranath/qualtor/docs/BOARD"
...
id(str) {
	return str == "board" || str == "bulletin board" || str == "guildboard";
}
...
short() {
	return "A bulletin board <"+msg_num+"/50>";
}
...
remove_msg(what_msg) {
    ...
    if((this_player()->query_real_name() != lower_case(player) &&
       this_player()->query_level() < 70) ||
		!query_ip_number(this_player())) {
		write("Only Daranath may remove other peaple's notes.\n");
