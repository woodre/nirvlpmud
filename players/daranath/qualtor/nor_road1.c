/* Damage from ash is doubled here, due to proximity to demonic hole */
...
reset(int arg){
if(!present("demon_hole")) {
move_object(clone_object(ROOT+"obj/demon_hole.c"),this_object());
}
  if(!arg){
...
"               "+BOLD+"North Road"+NORM+" (east, west)\n"+
     "North road runs east and west from here, connecting to the old\n"+
     "road and running to the stables further into the city. The\n"+
     "roadway is made of a dark grey stone, easily withstanding the\n"+
     "many problems the city has had in the last several years.\n";
    ...
    dest_dir=({
      "/players/daranath/qualtor/nor_road2.c","east",
      "/players/daranath/qualtor/old_road4.c","west",
    });
  ...
  tell_room(this_object(),msg);
   ...
   while(ob) {
      ...
      if(living(ob) && ob->is_player() && !present("br_scarf", ob))
         if(a > 1) ob->hit_player(6, "other|fire");
      ob=next;
   }
   call_out("ash", random(10) + 3); 
}
