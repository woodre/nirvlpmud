...
init(){
 ...
 if(tp->query_guild_name() == "necromancer" || call_other(tp,"id","undead") || 
  call_other(tp,"id","zombie") || call_other(tp,"id","undead") ||
  call_other(tp,"id","ghost") ){ return 1; }
    write(HIR+"\nThe ward flares in color.\n"+OFF);
    write("\nYou are pushed back against the room.\n");
    say(HIR+"The ward flares against the presence of "+tp->query_name()+".\n"+OFF);
   if(direction)
    command(direction,tp);  
   else
    tp->run_away();
