...
reset(arg){
  ...
  set_silently(1);
  set_id(GHOULSHIELD);
  set_long("How are you looking at this?  Contact Wocket.\n");
  numof = 3;
}
...
if(environment()){
  ...
  ::init();
  if(!GOB){ destruct(this_object()); return 1; }
  say(BOLD+"A series of "+numof+" ghouls fly up through the earth and start circling "+tpn+".\n"+OFF);
