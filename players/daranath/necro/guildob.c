...
query_auto_load() {return "/players/daranath/necro/guildob.c:"; }
id(str) {
  if(previous_object()) {
...
int int1,int2;
  ...
  msg = "\tYour pouch currently contains:\n";
      if(blood){ msg+= "\t  "+BOLD+blood+OFF+" vial";   if(blood > 1) msg+="s"; msg+=" of blood\n"; }
      if(skin){  msg+= "\t  "+BOLD+skin+OFF+" skin";    if(skin > 1)  msg+="s"; msg+="\n";          }
      if(eye){   msg+= "\t  "+BOLD+eye+OFF+" eye";      if(eye > 1)  msg+="s"; msg+="\n";           }
      if(spine){ msg+= "\t  "+BOLD+spine+OFF+" spine";  if(spine > 1)  msg+="s"; msg+="\n";         }
      if(heart){ msg+= "\t  "+BOLD+heart+OFF+" heart";  if(heart > 1)  msg+="s"; msg+="\n";         }
      if(soul){  msg+= "\t  "+BOLD+soul+OFF+" soul";    if(soul > 1)  msg+="s"; msg+="\n";          }
  write(msg);
  say(tpn+" searches through "+environment()->query_possessive()+" component pouch for a moment.\n");
  } 
}
...
extra_look() {
write(environment()->query_name()+" has a small "+BOLD+"skull"+OFF+" tattoo upon "+
  environment()->query_possessive()+" forehead.\n");
if(icutmyself)
write(environment()->query_name()+" has several lacerations across "+environment()->query_possessive()+
   " wrists.\n");
}
...
if(environment(this_object()) == this_player()) {
  if(!restore_object(SAVE+tprn)) {
    if(restore_object(SAVE+"bak/"+tprn)){
      write("Your services are no longer needed for testing the necromancer guild.\n"+
  ...
  if(tp->query_level() < 20) {
    ...
    tp->set_home("/players/daranath/necro/room/necro2.c");
    tp->set_pretitle("|");
    necro_title();
...
necro_block() {
  if(tp->query_level() < 20) {
    write(ROD+" The Realm of Death distains the use of that spell.\n");
