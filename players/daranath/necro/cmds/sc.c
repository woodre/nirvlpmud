...
me = tp;
  ...
  if(this_player()->query_wimpy())
      write("("+HIR+"wimpy"+OFF+") ");
  write("\n          ");
write("["+BOLD+BLK+"Intox: "+OFF+BOLD+((tp->query_intoxination()*100)/(tp->query_level()+3))+OFF+"%]  " );
write("["+BOLD+BLK+"Food: "+OFF+BOLD+((tp->query_stuffed()*100)/(tp->query_level()*8))+OFF+"%]  " );
write("["+BOLD+BLK+"Soak: "+OFF+BOLD+((tp->query_soaked()*100)/(tp->query_level()*8))+OFF+"%]  " );     
