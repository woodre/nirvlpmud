...
short(){
if(!this_player()) return "A Druid's Private Grove"; /* err... that was lame. [7.8.01] by verte */
if(this_player()->query_level() > 21) return owner ? owner + "'s  Private Grove" : "Someone's  Private Grove";  /* by verte [7.8.01] */
if(present("link_comm", this_player())){
 if(present("link_comm", this_player())->query_slave_master() == owner){
  return "A stand of trees";
  }
}
   if(environment(this_player()) != environment()) return "A stand of trees"; /* by verte [7.8.01] */
}

long(){
if(environment(this_player()) == this_object()){
write("The druid's grove should go in here for stuff.\n");
...
id(str){ return str == "grove" || str == "druid_grove" || str == "stand"; }
