...
id(str) { return str == "wizclan_chatter"; }
...
chat_line(string msg,string who,int emote) {
  int a;
    for(a=0;a<sizeof(guild_chat);a++) {
	  if(guild_chat[a]) {
if(!emote) tell_object(guild_chat[a],HIR+"["+HIW+"wizclan"+HIR+"]"+NORM+" "+capitalize(this_player()->query_real_name())+" says: "+msg+"\n");
else tell_object(guild_chat[a],HIR+"["+HIW+"wizclan"+HIR+"]"+NORM+" "+capitalize(this_player()->query_real_name())+" "+msg+"\n");
	  } 
	}
}
