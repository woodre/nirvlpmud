...
id(str) { return str == "guardian" || str == "guardian" || str == "guard"; }

short() { return "A Large "+HIW+"Guardian"+NORM+""; }

long() {
   write("A very large and impressive guardian placed here by "+
         capitalize(owner)+".\n");
}
...
follow() {
   if(this_player()->query_real_name() != owner) {
     write("The Guardian glares at you!\n");
     return 1;
   }
   if(follow == 1) {
...
heart_beat() {
   if(time() > start_time + 1800) {
    ...
    say("The guardian disappears in a puff of smoke.\n");
    destruct(this_object());
    return 1;
   }
   if(follow) {
