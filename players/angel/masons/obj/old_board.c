...
#define BOARD_NAME DATA_OLD_BOARD
...
init() {
	...
	if (!looked_at) {
	        int i;
		string arr;
		messages = allocate(31);
		headers = allocate(31);
		looked_at = 1;
		if (!restore_object(BOARD_NAME))
		    return;
		arr = explode(tmp_head, "\n**\n");
write("SIZEOFARR: "+sizeof(arr)+"\n");
write("SIZEOFHEADERS:"+sizeof(headers)+"\n");
		i = 0;
		while(i < sizeof(arr)) {
		    headers[i] = arr[i];
		    i++;
...
new_msg(msg_head) {
	line = 1;
write("This board is here for the sake of tradition.\nNo changes can be made to its contents.\n");
return 1;
	if (!msg_head)
		return 0;
	...
	if (str == "**") {
		...
		say(curr_writer->query_name() + " has completed a note : " +
			new_head + "\n");
                ...
                "(rank "+present("seal",this_player())->query_guild_rank()+
...
return 1;
	...
	if ((this_player()->query_name() != player &&
		...
		write("Only Wizards may remove other's notes.\n");
