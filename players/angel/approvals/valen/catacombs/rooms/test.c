...
reset(arg)
{
  ...
  long_desc =
  HIBLK+"  The"+WHT+HBCYN+" catacomb"+NORM+HIBLK+" path has come to a end. The arched\n\
...
with a single"+RED+" torch"+HIBLK+"on the opposite wall."+NORM+"\n";
  ...
  ({
    "torch", 
    "The torch is secured to the wall and you could"+RED+" twist torch"+NORM+"\n",
    "dust", 
...
{ 
  if(str == "torch")
  {
    if(door1 == 0)
    {
      ...
      "The old "+HIBLK+" wooden door"+NORM+" swings open. Now someone could "+RED+" enter door"+NORM+"\n");
      return 1;
    }
