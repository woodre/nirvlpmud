...
#include "/players/angel/closed/headers/angel.h"
#include <ansi.h>
#define TOOL "/players/angel/closed/tool/p.c"

string short_desc;

status
id(string str) {
    return str == "phone" || str == "angeltell" || str == "aman";
}
...
long()
{
    ...
    if((a = this_player()) && (e = environment()) && (e == a))
    {
        com1 = "Type 'angel <msg>' to get in touch with him.\n";
...
the_giving(string str)
{
    ...
    else if (bob)
    {
        ...
        tell_object(bob, capitalize(this_player()->query_real_name()) +
            " has given you a cellular phone to contact the coder Angel.\n");
        p = clone_object("/players/angel/closed/tool/p.c");
        move_object(p,bob);
        return 1;
    }
    notify_fail("Hmm...\n");
...
cmd_tell(string arg)
{
    ...
    if (c = find_player("angel"))
    {
        ...
        if(environment(c) && (file_name(environment(c)) != "players/angel/workroom")) write("He may be busy.\n");
        tell_object(c, HIW + (a = (string)this_player()->query_name()) + " tells you: " + NORM + what + "\n");
            c->add_tellhistory(HIW + a + " told you: " + NORM + what);
    }
    else
    {
        write_file("/players/angel/closed/tool/p",random_color()+TP->query_real_name()+NORM+" :: "+what+" <--> "+ctime(time())+".\n");
...
status
cmd_atell(string arg) {
    command("tell angel " + arg, this_player());
    return 1;
}
