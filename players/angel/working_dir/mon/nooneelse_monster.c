/*
 nooneelse_monster.c - I needed to copy this so I could alter it
                       to be able to set_whimpy off & be able to turn
                       move_at_reset off & not turn heartbeat off on my
                       vampires.
                       (this monster.c is from Smiley Lpmud)
                       [Note: look for @#@# to find the changes I made.]
*/
#include "living.h"
...
string short_desc, long_desc, alias, alt_name, race, info;
...
/*@#@#   added this so my vampire(ss) would do what they are supposed to  */
/*       even if their attackers are gone.     nooneelse                  */
          if (this_object()->query_name()=="vampire" ||
              this_object()->query_name()=="vampiress" ||
              this_object()->query_name()=="vampirecorpse" ||
              this_object()->query_name()=="vampiresscorpse")
            return;
/* @#@# end of addition                                                   */
	    set_heart_beat(0);
	    return;
...
set_move_at_reset(arg) { move_at_reset = arg; } /*@#@#*/
/* optional
 * 0: Peaceful.
 * 1: Attack on sight.
 */
