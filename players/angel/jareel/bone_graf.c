...
long() {
    if (num_messages == 0) {
        write("There is no messages for Bone.\n");
        return;
    }
...
short() {
    return ("A "+BOLD+BLK+"Dark Onyx Board"+NORM+"(" + num_messages + " msgs)");
}
...
reset(arg) {
    if (arg)
        return;
    restore_object("players/jareel/general_stuff/graf/bone");
}
...
string hd;
    ...
    if (who && environment(who) == environment(this_object())) {
        write(call_other(who, "query_name") + " is busy leaving Bone a note.\n");
        return 1;
    }
    if (num_messages == 10) {
...
get_body(str) {
    if (str == "**") {
        ...
        new_body = 0;
        new_hd = 0;
        save_object("players/jareel/general_stuff/graf/bone");
        write("Ok.\n");
        who = 0;
        return;
    }
...
remove(str) {
    ...
    while(rest != 0 && rest != "") {
        ...
        if((call_other(this_player(),"query_level") < 40) && (tp != "bone")) {
