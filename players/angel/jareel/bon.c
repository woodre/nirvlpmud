...
#include "/players/jareel/define.h"
     ...
     long_desc=(
       ""+BRED+BLK+"@@@@@"+NORM+"     An eerie fog hovers over the ground, leaving the floor virtually\n"+
       ...
       ""+BRED+BLK+"@"+NORM+"-+D"+BLK+BRED+"@"+NORM+"     An onyx coffin rests upon the table, its lid sealed and locked. The\n"+
     ...
     dest_dir = ({
      "/players/jareel/areas/statue/","west",
   });
}
     int peeps;void init() {
       ::init();
   ...
   if("/players/jareel/areas/statue/arena/rm5.c"->query_enter() == 1){
     write("Someone is in the arena.\n");
   return 1;
   }
   write("You start the arena.\n");
     say(this_player()->query_name()+" dissapears.\n");
     move_object(this_player(),"/players/jareel/areas/statue/arena/rm5.c");
     say(this_player()->query_name()+" enters.\n");
     command("glance",this_player());
       "/players/jareel/areas/statue/arena/rm5.c"->set_enter(1);
   return 1;
}
