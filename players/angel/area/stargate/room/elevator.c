...
long() {
    write("You are in the elevator. On the wall are twenty eight buttons,\n");
    write("numbered 1 to 28. You can only access 26-28.\n");
    if (door_is_open)
        write("There is an open door to the south.\n");
    if (!door_is_open)
        write("There is a closed door to the south.\n");
}
...
press(button) {
    ...
    if (b == "26" || b == "twenty six")
        dest = 26;
    if (b == "27" || b == "twenty seven")
        dest = 27;
    if (b == "28" || b == "twenty eight")
        dest = 28;
    if (dest == level) {
    ...
    if (dest > level) {
        ...
        write("The elevator jerks downward.\n");
        say("The elevator jerks downward.\n");
    }
    if (level > dest) {
        moving_time = level - dest;
        write("The elevator starts moving upward.\n");
        say("The elevator starts moving upward.\n");
    }
    if (dest == 1 || level == 1)
        moving_time += 10;
    moving_time += 1;
...
heart_beat() {
    ...
    level = dest;
    if (level == 27)
        call_other("/players/angel/area/stargate/room/hall2", "elevator_arrives", 0);
    if (level == 28)
        call_other("/players/angel/area/stargate/room/hall3", "elevator_arrives", 0);
}
...
go_south() {
    ...
    if (level == 26)
        call_other(this_player(), "move_player", "south#/players/angel/area/stargate/room/hall1");
    if (level == 27)
        call_other(this_player(), "move_player", "south#/players/angel/area/stargate/room/hall2");
    if (level == 28)
        call_other(this_player(), "move_player", "south#/players/angel/area/stargate/room/hall3");
    return 1;
}
