/*                                                                    *
 *    File:             /players/angel/area/stargate/room/hall3.c     *
 *    Function:         room                                          *
...
reset(arg)
{
  ...
  dest_dir =
  ({
    "/players/angel/area/stargate/room/hall4",   "south", 
  });
}
...
{    
    ...
    write("The are two obvious exit: north and south.\n");
    if (lamp_is_lit)
...
id(str) {
    ...
    north() {
        if (call_other("/players/angel/area/stargate/room/elevator", "query_door", 0) ||
            call_other("/players/angel/area/stargate/room/elevator", "query_level", 0) != 28) {
            write("The door is closed.\n");
            return 1;
    }
    ...
    south() {
    call_other(this_player(), "move_player", "south#/players/angel/area/stargate/room/hall4");
    return 1;
    }
    open(str)
    {
        if (str != "door")
            return 0;
        if (call_other("/players/angel/area/stargate/room/elevator", "query_level", 0) != 28) {
    ...
    push(str)
    {
        if (!str || str != "button")
            return 0;
        if (call_other("/players/angel/area/stargate/room/elevator", "call_elevator", 28))
            lamp_is_lit = 1;
