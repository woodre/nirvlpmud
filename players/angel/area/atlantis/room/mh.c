/*                                                                    *
 *    File:             /players/angel/area/atlantis/room/mh.c        *
 *    Function:         room                                          *
...
reset(arg)
{
  ...
  if( !present("athosian", this_object()) )
  {
    for(x=0; x<4; x++)
    {
    move_object(clone_object("/players/angel/area/atlantis/npc/athosians.c"), this_object());
    }
}
if(arg) return;
  set_light(1);
...
wall will close in the room."+NORM+"\n";
  ...
  dest_dir =
  ({
	"/players/angel/area/atlantis/room/hall_03.c",     "north",
  });
}