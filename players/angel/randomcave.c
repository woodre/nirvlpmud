...
switch(random(7)){
case 0: return "Under the Ooze";
break;
case 1: return ""+HIM+"Under the Ooze"+NORM+"";
break;
case 2: return ""+HIY+"Under the Ooze"+NORM+"";
break;
case 3: return ""+HIR+"Under the Ooze"+NORM+"";
break;
case 4: return ""+HIB+"Under the Ooze"+NORM+"";
break;
case 5: return ""+HIC+"Under the Ooze"+NORM+"";
break;
case 6: return ""+HIG+"Under the Ooze"+NORM+"";
break;
}
}
...
long(str){
	...
	if(!str){
		...
		write("\n"+GRN+"Obvious exits are:"+NORM+" ");

		if(room_num <= 90){

			if(blocked[room_num + 10] > 0){

				write("["+HIY+"north"+NORM+"] ");
...
creation(){
	...
	blocked[spec_mob - 10] = 1;

	blocked[spec_mob + 1] = 1;
	...
	if(encounter[room_num] < 1)
        {
        	...
        	if(room_num == spec_mob && x > 18)
                {

                          switch(random(2))
                          {
                            case 0: move_object(clone_object("/players/jaraxle/cave/mobs/special.c"), this_object());
                              break;
                            case 1:  move_object(clone_object("/players/fred/misc/bknight.c"), this_object());
                              break; 
                          } 
			...
			if(random(100) < 50)
                        {
                           mob = clone_object("/players/jaraxle/cave/mobs/newbie.c");
			...
			mob = clone_object("/players/jaraxle/cave/mobs/newbie.c");
       		...
       		else if(x < 15)
                {
       			if(random(100) < 50)
                        {
                           mob = clone_object("/players/jaraxle/cave/mobs/midlevel.c");
                           move_object(mob, this_object());
                           mob->set_al(-a);
         		}
	...
	/*
		...
		if(encounter[room_num] < 1){
			...
			if(room_num == spec_mob && x > 18){
                         ...
                         move_object(clone_object("/players/jaraxle/cave/mobs/special.c"), this_object());
				...
				if(random(100) < 50){
					...
					move_object(clone_object("/players/jaraxle/cave/mobs/gnewbie.c"), this_object());
				...
				if(random(100) < 50){
					...
					move_object(clone_object("/players/jaraxle/cave/mobs/gmidlevel.c"), this_object());
				...
				if(random(100) < 50){
					...
					move_object(clone_object("/players/jaraxle/cave/mobs/ghighlevel.c"), this_object());
...
move_north(){
	...
	renew_room(this_player()->query_level(), this_player()->query_alignment());
...
move_leave(){
	...
	this_player()->query_name()+" leaves the underground.\n", ({ this_player() }));
...
dest_me(){
	...
	for(i=0; i<sizeof(inv3); i++) {
		...
		else{ move_object(inv3[i], "/players/jaraxle/cave/rooms/entrance.c"); }
...
catch_tell(string str){
	...
	tell_room("/players/jaraxle/work.c",
	str);
