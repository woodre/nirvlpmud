/*                                                                    
 *    File: /players/chip/room/apartment/apt12.c                
 *    Function: room
...
{
  ::reset(arg);
  if( !present("lunatic", this_object()) )
    move_object(clone_object("/players/chip/mobs/apartment/lunatic1.c"),  this_object());
  if(arg) return;
  set_light(1);
  ...
  dest_dir =
  ({
    "/players/chip/room/apartment/f2h2.c",     "hall",
  });
}