...
reset(arg) {
  ...
  set_light(1);
  long_desc=
"  This is the top chamber of "+capitalize(OWNER)+"'s large intestine.  It is lined with\n"+
...
return str+")-=-("+NORM+capitalize(OWNER)+"'s Large Intestine"+str+")-=-("+NORM;
  }
}
...
teleport(str) {
  ...
  if(!str) {
    write(
"You may teleport to: church, post, portal, shrine, play\n"
    );
    return 1;
  }
  if(locked) {
...
case "play": dest="/players/chip/room/newplayroom.c"; break;
