...
arr_control = ({ "Cleanse", "Absorbi", "Burnoff", "Gtitle", "Heal",
                 "Regeneration" });
arr_combat = ({ "Wep", "Darts", "Fatal", "KO", "FormArm", "Hypermode" });
arr_creation = ({ "ShapeArmor", "Alarm", "Camouflage", "CreateBag", "---",
                  "CreateBall" });
...
write("Level 1  - ["+HIW+"Threat"+NORM+"]  ["+HIW+"Absorb"+NORM+"]");
write("  ["+HIW+"Wield"+NORM+"] ["+HIW+"Tentacles"+NORM+"]\n");
if(glvl > 15){
write("level 16 - ["+HIW+"Lair"+NORM+"]  ["+HIW+"Recruit"+NORM+"]\n");
}
write("\n");
write(COLOR+"Other Skills and Abilities:"+NORM+"\n");
write("[mp] [sc] [ssave] [convert] [raise_skill] [setcolor]\n");
write("[pklist] [supdate] [neobug] [beta] [news]\n");
