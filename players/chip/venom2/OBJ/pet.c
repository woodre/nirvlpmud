...
a 37% chance per round...at best.  The pet can and
...
short()
{
    if(nowner) return short + " " + BOLD + "[" + nowner + "]'s "+HIK+"Dark Matter"+BOLD+" Slave" + NORM;
}
long(){
	write(
		"This is the enslaved pet of "+capitalize(nowner)+".  The "+name+" has strands\n"+
		"of "+HIK+"Dark Matter"+NORM+" covering its body in long streaks and splotches.  It's\n"+
	...
	if(this_player() == owner){
                write("As the Master, you may '"+HIR+"destroy"+NORM+"' this creature at any time.\n");
	}
...
if(!owner && !root()) {  /* root() check by verte */
 countdown += 1;
 if(countdown > time) destruct(this_object()); /* doubled the lifespan, as most other guilds have longer lasting or even permanent pets -Chip(4-15-09)*/
 return;
 }
...
countdown = 0;
		...
		/******** About a 37% chance it will do an attack per round, and only against Npc's (chance to hit raised by 11% by Chip(4-15-09)) *****/
		if(owner->query_attack() &&
			random(100) < owner->query_attrib("wil") * 12 + random(6) &&
			ATTACKER->is_npc()){
			do_attack(ATTACKER);
		}
	}
...
query_count(){ return countdown; }

call_exit(){ 
 tell_room(environment(), 
 name+" hurries out of the room behind "+owner->query_name()+".\n");
 }

call_enter(){
 tell_room(environment(), 
 name+" hurries into the room shadowing "+owner->query_name()+".\n");
 }
...
do_attack(object attacker){
   ...
   if(!owner) return;
	...
	tell_room(environment(),
	capitalize(name)+" rages forward to attack "+capitalize(attacker->query_name())+"!\n");
	damage = random(wc) + 1;
	if(attacker->query_hp() < damage){
		tell_object(owner, capitalize(name)+" circles and waits for "+attacker->query_name()+"'s life to come to an end.\n");
		return;
	}
	switch(damage){
	...
	switch(random(5) + 1){
		...
		case 4: boo = "raking "+HIK+"Dark Matter"+NORM+" claw"; break;
	...
	if(random(owner->query_attrib("wil") + ac) < random(attacker->query_wc())){
		...
		tell_room(environment(),
		capitalize(name)+" dodges the side-swing of "+attacker->query_name()+"!\n");
	}
	return;
}
...
dest_this(str){
	...
	tell_room(environment(this_player()),
		...
		HIK+"Dark Matter"+NORM+" rises from its body in tendrils and reunites with "+this_player()->query_name()+".\n"+
	"The body disintegrates before your very eyes.\n", ({ this_player() }));
	tell_object(this_player(),
		"You point at "+capitalize(name)+"...\n"+
		HIK+"Dark Matter"+NORM+" rises from it in tendrils and reunites with your own body.\n"+
	"Your slave disappears in a lump of bubbling corrosion.\n");
	destruct(this_object());
	return 1;
}
