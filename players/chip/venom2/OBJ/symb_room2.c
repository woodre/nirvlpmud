...
long_desc = 
BOLD+"  You find yourself standing in a room made of "+HIK+"Dark Matter"+NORM+".  The walls\n"+
...
short(){ return ""+HIK+"Dark Matter"+NORM+""; }
...
open_challenge(str){
 ...
 if(str == "crack"){  
   ...
   write("You pry open the "+HIK+"Dark Matter"+NORM+" wall...\n"+
          "A large crack forms allowing entrance for just one person.\n"+
          "Your chosen one may now '"+HIR+"challenge symbiote"+NORM+"'.\n");
    say(this_player()->query_name()+" grabs the wall of "+HIK+"Dark Matter"+NORM+" tightly and tears it open...\n");
    allow_challenge = 1;
...
blahblah(){
 if(!allow_challenge) return;
  ...
  allow_challenge = 0;
  tell_room(this_object(), "The opening in the wall suddenly closes...\n");
  }
}
...
challenge(str){
 ...
 if(!str){ write("You must '"+HIR+"challenge symbiote"+NORM+"'.\n"); return 1; }
 ...
 tell_object(tp, 
 "\n\n\n"+HIR+"As you are pulled into the pool of "+HIK+"Dark Matter"+NORM+", you feel as though you are\n"+
 "torn in half!"+NORM+"\n\n\n"); 
 tp->hit_player(100 + random(50)); 
tp->shadow_symboff();
 command("look", tp);
 tell_object(tp,
 HIR+"Your symbiote is pulled from your body and forms itself before you..."+NORM+"\n\n");
 tell_object(tp,
 BOLD+capitalize(named)+" hisses: I shall have my freedom! I will destroy you!"+NORM+"\n");
...
write("You must revert to your human form first!\n");
return 1;
}
command("ssave", tp);
