...
gen_wc_bonus(){
   ...
   if(ATT && spikes){ 
   if(ATT->query_hp() > 20 && !random(20 - glvl)){
   sdam = random(combat) + 2;
   tell_object(USER,
   "// ////"+COLOR+"SPIKES"+NORM+"\\ \ Your body spikes tear into "+ATT->query_name()+"!\n");
   tell_object(ATT,
   "Your body is pierced by the "+BOLD+"Dark Matter"+NORM+" spikes of "+
   ""+USER->query_name()+"!\n");
	ATT->heal_self(-sdam);
   ATT->add_spell_point(sdam);
   spikes -= 1;
   if(!spikes) tell_object(USER, "Your spikes retract from your body.\n");
    }
   }
	...
	if(ATT->webbed() || present("venom_web_object", ATT) && combat){
		total += random(combat * 2) + combat;
	}
if(USER->query_real_name() == "maledicta") tell_object(USER, "TOTAL IS: "+total+".\n");
	return total;
}
