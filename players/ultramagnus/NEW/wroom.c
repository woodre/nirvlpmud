...
if (secure == 0 && invader !=invited && invader !="mouzar")  {
  ...
  write("Were you invited...I don't think so!\n");
   ...
   if(secure == 1 && alarm == 0)  {
     say(capitalize(invader) + " has entered this room\n");
     return 1;
   }
   return 1;
}
...
exit() {
  if(alarm == 0)  {
  invader = this_player()->query_real_name();
  say(capitalize(invader) + " get the hell out\n");
  return 1;
  }
  return 1;
}
...
force(str)  {
  ...
  write(capitalize(str) + " has arrived as you commanded\n");
  return 1;
}
