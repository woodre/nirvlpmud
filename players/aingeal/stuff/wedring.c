/*  Francesco's wedding ring code, used by permission */
...
marry(str) {
  ...
  ob=clone_object("/players/dreamspeakr/WEDDING/OBJ/wedding_ring");
  ob->set_spouse(find_living(him)->query_real_name());
  ob->set_married( 2 );
  move_object(ob,find_living(her));
  tell_object(find_living(her),"You have just been married to "+cap(him)+".\n");
  ob2=clone_object("/players/dreamspeakr/WEDDING/OBJ/wedding_ring");
  ob2->set_spouse(find_living(her)->query_real_name());
  ob2->set_married( 2 );
