...
#include "std_old.h"
/* Old experience cost was at one flat static rate.  The updates have a three-tier scale of rates. */
/* #define XPCOST 7000   */
 ...
 *  Modified by Aingeal 8/24/03 in consultation with
 *  Illarion to remove the stat caps and to make 
 *  it more consistent with the general advance and 
 *  attribute increases after Illarion's stat updates.  */
 *  
rise(str){
int sb, minexp, XPCOST;
int current, new;
string attrib, raise, cha, dex;
  minexp = sb = 0;
  XPCOST = 7000;
  if(!str) { 
