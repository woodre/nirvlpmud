        ...
        who=users();
         count=0;
           while(count<sizeof(who)) {
            if(call_other(who[count],"query_level",0)<26) {
    tell_object(who[count],"You see the Enterprise move overhead....\n");
    tell_object(who[count],"It looks like it is changing it orbit.\n");
 }
             count+=1;
  }
