...
object test,ob,ob1,test1;
    ...
    ob=clone_object("players/haji/diap.c");
...
reset(arg) {
    ...
    string dest;
     dest=allocate(10);
       ...
       dest[8]="room/plane8";
       dest[9]="room/ruin";
    local=find_object(dest[random(10)]);

    write(local);
    move_object(this_object(),local);
    shout("You see the Enterprise move overhead....\n");
    shout("It looks like it is changing it orbit.\n");
    shout("You hear a voice from a communicator carried\n");
    shout("by the guy down the road say 'The beam up point\n");
    shout("for the Enterprise has moved.\n");
    if (arg)
	return;
}
