...
ONE_EXIT("players/morgoth/CASTLE/vil_road_1", "west",
	 "The shop",
...
*/
    ...
    call_other("players/morgoth/CASTLE/store", "store", ob);
    return 1;
}
...
value(item) {
    ...
    if (!name_of_item) {
	if (call_other("players/morgoth/CASTLE/store", "value", item))
	    return 1;
	write("No such item ("); write(item); write(") here\n");
...
buy(item) {
    if (!item)
	return 0;
    call_other("players/morgoth/CASTLE/store", "buy", item);
    return 1;
}

north() {
    if (this_player()->query_real_name() != "morgoth") {
    ...
    call_other(this_player(), "move_player", "north#players/morgoth/CASTLE/store");
    return 1;
}

list(obj) {
    call_other("players/morgoth/CASTLE/store", "inventory", obj);
    return 1;
}
