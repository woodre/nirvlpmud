  ...
  { tmp=sscanf(rest2,"%s**%s**%s**\n%s",puzzle_name,answer,this_puzzle,rest2);
    ...
    { rest=puzzles;
      ...
      { tmp=sscanf(rest,"%s$%s",p_name,rest);
        ...
        if(p_name==puzzle_name) { found=1; }
      }
      if(found==1) { nr_to_do=nr_to_do-1; }
      else { str_to_do=str_to_do+puzzle_name+"**"; }
    }
    else
    { write("You didn't solve puzzles yet. Prepare for your first one!\n");
