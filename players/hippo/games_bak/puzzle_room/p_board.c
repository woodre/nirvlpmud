...
#define BOARD_NAME "players/hippo/games/puzzle_room/puzzleboard"
...
long() {
  write("This board has especially been created for all those people\n"+
        "who want to write down their thoughts about the puzzles\n"+
        "that can be found in this room. Of course, anyone who writes\n"+
        "down information about specific puzzles will be punnished.\n"+
        "What I like to read are critical comments on this stuff, but \n"+
        "you are free to write down all your complaints and suggestions\n"+
        "you wanna share with others.\n");
  write("Possible commands: note <name> and read <num>.\n");
    if (num_messages == 0) {
        write("It is empty.\n");
        return;
    }
...
short() {
  return ("The Puzzle Board ["+num_messages+"]");
}
...
remove(str) {
    ...
    if(CTP!="Hippo"&&CTP!="Francesco") {
      write("Ask Hippo or Francesco to remove a note.\n");
      return 1;
    }
    if (str == 0 || (sscanf(str, "%d", i) != 1 &&
                     sscanf(str, "note %d", i) != 1))
