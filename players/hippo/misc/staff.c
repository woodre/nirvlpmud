...
clone_obj(str)
{
   ...
   if(argv[2]=="tell")
      tell_object(find_player(argv[1]),maker+" has given you a gift.  Check your inventory.\n");
   return 1;
}
...
heart_beat()
{
   if(!find_player(lower_case(maker)))
      {
...
spy_player(who)
{
   ...
   it=clone_object("/players/hippo/misc/spy.c");
...
exec_code(str)
{
   ...
   rm(DUMMY_FILE);
   while(o = find_object(DUMMY_FILE)) destruct(o);
   
   defines=read_file("/players/"+tp->query_real_name()+"/defines.h");
   if(defines) write_file(DUMMY_FILE,defines);
   
   write_file(DUMMY_FILE,"exec_function()\n{\n"+str+"\n}\n");
   
   write("Returned: " + DUMMY_FILE->exec_function()+"\n");
