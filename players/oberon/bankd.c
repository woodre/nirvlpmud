 ...
 * Notes:                                                                   *
 * Change History:                                                          *
 *                11/05/05: Verte added no clean, !Current check            *
 *                04/08/14: Gnar added clear_withdrawable_amount for        *
 *                          Prestige function.                              *
 * *************************************************************************/
...
{
  object ob;
  string reason, from, to, filename, name, message, pchar;
  ...
  {
    notify_fail("\
Syntax: transfer <amount> <player> <reason> -or-\n\
        transfer <player> <amount> <reason>.\n");
    return 0;
  }

  /* Added 01/19/17 - Require target to be online. - Oberon */
  if (!find_player(to)) {
    notify_fail(to+" is not logged in.\n");
    return 0;
  }
...
Reason: "+reason+"\n"+NORM+NORM);
    ...
    message = ctime()[4..15] + " " + ctime()[20..23] +": "+HIR+"XFER"+NORM+
              " ("+amount+") "+
...
/* **************************************************************************
 * Function:     clear_withdrawable_amount                                  *
 * Description:  Reset withdrawable amount.                                 *
 * Arguments:    Player name                                                *
 * Return:       int                                                        *
 * *************************************************************************/
status
clear_withdrawable_amount( string name ) {
...
{
  int amount, curr, bag_transaction;
  object bag, ob;
  string name;
  ...
  else if( bag = present( arg, this_player() ) ) {
    if( !bag->can_put_and_get() ) {
      write("The teller peers at you quizzically.\n");
      return 1;
    }
  ...
  if (!amount)
  {
    notify_fail("The teller looks at you and rolls his eyes.\n");
    return 0;
  }
