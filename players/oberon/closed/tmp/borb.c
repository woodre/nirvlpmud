...
/* this is what i was planning on using to do the if guild
object gob;
     if(this_player()->query_guild_name() == "bloodfist") {
       if(gob = present("scar", this_player())) {
         if( random(3) ) gob->add_bloodhonor((-5));
         write("Your weapon feeds on your"+RED+"Bloodhonor"+NORM+" \n");
		 write("You feel stronger as "+RED+"Blood"+NORM+" racing through your veins.\n");
         return 1;
		 }
		 }
		 
		 "/players/fred/closed/bloodfist/daemon/honor_daemon"->query_
honor("sorrow")
Dragnar says: will return 26

Dragnar says: rm_honor( "sorrow", 5 )

Dragnar says: would lower honor by 5

->> Dragnar says: so you would query to make sure they have it

Dragnar says: then call rm_honor to lower it
*/