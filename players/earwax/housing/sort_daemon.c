 ...
 * Change History: 10/10/05 - Earwax - Shows filenames for 'list index' for devs
 * **************************************************************************/
...
status sort_ascending(int a, int b) { return (sort_descending(a, b) ? 0 : 1); }
...
varargs
string *
generate_output(string verb, mixed *info, string arg, string *filenames)
{
  string *output, *args;
  string temp; 
  ...
  if (!arg || arg == "all")
   ...
   for (i = 0; i < siz; i++) /* For loop to preserve the order */
   {
     switch(args[i])
     {
       case "available"   : indices = filter_objects(indices, "filter_available", me); 
                            break;
       case "unavailable" : indices = filter_objects(indices, "filter_unavailable", me);
                            break;
       case "large"       : indices = filter_objects(indices, "filter_large", me); 
                            break;
       case "medium"      : indices = filter_objects(indices, "filter_medium", me);
                            break;
       case "small"       : indices = filter_objects(indices, "filter_small", me);
                            break;
       case "developed"   : indices = filter_objects(indices, "filter_developed", me); 
                            break;
       case "undeveloped" : indices = filter_objects(indices, "filter_undeveloped", me);
                            break;
       case "creator"     : if ((i + 1) < siz) Creator = args[(i+1)];
                            indices = filter_objects(indices, "filter_creator", me);
                            break;
   ...
   for (i = 0; i < siz; i++)
   {
     ...
     if (flag_sort)
     {
      ...
      output += ({ (AVAILABLE ? "" : HIK)
        ...
        + (string)WAXFUNS->lalign(LSHORT, 36)
        /* Earwax 10/10/05 Added this for wizards */
        + ((int)this_player()->query_level() > 19 && arg == "index" ? filenames[i] : "")
        + "\n" +NORM + NORM});
    }
    else
    {
