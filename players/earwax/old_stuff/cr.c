...
object return_room_obj, following_obj, item, where_obj, item_obj_array;
  query_auto_load() {
return("players/cream/closed/cream.c:");
   }
reset(arg) {
  if (arg) return;
  :: reset(arg);
...
rhelp() {
  ...
  command("more /players/cream/closed/toy.help", this_player());
  return 1;
}
...
rfake_quicktyper(str)  {
  ...
  if (str=="log" || str=="elog") {
    write("\n=====> Error log:\n");
    command("cat /log/cream", this_player());
    write("\n=====> Bug/Idea/Typo log:\n");
    command("cat /log/cream.rep", this_player());
    return 1;
  }
  return;
}
...
rsummon(str) {
 ...
 there = environment(ob);
  ...
  say(capitalize(str) + " is deposited at Earwax's feet.\n");
