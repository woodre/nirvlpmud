...
move_player(dir_dest)
{
  ...
  if (ghost)
    ...
    tell_room(environment(), cap_name + " " + msgout + " " + dir + ".\n", ({ this_object() }));
  }
  ...
  if(ghost && is_light)
    ...
    tell_room(environment(), cap_name + " " + msgin + ".\n", ({ this_object() }));
...
RemoveWeaponBonus(object gear) {
  ...
  WeaponBonuses=m_delete(Weapons,gear);
  return 1;
}
...
do_damage(int *damage, string *method){
   ...
   int i, j, k, y;
   string type;
   int ac_tmp, dam, total, class, res, temp, itemp;
   string cat,subcat;
   mixed *armors2;
   object *armors;
...
phase in a fix */
         if(attacker_ob){
           if (attacker_ob->is_player() && attacker_ob->query_level() < EXPLORE &&
...
to the ground to die.\n");
    ...
    if(!this_object()->is_kid() && !this_object()->is_pet())
      if(attacker_ob) corpse->set_killer(attacker_ob->query_real_name());
    if(!is_npc)
    {
    ...
    else if(attacker_ob && is_npc)
    {
      ...
      if(!no_exp_value)
      {
        if(party) 
          ...
          tell_object(attacker_ob,"Experience Gained: "+exp_worth+"\n");
        ...
        if(attacker_ob && !weps[s]->query_silent_damage() && 
          ...
          if(environment())
            tell_room(environment(),
              cap_name+" "+dmess[1]+" "+U+name_of_attacker+NORM+dmess[0]+".\n",
              attacker_ob?({attacker_ob,this_object()}):({this_object()}));
        }
      }
  }
/* End of hits and damage for other registered weapons */
  ...
  if(this_object() && tmp == 0) {
    if(!present("no_msg_hit", this_object())) {
      ...
      checked_say(cap_name + " missed " + U+name_of_attacker+NORM + ".\n");
    }
    if(!is_npc) {
      if(second_weapon && pro_object && pro_object->query_combat_method() == 3)
  ...
  if(attacker_ob && call_other(attacker_ob, "query_name", 0) != NAME_OF_GHOST) {
    ...
    if(this_object() && !present("no_msg_hit", this_object())) {
      ...
      tell_object(this_object(), "You " + mess[1] + " " + U+name_of_attacker+NORM +
                  mess[0] + ".\n");
      if(query_invis() >= INVIS_ACTION)
        tell_object(attacker_ob, cap_name + " " + mess[1] + " you" + mess[0] +
                    ".\n");
      checked_say(cap_name + " " + mess[1] + " " + U+name_of_attacker+NORM + mess[0] +
                  ".\n");
    }
    if(!is_npc) {
...
/*
    ...
    if (query_invis() >= INVIS_ACTION)
      tell_object(attacker_ob, cap_name + " " + what + " you" + how +
		  ".\n");
    checked_say(cap_name + " " + what + " " + U + name_of_attacker + NORM + how +
		".\n");
*/
    string *mess;
    if(this_object())
...
add_money(m) {
  ...
  if(this_player() == this_object() && !flag) {
    money = money + m;
    if(money > MAXCOINS) money = MAXCOINS;
    flag = 1;
  }
  if(!flag) {
    money = money + m;
