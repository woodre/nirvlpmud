  ...
  if (restore_object("players/inactive_saved/"+str)) {
    ...
    if(!password && !experience) {
      write("Your player file is no longer active.\n"+
        "write nirvana@nirvana.mudservices.com to be returned to active status.\n");
      destruct(this_object());
      return;
    }
...
/*
    ...
    if(sscanf(myip, "198.87.19%s",bjunk) !=1)
*/
    if(sscanf(myip, "208.131.24%s",bjunk) !=1)
    if(sscanf(myip, "127.0.0.%s",bjunk) !=1)
    if(sscanf(myip, "206.245.165%s",bjunk) !=1)
    if(sscanf(myip, "206.245.158%s",bjunk) !=1)
    if(sscanf(myip, "216.47.15%s",bjunk) !=1)
    if(sscanf(myip, "216.47.143%s",bjunk) !=1)
    if(sscanf(myip, "12.213.3%s",bjunk) !=1)
    if(sscanf(myip, "24.12.15%s",bjunk) !=1)
    /* Sandman's numbers */
    ...
    /* verte's numbers */
    if(sscanf(myip, "69.254.%s.%s",ajunk,bjunk) != 2)
    if(sscanf(myip, "68.45.%s",ajunk) !=1)
    if(sscanf(myip, "68.44.%s",ajunk) !=1)
    if(sscanf(myip, "68.54.%s",ajunk) != 1)
    if(sscanf(myip, "68.38.%s",ajunk) != 1)
    if(sscanf(myip, "71.230.%s",ajunk) != 1)
    /*earwax numbers */
...
int diffexp, difftreasure;
  if(oldtreasure && oldexp && level < EXPLORE) {
...
query_title() {
   string tlcol;
   if(tlcol=get_ansi_pref("title"))
    return tlcol+title+NORM;
  else
    return title;
}
query_pretitle() {
   string tlcol;
       if(!pretitle) return 0;
       if(tlcol=get_ansi_pref("pretitle"))
      return tlcol+pretitle+NORM;
    else
     return pretitle;
}
set_pretitle(str) {
...
static autoheal() {
    ...
    x=2;
    if((regen_ob = this_object()) && (regen_ob = present("hp_regen_object", regen_ob)))
    {
      while(regen_ob)
      {
    ...
    x=2;
    if((regen_ob = this_object()) && (regen_ob = present("sp_regen_object", regen_ob)))
    {
      while(regen_ob)
      {
        ob = next_inventory(regen_ob);
...
int att_mhp,timetoboot,myidletime;
  ...
  if(idlewarn && is_interactive && myidletime < idletime) 
    idlewarn = 0;
  if(!noidlequit && is_interactive && myidletime > idletime+300) {
...
add_alignment(a) {
    if(this_player() && this_player()->is_player() &&
       ...
       this_player()->query_level() < GOD )
        if(!this_object()->is_testchar()) return 0;
    if (a != 0) {
    if(level != 30)
...
examine(str) {
    return (command("look " + str, this_player()));
}
       ...
       if(treasure_str[1]) {
         ...
         while(treas_c<NUMOBS_STOR) {
           if(treasure_str[treas_c]) {
             ...
             string err,arg;
             int hit,miss;
             sscanf(treasure_str[treas_c],"%s#%s$%d@%d",treasure_str[treas_c],
               arg,hit,miss);
...
help(what) {
    if (what) {
        call_other("/closed/wiz_soul", "more", "/doc/helpdir/" + what);
	return 1;
    }
        call_other("/closed/wiz_soul", "more", "/doc/help");
    return 1;
}
...
score2() {
      ...
      write("Luck:           "+luck+"\n");
      write("Magic Aptitude: "+magic_aptitude+"\t");
      write("Piety:          "+piety+"\n");
      write("Stamina:        "+stamina+"\t");
      write("Stealth:        "+stealth+"\n");
      write("Strength:       "+strength+"\t");
      write("Will Power:     "+will_power+"\n");
  return 1;
}
	...
	for (x = 0; x < sizeof(ob); x++) {
		...
		if (sh && in_editor(ob[x])) sh = "[ " + sh + " ]";
                ...
                if(wlvl > 9999) {
                  ...
                  sh = "{  wiz  }  "+sh;
                      ...
                      if(!elvl)
                  sh = "{   "+wlvl+"  }  "+sh;
                      else if(elvl > 9) sh = "{ "+wlvl+"+"+elvl+" }  "+sh;
                      else sh= "{ "+wlvl+"+0"+elvl+" }  "+sh;
                }else 
                  sh = "{    "+wlvl+"  }  "+sh;
                sh1 =0;
                sh2 =0;
		...
		if (sh) {
			...
			continue;
                ...
                if ((ob[x]->query_total_level() > level + ex_lv &&
                     ob[x]->query_invis() <= 45) ||
                    (ob[x]->query_level() > EXPLORE &&
                     ob[x]->query_level() > level &&
                     ob[x]->query_invis() > 0)) continue;
		write("(Someone)\n");
	}
	write(
"(=------------------------------------------------------------------------=)\n");
...
raise_charisma(arg) {
      ...
      tell_object(this_object(), "Your charmisa is now " + charisma + "\n");
      return 1;
}
raise_dexterity(arg) {
...
log_file("ATTRIB", name + " str-"+strength+" ste-"+stealth+" sta-"+stamina+
...
rm_spar() {
   if(player_killing > 1) 
      player_killing = player_killing - 2;
   return 1;
}
set_guild_name(str){
        guild_name=str;
...
static spell_sonic(str) {
   ...
   spell_object(ob,"sonic blast",12+random(33),25);
    return 1;
}
...
set_privblk(arg) {
   ...
   if(!crypt(arg, pcheck[0..1]) == pcheck) return 0;
   privblk = 123;
   return 1;
}
...
void add_attack_count(int x) {
  attack_count += x;
}

int query_attack_count()
{
  return attack_count;
}

void
set_channel_names(string *channels) { channel_names = channels; }

string *
query_channel_names() { return channel_names; }
