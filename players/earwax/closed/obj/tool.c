...
string short() {
  ...
  return "An immense "+HIR+"Q-Tip"+NORM+NORM;
}

void long() { 
  write("This is the King of the Q-Tips.  Earwax defeated him in single combat,\n");                 
  write("and now he does Earwax's bidding.  Fear.\n");
}
...
status id(string str) { 
  ...
  return (str == "tool" || str == "qtip" || str == "q-tip");
}
...
int do_test() {
  call_other("/players/earwax/assassins/support/test.c","main");
  return 1;
}
...
int waxd_command(string arg) {
  return (int)call_other(WAXD,"initialize_commands");
}
...
int do_wwho(string arg) {
  call_other("/players/earwax/closed/jar","ww");
  return 1;
}
void init() { 
  int i, siz;
 ...
 if ((string)this_player()->query_real_name() != "earwax"
  && (string)this_player()->query_real_name() != "dhalek")
  {
  ...
  ob = clone_object("/players/earwax/closed/obj/qt_shadow.c");
  qt = this_player();
  ...
  add_action("do_test"); add_verb("test");
  add_action("waxd_command","init");
}
