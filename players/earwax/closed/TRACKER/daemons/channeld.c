...
*return_all_members()
{
  ...
  while (i--) if (present(TRACKER_ID,obs[i])) temp += ({ obs[i] });
  obs = ({ });
  return temp;
}
...
*filter_invis(object *t)
{
  ...
  while (i--) 
  {
    ...
    /* added .. nice illegalities :) */
      if(lvl < (int)t[i]->query_invis()) continue;
      if (lvl > l)
        temp += ({ t[i] });
      else
      if (lvl == l)
      {
        ...
        if (xlvl = (int) t[i]->query_extra_level()) /* verte changed from an invalid query */
          temp += ({ t[i] });
      }
    }
    else
