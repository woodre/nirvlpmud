...
#define SHOME (PATH + "shop")
#define STORAGE (PATH + "backshop")
#define AT_HOME (file_name(environment()) == SHOME)
...
{
    return (status) SHOME->sell(str);
}

status cmdList(string str)
{
    return (status) SHOME->list(str);
}

status cmdValue(string str)
{
    return (status) SHOME->value(str);
}

status cmdBuy(string str)
{
    return (status) SHOME->buy(str);
}
...
status dismiss(string str)
{
	...
	if (AT_HOME) {
	    ...
	    this_object()->move_player("X#" + SHOME);
	}
	return 1;
    }
