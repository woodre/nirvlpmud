 ...
 * 030715 - added color, tweaked desc, and added exits that are
 *          visible to shardaks only [linus]
...
{
    ::create();
    set_short(RED+"The demon merchant's"+NORM);
    set_long(HIK+"\
The flickering torch light has little strength in this secluded corner\n\
of the grotto.  A dominating red glow flows through a doorway in the\n\
northern wall, producing strange shadows among the stalagmites.\n"+NORM);
    set_exits( ([
	"north" : (PATH + "smithy"),
	"west"  : (PATH + "grotto"),
	]));
...
long(string arg) {
 if(!arg) {
  if((int)this_player()->query_brief() != 2) 
   write(RED+"The demon merchant's\n"+NORM);
    ::long(arg);
  if (present("shardak_mark",this_player())) 
   write("\t"+NORM+RED+"\The exits are: west and north.\n"+NORM);
    return;
 }
  else ::long(arg);
}
...
do_sell( object ob)
{
    ...
    if (value > 1000) {
	/* pay something in range 1000..1199 */
	value= (value > 1200) ? 200 : value - 1000;
	value= 1000 + random( value);
