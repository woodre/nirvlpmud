...
void reset(status arg)
{
    ...
    set_name("pyromancer");
    set_gender("male");
    set_short(HIR+"The pyromancer"+NORM);
    set_long(RED+"\
...
HIR + "\tAn aura of fire surrounds him.\n" + NORM);
    set_level(22);
    set_ac(10);
    set_wc(70);
    set_hp(400);
    ...
    load_chat("The pyromancer arches his eyebrow at you.\n");
     load_chat(HIR+"Lava flows from the dragon's mouth into the obsidian dish below...\n"+NORM);
}
...
void long(string str)
{
	...
	text= "\
The pyromancer nods at you and says, \"Ah.  A fellow brother of\n\
...
"+BOLD+BLK+"Smoke"+NORM+" puffs from the stone dragon's head...\n");
        tell_object(from, "\
Your body locks up...\n");
...
void congrat(object from)
{
    ...
    if (from) {
	say(RED+"A red aura"+NORM+" envelopes " + from->query_name() +
	    " for a mere instant.\n", from);
	tell_object(from, "\
Enveloped in a frail "+RED+"red aura"+NORM+" you hear a message from Shardak,\n\
...
\"I carry the following items with me:\"\n\
   Name                   Price\n\
 --+ ---------------------- +--\n\
   "+BOLD+BLK+"Black Obsidian"+NORM+"         356\n\
   A flask of "+HIR+"Nitropyr"+NORM+"    453\n\
   "+RED+"Blood"+NORM+" Chalk            632\n\n\
