...
enter(string str)
{
    ...
    gname = (string)this_player()->query_real_name();
/* commented out as shardaks are open - wocket
    if(this_player()->is_player() && this_player()->query_level() < 21)
    if(gname != "worm" && gname != "revenge" && gname != "entropy" && gname != "stochos" && gname != "cerberus")
    {
...
*/
	...
	this_player()->move_player( "into the cave#" + PATH + "cave");
    return 1;
}
