 ...
 * 030715 - Added color, tweaked descs, added exits readable only by
 *          shardaks [linus]
 * 030719 - Modified long desc to allow for set_item() to work [linus]
 */
...
create ()
{
    ...
    set_short(RED+"Altar of Shardak"+NORM);
    set_long(HIK+"\
...
the "+RED+"blood"+NORM+" stained top surface which is blank and smooth.\n",
             ({ "floor", "tiles" }):"\
The tiles on the floor are cut from porphyry, stone in hues "+YEL+"brown"
+NORM+"\n\
to dark "+RED+"red"+NORM+" born in the volcanoes millions of years ago.\n",
             ({ "candles", "recesses", "wall" }):"\
"+HIY+"Yellow"+NORM+" wax candles are guttering in every recess of the wall.\n",
    ({ "rock", "hollow", "here" }):"\
...
get_next_title (int level)
{
    switch (level) {
        ...
        case 4: return "the " + HIK + "Necrolyte" + NORM;
        case 5: return "the Summoner of " + HIK + "Dragons" + NORM;
        case 6: return "the " + HIK + "Head Assassin" + NORM;
        case 7: return "the Controller of " + HIR + "Dragonbreath" + NORM;
        case 8: return "the " + HIK + "GateKeeper of the UnderWorld" +
NORM;
...
{
  switch (level) {
       case 26: return "the "+ HIK +"Warrior of Shardak" + NORM;
       case 27: return "the "+ RED +"Assassin of Shardak" + NORM;
       case 28: return "the "+RED+"Berzerker of Shardak"+NORM;
       case 40: return "the "+HIK +"Law Enforcer "+NORM+"of "+RED+"Shardak"+NORM;
       case 41: return "the "+ HIK +"WarLord of Shardak" + NORM;
       case 42: return "the "+RED+"Right Hand of the Master"+NORM;
       case 45: return "the "+ HIK +"Master of the UnderWorld" + NORM;
       case 50..100: return "the "+HIR+"Magus"+NORM+" of "+HIK+"Shardak"+NORM;
       default: return 0;
       }
}
...
{
    object titles;
    if(!str || (str != "class" && str != "rank")) return (notify_fail("Usage: 'title <class/rank>'\n"), 0);
    if(str)
    if (str == "class"){
...
advance ()
{
    ...
    if ((int) this_player()->query_guild_rank() >= 8) {
             write("The Priest tells you: You have already reached the highest possible level.\n");
             return 1;
    }
...
if(this_player()->query_extra_level() < 1 && this_player()->query_level()< 21)
  spare = this_player()->query_exp() - call_other("room/adv_guild",
"get_next_exp",this_player()->query_level()-1);
if(this_player()->query_extra_level() > 0 && this_player()->query_level()< 21)
  spare = this_player()->query_exp() - call_other("room/exlv_guild",
"get_next_exp",this_player()->query_extra_level()-1);
    if (spare < needed) {
             /* need more xp */
             write("\
