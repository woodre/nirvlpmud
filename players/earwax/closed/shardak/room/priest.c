...
void reset(int x)
{
    ...
    set_name("priest");
    set_gender("male");
    set_short("The priest");
    set_long("\
The priest is shrouded in a dark black cowl completely concealing his\n\
face. The cowl reflects little light and has "+CYN+"ultramarine embroideries"+NORM+"\n\
...
most levitating priests seem to fancy.\n");
    ...
    set_ghost(1);
    set_chat_chance(2);
    load_chat("The priest moves to replace a candle as its "+HIR+"flame"+NORM+" sputters and dies.\n");
}
...
"+HIW+"`advance'"+NORM+" in guild rank, restore your "+HIW+"'title'"+NORM+", and see your unsolved\n\
"+HIW+"'trials'"+NORM+".\"\n";
	call_out("DoTalk", 2);
    }
}
...
and intonates,"+RED+" \"Shard'k, ezem gk'dhar tze ke'hkt.\"\n"+NORM);
	    say("\
He lowers the body to the altar and you see a vague "+HIK+"shadow"+NORM+" leaving\n\
the corpse before it vanishes in a "+HIY+"flash of light"+NORM+", followed by the\n\
...
It is dissolved in a "+HIY+"flash of light"+NORM+", but nothing happens.\n");
	}
    }
...
void congrat(object from)
{
    ...
    if (from) {
	say(RED+"A red aura envelopes " + from->query_name() +
	    " for a mere instant.\n"+NORM, from);
	tell_object(from, RED+"\
