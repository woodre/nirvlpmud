 ...
 * 030719 - fixed long to allow items, and added items, heh [linus]
 */
...
dragon-man himself, Shardak.\n"+NORM);
    ...
    add_item("ceiling",
             "The ceiling is domed and difficult to see due to the dim light.\n");
    add_item("tiles",
             "Marble tiles, black in color.\n");
    add_item("throne",
             "An ornately decorated throne that the dragon-man named Shardak sits upon.\n");
    add_item("steps",
             "Three steps leading up to the throne.  They are covered with the golden\n"+
              "hoard belonging to Shardak.\n");
    add_item("hoard",
             "The gold colored marks of Shardak.\n");
    set_light(1);
...
long(string arg) {
 if(!arg) {
  if((int)this_player()->query_brief() != 2) 
   write(RED+"The Throne Room\n"+NORM);
    ::long(arg);
  if (present("shardak_mark",this_player())) 
   write("\t"+NORM+RED+"\The exits are: west, south, and the law room is east.\n"+NORM);
  if (Shardak && present(Shardak)) 
    write("\t\t"+NORM+RED+"\Shardak"+HIK+" sits upon his throne.\n"+NORM);
     return;
 }
 else
       write(HIK+"The throne is empty"+NORM);
        return;
}
