...
/*
  if(!ob1){
     ob1 = clone_object("players/earwax/monster/smith");
     move_object(ob1,this_object());
}
if(!ob2){
     ob2 = clone_object("players/earwax/monster/broker");
     move_object(ob2, this_object());
}
if(!ob3){
     ob3 = clone_object("players/earwax/csign");
...
*/
    ...
    dest_dir = ({ 
    "players/earwax/tp", "north",
    "players/earwax/ts", "west",
    });
}
...
north()
{
    call_other(this_player(), "move_player", "north#players/earwax/tp");
    return 1;
}

west()
{
    call_other(this_player(), "move_player", "west#players/earwax/ts");
    return 1;
}
...
*/
    ...
    filename = "/players/earwax/storage/"+this_player()->query_real_name();
    ...
    rm("/players/earwax/"+this_player()->query_real_name()+".temp");
    write_file("/players/earwax/"+this_player()->query_real_name()+
           ".temp", maxitems+1+"\n");
    i=2;
    ...
    rm(filename);
    cp("/players/earwax/"+this_player()->query_real_name()+".temp",
       filename);
    rm("/players/earwax/"+this_player()->query_real_name()+".temp");
    return 1;
    }
...
list()
{
    ...
    if (!read_file("/players/earwax/storage/"+
           this_player()->query_real_name(),1,1)) {
    write("You don't have a locker.\n");
    return 1;
    }
    tempstr = read_file("/players/earwax/storage/"+
            this_player()->query_real_name(),1,1);
    tempstr=extract(tempstr,0,strlen(tempstr)-2);
    sscanf(tempstr,"%d",maxitems);
    tempstr = read_file("/players/earwax/storage/"+
...
*/
    ...
    tempstr = "/players/earwax/"+this_player()->query_real_name()+".temp";
    rm(tempstr);
    filename = "/players/earwax/storage/"+this_player()->query_real_name();
...
deposit_item(item)
{
    ...
    filename = "/players/earwax/storage/"+this_player()->query_real_name();
    tempfile = "/players/earwax/"+this_player()->query_real_name()+".temp";
...
/*
    write_file("/players/earwax/LOGS/lockers",
