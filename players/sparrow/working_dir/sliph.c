 ...
 */
 if(arg || root()) return;
  call_out("self_destruct",50);
   ...
   set_short(HIW+"The "+NORM+HIK+"Silver "+NORM+HIW+"Sliph"+NORM);
   set_long(HIW+
	  ...
	  "her into the Silver Sliph. She never betrays her clients.\n"+NORM);
...
/* Traveling, when a person travels the objects dests itself
...
self_destruct()
{
 ...
 {
   while(remove_call_out("self_destruct") != -1); /* added this */
   call_out("self_destruct", 50);
   return 1;
 }
  here=Here;
  if(here)
  tell_room(here,HIK+"The Sliph yawns and dissapears down her well...obviously she was bored.\n"+NORM);
  move_object(clone_object("/players/data/well.c"), environment());
  destruct(this_object());
  return 1;
}
