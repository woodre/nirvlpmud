...
set_short(GRN+"The Broadsword of Balance"+NORM);
set_long(
...
int lev, xlev, hp, sp, mhp, msp, num;
  ...
  att = ob;
  if(lev < 10) return 0;
  if(!random(30)) {  /* 30% chance of balance */
  if(hp > (sp + 2) && !(hp > msp)) {
    num = hp - sp;
    num = num / 2;
  ...
  if(random(100) > 30){
     write(
  HIR+"You draw on the power of "+HIW+"C"+HIR+"haos"+HIW+"...\n\n"+
      "         - "+HIR+"S  L  I  C  E"+HIW+" -\n\n"+
      "   C"+HIR+"haos is imposed upon "+att->query_name()+"!\n"+NORM);
  return 25;
  }
  if(random(100) > xlev) return 15;
