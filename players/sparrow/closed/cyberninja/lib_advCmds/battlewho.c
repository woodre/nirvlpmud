...
/* this is practically a wizard command, downgrading, -Bp 12-2002 */
...
status main(string str) {
  ...
  No location, that's what the regular who's are for -Bp */
  write("Player          Opponent            Opp's Health \n");
  write(OFF+LINE);
  for(i=0; i < sizeof(all); i++) {
    if(all[i]->query_attack() &&
      ...
      if(!oppn) {
        int aln;
        aln = (int)opp->query_alignment();
        if(aln > 0) oppn = "Something Good";
        if(aln < 0) oppn = "Something Evil";
        if(aln = 0) oppn = "Something Netural";
      }
