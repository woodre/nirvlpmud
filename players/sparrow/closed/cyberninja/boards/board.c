...
if(IPTP->query_nohonor() && TP->query_level() < 20)
  {
write("As a Renegade, you may not use this board.\n");
    return 1;
  }
  ::new(str);
  return 1;
...
/* change by mizan */
/* added this as a fix, so all 'read' cmds arent hogged.
   there was a historic reason as to why the entire read cmd was
   hogged but i could not find it, so hence the change. */
  if(!present(AFFILIATE, TP) && TP->query_level() < 20 && str)
  {
