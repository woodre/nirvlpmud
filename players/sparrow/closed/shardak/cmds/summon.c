...
SummonDragon()
{
    ...
    needmana(COST);
    if((string)previous_object()->DragonName()==(string)this_player()->query_real_name()) return (write("You guys blew it so you have to change your dragon names.\n"), 1);
...
SummonSpinal()
{
   ...
   if(present("shardak_mark", this_player())->Spinal()) {
    write("You already have a Spinal.\n");
    return 1;
  }
...
sparkle in the black eye sockets.\n");
    ...
    destruct(bones);
    present("shardak_mark", summoner)->Spinal(dead);
