...
reset(int x)
{
  ...
  set_race("ghoul");
  set_gender("male");
  set_name(""+HIK+"fiend"+NORM+"");
  set_alias("evil fiend");
  set_alt_name("dark fiend");
  set_short("A "+HIK+"Fiend"+NORM+"");
  msgin="drifts in";
  msgout = "drifts"; 
...
catch_tell(string txt) {
   if (listen && owner) tell_object(owner, "Fiend> " + txt);
}
#endif
...
monster_died(object me)
{
   ...
   if (ob = present("corpse", environment()))
      destruct(ob);
   say("\
      Darkness surrounds " + query_name() + " and his\n\
      body vanishes.\n");
...
heart_beat()
{
   ...
   /* if (environment(owner) && !present(owner, environment())) {
     fixed this -Bp */
   if (environment(owner) && !present(owner->query_real_name(), environment(this_object()))) {
...

   {
      tell_object(att, query_name() + " visciously attacks with its " +
         weap->query_name() + ".\n");
      say(query_name() + " begins to F up " + att->query_name() +
         " with its " + weap->query_name() + ".\n", att);
      attacker_ob = att;
   }
  if(TO->query_attack()){
    tell_object(owner, 
     HIY+owner->query_name()+ "'s Fiend-> "+HIW+((hit_point * 10)/max_hp)+" / 10 health"+NORM+"\n");
  } 
   ::heart_beat();
}
...
confused()
{
   write(query_name() + " doesn't " + query_possessive() + " understand you.\n");
}
...
cmd_kill(string arg) {
  ...
  if(!meat->query_npc()) {
    write("The Fiend will not attack players!\n");
    return 1;
  }
  if(meat == this_object()) {
  ...
  tell_room(environment(this_object()),""+HIK+"Fiend"+NORM+" jumps into battle!\n");
