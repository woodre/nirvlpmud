...
heart_beat(){
  int stalking,i;
  object oldloc, newmonk;
  string *stalk_mess;
  ::heart_beat();
  
  /* if we are attacking and do not have Target defined, set it as the current target */
  if(att_object && !Target) {
    Target = this_object()->query_attack();
    stalking = 0;
  }
  
  /* if we have a target but are not in a fight appearantly they pussed out and we need
     to start stalking the punk 
  */
  if(Target && !att_object && (stalking == 0)) {
    oldloc = environment(this_object());
    /* move me to the stalking room */
    this_object()->move("suddenly, blending into the shadows#/players/sparrow/areas/bwo/rooms/shadows.c");
    /* check to see if we actually moved */
    if(!environment(this_object()) == oldloc) {
      /* put me into stalking mode */
      stalking = 1;
  }
  
  /* Check to see if the Monk is stalking anyone */
  if(stalking == 1) {
    /* Make sure that there is a target and that the target has moved from the
       original location of the battle. I don't want people just being able to 
       wait around for the monk to pop back in.
    */
    if(Target && member(users(),Target)){
      switch(random(5)) {
        case 0:
          if(!environment(Target) == oldloc) {
            /* Send the monk to the target and attack */
            stalking = 0;
          }
          break;
        case 1: case 2:
          /* Send a random stalking message to the target */
          stalk_mess = ({"The shadows seem to watch your every move.\n",
                         "You feel a strange weight pressing down on you.\n",
                         "You hear strange whispering coming from the shadows.\n"});
          tell_object(Target, stalk_mess[random(sizeof(stalk_mess))]);
          break;
        case 3: case 4:
          /* Do nothing */
          break;
    ...
    /* We either have no target or the target left the game, heal the monk and transport
       him to a new random room.
    */
    else {
      stalking = 0;
      newmonk = clone_object("/players/sparrow/areas/bwo/npc/bwo_template.c");
      move_object(newmonk, environment(this_object()));
      newmonk->reset(1);
      destruct(this_object());
    }
  }
  
  /* Check to see if we are in battle. If so, check our hps and attempt to slink
     into the shadows if hps are less than 300.
  */
  if(att_object && Target) {
    /* if hps are less than 300 and we have a target that is a player */
    if((this_object()->query_hp() < 300) && (Target->is_player())) {
      /* 30% chance of fleeing */
      if(random(10) < 3) {
