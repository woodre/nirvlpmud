...
*/
 ...
 if(sscanf(str, "%s gives %s to Storm.", a, b) == 2) {
  ...
  say("Storm says: Oh listen mighty warriors this is a task only the bravest\n" +
      ...
      "            free Stormhold of the evil that has befallen it and\n" +
      "            return it to peace and tranquility. If you complete\n" +
      "            this dangerous task return to me the head of Jeny for\n" +
      "            a great and powerful reward before I send you on your\n" +
      "            way I must warn you this task is both hard and treacherous\n" +
      "            not all is what it seems.\n\n" +
      "            Storm bows and changes back into a statue\n\n");
  stas=clone_object("players/persephone/closed/statc");
  move_object(stas, environment(TOB));
   move_object(TOB, "players/pavlik/rooms/dungeon.c");
  destruct(TOB);
  return;
 }
}
