...
extra_init(){
  if(call_other(this_player(),"query_real_name",0) == "persephone")  {
...
if (this_player()->is_invis() > 0 && invader !="persephone")  {
   this_player()->force_us("vis");
  }
if (secure == 0 && invader !=invited && invader !="persephone")  {
...
force(str)  {
  ...
  invited =str;
  tell_object(person,"You are brought to Persephone's workroom...\n");
...
TWO_EXIT("room/church","north",
         "room/shop","down",
         "You are atop the Fabled Olympos inside the House of Persephone",
         "\n"
