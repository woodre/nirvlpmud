...
reset(arg){
 if (!arg) {
  ...
  items=({"walls","The walls are painted in soothing shades of blue and have borders",
          ...
          "number","These are just room numbers. The one to the south is 102 and the\n" +
                   "one to the north is 101 definately not your rooms",
          "rooms","You can not really tell what the rooms look like you will have\n" +
                 "enter them if you are interested",
          });
  dest_dir=({"players/persephone/rooms/hotcor1a","west",
  });
 }
}
realm() { return "NT"; }
