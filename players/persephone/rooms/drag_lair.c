...
reset(int arg){
 if(!present("puf")) {
  move_object(clone_object("players/persephone/monster/magdrag"), TOB);
 }
  if (!arg){
  set_light(1);
  ...
  dest_dir=({"players/persephone/rooms/lair_ent","southeast"
  });
 }
}
init(){
...
pick(arg){
 if(!arg){
  write("Puf is too quick for your feeble attempt to rob from him to work.\n\n");
 return 1;
 }
 if(arg){
  write("Puf is too quick for your feeble attempt to rob from him to work.\n\n");
...
fang(arg){
 if(!arg){
  write("The dragon hide is too tough to fang.\n\n");
 return 1;
 }
 if(arg){
  write("The dragon hide is too tough to fang.\n\n");
