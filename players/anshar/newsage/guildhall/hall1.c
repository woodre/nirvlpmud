...
join_sage(str) {
  ...
  if(str == "sage" || str == "sages") {
    ...
    if(TP->query_guild_name() == "sage") {
      write("You are given your book.\n");
      move_object(clone_object("players/anshar/newsage/gob"),TP);
      return 1; }
...
add_to_guild(ob) {
  ...
  ob->save_me();
  ob->reset();
  gob = clone_object("/players/anshar/newsage/gob.c");
  write_file("/players/anshar/newsage/JOIN",ctime(time())+" "+
  TP->query_real_name()+"\n");
  move_object(gob, ob);
