/* INIT */
  ...
  setup(ENV(TO));
  add_action("sage_reset","sreset");
  add_action("shelp","shelp");
  add_action("shome","shome");
  add_action("stalk", "sage");
  add_action("semote", "sagee");
  add_action("stitle", "stitle");
  add_action("sage_score","score");
  add_action("sage_score","sc");
  ...
  environment()->set_guild_name("sage");
  "/players/anshar/newsage/objects/chatob.c"->resetchannel();
