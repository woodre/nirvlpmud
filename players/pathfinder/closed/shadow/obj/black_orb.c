...
#define GR this_player()->query_guild_rank()
#define XP this_player()->query_experience()
...
long() {
  ...
  write("You can 'swap' xps for guild xps.  and then 'advance' when you have enough.\n");
  return 1;
}
...
swap_exp_gxp(str) {
  ...
  if(TP->query_guild_name() != "shadow") {
    ...
    call_other(TP, "move_player", "cheating#room/church");
    return 1;
  }
  ...
  if(amt < 0) {
    write("You cannot change a negative number of xps.\n");
    return 1;
  }
...
advance_glevel() {
  ...
  if(TP->query_guild_name() != "shadow") {
    ...
    write("Guild!!\n");
    say(CAP(TP->QRN)+" is a cheater!\n");
    call_other(TP, "move_player", "cheater#room/church");
    return 1;
  }
...
/* if they do not have enough guild experience */  
  ...
  if(gxp < xpfor[(grank + 1)]) {
    write("You have: " + gxp + " gxp.\n");
    write("You need: " + xpfor[(grank+1)] + " gxp.\n");
