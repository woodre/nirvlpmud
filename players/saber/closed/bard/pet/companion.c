...
object ob;
  ...
  if(ob->is_player() || ob->is_pet())  {
    write("Pets can't attack players or pets.\n");  return 1;  }
...
/* Bard takes damage when pet dies */
...
/* Bard has pet give item to other player */
...
object target, item;
  ...
  if(call_other(what, "drop", 0))  {
    notify_fail(NAME+" can not take that from you.\n"); return 0;  }
  if(flag)  {
  say(PET+" flies off into the sky.\n\n"+
...
/* Bard has pet eat corpse to get healed */
...
/* Have pet follow master and tell pets health */
...
if(!present(MASTER,environment(this_object())))  {
  say(PET+" flies from the room after its master.\n");
  move_object(this_object(),environment(there));
  say(PET+" flies into the room.\n");
        }
call_out("pet_follow",6);
  return 1;
        }
...
/* Have pet give bard something that it is carrying */
...
/* Discouragement from attacking bardic pets */
...
/* Have pet peek into an adjacent room */

scout(str)  {
object ob2;
string there, short_str;
  if(!str)  {
    write("What direction do you want your "+NAME+" to scout?\n");
    return 1;
        }     
   command(str, this_object());
   write("You see through your "+NAMES+"'s eyes:\n");
...
/* Have pet buzz an object */

buzz(str)  {
object ob;
  if(!str)  {  
    write("What would you like your "+NAME+" to buzz?\n");  return 1;  }
  if(!(ob = present(str, environment(this_object()))))  {
    write("There is no "+str+" here to buzz.\n");  return 1;  }
  say(PET+" dives towards "+capitalize(str)+".\n");
  say("At the last moment the "+NAME+" banks and flies into the sky.\n");
  return 1;
       }
