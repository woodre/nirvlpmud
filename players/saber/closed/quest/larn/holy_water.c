/*
  Holy water from the Church of Larn!
 */
...
int count;
 ...
 reset(arg) {
    ...
    set_long("A bottle of holy water, blessed by the priests of Larn.\n"+
...
if(ob->is_player())  {
  write("Holy water may not be used against players.\n");
    return 1;
        }

if(ob->query_alignment() > 0)  {
  write("You douse "+ob->query_name()+" with holy water.\n"+
  ...
  say(tp+" douses "+ob->query_name()+" with holy water!\n"+
    ob->query_name()+" screams in pain as the water eats away at "+ob->query_objective()+"...\n");
  destruct(this_object());
  return 1;
}
