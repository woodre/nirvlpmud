...
reset(int arg){
if(!present("kitten"))  {
move_object(clone_object("/players/saber/closed/quest/items/kitten.c"),this_object());  }
 if(!arg){
...
items=({
 ...
 "ceiling","The ceiling is dripping.  That don't look too stable",
  ...
  dest_dir=({
 "/players/saber/closed/quest/ice/x2.c","east",
 "/players/saber/closed/quest/ice/x4.c","south",
           });
  }   }
init(){
 ::init();
