...
#define ERROR_LOG "/players/saber/closed/bards/log/BOARD"
#define BOARD_NAME "players/saber/closed/fem/femnotes"
...
id(str) { return str == "diary" || str == "silver diary"; }

long() {
   int ind;
   write("This is a silver diary with many pages full of notes.\n");
   write("Usage: note <headline>, read/remove <message number>\n");
   if(this_player()->query_level() > 20)
     write("        store <message number> <file name>\n");
   if(!msg_num) {
     write("The diary is empty.\n");
     return;
   }
   write("The silver diary contains "+msg_num);
   if(msg_num == 1)
     write(" note:\n\n");
   else 
     write(" notes:\n\n");
...
short() { 
   ...
   return "A silver bound diary ("+msg+")"; 
}
