...
*/
write(ob->query_name()+" has a "+blahstr+" soul.\n");
  call_other(TP, "add_spell_point", -5);
  return 1;
       }
...
object ob;
 ...
 if(ob = present(str, TP))  {
  ...
  else if(ob = find_player(str) && find_player(str)->query_invis()) {
...
/*  Fixed by saber - weapon and armor class lowering removed */
/*  spell now does 50 damage - only useable once  */
true_name(str)  {
object ob;
int aaa,bbb,ccc;
  if(!str)  {
  ...
  if(attack_spell(-189,9,50,ob) == -1)  return 0;
