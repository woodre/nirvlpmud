...
move_object(mark,this_object());
   ...
   if (!arg){
     set_name( "phoenix" );
     set_alias("pet");
     set_short("A firey phoenix with blazing eyes");
     set_alt_name("bird");
     set_gender("creature");
     set_race( "bird");
     set_long(
     ...
     load_chat("The phoenix looks up with a sparkle in its firey eyes.\n");
     load_a_chat("The phoenix circles its foe.\n");
     load_a_chat("The phoenix rakes its foe with razor sharp claws.\n");
      set_spell_mess1("The phoenix flies to its foe and EXPLODES in an inferno of fire.");
     set_spell_mess2("The phoenix flies up to you and EXPLODES in an inferno of fire.\nOuch!");
     set_chance(15);
...
init()  {
  ...
  if(present("instrument",this_player()))  {
   add_action("dart_command","phoenix");
        }
        }
...
say("The phoenix explodes in an inferno of crimson fire.\n\n");
...
if(!ob) { return 1; }
say("The phoenix launches itself towards "+capitalize(str)+".\n");
...
ob->hit_player(random(15) + 25);
remove_call_out("pet_follow");
tell_room(environment(this_object()),
     "The phoenix explodes in an inferno of crimson fire.\n"+
...
if(!item)  {
  ...
  if(what == "instrument" || what == "base_obj" || what == "rock" || what == "soul")  {
  return 1;  }
  if(flag)  {
  say("The phoenix flies off into the sky.\n\n"+
     "The phoenix returns and nods to "+this_object()->query_master()+".\n");
  } else {
  say("The phoenix takes an item from "+this_object()->query_master()+" and flies off into the sky.\n\n"+
     "The phoenix returns and nods to "+this_object()->query_master()+".\n");
         }
  tell_room(environment(target), "A phoenix flies in and gives a "+what+" to "+who+".\n\nThe phoenix flies off into the sky.\n");
...
object corpse;
 ...
 if(!corpse)  { notify_fail("There is no corpse here.\n"); return 0; }
  say("The phoenix descends upon the corpse and devours it.\n");
  destruct(corpse);
  heal_self(20);
  return 1;
         }
...
switch(word)  {
  ...
  default: notify_fail("Command the phoenix to do what?\n"); return 0; break;
  }
       }

talk()  {
this_object()->set_chat_chance(5);
write("You look over at your phoenix and smile.\n");
return 1;
        }

silence()  {
this_object()->set_chat_chance(0);
write("You look over at your phoenix and shake your head.\n");
  return 1;
        }
...
if(this_object()->query_hp() < 200)  {
  tell_object(ob, "Your Phoenix is at "+HP_LVL+"% of its health.\n");
        }

if(!present(master,environment(this_object())))  {
  say(
   "The phoenix soars off into the sky.\n");
  move_object(this_object(),environment(there));
  say(
   "A firey phoenix flies in after "+capitalize(this_object()->query_master())+".\n");
        }
call_out("pet_follow",6);
  return 1;
        }
...
string who_to,what;
object target, item;
if(!str)  { notify_fail("Have the phoenix give you what?\n"); return 0; }
if(sscanf(str,"%s to %s",what,who_to) < 2)  {
    notify_fail("Have the phoenix give you what?\n");  return 0;  }
target = present(who_to,environment(this_object()));
item = present(what,this_object());
if(!target || !living(target))  {
  notify_fail("Have the phoenix give the "+what+" to who?\n"); return 0; }
if(!item)  {
  notify_fail("The phoenix does not have a "+what+".\n"); return 0; }
if(what == "instrument" || what == "base_obj" || what == "rock" || what == "soul")  { return 1; }
tell_room(environment(this_object()),
  "The phoenix drops a "+what+" into "+capitalize(who_to)+"'s hand.\n");
move_object(item,target);
  return 1;
        }
...
tell_room(environment(this_object()),
  ...
  "\n       \"\This phoenix is under my protection.\"\n\n"+
...
tell_room(environment(this_object()),
  ...
  write("Your phoenix is not under attack by a player or pet or kid.\n");
  return 1;
        }
