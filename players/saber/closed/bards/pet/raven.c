...
say("The raven flies off into the sky.\n");
...
if(!ob) { return 1; }
say("The raven flies towards "+capitalize(str)+".\n");
...
tell_room(environment(this_object()),
   "The raven falls to the ground with a thud.\n\n");
tell_room(environment(this_object()),
...
if(!item)  {
  ...
  if(flag)  {
  say("The raven flies off into the sky.\n\n"+
     "The raven returns and nods to "+this_object()->query_master()+".\n");
  } else {
  say("The raven takes an item from "+this_object()->query_master()+" and flies off into the sky.\n\n"+
     "The raven returns and nods to "+this_object()->query_master()+".\n");
         }
  tell_room(environment(target), "A raven flies in and gives a "+what+" to "+who+".\n\nThe raven flies off into the sky.\n");
...
object corpse;
 ...
 if(!corpse)  { notify_fail("There is no corpse here.\n"); return 0; }
  say("The raven flies down to the corpse and plucks out its eyeballs.\n");
  destruct(corpse);
  heal_self(20);
  return 1;
         }
...
switch(word)  {
  ...
  default: notify_fail("Command the raven to do what?\n"); return 0; break;
  }
       }
...
string who_to,what;
object target, item;
if(!str)  { notify_fail("Have the raven give you what?\n"); return 0; }
...
tell_room(environment(this_object()),
  "The raven drops a "+what+" into "+capitalize(who_to)+"'s hand.\n");
move_object(item,target);
  return 1;
        }
