...
#define TPQ environment(this_object()) 
  ...
  query_auto_load()  { return "/players/saber/closed/virus/flu.c:"; }
  
  init()  {
   if(!TPQ) { destruct(this_object()); return 1; }
  if(TPQ != TP) return;
    add_action("sneeze","sneeze");
    if(TPQ->query_level() < 6) return destruct(this_object());
    if(TPQ->query_level() > 20)  {
