...
if(!present("trazzik")) {
move_object(clone_object("/players/saber/monsters/trazzik.c"),this_object());
}
if(!present("manes")) {
  for(i=0;i<6;i++){
move_object(clone_object("/players/saber/monsters/manes2.c"),this_object());
}
 if(!arg){
  set_light(1);
...
items=({
 ...
 "floor","The floor is covered with crimson flames",
  ...
  dest_dir=({
 "/players/saber/tower/tower9.c","south",
           });
  }   }
}
init(){
