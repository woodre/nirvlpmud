...
if(!present("dream")) {
  ...
  d1->set_short("Winnie the Vampire Cult Mistress (dream)");
  move_object(d1,this_object());
  d2 = clone_object("/players/saber/monsters/dream.c");
  d2->set_short("Whisper is a Cat with a sword (dream)");
  move_object(d2,this_object());
  d3 = clone_object("/players/saber/monsters/dream.c");
  d3->set_short("A figure carrying a corpse (dream)");
 ...
 if(!arg){
  ...
  long_desc=
"Hazy images of dark lights flash by and you realize that you are running.\n\n"+
"       running\n            running\n                 running\n\n"+
"You see a figure ahead of you carrying a corpse.\n"+
"He seems to be fleeing from a cat and a vampire.\n";

dest_dir=({
 "players/saber/tower/dream8.c","blink",
