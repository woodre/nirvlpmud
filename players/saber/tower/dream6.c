...
if(!present("dream")) {
  ...
  d1->set_short("A cat with a bird is Boris (dream)");
  ...
  d3->set_short("A cat with a mission is Tigerlily (dream)");
  move_object(d3,this_object());
  d4 = clone_object("/players/saber/monsters/dream.c");
  d4->set_short("A large black panther named Bagera (dream)");
 ...
 if(!arg){
  ...
  long_desc=
"A hazy image of a room filled with mounds and mounds of kitty litter.\n"+
"There seems to be several felines playing here.\n";

dest_dir=({
 "players/saber/tower/dream7.c","blink",
