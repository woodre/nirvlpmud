...
reset(int arg){
  if(!present("sign"))  {
move_object(clone_object("/players/saber/food/tshop.c"),this_object());  }
 if(!arg){
  set_light(1);
 short_desc="Tower Storage";
long_desc=
 ...
 "across the chamber walls.  You get the distince impression\n"+
...
items=({
 ...
 "room","A cold, damp room of worked stone",
  ...
  dest_dir=({
 "/players/saber/tower/tower2.c","west",
           });
  }   }
init(){
 ::init();
