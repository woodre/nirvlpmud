...
inherit "obj/weapon.c";
 ...
 reset(arg) {
    ...
    set_weight(2);
    set_value(900);
    /* changed to a fire damage special by illarion Dec 2004
...
fire_hit(attacker){
   ...
   if (w>6) {     
      ...
      /* direct hit_player call no longer needed
         the return is already randomized and doesn't need it again
      n = random(5);
      ahp = attacker->query_hp();
      if(n>ahp) { n = ahp - 1; }
      attacker->hit_player(n);
      */
      return 6; /* results in 0-5 fire damage */
    }
    return;
}
