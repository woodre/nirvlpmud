...
south() {
    call_other(this_player(), "move_player", "leaves#/players/saber/ryllian/orin1.c");
   return 1;
}
...
reset(arg) {
   ...
   if(!present("rose"))  {
    move_object(clone_object("/players/saber/stuff/rose.c"),this_object());  }
   if(!present("jacket"))  {
   move_object(clone_object("/players/saber/armor/ljacket.c"),this_object());   }
   if(!present("sword"))  {
   move_object(clone_object("/players/saber/weapons/longsword.c"),this_object());   }
   if(!we_is_ok)
      inventory("all");
}
