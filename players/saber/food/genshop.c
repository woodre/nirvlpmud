...
short() { return "A merchant buying and selling items"; }
long() { write("\n"+
 "A small merchant who will buy and sell items.\n"+
"He will let you 'list' & 'sell' & 'buy' & 'value' items.\n");
write("\n");
}
...
do_sell(ob) {
   ...
   if(value > 6000 || value < 50)
	do_destroy = 1;
    if (value > 1000) {
	write("The merchant is low on money...\n");
   value = 1000 + random(100);
    ...
    if (do_destroy) {
  write("This item is placed in the back room of the store.\n");
	destruct(ob);
	return 1;
    }
...
down() {
    ...
    call_other(this_player(), "move_player", "down#/players/saber/food/store2.c");
    return 1;
}
