...
reset(int arg){
if(!present("sickle")) {
move_object(clone_object("/players/saber/monsters/sickle.c"),this_object());  }
if(!present("fang")) {
move_object(clone_object("/players/saber/monsters/kitty.c"),this_object());  }
if(!present("relia")) {
move_object(clone_object("/players/saber/monsters/relia.c"),this_object()); }
 if(!arg){
  set_light(1);
...
items=({
 ...
 "stains","There are dark crimson stains upon the ground",
  ...
  dest_dir=({
 "/players/saber/ryllian/arena8.c","north",
 "/players/saber/ryllian/arena6.c","west",
           });
  }   }
init(){
 ::init();
