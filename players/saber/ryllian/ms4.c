...
reset(int arg){
if(!present("punk"))  {
move_object(clone_object("/players/saber/monsters/streetpunk.c"),this_object());
        }
if(!present("merchant"))  {
move_object(clone_object("/players/saber/food/atoneshop.c"),this_object());
       }
 if(!arg){
  set_light(1);
...
items=({
 ...
 "moon","A luminous full moon looms above",
  ...
  dest_dir=({
 "/players/saber/ryllian/ms1.c","north",
 "/players/saber/ryllian/ms5.c","east",
 "/players/saber/ryllian/ms7.c","south",
           });
  }   }
init(){
 ::init();
