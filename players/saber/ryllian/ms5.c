...
reset(int arg){
if(!present("merchant"))  {
move_object(clone_object("/players/saber/food/foodshop.c"),this_object());
       }
if(!present("guy"))  {
move_object(clone_object("/players/saber/food/gunshop.c"),this_object());
        }
 if(!arg){
  set_light(1);
...
items=({
 ...
 "moon","A luminous full moon looms above",
  ...
  dest_dir=({
 "/players/saber/ryllian/ms2.c","north",
 "/players/saber/ryllian/ms6.c","east",
 "/players/saber/ryllian/ms8.c","south",
 "/players/saber/ryllian/ms4.c","west",
           });
  }   }
init(){
 ::init();
