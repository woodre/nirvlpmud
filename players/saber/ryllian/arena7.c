...
reset(int arg){
if(!present("donrock"))  {
move_object(clone_object("/players/saber/monsters/donrock.c"),this_object());
}
if(!present("donnae")) {
move_object(clone_object("/players/saber/monsters/donnae.c"),this_object()); }
 if(!arg){
  set_light(1);
...
items=({
 ...
 "stains","There are dark crimson stains upon the ground",
  ...
  dest_dir=({
 "/players/saber/ryllian/arena8.c","south",
 "/players/saber/ryllian/arena4.c","west",
           });
  }   }
init(){
 ::init();
