...
#define SAVE_PATH "/players/pestilence/closed/destroyer/boards/"
#define MEMOBJ(x) (x ? present("destroyer_ob", x) : 0)
inherit BASE;
...
status cmdGook() {
    ...
    if (!messages || this_player()->query_level() < 40) return 0;
...
valid_edit(string author)
{
    ...
    if ((int) this_player()->query_level() >= 21) return 1;
...
cmdNote (string header)
{
    ...
    write("You do not have the proper dna to post here.\n");
    return 1;
                }
...
cmdRead (string str)
{
    ...
    mixed           header;
    if(!present("destroyer_ob",this_player())) {
    write("You can't understand this writing.\n");
    return 1;
                }

/*
    write("You can't read this board.\n");
*/
