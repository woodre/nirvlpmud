...
if(ETO) {  /*  needed for startup  */
  if(living(ETO)) {
    if(USER->query_alignment() > 100) {
    tell_object(environment(this_object()),
      ""+HIK+"You feel pain from the goodness inside you"+NORM+".\n");
    if(interactive(ETO)) 
    ETO->hit_player(10 + random(20), "other|poison");
...
f_you(){
  if(USER->query_real_name() == "grit") {
    ...
    USER->add_guild_xp(- (USER->query_guild_xp()));
    USER->set_guild_rank(0);
    USER->set_home("/room/church");
write("You lazy bastard you are no longer part of this guild.\n");
