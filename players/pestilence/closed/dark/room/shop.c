...
reset(arg) {
  ...
  short_desc = HIK+"Dark Order Consignment Shop"+NORM;
  long_desc=
"This is a shop located in the cave system of the Dark Order.\n",
  set_light(-1);
  items=({
...
\n\tContents from bags will always be sold when selling",
  ...
  dest_dir=({
    "/players/pestilence/closed/dark/room/dhall2.c", "south",
  });
  if( !find_object( "/players/pestilence/amity/room/shop.c" ) )
    call_other( "/players/pestilence/amity/room/shop.c", "???" );
