...
if(target->query_npc() && living(target)){
                if(USER->query_spell_point() < 30){
                        ...
                        if(target->webbed()){ 
                        tell_object(USER, "You cannot distraught "+target->query_name()+" again!\n"); 
                        return 1; 
                }
if(random(100) > (USER->query_attrib("int") + USER->query_attrib("wil") + random(100))){
tell_object(USER,
"You attempt to distraught "+target->query_name()+", but fail!\n");
USER->add_spell_point(-20);
return 1;
}
tell_room(environment(USER),
"A wave of "+HIK+"darkness"+NORM+" hits "+target->query_name()+" leaving them distraught!\n", ({USER}));
tell_object(USER,
"A wave of "+HIK+"darkness"+NORM+" hits "+target->query_name()+" leaving them distraught!\n");          
