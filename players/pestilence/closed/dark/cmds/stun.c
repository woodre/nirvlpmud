...
if(!str){
  if(this_player()->query_attack()) {att = this_player()->query_attack(); }
  else { write("Stun who???\n"); return 1; }
}
...
return 1; }
  if(!att->query_attack()) att->attack_object(this_player());
...
say(this_player()->query_name() +" gathers power from "+HIK+"dark"+NORM+" within the depths of "+HIR+"Hell"+NORM+".\n");
say(this_player()->query_name() +" weakens "+att->query_name()+" with a "+HIK+"dark"+NORM+" incantation.\n");
