...
query_message_hit(tmp) {
 ...
 if (tmp > 55) {
  ...
  mess = (""+HIW+"Smashed"+NORM+""," with a bone crushing sound");
  }
  else if (tmp > 20) {
...
gen_wc_bonus(){ 
        int total,str_bonus,dex_bonus,otherbonus,inf_bonus;
        total = 0;
   ...
   no_go_over += 1;
        if(!USER) return 0; /* just a test!  added by verte 6.23.01 */
            ...
            inf_bonus = present("destroyer_ob",this_player())->query_infantry();

        if(!USER->query_weapon()){
                total += inf_bonus * 3 + random(6);
}
        else {
