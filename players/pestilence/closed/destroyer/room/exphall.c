...
reset(arg) {
        if(!present("templar_trainer")){
                owner = clone_object("/players/pestilence/closed/destroyer/mob/brigden.c");
...
short() {
    return ""+YEL+"Explosives Training Hall"+NORM+"";
}
...
train(str) {
        ...
        if(!str) {
                write("You must type train explosives.\n");
                return 1;
        }
        if(present("destroyer_ob",this_player())->query_infantry()) {
                write("I'm sorry your already in the infantry group.\n");
                return 1;
        }
        ...
        if(str=="explosives") {
                ...
                current = guild_ob->query_bomb();
                if(current > 9) {
                        write("You have reached the highest Explosion level within the Army.\n");
                        return 1;
                }
                ...
                present("destroyer_ob", TP)->add_bomb(1);
                this_player()->save_me();
