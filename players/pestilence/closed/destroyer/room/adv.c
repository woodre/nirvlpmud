...
reset(arg) {
        if(!present("templar_trainer")){
                owner = clone_object("/players/pestilence/closed/destroyer/mob/rolin.c");
...
short() {
    return ""+YEL+"Training Barracks"+NORM+"";
}
...
advance(str) {
        ...
        if(!str) {
                write("You must type advance rank.\n");
                return 1;
        }
        ...
        if(str=="rank") {
                ...
                if(current > 9) {
                        write("You have reached the highest level within the Army.\n");
                        return 1;
                }
