...
reset(arg) {
        if(!present("templar_trainer")){
                owner = clone_object("/players/pestilence/closed/destroyer/mob/slaven.c");
...
short() {
    return ""+YEL+"Recon Training Hall"+NORM+"";
}
...
train(str) {
        ...
        if(!str) {
                write("You must type train recon.\n");
                return 1;
        }
        ...
        if(str=="recon") {
                ...
                current = guild_ob->query_recon();
                if(current > 9) {
                        write("You have reached the highest Recon level within the Army.\n");
                        return 1;
                }
                ...
                present("destroyer_ob", TP)->add_recon(1);
                this_player()->save_me();
