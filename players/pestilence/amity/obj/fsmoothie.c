/*
       Heal Cost Calculator (c) Fred 2009
        ...
        [4] Soak amount per charge:     8
        [5] Stuff amount per charge:    8
        ...
        The cost for this heal would be: 1328 coins.

Looking to sell for 1200, has a 10% chance to lose 10 hp and 10 sp per
heal attempt
*/
...
void reset(int arg)
{
  ::reset(arg);
  if(arg) return;
  set_name("a "+HIM+"Frozen Fruit Smoothie"+NORM+"");
  add_alias("fruit");
  add_alias("smoothie");
  add_alias("fruit smoothie");
  set_short("a "+HIM+"Frozen Fruit Smoothie"+NORM+"");
  set_long("This is a "+HIM+"Frozen Fruit Smoothie"+NORM+".\n"+
           "It is made of several different frozen fruit and provides a good boost to ones mana.\n");    
  set_type("sips");
  set_msg("You take a big gulp of the "+HIM+"Frozen Fruit Smoothie"+NORM+".\n");
  set_msg2(" takes a big gulp of the "+HIM+"Frozen Fruit Smoothie"+NORM+".\n");
  add_cmd("drink");
  add_cmd("gulp");
  add_cmd("sip");
  set_heal(0,50);
