...
reset(arg)
{
    ...
    toggle_off(MODE_PET_KILLER);           /* if the monster will attack pets spontaneously */
    toggle_off(MODE_PK_INSTIGATOR);        /* if the monster will attack PK players spontaneously */
    ...
    toggle_off(MODE_CORPSE_EATER);         /* If the monster consumes corpses whole instead of just taking them (emotes taken from Boltar's hoth monster */
