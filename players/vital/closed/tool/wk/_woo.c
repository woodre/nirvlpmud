...
"stf" || str == "soak" || str == "invis" || str == "pexp" || str == "prate")
 
#define DEFAULT ({"lev","xlev","room","gn"})
...
"stf":"query_stuffed","soak":"query_soaked","invis":"query_invis","prate":"party_rate","pexp":"party_exp"]);
    aligns = (["h":25,"lev":-5,"xlev":-4,"xp":-8,"cash":-6,"gn":10,"gr":-4,
"gx":-8,"gf":30,"wc":-2,"ac":-2,"ix":-2,"stf":-3,"soak":-3,"invis":-3, "prate":9, "pexp":9]);
    header = (["ip":"    Address    ","h":"        Hostname         ","lev":" Lvl ",
"xlev":"Xlvl.","age":"Age","env":"  Location                   ","room":
...
"stf":"STF","soak":"SOA","invis":"Inv","idle":"Idl","name":"  Name      ","prate":" XP/min  ","pexp":"Party XP "]);
}
...
print_help() {
    ...
    write("invis   -Print invis level\n");
    write("idle   -Print idle time\n");
    write("[f]    -Display return of function query_f\n");
    write("prate  -Print Party Experience rates\n");
    write("Defaults:\n");
...
add_portion(user, arg) {
    ...
    if(arg == "idle") {
        ...
        out = color_pad((arg=="env"?file_name(o):o->short()), 30);
        ...
        if(o = user->query_attack()) out = color_pad(o->query_name(), 15);
        else
        out = pad("Not fighting", 15);
    } else if(arg == "aatk") {
        if(o = user->query_alt_attack())
        out = color_pad(o->query_name(), 15);
        else 
        out = pad("Not fighting", 15);
    } else if(arg == "weap") {
        if(o = user->query_weapon())
        out = color_pad(o->query_name(), 10);
        else
        out = pad("No weapon", 10);
    } else if(arg == "hp") {
        if(user->query_level() > 20) out = pad("Coder",-7);
        ...
        if (o = present("party",user))
        {
          if (o->QPartyTime() < time())
            out = color_pad((o->QPartyExperience()) /
                ((time() - o->QPartyTime())/60),9);
          else
            out = color_pad(o->QPartyExperience()/1, 9);
        }
        else
          out = pad("No Party", 9);
    } else if (arg == "pexp") {
        if (o = present("party",user))
        {
          out = pad(o->QPartyExperience(), 9);
        }
        else out = pad("No Party", 9);
    }else {
...
{
  string help;
  help = "A Better Who! 'woo help' for more informtation";
  return help;
}
