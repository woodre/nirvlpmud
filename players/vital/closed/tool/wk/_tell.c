...
main(str)
{
  ...
  if(who->query_level() < 20 && this_player()->query_invis()) 
  {
    ...
    tell_object(who,format(color+"Someone tells you: "+NORM+foo2+" \n",70));
  }
  else
  {
    who->add_tellhistory("Vital tells you: "+foo2+" \n");
    tell_object(who,format(color+"Vital tells you: "+NORM+foo2+" \n",70));
    write(format("You tell "+color+capitalize(foo)+NORM+": "+foo2+" \n",70));
    who->Replyer("vital");
  }
  return 1;
}
