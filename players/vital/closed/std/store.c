 ...
 *                  All Rights Reserved.
 *  
 *  Source:			Started 2/10/01.
 *  
 *  Notes:			Adapted from store.c on Nirvana
 *  
 *  Change History: 
 *  
 *  
...
short() {
   return "Vital's Shop's Storerooms";
}
...
add_blaster() {
      object blaster;
      blaster = clone_object("/players/vital/dest/weapon/blaster.c");
      move_object(blaster, this_object());
}

add_jumpsuit() {
    object jumpsuit;
    jumpsuit = clone_object("/obj/armor.c");
    jumpsuit->set_id("jumpsuit");
    jumpsuit->set_name("jumpsuit");
    jumpsuit->set_alias("coverall");
    jumpsuit->set_short("All-purpose Jumpsuit");
    jumpsuit->set_long("This all-purpose jumpsuit is designed to protect \npeople from the general elements of the world as they travel. \nThe fabric is a light, synthetic blend that offers \nprotection from the elements.\n");
    jumpsuit->set_value(500+random(185));
    jumpsuit->set_weight(1);
    jumpsuit->set_ac(2);
    jumpsuit->set_type("armor");
    move_object(jumpsuit, this_object());
}
