...
move_north() {
    if(!present("vermina",TO) && !present("verminb",TO)) {
      ...
      this_player()->move_player("stumbling into darkness#/players/vital/dest/acadie/room/alley6.c");
	return 1;
    }
...
move_south() {
    if(!present("vermina",TO) && !present("verminb",TO)) {
      ...
      this_player()->move_player("stumbling out of the darkness#/players/vital/dest/acadie/room/sidewalk17.c");
	return 1;
    }
    write("You can't get past the vermin running around in front of you.\n");
    return 1;
