...
reset(arg) {
    ...
    long_desc =
    "  The Sewers\n\
This is a long tunnel that curves and twists off to the \n\
northeast. The floor curves up to become the walls which \n\
in turn curve on up to become the ceiling. The darkness \n\
could hide anything down here.\n ";
...
dest_dir = ({
    SPROOM+"under1.c","grate",
    SPROOM+"sewer2.c","northeast"
  });
...
add_monster() {
    ...
    ob->set_short(HIG+"Sewer "+GRN+"Monster"+NORM);
    ob->set_long("\
...
cannot hide!\n");
    ...
    MO(ob, TO);
    wep = CO("/players/vital/dest/weapon/lance.c");
    MO(wep,ob);
    command("wield lance");
    command("power up lance");
    }
    return 1;
}
