...
reset(arg) {
   if (arg) return;
   if(!present("trashcan")) {
      move_object(clone_object("/players/eurale/VAMPIRES/OBJ/trashcan.c"),TO); }
   set_light(0);
   ...
   items = ({
         "tables","Assorted sizes and shapes with many battle scars from\n"+
         "fangs, nails and objects places recklessly on them",
         "chairs","Stuffed chairs with large comfly arms and high backs.\n"+
         "Some are threadbare",
         "bar","A high bar with a brass footrail along the bottom. The cafe\n"+
         "drink stick comes from behind it",
         "sign","An large wooden sign painted in red with the Cafe's current\n"+
         "foods and refreshments along with their prices. Maybe you\n"+
         "would like to 'read' it",
         "plaque","A polished brass plaque with blazing red letters on it",
 });
