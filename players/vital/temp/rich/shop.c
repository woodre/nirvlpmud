...
THREE_EXIT("players/rich/room1.c", "south",
         "players/rich/medshop.c", "east",
         "players/rich/surstomai.c" ,"west",
         "a room",
	 "You are in the quartermasters. You can buy or sell things here.\n" +
 ...
 "To the east is the medical supply shop,to the west is the \n" +
 "starfleet surplus store and to the south is the exit.\n" +
 "Finally if there is a player out there that you really " +
  "\ncan't stand then go down to the photon torpedo room and buy\n" +
 "a torpedo key to launch a torpedo at that player.\n",
 1)
...
sell(item) {
    ...
    if(value > 1000 && value < 1300) value = 1000;
    if (value > 1000) {
	write("The shop is low on money...\n");
	value = 1000 + random(10);
    }
...
value(item) {
	...
	write("But the store is low on money, so you would only get 1300.\n");
    }
    return 1;
}
