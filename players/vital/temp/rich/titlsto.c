...
short() { return "store room for the title change store"; }
...
fill() {
    if (!present("titlchn")) {
        move_object(clone_object("players/rich/titlchn.c"), this_object());
    }
}


long() { write("All things from the title change store are stored here.\n"); }
