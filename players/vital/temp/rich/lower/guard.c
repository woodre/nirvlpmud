...
"It is professional looking short sword, used by warriors and guards");
	...
	call_other(key, "set_alias", "jeffries key");
	call_other(key, "set_short", "A jeffries key");
...
ONE_EXIT("players/rich/lower/room1d.c","north",
	 "A room",
	 "You are in a room.\n" +
...
unlock(str) {
	...
	return 0;
    if (!present("jeffries key", this_player())) {
...
east() {
    ...
    call_other(this_player(), "move_player", "east#players/rich/lower/jeffries.c");
    return 1;
}
