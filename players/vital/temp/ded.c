 ...
 * be improved.  (see functions read_filearr and write_filearr)
 *
...
/* How to print out the range of numbers from a to b */
...
/* need to check this in inserts and whatnot */
...
/* for undo */
string action;  /* what was the action */
mixed old;      /* the old stuff (usually text or array of text) */
int where,towhere;      /* what line(s) */

short() { return "ed"; }
id(str) { return str==short(); }
long() { write(short()+".\n"); }
get() { return 1; }

init() {
  if(TP->query_real_name()!="vital") return 1;
...
comm(str) {
  ...
  if(ret==-1) return;  /* either quit or another input_to */
  if(!ret)
    if(settings[TRANS]) command(str,TP);  /* str might not be full command */
