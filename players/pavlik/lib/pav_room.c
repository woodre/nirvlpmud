 ...
 * Thanks to Balowski for this func
 */
...
area_yell(str){
  ...
  else if(environment(this_player())->query_area()) {
    loc = environment(this_player())->query_area();
    ob = users();
    for(i=0; i<sizeof(ob); i++) {
      if(environment(ob[i])->query_area() == loc)
        tell_object(ob[i],
        capitalize(this_player()->query_real_name())+" yells: "+str+"\n");
    }
    return 1;
  }
