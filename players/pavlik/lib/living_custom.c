...
hit_player(dam,zap) {
  ...
  if (this_player() && this_player() != this_object() && this_player()->query_level() < EXPLORE) {
	...
	if ( this_player()->is_player() &&
        ...
        !this_player()->valid_attack( this_object())) { /* bad attack, could be logged */
...
/***********************************/
  ...
  tell_room(environment(this_object()), "[Hit Player]: Damage before AC: "+dam+".\n");

  dam -= random(ac_tmp + 1);
  tell_room(environment(this_object()), "[Hit Player]: Damage after AC: "+dam+".\n");
...
attack()
{
  ...
  tell_room(environment(this_object()), "Weapon Class: "+weap_tmp+"\n");
  ...
  if(whit) {
	...
	tell_room(environment(this_object()), "Weapon Class + Weapon Hit: "+weap_tmp+"\n");
  }
  pal_tmp=weap_tmp;
...
/******************************/
  ...
  if(whit != "miss" && attacker_ob) {
	tell_room(environment(this_object()), "Calling hit_player with weap_tmp ["+weap_tmp+
	"] and spell_dam ["+spell_dam+"]\n");
	tmp = call_other(attacker_ob, "hit_player", 
	knt_tmp+random(weap_tmp - intox_no) + spell_dam);
	tell_room(environment(this_object()), "Hit Player returned: "+tmp+"\n");
  }
  else
	tmp = 0;
...
drop_all_money(verbose) {
  ...
  {
      save_me();
  log_file("ADD_MONEY", ctime(time()) + " " + object_name(environment(this_object())) + " " + name + " droped " + tmp + " coins\n");
  }
}
