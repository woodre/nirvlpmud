...
reset(arg) {
  ...
  set_wc(15);        /* low AWC cuz the weapon has really high return */
...
stop_fight() {
  if(find_player("pavlik")) {
    if(!this_player())
       tell_object(find_player("pavlik"), "Stop_Fight: No this_player\n");
    else
       tell_object(find_player("pavlik"), "Stop_Fight: this_player is: "+
       this_player()+"\n");
  }
