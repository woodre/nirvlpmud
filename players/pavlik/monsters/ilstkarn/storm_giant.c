...
reset(arg){
 ...
 move_object(clone_object("players/pavlik/armor/storm_shield"), this_object());
 move_object(clone_object("players/pavlik/armor/storm_cloak"), this_object());
...
stun(){
 ...
 if(present("concussion_obj", obj)){
    ...
    ob = clone_object("players/pavlik/items/concussion");
    move_object(ob, obj);
    ob->do_conc();
  }
}
...
monster_died(){
 ...
 if(this_player())
  if(this_player()->query_real_name())
    ...
    capitalize(this_player()->query_real_name())+" killed the Storm Giant. ["+ctime()+"]\n");
