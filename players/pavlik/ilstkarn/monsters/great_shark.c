...
reset(arg) {
  ...
  enable_commands();
  set_living_name("great_shark"); /* Rumplemintz */
}
...
go_move() {
  ...
  if(this_object()->query_attack() && !random(6)) {
	devour();
	return 1;
  }
  i = random(4);
...
devour() {
  ...
  "\n");
  tell_object(obj,
  "You are "+HIW+"swallowed whole"+NORM+"!\n");
...
monster_died() {
  ...
  ob = clone_object("players/pavlik/ilstkarn/treasure/eaten_corpse");
  move_object(clone_object("players/pavlik/ilstkarn/armor/rusty_plate"), ob);
  move_object(clone_object("obj/money"), ob);
  present("money", ob)->set_money(2200+random(1000));
