...
reset(arg) {
  ...
  move_object(clone_object("players/pavlik/ilstkarn/obj/basalisk_weapon"),
  this_object());
...
hit_player(f, y)
{
  ...
  if(y=="physical") {
    int x;
     x = 2+random(3);
     if(this_player() && present("look_away", this_player())
      )
    {
    f = f - f/x;
    }
    else if(this_player() && !random(3)) petrify(this_player());
   if(f < 1) f = 1;
   }
    dam = ::hit_player(f, y);
   return dam;
...
go_blind(str) {
  ...
  if(str == "away") {
    ...
    obj = clone_object("players/pavlik/ilstkarn/obj/look_away");
...
petrify(ob) {
  ...
  if(random(100) < chance) {
    obj = clone_object("players/pavlik/ilstkarn/obj/stone_curse");
...
monster_died() {
  ...
  move_object(clone_object("players/pavlik/ilstkarn/armor/stone_armor"),
  environment(this_object()));

  move_object(clone_object("players/pavlik/ilstkarn/weapons/lightning_rod"),
  environment(this_object()));
