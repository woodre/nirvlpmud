...
reset(arg) {
  ...
  "find a place with some air.  The sandy lake bottom slopes gently up\n"+
  "here.  You might be able to walk right out of the lake from this point.\n";
  dest_dir=({
	"/players/pavlik/ilstkarn/lake4", "north",
	"/players/pavlik/ilstkarn/null", "up",
	"/players/pavlik/ilstkarn/null", "out",
  });
}

query_ilstkarn(){ return "lake7"; }
realm(){ return "NT"; }
...
go_out() {
  write("You swim out of the water.\n");
  say(capitalize(this_player()->query_name())+" swims out of the water.\n");
  this_player()->move_player("out#players/pavlik/ilstkarn/beach4");
  return 1;
}
