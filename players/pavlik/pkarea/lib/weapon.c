...
/*
 * Based on /obj/armor.c modified for Pavlik's PK-Area.
 * Changes (other than formatting) to this file can be found by
 * searching for "PAV".
 */

status wielded;
status debug;			/* PAV: addition */
...
set_damage_type(type, amount) {
  damage[type] = amount;
}

query_damage_type(type) {
  return damage[type];
}

add_damage_type(type, amount) {
  damage[type] += amount;
}
...
init() {
  ...
  add_action("weapon_debug","weapon_debug");
}
...
id(str) {
  return str == type || str == name_of_weapon || str == alt_name || str == alias_name || 
  str == "weapon" || str == "MATRIX_OBJ" ;
}
...
/*
 * PAV: addition
 * This is the badboy that needs workin.
 * attack calls this function and adds the result to the weapon class
 * of the weapon.  The resulting number is called to hit_player() to
 * do damage.
 *
 * I will be keeping weapon class on weapons set to 0 and doing the damage
 * through the hit() function, after adjusting the damage amount based on
 * the victims armor class vs. the weapon type.
 */
hit(attacker) {
  int i, dmg, total, protection;
  string tmp;
  dmg = allocate(DAMAGE_TYPES);
  protection = allocate(DAMAGE_TYPES);

  if(hit_func) {
	dmg =  call_other(hit_func,"weapon_hit",attacker);
  }
  if(dmg) {
	for(i=0; i<DAMAGE_TYPES; i++) dmg[i] += damage[i];
  }
  else {
	dmg = damage;
  }
  if(present("HEAD_ARMOR", attacker)) {
	protection = present("HEAD_ARMOR", attacker)->query_protection_total();
	for(i=0; i<DAMAGE_TYPES; i++) {
		if(dmg[i] > 0) dmg[i] -= random(protection[i]);
	}
  }
  total = 0;
  tmp = "Damage ";
  ...
  if(debug) write(tmp+" Total: "+total+"\n");
  return total;
}
...
weapon_debug() {
  if(this_player()->query_level() >= 21) {
	if(debug) {
		write("Weapon debug is now off.\n");
		debug = 0;
	}
	else {
		write("Weapon debug is now on.\n");
		debug = 1;
	}
	return 1;
  }
