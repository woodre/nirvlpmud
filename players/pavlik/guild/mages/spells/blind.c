...
spell()
{
  ...
  if( (x >= MAX_ALTERATION_NUM) || (x + ALTERATION_NUM > MAX_ALTERATION_NUM) ||
      (present("blind_obj", obj) && present("blind_obj", obj)->query_owner(tp->query_real_name())) )
  {
...
/*      Changing this call, do_spell doesn't have any arguments - Rump
        if(temp && living(obj)) call_other(temp, "do_spell", tp->query_real_name());
*/
        if(temp && living(obj)) call_other(temp, "do_spell");
