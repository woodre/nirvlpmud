...
#define NOTES "players/pavlik/guild/mages/obj/bboard_notes"
...
long() {
  write(
 "A small section of this wall is used for Mages to post thoughts and\n"+
 "ideas.  This collection of knowledge is to better the Guild for\n"+
 "everyone.  Use '"+HIC+"post <subject>"+NORM+"' and '"+HIM+"read <#>"+
 NORM+"' to use.\n"+
 "This board is open for all guild members, please be considerate.\n");
    if (num_messages == 0) {
      write(HIR+"It is empty.\n");
        return;
    }
    write(HIC+"The bulletin board contains "+num_messages);
    if (num_messages == 1)
        write(" note:\n\n");
    else
        write(" notes:\n\n");
...
short() {
  return (HIC+"The Mages Board "+NORM+CYN+"["+num_messages+"]"+NORM);
}
...
headers() {
    ...
    write(HIM+"["+i+"]"+NORM+ ":\t\b\b\b"+ rancol2(hd) +"\n");
        i += 1;
    }
}
