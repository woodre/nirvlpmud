...
#define MM capitalize(drag->query_nme())
...
heal_spell(drag){
 ...
 capitalize(drag->query_owner())+" for a moment.\n");
 tell_object(find_player(drag->query_owner()),
 "Your dragon casts a "+HIB+"spell of healing"+NORM+" and you feel rejuvantated\n"+
 "as the magic replenishes your soul.\n");
 find_player(drag->query_owner())->heal_self(heal);
...
lightning_bolt(drag){
 ...
 if(random(4) >= 2){
  ...
  "A blazing arrow of magic shoots forth from "+II+"'s fingertip!\n");
  else tell_room(environment(drag),
