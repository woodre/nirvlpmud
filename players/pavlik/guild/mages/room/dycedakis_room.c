...
reset(arg) {
  if(arg) return;
  set_light(1);
  short_desc=RED+"Dycedakis' "+NORM+HIBLK+"Chambers"+NORM;
  long_desc=
	"You have entered the chambers of "+HIBLK+"shadow"+NORM+" and "+MAG+"illusion"+NORM+".  A dark foreboding\n"+
	...
	"A "+HIBLK+"black cat"+NORM+" lies dormant before a narrow doorway to the east.  The\n"+
	"disturbing caw of a perched raven pierces your concentration.  The hallway behind you\n"+
	"seems very bright in comparison to this dark and shady chamber.\n";
  ...
  dest_dir=({
	ROOM_DIR+"spell_masters",	"north",
	"null",				"east",
  });

  if(!present("dycedakis", this_object()))
	move_object(clone_object(NPC_DIR+"dycedakis"), this_object());
...
no_way()
{
  ...
  say("The "+HIBLK+"shadows"+NORM+" grow deeper and a bead of sweat rolls down your neck.\n");
  return 1;
}
