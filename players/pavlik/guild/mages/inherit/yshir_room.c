...
reset(arg) {
  ...
  items=({
	...
	"weavings.  Although the couch is obviously a priceless artifact.\n"+
	"You feel welcome to sit upon it",
    "piano",
	"The piano is a sleek black baby grand.  The piano plays along\n"+
	"quietly, even though there is noone seated at its bench.\n"+
	"There is some sheet music on top of the instrument",
    "sheet music",
  ...
  dest_dir=({
	ROOM_DIR+"spell_masters",	"south",
	ROOM_DIR+"null",		"north",
  });

  if(!present("couch", this_object()))
	move_object(clone_object(OBJ_DIR+"yshir_couch"), this_object());
...
north()
{
  write(
    ...
    "suddenly blasts a few menacing chords.\n"+
