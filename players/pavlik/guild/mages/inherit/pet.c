...
pet_follow()
{
  ...
  if(!(owner = query_owner())) return;
  if(!environment(owner)) return;
  if(!(f_msg = query_follow_msg())) f_msg = "wanders";
  if(!(pet_name = query_name())) pet_name = "generic";

  if(query_follow() && environment(this_object()) != environment(owner))
  {
	tell_room(environment(this_object()), capitalize(pet_name)+" "+f_msg+" out.\n");
	move_object(this_object(), environment(owner));
	tell_room(environment(this_object()), capitalize(pet_name)+" "+f_msg+" in following "+OWN+".\n");
  }
}
