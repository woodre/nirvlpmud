...
TWO_EXIT("players/mouzar/castle/newbie/trail12", "south",
        "players/mouzar/castle/newbie/storage", "west",
         "The Town Shop",
"You have entered the Town Shop. Feel free to look around a buy or sell thing.\n" +
"Commands are: 'buy item', 'sell item', 'sell all', 'list', 'list weapons'\n"+
"'list armours' and 'value item'.\n" +
"There is a door that is locked up tight to the north with big letters on it\n" +
"saying 'KEEP OUT!'.\n", 1)
...
value(item) {
    ...
    if (!name_of_item) {
        if (call_other("players/mouzar/castle/newbie/store", "value", item))
            return 1;
        write("No such item ("); write(item); write(") here\n");
...
buy(item) {
    if (!item)
        return 0;
    call_other("players/mouzar/castle/newbie/store", "buy", item);
    return 1;
}
...
north() {
    if (call_other(this_player(), "query_level", 0) < 20) {
        ...
        say(call_other(this_player(), "query_name", 0) +
            " tries to walk through the north door, but bounces off.\n");
        return 1;
    }
    write("The door magically opens up for you to enter.\n");
    this_player()->move_player("north#players/mouzar/castle/newbie/store");
    return 1;
}
 
 
list(obj) {
    call_other("players/mouzar/castle/newbie/store", "inventory", obj);
    return 1;
}
