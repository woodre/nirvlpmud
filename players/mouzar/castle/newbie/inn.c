...
ONE_EXIT("/players/mouzar/castle/newbie/trail3","north",
"The Brass Tankard Inn",
"You are in the The Brass Tankard Inn. Here you can buy food to still your\n"+
...
show_menu() {
 ...
 if(!(cm||mm||rmm)) 
  ...
  write("1: Newbie's Meal        ");write(cm);write(" at 240 gp\n");
  write("2: Adventurer's Meal     ");write(mm);write(" at 480 gp\n");
  write("3: Wiz's Meal            ");write(rmm);write(" at 720 gp\n");
  write("4: A Mug of Ale       ");write("alot at");write("  100 gp\n");
  write("\n");
...
pays(n) {
 if(this_player()->query_gold()<n) {
  Speak("You cannot afford that!!");
  return 0;
 }
 this_player()->add_gold(-n);
 return 1;
}
...
if(s=="1"||s=="<1>") {
  ...
  if(!pays(20)) return 1;
  if(Level>6) {
   Speak("You don't look like a newbie to me.");
   if(mm||rmm) {
  ...
  write("You are served a newbie's meal - very nourishing\n");
  say(Name+" orders a newbie's meal\n");
  cm=cm-1;
  return tease(4);
 }
/* merchant's meal, price 50, heals 8, preferably for levels 7-12 */ 
 if(s=="2"||s=="<2>") {
  ...
  if(!pays(50)) return 1;
  if(Level>21) {
   Speak("You look more like a wiz to me.");
   if(rmm) {
    Speak("You should eat food more suited for you.");
    return 1;
   }
  ...
  write("You are served a Adventurer's meal - very good\n");
  say(Name+" orders an Adventurer's meal\n");
  mm=mm-1;
  return tease(8);
 }
/* rich man's meal, price 90, heals 12, preferably for levels 13+ */ 
 if(s=="3"||s=="<3>") {
  ...
  if(!pays(90)) return 1;
  write("You are served a wiz's meal - very delicious\n");
  say(Name+" orders a wiz's meal\n");
  rmm=rmm-1;
  return tease(12);
 }
 if(s=="4"||s=="<4>"||s=="mug"||s=="beer") {
  ...
  write("You down a mug of hot ale - That feels good.\n");
  say(Name+" downs an ale.\n");
  return 1;
 }
