...
reset(arg){
 if(!arg){
  ...
  dest_dir=({"players/mouzar/quest/lroom1","north",
             "players/mouzar/quest/lroom4","west",
             "players/mouzar/quest/lroom6","east",});
 }
 if(!present("guard",this_object())){
   move_object(clone_object("/players/mouzar/quest/mon/mguard"),this_object());
}
init(){
  ::init();
...
south() {
  if(open == "yes"){
    ...
    ME->move_player("south#/players/mouzar/quest/proom2");
