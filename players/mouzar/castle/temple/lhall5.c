...
reset(arg) { 
  if(!present("elemental", this_object())) {
    move_object(clone_object("/players/mouzar/quest/mon/water"), this_object());
  }
  set_light(1);
}
init() {
...
south() {
  ...
  this_player()->move_player("south#/players/mouzar/quest/lhall6");
  return 1;
}
north() { 
  this_player()->move_player("east#/players/mouzar/quest/lhall4");
  return 1;
}
