...
short() {
  ...
  if(set_light(0)) {
    short=short_desc;
    if(!dest_dir || sizeof(dest_dir)==0) return short;
    short+=" [";
    x=1;
    while(x<sizeof(dest_dir)) {
      if(dest_dir[x]=="north") short+="n"; else
      if(dest_dir[x]=="northeast") short+="ne"; else
      if(dest_dir[x]=="east") short+="e"; else
      if(dest_dir[x]=="southeast") short+="se"; else
      if(dest_dir[x]=="south") short+="s"; else
      if(dest_dir[x]=="southwest") short+="sw"; else
      if(dest_dir[x]=="west") short+="w"; else
      if(dest_dir[x]=="northwest") short+="nw"; else
      if(dest_dir[x]=="up") short+="u"; else
      if(dest_dir[x]=="down") short+="d"; else
      short+=dest_dir[x];
      if(x<=sizeof(dest_dir)-2)
        short+=",";
      x+=2;
    }
    short+="]";
...
add_exit(string where, string what) {
dest_dir = dest_dir + ({where,what,});
  init();
}
set_outdoors(x) { int y; y = x; }
