...
id(str) {
    return str == "board" || str == "junk board";
}
 
long() {
    write("Mouzar's junk board.\n");
    if (num_messages == 0) {
        write("It is empty.\n");
        return;
    }
    ...
    say(call_other(this_player(), "query_name") +
          " studies the junk board.\n");
    headers();
}
 
short() {
    return ("The Junk board");
...
reset(arg) {
    if (arg)
        return;
    restore_object("players/mouzar/LOGS/worknotes");
}
...
get_body(str) {
    if (str == "**") {
        ...
        new_body = 0;
        new_hd = 0;
        save_object("players/mouzar/LOGS/worknotes");
        write("Ok.\n");
        who = 0;
        return;
    }
