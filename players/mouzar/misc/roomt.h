...
#define RN           capitalize(this_player()->query_real_name())
#define TOB          this_object()
#define ENV(x)       environment(x)
#define HERE         ENV(TPL)
#define NPC(x)       (x)->query_npc()
#define ISPL(x)      query_ip_name(x)
#define put_monster(str) { move_object(clone_object("players/mouzar/monsters/"+str),TOB); }
#define put_object(str) { move_object(clone_object("players/mouzar/obj/"+str),TOB); }
