...
#include "/players/data/weather/new/basic/timedep.h"




inherit "/players/data/weather/new/room.c";
...
void configure() {
  ...
  set_exits(({ "./start","./own_tdep" }),({"out","north" }));
  
  // We want to have an NPC here, too, so let's load it. POP_BLUEPRINT is
  // defined in <populate.h>. Further examples for usage of populating a room
  // via basic/populate (which is inheritted by complex/room already) with
  // more description can be found in other example rooms.
  
  set_populate_living(({ EXAMPLES_NPCS"beerie", POP_BLUEPRINT }));
}
