// The central parser module. Not an easy read.
...
#if 0 // Ugh Aug-01-1995 because inheriting objects can't call the simul_efun
// make_shared_string() is internal wizardry. It returns the
// same string that has been passed as an argument, but the
// result will use less space by having identical strings
// share memory.
private string make_shared_string(string s)
{
  return order_alist( ({ s }) )[0][0];
}
#endif
...
// Ugh May-13-1998
    ...
    G_ADVERBS:          LIB_GRAMMAR;"parse_adverbs",// Coogan, 27-Feb-2002
    G_COLOUR:           LIB_GRAMMAR;"parse_colour", // Coogan, 24-Feb-2000
    G_RACE:             LIB_GRAMMAR;"parse_race",   // Coogan, 26-May-2003

// #ifdef INTERMUD3  // commented out, Coogan, 19-Oct-2001
...
static varargs void add_rule(mixed rulestring,string action,string handler,
  ...
  if (pointerp(rulestring)) {
    ...
    // Strip off the verb and all leading words.
    verb = tokens[0];
    // Ugh 1995-Apr-19 (substituted the next part by the function call to 
    // make_rule()
    rule = make_rule(tokens);
  }
  family = family || DEFAULT_FAMILY;
  // Ugh Jan-06-1998 don't use query_rules() anymore, could be overloaded
  if (!(list = rules && rules[family] && rules[family][verb]))
    list = ({ ({ }), ({ }), ({ }) });
  list[0] += ({ rule });
  list[1] += ({ handler || this_object() });
  // Coogan: here the handler is still valid
  list[2] += ({ action });
  if (!rules)
  ...
  // Coogan: here the handler is still valid, even when a cloned room.
  // After returning from all the add_rule()s a query_rules() doesn't give
  // any handlers, when the particular handlers are cloned rooms.
}

// Ugh Aug-04-1995 -- for saved rules (in a global parser?)
static varargs void
...
// Ugh Aug-04-1995 -- for saved classes (in a global parser?)
...
private string class_scope;  // made it private, Coogan, 24-Aug-2001
                        ...
                        mixed handler,string action)
{
  ...
  // for some unknown reasons, the handler is sometimes lost (as far as
  // I experienced only in case of cloned rooms with actions.
  // So I added the !handler-check, Coogan, 06-Jun-2001
  if (!handler)
    handler = this_object();
      ...
      for (i=0;i<sizeof(rule);i+=2)
      {
        ...
        else // Two classes to parse.
        {
          ...
          for (mid = first;mid<last;mid++)
          {
            ...
            class_scope = rule[i][0];
       //DEBUG_POINT_TP("coogan", ({ call1ob, call1func, first, mid, words, text }));
...
// for a given verb.
// Changed from return type int to mixed, Coogan, 26-Sep-2010
varargs mixed parse_sentence(mixed sentence, mixed family)
{
  ...
  if (stringp(sentence))
    text = 
#if 0  // Alfe 1997-Oct-2002
      regexplode(sentence,"[a-zA-Z0-9][-a-zA-Z0-9'_]*[a-zA-Z0-9]|[^ \t\n]");
#else
      regexplode(sentence,REGEXP_PARSER_WORD);
#endif
  else
    text = sentence;
  // words doesn't hold the whitespace part.
  if (sizeof(text) < 2)  // changed <1 to <2 to avoid the error that arose when
                         // the player just typed a tab as command
                         // Alfe 1996-Jun-17
    return 0;
  ...
  else if (stringp(message) || closurep(message)) //closurep() added by
...
varargs static void add_actions(string func, string family) {
  ...
  // Ugh 1995-Apr-2009 (used query_rule_verbs())
