...
long()
{
  ...
  write( HIW+
	  ...
	  "her into the Silver Sliph. She never betrays her clients, so feel\n"+
	  "free to"+HIB+" Ask"+HIW+" her a question or you may wish to"+HIB+" Travel."+NORM+"\n");
	return 1;  
	 }
...
if(str == "sliph" || str == "well")
    {
   write(HIG+"You stand before the well and using your great power you call forth the Silver Sliph!\n"+NORM);
   say(HIG+TPN+" stands before the well and uses their great power to call forth the Silver Sliph!\n"+NORM);
   summoned = 1;
   add_action("travel_place", "travel");
   add_action("listen_to", "ask");  
   call_out("self_destruct",100);
 return 1;
   }
}
listen_to(str)
...
mother_cry01(object who)
{
  ...
  tell_room(Here, GRN+"The Silver Sliph Whispers"+HIW+": Welcome "+who->query_name()+". I am called the Silver\n"+
...
mother_cry02(object who)
{
   ...
   if(!who) return 0;
   if(moving_time) return 0;
   tell_room(Here, HIG+"The Silver Sliph shifts her mass and peers with great interest,\n"+
                       "her face inches from yours as she gives you a sly smile.\n"+NORM+
                    "\n");
...
mother_cry03(object who)
{
   ...
   if(!who) return 0;
   if(moving_time) return 0;
   tell_room(Here, GRN+"The Silver Sliph whispers"+HIW+": Centuries later, War Wizard Data Rahl\n"+
                       "brought me back and asked me to provide my services if anyone\n"+
					   "was in need of traveling help. As I am always happy to please this\n"+
					   "wasn't hard for me to accept such responsibilities. No one else shall\n"+
	                   "know of our pleasure "+TPN+"\n"+NORM+
                       "\n");
...
mother_cry04(object who)
{
   ...
   if(!who) return 0;
   if(moving_time) return 0;
   tell_room(Here, GRN+"The Silver Sliph asks"+HIW+": "+who->query_name()+", do you wish to 'travel'?\n"+NORM+
                    "\n");
...
travel_place(str)
{
    ...
    if(!str)
 {
    write(HIM+"The Sliph looks up and thinks for a moment before replying:\n"+NORM+HIW+
		  ...
		  "magic that Data used has had side effects on me.\"\n"+NORM+HIB+
		  "\n"+
		  "\n"+
		  "Nirvana\n"+NORM+HIK+
    ...
    switch(str)
 {
    case "outer boundary": location="/players/data/OUTERB/ROOMS/entrance.c"; break;
    case "nirvana": location="room/south/sforst3"; break;
    default: write(HIM+"The Sliph says: I don't know that place, please choose another.\n"+NORM);
             return 1;
             break;
 }
  write("You tell the Sliph that you wish to travel to the "+str+".\n");
  say(HIW+TPN+" dives into the Silver Sliph's Froth."+NORM);
  gifted=this_player();
	holdpen = clone_object("/players/data/sliphhold.c");
	holdpen->set_own(TPN);
	move_object(TP,holdpen);
    mover = clone_object("/players/data/sliph.c");
	mover->set_location(location);
...
self_destruct()
{
  ...
  if(here && here->is_player())
 {
  ...
  here=Here;
  if(here)
  tell_room(here,HIK+"The Sliph excuses herself and dissapears down her well as her services\n"+
                     "are needed else where.\n"+NORM);
  summoned = 0;
   remove_action("travel");
   remove_action("listen");  
  return 1;
}