...
short(){
	...
	return "A Defibrillator";
        ...
        long(){
	write("This defibrillator can be used to resurrect your mud child\n"+
...
/* Need to know what kid to rez*/
if(!str) {
write ("We need a name.\n");
return 1;
}
/* Checks all the daycares for the child name, when found it will restore that daycare file */
nom = this_player()->query_real_name();
 ...
 cap_name=capitalize(me);
	...
	/* Check to see if child is in daycare */
   if(!kidname) {
  write("Your child is in daycare.\n");
   return 1;
    }
	
	/* Check to see if the child is alive, and if so where */
	...
	/* If child isnt in daycare, or alive then it proceeds with the following */
