...
/*************** ASSASSIN ****************************************************************/
if(assassin){
	 "/players/data/closed/wizard/test_obj5/heart_beat.c"->main(0, this_object(), USER);
}
/*************** BERSERKER ***************************************************************/
...
/******************************************************************************************/
	...
	if(strength_bonus || xstr){
		if(USER->query_attrib("str") >= 20){
			if(USER->query_attrib("str") < USER->query_attrib_max("str") + strength_bonus + xstr){
				USER->set_attrib("str", USER->query_attrib_max("str") + strength_bonus + xstr);
       ...
       if(stamina_bonus || xsta){
		if(USER->query_attrib("sta") >= 20){
			if(USER->query_attrib("sta") < USER->query_attrib_max("sta") + stamina_bonus + xsta){
				USER->set_attrib("sta", USER->query_attrib_max("sta") + stamina_bonus + xsta);
				tell_object(USER, "You feel stamina enter your body as you grow in experience.\n");
			}
		}
	}
   if(!stealth_bonus && !xste && USER->query_attrib("ste") > USER->query_attrib_max("ste")){
      USER->set_attrib("ste", USER->query_attrib_max("ste"));
      }
   
	if(stealth_bonus || xste){
      if(USER->query_attrib("ste") > USER->query_attrib_max("ste") + stealth_bonus + xste){
        	USER->set_attrib("ste", USER->query_attrib_max("ste") + stealth_bonus + xste);
         }
