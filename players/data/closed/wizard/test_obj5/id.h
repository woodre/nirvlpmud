id(str){
	...
	if(previous_object()){
		if(!test_my_wear(previous_object())) {
			...
			if(offhand_name){
				...
				str == "no_pet" || str == "wizard_gob" || str == "no_spell" || str == "generic_wc_bonus" ||
  ...
  (skills[70] > 2 ? (str == "dark_sight_object") : (str == "warrior_object"));
			}
			else{
				return str == "guild_object" || str == "warrior_gob" || str == "prof_guild_object" ||
				str == "no_pet" || str == "wizard_gob" || str == "no_spell" || str == "generic_wc_bonus" ||
...
if(riposte && ATT && offhand && USER->query_pl_k()){
	    "/players/data/closed/wizard/power/riposte_damage.c"->main(this_object(), USER, damage, p);
	}
if(evil_aura){
      if(random(100) < skills[91] * 15){
		"/players/data/closed/wizard/power/prot_evil.c"->main(this_object(), USER, damage, p);
      }
    }
if(good_aura){
      if(random(100) < skills[90] * 15){
		"/players/data/closed/wizard/power/prot_good.c"->main(this_object(), USER, damage, p);
      }
     }
}
