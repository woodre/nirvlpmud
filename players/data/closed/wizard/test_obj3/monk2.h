...
monk_bonus(){
	...
	if(samt){
		...
		if(perc_end() >= 9) bonus = 17 + random(samt * 2);
		else if(perc_end() > 1) bonus = 14 + random(samt * 2);
		else bonus = 5 + random(samt);
		
              if(strmod > 0 && strmod < 16)  bonus += 0;
      	else if(strmod > 15 && strmod < 21) bonus += random(4) + 1;
   		else if(strmod > 20 && strmod < 22) bonus += random(5) + 1;
			else if(strmod > 21 && strmod < 24) bonus += random(6) + 1;
			else if(strmod == 24) bonus += random(7) + 1;
			else if(strmod == 25) bonus += random(8) + 2;
			else if(strmod == 26) bonus += random(9) + 3;
			else if(strmod == 27) bonus += random(10) + 4;
         else bonus += random(12) + 5;
		...
		if(!random(2)){
			switch(random(11) + 1){
				case 11: one = YEL+"Bear "; bonus += 3; break;
				case 10: one = HIR+"Dragon "; bonus += 3; break;
				case 9:  one = HIY+"Tiger ";    bonus += 2; break;
				case 8:  one = HIC+"Wolf ";   bonus += 1; break;
				...
				case 3:  one = HIB+"Hawk ";   bonus += 1; break;
		...
		if(random(100) < samt * 7) bonus += (samt * 2) + 2;
...
lower_ac(){
	...
	if(ATT->query_npc()){
		if(present("acwc_modifier", ATT)){
			...
			woot = clone_object("/players/maledicta/closed/w/OBJ/mob_mod.c");
		...
		if(present("acwc_modifier", ATT)){
			...
			woot = clone_object("/players/maledicta/closed/w/OBJ/player_mod.c");
			move_object(woot, ATT);
			woot->add_ac_mod(1);
		}
	}
