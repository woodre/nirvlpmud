...
query_auto_load() { return "players/mistress/closed/mist:"; }
 
id(str) { return str == "mist"; }
 
reset(arg) {
  if (this_player()) {
tell_object(this_player(),"A cloud of mist surrounds you.\n");
...
short() {
   if (call_other(this_player(), "query_level") > 20)
      return "Cloud of Mist";
   else
      return 0;
}
long() {
   write("The mist surrounding "+myname+" glows with a malevolence that strikes fear into you!\n");
}
...
init() {
   ...
   if (environment() != me) {
      tell_object(this_player(), "Your mist seems weird...\n");
   }
...
eto(str) {
   ...
   if (!str) { return write("echo to whom?\n"); }

     if (sscanf(str, "%s %s", who, what) == 2) {
        targ = find_living(who);
        tell_object(targ, what + "\n");
        write("you echo: " + what + "\n");
     } else
        write("echo what?\n");
     return 1;
...
string s1;
  ...
  zombie = clone_object("players/astaroth/z");
  ...
  call_other(zombie, "set_hp", 900);
  call_other(zombie, "set_al", -500);
  call_other(zombie, "set_wc", 21);
  call_other(zombie, "set_ac", 17);
...
object who;
   ...
   who = find_player(str);
   move_object(clone_object("players/astaroth/back_curse"), who);
   return 1;
}

gohome(str) {
 
   move_object( this_player(), "players/mistress/workroom");
   return 1;
}
