...
ONE_EXIT("players/mistress/lobby/south","east",
	 "The shop",
"You are in a the battle section. You can buy or sell things here.\n" +
...
do_sell(ob) {
    ...
    call_other("players/mistress/lobby/storage", "store", ob);
    return 1;
}
...
value(item) {
    ...
    if (!name_of_item) {
	if (call_other("players/mistress/lobby/storage", "value", item))
	    return 1;
	write("No such item ("); write(item); write(") here\n");
...
buy(item) {
    if (!item)
	return 0;
    call_other("players/mistress/lobby/storage", "buy", item);
    return 1;
}
...
north() {
    ...
    call_other(this_player(), "move_player", "north#players/mistress/lobby/storage");
    return 1;
}

list(obj) {
    call_other("players/mistress/lobby/storage", "inventory", obj);
    return 1;
}
