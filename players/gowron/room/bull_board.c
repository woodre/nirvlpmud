...
long() {
    write("This is the bulletin board for Springfield.  If you have any\n");
    write("questions, suggestions or flames, post them here to be heard.\n");
...
short() {
    return ("The Springfield Bulletin Board(" + num_messages + " msgs)");
}
...
reset(arg) {
    if (arg)
	return;
    if(!restore_object("/players/gowron/room/springbull")) 
      save_object("springbull");
}
