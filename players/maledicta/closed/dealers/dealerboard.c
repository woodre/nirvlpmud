/* Orginal code from Bern's Clerics, 
   ...
   Modified again to become the drug dealers board by Mal.
*/
...
id(str) {
    return str == "dealer_board" || str == "display" || str == "bulletinboard";
}

long() {
    write("You can set up new messages with the command 'note headline'.\n");
    write("Read a message with 'view num', and remove an old message with\n");
    write("'remove num'.\n");
    if (num_messages == 0) {
	write("It is empty.\n");
	return;
    }
    write("The computer display contains " + num_messages);
    if (num_messages == 1)
	write(" message:\n\n");
    else
	write(" messages:\n\n");
...
short() {
    return HIG+"Computer Display"+NORM;
}
...
reset(arg) {
    if (arg)
	return;
    restore_object("players/maledicta/closed/dealers/dealerboard");
}
...
new(hd) {
    ...
    if (strlen(hd) > 50) {
	write("Too long header to fit the display.\n");
	return 1;
    }
    new_hd = hd;
    input_to("get_body");
...
get_body(str) {
	...
	new_body = 0;
	new_hd = 0;
	save_object("players/maledicta/closed/dealers/dealerboard");
	write("Ok.\n");
	who = 0;
	return;
    }
