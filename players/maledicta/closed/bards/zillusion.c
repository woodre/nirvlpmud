   ...
   if(!living(ob)){ 
	...
	if (!ob->query_npc()){
      ...
      TP->add_spell_point(-20);
   	binding = clone_object("/players/reflex/guilds/bards/binding.c");
  ...
  if(!str){  write("song <darken, light, or out light>\n"); return 1; }
	if (str == "darken") {
      ...
      if(!creator(environment(TP))){
		...
		/* </change> */
		move_object(clone_object("/players/reflex/guilds/bards/dark.c"),
...
chord_of_distortion(str)
{
   ...
   default:        acboost = 0; break;
	...
	ob = clone_object("/players/reflex/guilds/bards/bard_defend.c");
	move_object(ob, TP);
...
song_of_masking(){
   ...
   m = clone_object("/players/reflex/guilds/bards/hide_room");
...
song_of_illusion(str)
{
	...
	if (spell(-194, 7, 40) == -1)
		return 0;
	if (temp == "here") {
		move_object(clone_object("/players/reflex/guilds/bards/ill_obj.c"),
					environment(TP));
