...
init(){
 if(!environment()) return;
  restore_object("players/maledicta/templar/members/"+TP->query_real_name());
...
save_ob() { 
 ...
 if(!nom) return;
 save_object("players/maledicta/templar/members/"+nom);
 return 1;
}
...
new_quit(){
  ...
  save_object("players/maledicta/templar/members/"+TP->query_real_name());
  write("Saved...\n");
...
leave_templar(){
  ...
  rm("/players/maledicta/templar/members/"+TP->query_real_name()+".o");
