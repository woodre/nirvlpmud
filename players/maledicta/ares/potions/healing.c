/* 50 add_hit_points, 3 charges, at 800 coins.  stuff and soak heal
...
short() { return "Potion of "+GRN+"Healing"+NORM+" "+BOLD+uses+NORM+""; }  
long(){
   write(
"This is a small vial that holds a magical healing liquid.\n"+
"To use it 'imbibe' the potion.\n"+
...
write(
"You sip from the potion and feel healed!\n");   
call_other(this_player(),"add_hit_point", 50);
call_other(this_player(),"recalc_carry");

write(
"     HP>>>"+HIM+""+HP+""+NORM+"/"+HIM+""+MHP+""+NORM+"  "+
" Stuff["+HIM+""+USER->query_stuffed()*100/(USER->query_level()*8)+"%"+NORM+"]  "+
" Soak["+HIM+""+USER->query_soaked()*100/(USER->query_level()*8)+"%"+NORM+"]\n");
uses -= 1;
write("The potion has "+HIR+"["+uses+"]"+NORM+" left.\n");
