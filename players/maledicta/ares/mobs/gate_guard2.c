...
reset(arg)  {
	...
	set_alias("templar");
	set_race("human");
	set_short(BOLD+"Templar "+NORM+GRN+"Castle Gate Guard"+NORM);
	set_long(
...
monster_died(){
if(DAEMON->query_gl(2)){
if(query_attack()){
   write_file("/players/maledicta/templar/logs/GUARD2",
   query_attack()->query_name()+" "+ctime(time())+".\n");
   }
   tell_room(environment(),
   HIG+"\nTemplar reinforcements arrive!"+NORM+"\n\n");
   move_object(clone_object("/players/maledicta/ares/mobs/gate_guard2.c"), environment());
   DAEMON->sub_guards(2);
   send();
   }
}
...
send() {
   ...
   for(i=0; i<sizeof(list); i++) {
      ...
      if (cross && !cross->query_block()) {
          tell_object(list[i], 
          HIR+"(Templar War) A Castle Gate guard has been killed!"+NORM+"\n");
       }
   }
}
