 ...
 * and call local functions to configure it.
 Added the option for a mob to block exits.
 USE:  exit_block = ({}); to set exits in an array.
       set_exit_mess() will set the message given when the mob blocks
       set_blocking()  will set to where a mob will NOT block if being
...
string *exit_block;    /*  Can use to block exits in room */
string exit_mess;
int blocking;
...
/* Block message and special blocking sets */
set_exit_mess(string str){ exit_mess = str; }
query_exit_mess(){ return exit_mess; }

set_blocking(int i){ blocking = i; }
query_blocking(){ return blocking; }
/*******************************************/
/* Optional */
set_alias(a) { alias = a; }
/* Optional */
...
init() {
  ...
  int i;  
    if(exit_block){
    while(i < sizeof(exit_block)){
    add_action("blocked_exit", exit_block[i]);
    i++;
    }
   } 
...
name+" jumps in front of "+this_player()->query_name()+"!\n");
else
tell_object(this_player(), exit_mess);
return 1;
  }
if(!attacker_ob){
if(!exit_mess)
