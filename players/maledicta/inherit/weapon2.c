 ...
 * The reason of this, is that the above saves a lot of memory.
 ...
 Added:  Strength bonus to the weapons.  This can be set to have an 
         effect by using - set_str_wep()  1 = in effect. 0 = none
         It defaults to 1, because 95% of the weapons on the mud
         will gain this bonus.  This makes changing the guns,etc.
         over to 0 easier than vice-versa.
 */
...
strength = environment(this_object())->query_attrib("str");
...
write("Strength Mod: "+str_mod+"\n");
    if (hit_func)
        str_mod += call_other(hit_func,"weapon_hit",attacker);
        
write("Strength & Whit: "+str_mod+"\n");
    return str_mod;
...
/********* Strength Bonus*************/
set_str_wep(int i){ str_wep = i; }
query_str_wep(){ return str_wep; }
/*************************************/ 
