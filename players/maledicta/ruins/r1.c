...
secret_way(string str){
	if(!str){ write("What?\n"); return 1; }
	if(str == "ruins"){
        /*set my title with altair - its good to be da king */
        if((string)this_player()->query_real_name() == "altair"){
        this_player()->set_pretitle(BOLD+BLK);
        this_player()->set_title(""+NORM+""+HIR+"Knight of Blood"+NORM+"");
         }
