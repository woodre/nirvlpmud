...
id(str) {
  ...
  if(!test_my_wear(previous_object())) {
    return (::id(str) || str == "glove" || str == "gloves" ||
    str == "notarmor" || str == "knife");
     }
   }
  return (::id(str) || str == "glove" || str == "gloves" || str == "knife"); 
}
...
if(W >= 7)  {
      ...
      if((attacker->query_hp() - 10) > num){ 
       attacker->hit_player(num, "other|dark");

if(random(80) < USER->query_attrib("ste")){ 
write(
HIM+"You grasp your opponent with both gloves and feel a rush of power!"+NORM+"\n\n");
        USER->add_hit_point(random(8) + 1);
        }
        return 5;
     }
if(random(80) < USER->query_attrib("ste")){ 
...
collect_reward(string str){
  int coins;
   ...
   if(wielded){ write("You must unwield it first!\n"); return 1; }
