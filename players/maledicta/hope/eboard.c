...
#define BOARD_NAME "players/maledicta/hope/eboard"
#define TEMP present("KnightTemplar", this_player())
#define GLVL this_player()->query_guild_rank()
...
long() {
	int ind;
	write(
      "This is the Elite Board. Here topics pertaining to the\n"+
      "order may be discussed in an open manner.\n");
	write("Usage : note <headline>, study/remove <message number>\n");
	if (!msg_num) {
...
reset(arg) {
	if (arg)
		if (!random(5)) {
			say("A squire appears and secures some " +
			"notes on the board that were loose.\n");
			say("The squire leaves again.\n");
		}
}
...
remove_msg(what_msg) {
	...
	if (!TEMP || GLVL < 9 || !query_ip_number(this_player())) {
                return 0;
	}
