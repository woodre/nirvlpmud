...
if(!arg){
  ...
  set_id("clip");
  set_short("a clip [Blaster Rifle]");
  set_long(
  "A large metal clip for a blaster rifle. It carries 1000 rounds\n"+
  "of energy.\n"+
  "Usage: insert clip.\n"+
  "       force_insert clip.\n");
  set_weight(1);
...
object fgun;
fgun = present("gun_rifle_object", environment(this_object()));	
    ...
    write("You don't have a blaster rifle!\n");	
    return 1;
    }
return 0;
}
...
object gun;
gun = present("gun_rifle_object", environment(this_object()));	
    if(!str){ write("insert what?\n"); return 1; }
    if(str == "clip"){
    	if(gun){
