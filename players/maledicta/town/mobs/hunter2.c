...
reset(arg)  {
	...
	move_object(clone_object("players/maledicta/town/armor/wshield.c"), this_object());
	heals = 9;
}
...
heart_beat(){
	...
	if(!hunting_now && !hunting_it &&
	   sscanf(environment()->short(), "%s[restaurant%s", place, booyah) != 2){
		...
		if(query_attack() && random(100) < 25){
			tell_room(environment(),
				"Whiteblade fires a deadly round of blaster fire at "+opp->query_name()+"!\n");
                        opp->hit_player(15+random(35),"other|laser");
		}
		...
		if(hunting_now && hunting_now->query_ghost() &&
		   !present(hunting_now, environment())){
			tell_room(environment(),
		...
		if(hunting_now && present(hunting_now, environment()) && !query_attack()){
			if(sscanf(file_name(environment(hunting_now)), "players/maledicta/ruins/%s", place) ||
				sscanf(file_name(environment(hunting_now)), "players/dragnar/Wastelands/%s", place) ||
				sscanf(file_name(environment(hunting_now)), "players/maledicta/town/%s", place) ||
				sscanf(file_name(environment(hunting_now)), "players/maledicta/cave/%s", place) ||
				sscanf(file_name(environment(hunting_now)), "players/maledicta/castle/%s", place) ||
				sscanf(file_name(environment(hunting_now)), "players/maledicta/cont/%s", place) ||
				sscanf(file_name(environment(hunting_now)), "players/maledicta/ares/%s", place) ||
				sscanf(file_name(environment(hunting_now)), "players/fakir/ways/%s", place) ||
				sscanf(file_name(environment(hunting_now)), "players/jaraxle/3rd/%s", place)){
		...
		if(hunting_now && !query_attack()){
			if(sscanf(file_name(environment(hunting_now)), "players/maledicta/ruins/%s", place) ||
				...
				sscanf(file_name(environment(hunting_now)), "players/fakir/ways/%s", place) ||
				sscanf(file_name(environment(hunting_now)), "players/jaraxle/3rd/%s", place)){
					if(!environment(hunting_now)->no_hunter()){
						tell_room(environment(),
		...
		if(!sscanf(file_name(environment(this_object())), "players/maledicta/town/rooms/%s", place) &&
			!query_attack()){
				tell_room(environment(),
					"Whiteblade leaves.\n");	
				move_object(this_object(), "/players/maledicta/town/rooms/t9.c");	
