...
inherit "players/mythos/amon/forest/airmon";
reset(arg)  {
  ::reset(arg);
  if(arg) return;
  set_name("firedrake");
  ...
  set_distance_effect("The Drake opens it's mouth and "+RED+"fire"+NORM+" spews forth!\n");
  ...
  if(random(5) == 0) {
  set_mult(2 + random(2));
  set_mult_chance(1 + random(10));
  set_mult_dam1(1);
  set_mult_dam2(random(10)); }
...
object leave;
  switch(random(4)) {
    case 0: leave = clone_object("/players/mythos/awep/forest/claw.c"); break;
    case 1: leave = clone_object("/players/mythos/aarmor/forest/carapace.c"); break;
    case 2: leave = clone_object("/players/mythos/amisc/forest/tooth.c"); break;
    case 3: leave = clone_object("/players/mythos/amisc/forest/meat.c"); break; 
  }
