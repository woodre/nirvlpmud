...
baglist() {
 ...
 for(h=0;h<sizeof(ob);h++)
 {
   ...
   if(ob[h] && ob[h]->short())
   {
     ...
     if(!ob[h]->query_save_flag() && (object_name(ob[h])[0..2] != "obj"))
       write("Saveable\n");
...
Note: objects of value 0 will be sold as well.\n\
  ...
  if(sscanf(str,"%s %s",null,name) == 2 && null == "inbag")
  {
    ...
    if(!ob)
    {
      notify_fail(capitalize(name)+" is not in the bag.\n");
      return 0;
    }
    j = ob->query_value();
    j = j - (2*(j/10));
    if(j > 1000) j = 1000;
    tp->add_money(j);
    write("You have received " + RED + j + NORM + " coins for selling " +
      ob->short() + ".\n");
     ...
     str == "nosave" || str == "noset")
  {
      ...
      if(ob[h])
      {
        if(ob[h] != this_object())
        {
          if((str == "nosave" && ob[h]->query_save_flag()) ||
            ...
            (str == "noset" && ((!ob[h]->weapon_class() ||
            ob[h] != tp->query_weapon()) && (!ob[h]->armor_class() || 
            !ob[h]->query_worn()))))
          {
    ...
    if(str=="all")
    {
      if(!this_object()->queryKept())
      {
        tp->add_money(query_value());
        tp->add_weight(query_weight());
        destruct(this_object());
        /* Had to get rid of this if we sell all */
      }
    }
    return 1;
  }
  ...
  if(!ob)
  {
    notify_fail(capitalize(str)+" is not in your inventory.\n");
    return 0;
  }
  if(!ob->short() || ob->query_auto_load() || !ob->query_weight() || 
    !ob->get() || ob->drop())
  {
    notify_fail("That item is not sellable!\n");
    return 0;
  }
  j = ob->query_value();
  j = j - (j/10);
