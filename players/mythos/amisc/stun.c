...
reset(arg) {
  ...
  set_id("tochau");
  set_short("The stunned body of Tochau");
  set_long("The stunned body of Tochau....You may wish to 'bond' yourself to it\n"+
...
bond() {
  ...
  mon=clone_object("/players/mythos/amisc/tochau.c");
  move_object(mon,this_player());
  write_file("/players/mythos/prac/ob",ctime(time())+"\t"+
  this_player()->query_real_name()+"\t\t"+this_player()->query_level()+"\ttoc\n");
  destruct(this_object());
  return 1;  }

vanish() {
  write("The body flickers and vanishes.\n");
  say("The stunned body of Tochau flickers and vanishes.\n");
  destruct(this_object());
return 1;
}
