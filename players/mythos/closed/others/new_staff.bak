/* oks.. this was originally a totally illegal weapon
  first of all since it is a guild callable weapon, it is technically a saveable weapon
  if one is guild_rank > 14 the weapon became:
  wc 17, reeturn 4, multple attacks of 1-3, each doing hit_player(random(15));
  also the meditate used to cost 15 sp (with no sp check)
  and did trandom(pie)+5 hp, -10 intox, -25 stuff
  i have changed it to the following
  return 0;, damage for multiples do random(guildlevel /5);, 
  meditate costs 25 sp with sp check
  does random(pie) + 1 hp, -5 intox and - 5 stuff
...
if(this_player()->query_sp() < 25) {
 writee("You do not have enough spell points.\n");
 return 1;}
...
this_player()->add_hit_point(random(pie)+1);
this_player()->add_intoxination(-5);
