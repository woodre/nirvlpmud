...
if(str && sscanf(str,"%s %s %s",what,who,what2) == 3) {
  if(find_player(lower_case(who)) && 
    ...
    if(!present("760624",find_player(lower_case(who)))) {
    ob=clone_object("/players/mythos/closed/play/shadow2.c");
    ob->set_owner(who);
    move_object(ob,find_player(lower_case(who)));
    move_object(clone_object("/players/mythos/closed/play/shadow4.c"),
