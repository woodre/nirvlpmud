...
static string title;
static int shop_dest_flag, level_not;
...
long() {
        cat("/players/mythos/closed/guild/books/"+title+".0");
...
if(str==title) {
  if(con==0) {
    ...
    say(capitalize(this_player()->query_real_name())+" opens "+ capitalize(title)+
      " and begins to read.\n");
    cat("/players/mythos/closed/guild/books/"+title+"."+page);
  return 1;}
...
close(str) {
  ...
  if(str==title) {
   if(con==1) {
    con=0;
    page=0;
    say(capitalize(this_player()->query_real_name())+" closes "+capitalize(title)+"\n");
