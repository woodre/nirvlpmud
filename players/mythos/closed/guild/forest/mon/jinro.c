...
reset(arg)  {
  ...
  set_alias("servant");
  set_race("hunters");
  set_short(HIR+"Jinro"+NORM+"- Half man/ half beast");
  ...
  /* Changed by sparrow to fit new spell format. Also to work in damage types 
  set_mass_chance(25);
  set_mass_dam(75);
  ...
  */
  add_spell("flash",
      HIC+"    *   *     *  FLASH  *    *   *\n"+NORM,
      HIC+"    *   *     *  FLASH  *    *   *\n"+NORM,
      30,({25,25,25}),({0,"other|dark","other|evil"}),1);
  set_mult(4);
  set_mult_chance(75);
  set_mult_dam1(1);
  set_mult_dam2(10);
...
grim() {
  ...
  if(this_object()->query_ac() < 17) { 
    this_object()->set_ac(17); 
  } 
  if(this_object()->query_wc() < 45) { 
    this_object()->set_wc(45); 
  }
  /*
  if(present("demon",ep)) { 
    destruct(present("demon",ep)); 
  }
  */
