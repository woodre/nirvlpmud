...
six(str) {
  ...
  write_file("/players/mythos/closed/guild/prop/s/"+epn,ctime(time())+" Spell  "+str+"\n");
...
seven(str) {
  ...
  write_file("/players/mythos/closed/guild/prop/s/"+epn,ctime(time())+" Price  "+str+"\n");
  tell_object(ep,"The man nods and jots it down.\n"+
...
eight(str) {
  if(str == "y") {
     ...
     return 1;}
  if(str == "n") {
     rm("/players/mythos/closed/guild/prop/s/"+epn);
