...
{
  object wc_mod;
  wc_mod = clone_object("/players/mythos/closed/guild/spells/wc_modifier");
  move_object(wc_mod, ob);
  wc_mod->start_it(-(2+random(4)),1000);
}
if(!(ob->query_attack())) { ob->attack_object(tp); }
ob->hit_player(20+random(60),"other|dark");
