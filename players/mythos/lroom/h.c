...
reset(arg) {
  if(!present("scribe")) {
    move_object(clone_object("/players/mythos/closed/guild/task/3/scribe.c"),this_object());
    }
  if(!arg) {
  set_light(1);
    short_desc = "Library: ** History Stacks **";
    long_desc =
    "You are in a room filled with books.  The books cover the\n"+
    "history of Nirvana from its beginnings to the present.  'Search'\n"+
    "the stacks to find the books to 'read' to your heart's content!\n";
  items = ({
    "back","Leads to the rest of the library",
  });

  dest_dir = ({
    "/players/mythos/lroom/library2.c","back",
  });
} }
