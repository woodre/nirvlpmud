...
reset(arg) {
  if(!arg) {
  set_light(1);
    short_desc = "Library: ** Guilds Stacks **";
    long_desc =
    "You are in a room filled with books.  The books here cover\n"+
    "the guilds that exist in Nirvana today.  They consist of the\n"+
    "help files of each guild.  'Search' the stacks to find the books\n"+
    "to 'read' to your heart's content!\n";
  items = ({
    "back","Leads to the rest of the library",
  });

  dest_dir = ({
    "/players/mythos/lroom/library.c","back",
  });
} }
