...
do_sell(ob) {
  ...
  write("This item is sent to the back store room of the shop.\n");
    ...
    call_other("/players/mythos/aroom/store1.c", "store", ob);
    return 1;
}
...
value(item) {
    ...
    if (!name_of_item) {
    if (call_other("/players/mythos/aroom/store1.c", "value", item))
	    return 1;
	write("No such item ("); write(item); write(") here\n");
...
buy(item) {
    if (!item)
	return 0;
    call_other("/players/mythos/aroom/store1.c", "buy", item);
    return 1;
}
...
north() {
    ...
    call_other(this_player(), "move_player", "north#/players/mythos/aroom/store1.c");
    return 1;
}

list(obj) {
    call_other("/players/mythos/aroom/store1.c", "inventory", obj);
    return 1;
}
