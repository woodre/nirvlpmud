...
/* 11/21/05 Earwax: made them misc instead of helmet 
 *          also added ::id(str) in id()
 */
...
reset(arg) {
  ...
  set_long("A small set of goggle-like eyes, which you could probably wear.\n"
          ...
          +"Type \"eyes <cmd>\" to do something in the dark.\n");
  set_ac(1);
  set_weight(2);
  set_type("misc");
  set_value(2500);
}
id(str) { return ::id(str) || str == "eyes" || str == "night eyes" || str == "goggles"; }
init() {
  ::init();
  add_action("wear_me", "wear");
  add_action("night_cmd", "eyes");
}
wear_me(str) {
  if(id(str))
...
night_cmd(str) {
  if(!str) {
    write("Type \"eyes <cmd>\" to do something in the dark.\n");
    return 1;
  }
  if(set_light(0)) {
