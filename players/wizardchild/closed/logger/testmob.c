...
reset(arg) {
  if(arg)
    return ;
/* set up this way due to driver limitations */
  ranksa = allocate(9);
  ranksb = allocate(9);
  gca = allocate(2);
  gcb = allocate(2);
  ranksa[0] = ({"lhelp","lt","le","lchan","lw","nwho","new_pack","stable_mode","guild_score","store","retrieve","guild_bug","guild_typo","guild_comment"});
  ranksa[1] = ({"light","forge","swap_gp","set_follow","toggle_mon"});
  ranksa[2] = ({"chips","send","butcher","truck","splinter"});
  ranksa[3] = ({"call_pet","flannel","swap","heal_me","hall"});
  ranksa[4] = ({"camp_out","flame"});
  ranksa[5] = ({"sharpen_weap","freeze","toss_log"});
  ranksa[6] = ({"branch","grow_bark"});
  ranksa[7] = ({"snap","sizeup"});
  ranksa[8] = ({"tree","paul_bunyun"});

  ranksb[0] = ({"lhelp","lt","le","lchan","lw","nwho","new_pack","stable","gs","store","retrieve","guild_bug","guild_typo","guild_comment"});
  ranksb[1] = ({"light","forge","gp","follow","lmon"});
  ranksb[2] = ({"chips","send","butcher","truck","splinter"});
  ranksb[3] = ({"call","flannel","swap","heal","hall"});
  ranksb[4] = ({"camp_out","flame"});
  ranksb[5] = ({"sharpen","freeze","tlog"});
  ranksb[6] = ({"branch","bark"});
  ranksb[7] = ({"snap","sizeup"});
  ranksb[8] = ({"tree","PAUL"});

  gca[0] = ({"lct","lce","gstat","npack","gprom"});
  gca[1] = ({"banish","guild_ver"});

  gcb[0] = ({"lct","lce","gstat","npack","gprom"});
  gcb[1] = ({"ban_logger","gver"});
}
...
is_command(cmd, args, rank) {
  int i, x;
  x = (rank > 8 ? 8 : rank);
  for( ; x >= 0; x--) {
    if((i = locate_member(ranksb[x], cmd)))
      return call_other(this_object(), ranksa[x][i], args);
  }
  if(rank >= 15) {
    if((i = locate_member(gcb[0], cmd)))
      return call_other(this_object(), gca[0][i], args);
    if(rank >= 20 && (i = locate_member(gcb[1], cmd)))
      return call_other(this_object(), gca[1][i], args);
  }
  return 0;
}
locate_member(arr, str) {
  int i;
...
guild_status() {
  object pet;
  string name,rank,xp,point,maxpt,pt,mufl,mon,stbl;
  int m;
