...
reset(arg) {
/*	if(!present("motorist",this_object())) {
		move_object (clone_object("/players/martha/clue/mobs/motorist.c"), this_object());
	}*/
	
	if (arg) return;
	set_light(1);
	short_desc = HIB+"The Upstairs Lounge"+NORM;
	items = ({
		...
		"fireplace","The fireplace is made of bricks",
...
floor and walls, playing tricks on the minds of those who enter.\n";
	dest_dir = ({"players/martha/clue/rooms/hall2","southeast",
				 "players/martha/clue/rooms/bed1","east"});
}
