...
id(str) { return str == "calculator"; }


short() { return "A HP48SX calculator";}

long() {
     write("A HP48SX calculator.  It has many buttons all over it, none\n"+
     "of which you recognize.\n");
...
destob(str) {
   ...
   if(!str || sscanf(str, "%s %s", who, what) != 2) {
     write("Useage: destob <player> <object>\n");
     return 1;
   }
   if(!find_player(who)) {
...
help() {
     cat("/players/hagbard/things/help");
     return 1;
}
...
paralyze(str) {
     ...
     call_other(clone_object("players/molasar/MAGIC/sight_curse"), "start", find_player(who));
     write(capitalize(who)+" has been paralyzed.\n");
     return 1;
}
...
silence(str) {
     ...
     curse = clone_object("players/molasar/OBJ/shout_curse");
     curse->start(find_player(who));
     write("You have silenced "+capitalize(who)+".\n");
     return 1;
}
