...
heart_beat()
{
  ...
  if(!random(4) && !attacker_ob) wander();
...
{
  string *where, move;
  where = ({"north","east","west","south","northwest","northeast","southeast","southwest"});
  move = where[random(sizeof(where))];
  command(move, this_object());
  return 1;
}

check_me()
{
  if(creator(environment()) != "fred")
  {
    tell_room(environment(),
     this_object()->short()+" looks confused and rushes out of the room.\n");
    move_object(this_object(), "/players/fred/closed/bloodfist/r/courtyard.c");
    tell_room("/players/fred/closed/bloodfist/r/courtyard.c",
     this_object()->short()+" enters the room.\n");     
  }
}
