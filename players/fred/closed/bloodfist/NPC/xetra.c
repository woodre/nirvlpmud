...
long()
{
  write("A "+eggs+" xetra that has been bound to "+ON+" until the end of time.\n");
}

id(str)
{
  return str == "bf_xetra" || str == "bf_mob" || str == "xetra";
}

query_bf_xetra(){ return 1; }
...
heart_beat()
{
  ...
  if(owner && !owner->query_ghost())
  {
    ...
    if(owner->query_attack() && !MEAT && !random(5))
    {
      tell_room(environment(owner),
