...
heart_beat()
{
  ...
  if(owner && !owner->query_ghost())
  {
    ...
    if(!random(30))
    {
      ...
      tell_object(owner, capitalize(poof)+" says to you, \"Oh "+ON+" you're so "+what+"!!\"\n");
      tell_room(environment(owner), capitalize(poof)+"exclaims, \"Oh "+ON+" you're so "+what+"!!\"\n", ({ owner }));
    }
    if(!random(40))
    {
      int y;
      ...
      tell_room(environment(owner), capitalize(poof)+" looks at "+ON+" and "+huh+".\n", ({ owner }));
    }
  }
}
