...
short(){
  if(num_messages)
    return ("A bulletin board "+HIR+"[" + num_messages + "]"+NORM);
  else
...
reset(arg){
  ...
  restore_object("players/fred/closed/bloodfist/log/mboard");
}
...
get_body(str) {
	...
	new_body = 0;
	new_hd = 0;
	save_object("players/fred/closed/bloodfist/log/mboard");
	write("Ok.\n");
	who = 0;
	return;
    }
...
remove(str) {
	...
	if (i == 0) {
        ...
        if(sscanf(lower_case(hd),"%s"+nm, jun) != 1 && nm != "fred")
        {
