	     ...
	     switch(random(4))
             {
               case 0:
                 tell_room(ENV(),
                   "Particles of sand whip into the room tearing the flesh from "+meat->query_name()+!\n");
                 tell_room(ENV(),
                   "When the sand dissipates only "+meat->query_name()+"'s lifeless body remains.\n");
                 break;
               case 1:
                 tell_room(ENV(),
                   "A wave of sand engulfs "+meat->query_name()+" slowly suffocating the life out of "+meat->query_objective()+".\n"); 
                 break;
               case 2:
                 tell_room(ENV(),
                   "The ground beneath "+meat->query_name()+" starts to shake...\n");
                 tell_room(ENV(),
                   "     A giant pillar of sand explodes from the ground smashing "+meat->query_name()+"\n");
                 break;
               case 3: 
                 tell_room(ENV(),
                   "The ground opens up underneath "+meat->query_name()+" causing them to fall to their death.\n");
                 z = 1;
                 break;
             }
             if(z)
               destruct(present("corpse", ENV()));
              
             if(TP->query_spell_point() > cost1)
               TP->add_spell_point(-cost1);
	     else
               present(GID, TP)->add_sand_points(-cost);	
