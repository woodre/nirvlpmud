...
long(){
     write(
"  This is the "+BOLD+"Dark Matter"+NORM+" Symbiote "+capitalize(symb_name)+". It\n"+
"is separated from its host body and prepared for combat.\n"+
"It is a wirey form that carries all of the same powers as\n"+
"it did before it's separation.\n");
 return 1;
 }
...
object woooo;
   ...
   if(!present(challenger, environment())) return;
   if(attacker_ob != challenger || alt_attacker_ob != challenger) return;
   write_file("/players/maledicta/log/challenged!",
   attacker_ob->query_name()+" defeated symbiote on "+ctime(time())+".\n");
   tell_object(challenger,
   "\n\n\n\nYou have beat your Symbiote into submission!\n\n\n\n"+
   "It bows its head in shame and reenters your body.\n");                
woooo = clone_object("/players/maledicta/venom2/neo_venom.c");
