inherit "/players/softly/point/rooms/std_lh.c";
  ...
  reset(arg) {
  if(!present("din")) 
  move_object(clone_object("players/softly/point/mobs/kheir"),this_object());
  if(!present("aruj")) 
  move_object(clone_object("players/softly/point/mobs/aruj"),this_object());
  if(arg) return;
  ::reset();
set_light(-1);
long_desc =
...
dest_dir = ({
  "/players/softly/point/rooms/lh12.c","up",
  "/players/softly/point/rooms/lh10.c","down",
  });
}
