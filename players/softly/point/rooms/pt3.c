...
reset(arg) {
  if(!present("aralie")) 
  move_object(clone_object("players/softly/point/mobs/aralie.c"),this_object());
  if(arg) return;
  set_chance(3);
...
set_light(1);
short_desc=(RED + "R" + NORM + "ocky " + RED + "P" + NORM + "oint Jetty's End");
long_desc ="  The jetty ends abruptly, surrounded by the encroaching\n"+
           "sea.  Footing is treacherous on the large slick boulders.\n"+
           "The crashing surf hurls water from all directions.\n";
...
items = ({
  ...
  "water","The sea is a dull blue green color",
  });
}
...
call_other(this_player(),"move_player","west#/players/softly/point/rooms/pt2.c");
  return 1;
}
