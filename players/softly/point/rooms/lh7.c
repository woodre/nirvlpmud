inherit "/players/softly/point/rooms/std_lh.c";
  ...
  reset(arg) {
  if(!present("bonny"))
  move_object(clone_object("players/softly/point/mobs/bonny"),this_object());
  if(!present("read"))
  move_object(clone_object("players/softly/point/mobs/read"),this_object());
  if(arg) return;
  ::reset();
set_light(0);
long_desc =
...
dest_dir = ({
  "/players/softly/point/rooms/lh8.c","up",
  "/players/softly/point/rooms/lh6.c","down",
  });
}
