...
id(str) { return str == "healerob" || str == "mythos_healer" || str == "healer_staff" ||
...
/* Moved this to the board. - Feldegast 12-31-00
      write(BLU);
	  write("After much thought I have decided to leave Nirvana for the time being.\n");
	  write("I must apologize for the abruptness of it all but I finally decided\n"+
	        "that I must do this.  If you are wondering why the reason is as follows:\n");
	  write("I have become apathetic about Nirvana, both coding for and administrating it,\n"+
	        "and after dealing with various wizes who feel that 'they know better' I\n");
	  write("figured that if they consider me a hinderance I may as well leave.\n"+
	        "It comes down to this: I do not get paid for helping administrate this place.\n");
	  write("And if it is no longer fun for me, why should I continue doing it.\n"+
...
int nss;
  if(epl < 20) {
    ...
    if (epxl) {
      ...
      if(ep->query_guild_name() != "healer") ep->set_guild_name("healer");
      ep->set_home(HOME_H);
      ep->set_guild_file(root+"guildob.c");
      lev = epl + epxl;
	...
	if(mass_heal_var[0] - 1 < lev) {
      ...
      if(mss) {
      tell_object(ep,"Reseting mass healing capacity.\nNow you may mass heal up to "+lev/5+" beings.\n");
...
string monitor;
  ...
  if(!ep->query_attack() && mon_on) {
    ...
    monitor += " I "+(ep->query_intoxination()*100/(ep->query_level()+3))+"%";
    monitor += " F "+(ep->query_stuffed()*100/(ep->query_level()*8))+"%";
    monitor += " S "+(ep->query_soaked()*100/(ep->query_level()*8))+"%";
    monitor += "  >>>>\n";
...
int l;
  if(!str) {
    write("Your Need Monitor Members: \n");
...
string mee;
  if(!moo) moo = "";
  if(moo != "") {
    if(sscanf(moo,"e %s",mee)) moo = RED+"("+BLU+"] "+NORM+tp->query_name() + " " + mee;
    else if(sscanf(moo,"a %s",mee)) moo = RED+"("+BLU+"] "+NORM+mee + " <" + tp->query_name() + ">";
    else moo = RED+"("+BLU+"] "+NORM+tp->query_name() +" intones:"+ moo;
    moo = format(moo,70);
  }
...
squit() {
  ...
  if(ep->query_level() < 20) write_file("/players/mythos/healerguild/EXP.TXT",ctime(time())+"  "+epn+"  "+ep->query_exp()+"\n");
}
...
defend(str) { if(defend_var[0] > lev) return 0; call_other(POWER,"defend",lev,pow,str); return 1; }
shield(str) { if(shield_var[0] > lev) return 0; call_other(POWER,"shield",lev,pow,str); return 1; }
