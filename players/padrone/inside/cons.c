...
extra_reset(arg) {
...
#if 0 /* Rumplemintz */
	apprentice = clone_object("obj/monster_lars");	/* NEW, IMPROVED monster class! */
	...
	invisible_companion = clone_object("obj/monster_lars"); /* NEW, IMPROVED monster class! */	
...
/* Commenting out macro - not working as it's supposed to - doing the long way
   -- Rumplemintz
...
#else
    apprentice = clone_object("/players/padrone/inside/npc/cons_apprentice"),
    move_object(apprentice, this_object());
    invis_com = clone_object("/players/padrone/inside/npc/invis_com");
    move_object(invis_com, this_object());
#endif
    }
}
...
#if 0  /* Rest of this is unused now -- Rumplemintz */
handle_say(str) {
	 ...
	 */
	bird = clone_object("obj/monster_lars");	/* NEW, IMPROVED monster class! */
	bird->set_name("bird");
	bird->set_level(1);
	bird->set_hp(10);
	bird->set_al(100);
	...
	say(" - Just say 'change alignment to ...',\n" +
            " - and I'll do it.\n");
#elseG        say("The apprentice says: I know two spells to handle alignments,\n" +
