...
extra_reset() {
    ...
    if (!rat1 || !living(rat1)) {
	rat1 = clone_object("obj/monster_lars");
    ...
    if (!rat2 || !living(rat2)) {
	rat2 = clone_object("obj/monster_lars");
	...
	anticimex = clone_object("obj/monster_lars");
...
#else
  if (!anticimex || !living(anticimex))
    move_object(clone_object("/players/padrone/inside/npc/anticimex"),
                this_object());
  if (!rat1 || !living(rat1))
    move_object(clone_object("/players/padrone/inside/npc/f_rat"),
                this_object());
  if (!rat2 || !living(rat2))
    move_object(clone_object("/players/padrone/inside/npc/f_rat2"),
                this_object());
#endif
} /* extra_reset */
