#include "std.h"

/* FOUR_EXIT("players/padrone/inside/big_under", "north",  
   -- Removed north exit, player created rooms don't need to be on Nirv
      Rumplemintz
*/
THREE_EXIT("players/padrone/inside/dark_pass1", "south",
           "players/padrone/inside/wine_cellar", "west",
           "players/padrone/inside/boiler_room", "east",
	  "Dark passage",
	  "You are at the north end of a dark passage in the cellar.\n" +
	  "To the north there is a small wooden door that doesn't seem to\n" +
	  "have been opened in a long time, and other doors lead east and west.\n" +
	  "The passage continues south.\n", 0)

