...
extra_reset()
{
    if (!mermaid) {
#if 0 /* Rumplemintz */
	mermaid = clone_object("obj/monster_lars");	/* NEW, IMPROVED monster class! */
...
#else
    mermaid = clone_object("/players/padrone/tower/npc/mermaid");
    mermaid->set_dead_ob(this_object());
#endif
...
#if 0 /* Changing to already coded guard from tower1 */
    guard = clone_object("obj/monster_lars");
...
#else
    guard = clone_object("/players/padrone/tower/npc/sec_guard");
    move_object(guard, this_object());
#endif
