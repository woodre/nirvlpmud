...
reset(arg) {
   ...
   if (!arg) {
      ...
      /* Removing macro and setting doors up manually - macro has issues
      -- Rumplemintz
      ...
      */
      ...
      if(!present("door")) {
         ...
         obj_1->set_door_long(""+
            "This is a stout wooden door, several inches think.\n"+
            "There is a small note nailed to it:\n"+
            "    'Don't go in here. The guards have gone crazy again.'\n");
         obj_1->set_locked(1);
         obj_1->set_closed(1);
...
make_dragon()
{
   ...
   the_dragon = clone_object("obj/monster_lars");/* NEW, IMPROVED monster class! */
...
/* Commenting out macro and doing the key the long way
   -- Rumplemintz
