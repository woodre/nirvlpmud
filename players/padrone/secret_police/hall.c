/* secret_police/hall.c */

#include "std.h"
#if 0 /* Rumplemintz */
#include "../../../obj/door.h"
#else
#include "/obj/door.h"
#endif

object obj_1, obj_2;	/* Used by the MAKE_DOORS macro */

#undef EXTRA_RESET
#define EXTRA_RESET extra_reset(arg);

extra_reset(arg) {

    if(!arg) {
	/* First reset */
#if 0 /* Rumplemintz - macro doesn't work, have to code long way */
	MAKE_DOORS("players/padrone/secret_police/hall", "west", "players/padrone/secret_police/interrogation_room", "east", "iron", "interrogation", "There is a sign saying \"KEEP OUT!\" on this door.\n", 1, 1, 1)
#else
    if (!present("door"))
    {
    obj_1 = clone_object("/obj/door");
    obj_1->set_dir("west");
    obj_1->set_door_room("/players/padrone/secret_police/hall");
    obj_1->set_type("iron");
    obj_1->set_code("interrogation");
    obj_1->set_door_long("There is a sign saying \"KEEP OUT!\" on this door.\n");
    obj_1->set_closed(1);
    obj_1->set_locked(1);
    obj_1->set_can_lock(1);

    obj_2 = clone_object("/obj/door");
    obj_2->set_dir("east");
    obj_2->set_door_room("/players/padrone/secret_police/interrogation_room");
    obj_2->set_type("iron");
    obj_2->set_code("interrogation");
    obj_2->set_door_long("There is a sign saying \"KEEP OUT!\" on this door.\n");
    obj_1->set_door(obj_2);
    obj_2->set_door(obj_1);
    move_object(obj_1, this_object());
    move_object(obj_2, "/players/padrone/secret_police/interrogation_room");
    obj_1->set_both_status();
    }
#endif
    }
    else {
	obj_1->set_closed(1);
	obj_1->set_locked(1);
	obj_1->set_both_status();
    }
} /* extra_reset */

FOUR_EXIT("players/padrone/secret_police/office", "north",
	  "players/padrone/secret_police/backroom", "south",
	  "players/padrone/secret_police/sleeping_room", "east",
	  "players/padrone/secret_police/interrogation_room", "west",
	  "A small hall behind the shop",
	  "You are in a small hall behind the shop.\n" +
	  "There are doors in all directions, and on the wall there is a poster\n" +
	  "with the text: \"The Secret Police - for the Safety of the Realm!\".\n", 1)

