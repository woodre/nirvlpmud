 ...
 *	He is on a quest for Vasques...
 *  (Special version... hehehe)
 */

#include "std.h"

object sign;
object obliterator, follow, last_talker, vasques, sword;
string follow_name;
status found_him;
...
check_vasques(str) {
    ...
    if (!found_him) {
	vasques = present("vasques", environment(obliterator));
	if (vasques) {
	    found_him = 1;
	    /* Stop talking: */
	    ...
	    obliterator->attacked_by(vasques);
	    return 1;
	}
    }
    return 0;
...
handle_say(str) {
    ...
    if (check_vasques())
	return;
    else if (vasques && present(vasques, environment(obliterator)))
	return;
	...
	say("Sir Obliterator says: Well then, " + who +
	    ", now lead me to Vasques!\n");
	last_talker = 0;
	follow = talker;
	follow_name = capitalize(who);
	return;
    }
    if (sscanf(phrase, "%svasques%s", a, b) == 2) {
	say("Sir Obliterator says: Yes! Yes! Tell me where he is!\n");
	return;
    }
    say("Sir Obliterator says: Stop talking nonsense! Tell me where Vasques is!\n");
    return;
} /* handle_say */
...
handle_leaving(str)
{
	...
	obliterator->init_command(rest);
	if (!found_him) {
	    if (check_vasques() && present(follow, environment(obliterator))) {
		...
		tell_object(follow, "You feel more evil, having sold Vasques to the anti-paladin.\n");
...
setup_arrays() {
    ...
    chat_str = ({
	"Sir Obliterator says: Excuse me, Sir! Have you seen a man called Vasques?\n",
	"Sir Obliterator says: Can you show me to Vasques?\n",
	"Sir Obliterator says: You! Where is that bastard Vasques?\n",
	"Sir Obliterator says: In which direction can I find Vasques?\n",
	"Sir Obliterator says: You! Peasant! Show me to Vasques!\n",
	"Sir Obliterator says: Have you seen a person who calls himself Vasques?\n",
	"Sir Obliterator says: I need to find a man called Vasques.\n",
	"Sir Obliterator says: Take me to Vasques!\n",
	"Sir Obliterator says: I have a message for Vasques.\n",
	...
	"Sir Obliterator says: 200 gold coins if you lead me to Vasques!\n",
	"Sir Obliterator says: Take me to Vasques and earn 200 gold coins!\n"
    });
	...
	"Sir Obliterator says: So, you are a friend of Vasques, are you?\n",
	"Sir Obliterator laughs insanely.\n",
	"Sir Obliterator says: Don't think you can even TOUCH me, you nerd!\n",
	"Sir Obliterator says: Who put you up to this? Vasques?\n",
