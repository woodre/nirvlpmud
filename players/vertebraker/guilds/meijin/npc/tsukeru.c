...
catch_tell(str) {
  ...
  if(sscanf(str, "%s tells you: %stype%s", tmp, tmp2, tmp3) == 3) {
	call_other("/players/vertebraker/guilds/meijin/powers/types.c", "help", 0);
	return 1;
  }
  ...
  if(sscanf(str, "%s tells you: %swaiting%s", tmp, tmp2, tmp3) == 3) {
	...
	write_file("/players/vertebraker/guilds/meijin/log/WAIT_LIST",
...
join() {
  ...
  this_player()->set_home("/players/vertebraker/guilds/meijin/room/start");
  ...
  write_file("/players/vertebraker/guilds/meijin/log/JOIN",
   ...
   */
  write_file("/players/vertebraker/guilds/meijin/log/JOIN_STATS",
	"\n"+
