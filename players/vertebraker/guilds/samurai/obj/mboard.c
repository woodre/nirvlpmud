...
long(){
  if(TP->query_guild_name() != "samurai" && TP->query_level() < 60 ) return;
  write(
...
short(){
  if(num_messages)
    return ("A bulletin board "+GRN+"[" + num_messages + "]"+NORM);
  else
...
reset(arg){
  ...
  restore_object("players/vertebraker/guilds/samurai/obj/mboard");
}
...
get_body(str) {
	...
	new_body = 0;
	new_hd = 0;
        save_object("players/vertebraker/guilds/samurai/obj/mboard");
	write("Ok.\n");
	who = 0;
	return;
    }
