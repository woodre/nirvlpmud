...
resetchannel(){
  ...
  for(i=0,n=sizeof(players); i<n ; i++){
    if(present("mark of the samurai",players[i])){
      members += ({ players[i], });
      num_members++;
    }
  }
  for(i=0,n=sizeof(members); i<n ; i++){
    if(present("mark of the samurai",members[i])->muffled() != 1){
      listeners += ({ members[i], });
      num_listeners++;
    }
  }
...
send_msg(string str){
  ...
  for(i=0,n=sizeof(listeners);i<n;i++){
   ...
   "/obj/user/chistory"->add_history("Samurai", str);
}
...
tell(string str){
  ...
  {
    write("/obj/user/chistory"->query_history("Samurai"));
    return 1;
  }
  msg = "";
  msg += HIW+"[ishin-denshin] "+NORM;
...
emote(string str){
  string msg;
    ...
    if(file_size(file="/bin/soul/_"+em+".c") > 0) {
      return (int)file->guildcast("samurai", arg);
    }
  msg = "";
  msg += HIW+"[ishin-denshin] "+NORM;
...
add_history(m) { 
  "/obj/user/chistory"->add_history("Samurai", m);
}

query_tag() {
  return HIW+"[ishin-denshin]"+NORM;
}
