...
#define ALLOWED ({ "fakir", "earwax", "maledicta", "wocket", "feldegast", "vertebraker", "softly", "francesco" })
...
cmd_register()
{
    ...
    x = capitalize((string)this_player()->query_real_name());
    if ((status)MASTER->query_registered(x))
    {
      notify_fail("You are already registered for the Xp Race.\n");
      return 0;
    }
    write("You register yourself for the Experience Race.\n");
    say(x + " registers for the \
Experience Race.\n");
    MASTER->register(x);
    return 1;
}
