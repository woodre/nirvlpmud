...
reset(x) {
   ...
   set_light(1);
   short_desc = "Stable Yard "+GRN+"["+NORM+BOLD+"Knights Templar"+NORM+GRN+"]"+NORM;
   long_desc =
   ...
   add_exit("/players/jaraxle/templar/rooms/courtyard.c","northeast");
   add_exit(ROOMS+"s2","enter");
...
enter()
{
   if(present("templar_guard",TO) && !present("KnightTemplar",TP) && TP->query_level() < 21)
      {
