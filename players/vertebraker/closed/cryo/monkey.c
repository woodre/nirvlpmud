...
A cute little spider monkey.\n");
    ...
    set_chat_chance(10);
    move_object(clone_object("/players/pain/NEW/items/star"), this_object());
    renegades_of_funk();
}
...
init()
{
    ...
    s=sizeof(fl=get_dir("/bin/soul/"));
   while(s--) if(fl[s][strlen(fl[s])-2..strlen(fl[s])-1] == ".c") add_action("cmd_hook", fl[s][1..strlen(fl[s])-3]);
}
...
cmd_name(string arg)
{
    ...
    name = lower_case(arg);
    cap_name = capitalize(arg) + " the Spider Monkey";
...
catch_tell(string str)
{
...
#ifdef M_DEBUG
    tell_object(find_player("vertebraker"), HIR + "%" + NORM + str);
#endif
...
#ifdef M_DEBUG
      tell_object(find_player("vertebraker"), "bacon grease called\n");
#endif
       ...
       (x != this_object()) && !Busy)
    {
      ...
      {
#ifdef M_DEBUG
        tell_object(find_player("vertebraker"), "drop zone\n");
#endif
        call_out("nog_dat", 1, x);
...
bacon_grease(string *heh)
{
    ...
    a = heh[0]; b = heh[1];
#ifdef M_DEBUG
    tell_object(find_player("vertebraker"), "bacon grease\n");
    tell_object(find_player("vertebraker"), "bacon grease cmd: " + a + " " + b + "\n");
#endif
    command(a + " " + b, this_object());
}
...
int cmd_say(string str) {
 if(this_player())
  if(this_player() == this_object()) {
    say(this_player()->query_name()+" says: "+lower_case(str)+"\n");
    return 1;
  }
}

int is_player() { return 1; } /* for star */
