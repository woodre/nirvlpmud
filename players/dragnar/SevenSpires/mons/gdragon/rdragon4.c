...
set_race("dragon");
set_short(HIG+"Green "+NORM+GRN+"Dragon"+NORM+" [elite guard]");
set_long(
"This huge green dragon belongs to the elite guard of the\n"+
"King. It is the largest of the soldier class, and holds\n"+
"more devastating power than any two. Its body is over\n"+
"140 feet in length, and is covered in a tough green plate\n"+
...
set_spell_dam(25);
set_spell_mess1(
"The "+HIG+"Green "+NORM+GRN+"Dragon"+NORM+" bites down on its opponent!\n");
set_spell_mess2(
"You feel your flesh tear apart under the assault of the dragons bite!\n");
set_chat_chance (10);
load_chat("The green dragon glares at you with hatred.\n");
load_chat("The green dragon's huge tail swishes back and forth.\n");
scale = clone_object("/players/dragnar/SevenSpires/obj/gdragon/dragonscale.c");
move_object(scale, this_object());
scale2 = clone_object("/players/dragnar/SevenSpires/obj/gdragon/dragonscale.c");
...
if(random(100) < 70 && attacker_ob){
   ...
   tell_room(environment(),
   "The "+HIG+"Green "+NORM+GRN+"Dragon"+NORM+" slashes its opponent with its gleaming claws!\n",
   ({ attacker_ob }));
...
/* added environment check so shit dont break in hb -mizan */
if(environment() && attacker_ob && alt_attacker_ob && random(100) < 30){
