...
mixed
member_map(mixed arg, mapping map) {
  int i;
  mixed *TempArray;
  TempArray = m_indices(map);
  return member_array(arg, TempArray);
}

int
MappingSize(mapping map) {
  mixed *TempArray;
  TempArray = m_indices(map);
  return sizeof(TempArray);
}
...
CopyMap( mapping map ) {
  ...
  values = m_values( map );
  return make_mapping( indices, values );
}
