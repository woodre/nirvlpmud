...
long() {
   write(
"This is the scar you received when you became a champion of Mortal\n"+
...
remove_object() {
   ...
   CHANNELD->remove_channel( CHANNAME, environment() );
...
ev_pkill(object victim) {
   ...
   CHANNELD->transmit_message( CHANNAME, msg );
}
...
init() {
  ...
  if(this_player() != environment()) {
   ...
   CHANNELD->add_channel( CHANNAME, environment() );
   restore_me();
   LoginTime = time();
   if((user->is_player() && user->query_level() < 21) || user->is_npc()) {
...
restore_me() {
 string x;
   ...
   if (!user || !RN)
      return 0;
  if(sscanf(RN,"%s %s",x,x)==2) return 0;
