...
#define ERROR_LOG "players/dragnar/logs/BBOARD"
#define SAVE_PATH "players/dragnar/BBOARD"
#define BOARD_NAME "player_ideas"
...
long() {
	int ind;
	write("This is a bulletin board. The wizards on Nirvana are\n");
	write("interested in hearing what the players think would make\n");
	write("Nirvana more fun.  Read the first note for more details.\n");
...
remove_msg(what_msg) {
	...
	if ((this_player()->query_real_name() != player &&
		this_player()->query_level() < 60) ||
		!query_ip_number(this_player())) {
		write("Only Archwizards may remove notes.\n");
