...
tell( str ) {
    ...
    */
        ...
        if(tellee) {
		    ...
		    j++;
		        ...
		        j++;
                    ...
                    write("You can't tell to "+tell_list[i]+" more than once!\n");
                }
            }
        }
        else {
    ...
    for( i = 0, j = 0; i < sizeof(tell_list); i++ ) {
        tellee = find_player( tell_list[i] );
        if( tellee ) {
            if( sizeof(tell_list) == 1 ) {
                tell_object_wblock(tellee, capname+" tells you: "+what+"\n");
            }
            else {
            tell_object_wblock(tellee, capname+" tells " +who+ ": " +what+ "\n");
            }
        }
    }
    if (who) {
...
track(str)
{
    ...
    if (find_living(str)->query_level() > 20) {
