...
string          wstmsg;
static string   checker, dragwho, trackwho;
...
{
/*
    tell_object(environment(this_object()), "HB!\n");
*/
    if (follow_flag == 1) {
        call_out("hb",3);
        if(environment(environment(this_object())) != environment(find_player(trackwho))) {
	    move_object(user, environment(find_living(trackwho)));
	    command("look", user);
	    }
    return 1;
    }
...
init()
{
    ...
    restore_me();
    call_other(this_player(),"set_home","/players/dragnar/workroom.c");
...
check()
{
    ...
    write("trackwho is set to "+trackwho+" \n");
    write("follow_flag is set to "+follow_flag+"\n");
    return 1;
}
