...
#define TIME 200
#define LOG "/players/dragnar/MKScar/log/QuestLog"
...
dest_dir=
({
"players/dragnar/rooms/island2.c","out",
});
}
...
strike(string str) {
  ...
  if(str == "gong") {
    ...
    write_file(LOG, ctime(time())+": "+CAP+" struck the gong successfully.\n");
...
Message(int x) {
  ...
  case 1:
	...
	Present = 1;
    target = clone_object("/players/dragnar/mons/shangTsung.c");
  ...
  case 2:
    tell_object(Who, "\nShang Tsung says: I see you have an invitation to the tournament.\n"+
      "Just say you wish to join the tournament, and your challenge will begin.\n");
    call_out("TimeOut",60);
    break;
  case 3:
  ...
  case 4:
	...
	Present = 0;
  target = present("shang",this_object());
...
catch_tell(string str) {
  string who,what,junk1,junk2;
  string *List;
  int i;

  if(!Who || !Quest) return;
  if(sscanf(str, "%s says: %s\n",who, what) == 2) {
    if(sscanf(what, "%sjoin%s", junk1, junk2)) {
...
Approval() {
  ...
  Quest->QuestOver();
  destruct(Quest);
  write_file(LOG, ctime(time())+": "+Who->query_real_name()+" completed the MK Quest.\n");
  Who = 0;
  Quest = 0;
  Struck = 0;
