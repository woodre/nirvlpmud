...
reset(arg) {
	if(arg) return;
if(!present("ninja")) { move_object(clone_object("players/dragnar/mons/master"),TO); }
set_light(1);
...
dest_dir=
({
"players/dragnar/rooms/path1.c","east",
});
}
enter() {
	if(present("ninja")){
	...
	this_player()->move_player("into the cave#players/dragnar/rooms/cave.c");
	return 1;
}
