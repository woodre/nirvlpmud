...
#define SAVE_PATH "players/dragnar/code/code"
inherit "room/room";
realm() { return "NT"; }
int x,y,XYZ;
set_XYZ(ob) { XYZ=ob; }
...
dest_dir=
({
"players/dragnar/past/town5.c","south",
});
}
swipe(str) {
	string code;
...
north() {
	if(y > 2) {
	y=y-6;
	call_other(this_player(),"move_player","north#/players/dragnar/past/quest.c");
...
check(){
	write("The value of the code is "+XYZ+".\n");
	return 1;
}
hb(){
	restore_code();
