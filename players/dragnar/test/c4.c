...
int print_board();
string print_cell( int cell, int x, int y );
void update_score(int player, int x, int y);
int auto_move( int player, int level );
void push_state();
...
start_game() {
  ...
  states_allocated = 1;
	...
	call_out("check_move",2+random(4));
...
print_board() {
  ...
  for( x=0; x < sizeof(player_names); x++ ) {
    ...
    write("player"+(x+1)+": " + player_names[x] +"\n");
    if(!target) {
    	write("Target not found: "+player_names[x] +"\n");
      break;
    }
    ...
    tell_object(target, " ");
    for (i=0; i<= ROWS; i++)
        tell_object(target, " " + HIW + (i+1)+ NORM +"  ");
...
int
 play(string p_str) {
	...
	if( !game_in_progress ) {
		...
		player_names[PLAYER1] = (string) this_player()->query_real_name();
	  player_names[PLAYER2] = who;
		...
		if( (string) this_player()->query_real_name() != player_names[turn] ) {
