...
#define ERROR_LOG "/players/sandman/paladin/log/BOARD"
#define BOARD_NAME "players/sandman/paladin/cmdnotes"

#define OK_ACCESS call_other("/players/sandman/paladin/powers",\
"query_commanders")
...
long() {
   int ind;
   write("This is Guild Commanders Bulletin Board.\n");
...
new_msg(msg_head) {
   ...
   if(member_array(this_player()->query_real_name(), OK_ACCESS) == -1) {
     write("This board is for Paladin Guild Commanders only!\n");
     return 1;
   }
   say(capitalize(curr_writer->query_name())+" starts writing a note.\n");
   new_head = msg_head;
   new_text = "";
   input_to("get_msg");
