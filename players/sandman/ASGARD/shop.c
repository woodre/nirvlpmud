...
ONE_EXIT("players/sandman/ASGARD/mstreet2","north",
	 "The shop",
...
do_sell(ob) {
    ...
    call_other("players/sandman/ASGARD/store", "store", ob);
    return 1;
}
...
value(item) {
    ...
    if (!name_of_item) {
        if (call_other("players/sandman/ASGARD/store", "value", item))
	    return 1;
	write("No such item ("); write(item); write(") here\n");
...
buy(item) {
    if (!item)
	return 0;
    call_other("players/sandman/ASGARD/store", "buy", item);
    return 1;
}
...
south() {
    ...
    call_other(this_player(), "move_player",
              "south#players/sandman/AGSGARD/store");
    return 1;
}

list(obj) {
    call_other("players/sandman/ASGARD/store", "inventory", obj);
    return 1;
}
