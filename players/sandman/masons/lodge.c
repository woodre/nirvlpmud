...
reset(arg) {
    if(!arg)
      set_light(1);
    extra_reset();
    restore_object("players/sandman/masons/treasury");
}
...
extra_reset() {
     if(!present("plaque")) {
       plaque = clone_object("players/sandman/masons/plaque");
       move_object(plaque, this_object());
     }
     if(!present("stone")) {
       stone = clone_object("players/sandman/masons/stone");
       move_object(stone, this_object());
     }
     if(!present("board")) {
       board = clone_object("players/sandman/masons/bull_board");
...
pay() {
     ...
     treasury += amt;
     save_object("players/sandman/masons/treasury");
...
north() {
     ...
     if(!ob) {
       ...
       this_player()->move_player("north#players/sandman/masons/guild_inner");
       return 1;
     }
     ...
     this_player()->move_player("through the curtain#"+
         "players/sandman/masons/guild_inner");
     return 1;
}
...
south() {
     call_other(this_player(), "move_player",
                    "south#players/sandman/masons/guild_hall");
     return 1;
}
