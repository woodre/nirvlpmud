...
short() {
    return ("Elder bulletin board(" + num_messages + " msgs)");
}
...
reset(arg) {
    if (arg)
	return;
    restore_object("players/wicket/guild/board");
}
...
get_body(str) {
	...
	new_body = 0;
	new_hd = 0;
      save_object("players/wicket/guild/board");
	write("Ok.\n");
	ob->emote("has posted a new note has been posted on the Elder board!\n");
...
remove(str) {
  ...
  if(this_player()->query_level() < 21) {
    write_file("/players/wicket/guild/logs/BOARD",
