...
id(str) {
    return str == "bulletin board" || str == "board" || str == "guildboard";
}

long() {
  write("This is a board for any player of the Elementalist guild.\n");
...
short() {
    return ("Guild bulletin board(" + num_messages + " msgs)");
}
...
reset(arg) {
    if (arg)
	return;
    restore_object("players/wicket/guild/board2");
}
...
new(hd) {
    if (!hd)
	return 0;
  if(!present("wicket_element_orb", this_player())) {
...
get_body(str) {
	...
	new_body = 0;
	new_hd = 0;
      save_object("players/wicket/guild/board2");
	write("Ok.\n");
	pre = "";
	pre += MAG+" <"+NORM+"-"+MAG+"> "+RED+"Ele"+YEL+"men"+GRN+"tal"+CYN+"ist"+MAG+" <"+NORM+"-"+MAG+"> "+NORM;
	pre += this_player()->query_name()+" has posted a new note on the Guild Board!";
...
remove(str) {
	...
	if (i == 0) {
        if(!sscanf(hd, "%s" + (string)this_player()->query_real_name(), jun) && ((int)this_player()->query_level() < 40))
        {
  ...
  if(this_player()->query_level() < 21) {
    write_file("/players/wicket/guild/logs/BOARD2",
