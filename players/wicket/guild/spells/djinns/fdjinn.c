...
reset(arg)
{
	...
	set_alt_name("fire");
	set_race("djinn");
	set_alias("fire djinn");
	set_short("An "+HIW+"Ancient"+HIR+" Fire Djinn"+NORM+" ("+RED+"shrouded in flames"+NORM+")" );
	set_level(0);
	set_hp(0);
	set_ac(0);
	set_wc(0);
...
long(){
  	write("An ancient fire djinn that burns with a deep power from the element of\n");
	write("fire. It is made of nothing but complete fire and just blazes with a\n");
	write("glow and immense heat. It appears to pay attention to only its master\n");
	write("for which it was summoned.\n");
  if(TP == owner){
...
fight(str){
	if(!str && !owner->query_attack()){
		write("The Fire Djinn blinks at you.\n");
		return 1;
	}
	
	if(!str && TO->query_attack()){ 
		write("The Fire Djinn tells you: I'm already attacking "+TO->query_attack()->query_name()+".\n");
		return 1;
	}
	
	if(!str && owner->query_attack() && !attacker_ob && !owner->query_attack()->is_player()){
		say("The Fire Djinn bursts into flames as it charges  "+owner->query_attack()->query_name()+".\n");
	...
	if(!target){
		write("The Fire Djinn tells you: I don't see any "+str+" here\n");
		return 1;
	}
	
	if(target->query_npc() == 0) {
		write("The Fire Djinn tells you: Kill the player on your own!\n");
		return 1;
	}
	say("The Fire Djinn bursts into flames as it charges "+target->query_name()+".\n");
...
dismiss(){
	...
	say("The Fire Djinn bows to its audience and slowly its flames die out.\n");
	ob->set_has_djinn(0);
	ob->gsave();
	destruct(TO);
	return 1;
...
feed(){
	...
	if(!ob){
		...
		owner->hit_player(30 + random(10), "other|fire");
		return 1;
	}
	say("The ancient fires whip around the djinn closing up their wounds slowly as the corpse is absorbed.\n");
...
dmon(str){
	...
	if(!str){
		tell_object(owner, BOLD+HIR+"Fire Djinn: "+HIW+((hit_point * 10)/max_hp)+" / 10"+NORM+"\n");
		return 1;
	}
...
heart_beat(){
	...
	if(owner && stay != 1 && !owner->query_ghost()){
		if(ENV(TO) != ENV(owner)){
			say("The Fire Djinn follows after its owner, "+capitalize(owner->query_name())+".\n");
			move_object(this_object(), environment(owner));
			say("The Fire Djinn flows in behind "+capitalize(owner->query_name())+".\n");
			set_heart_beat(1);
		}
		if(TO->query_attack() && monitor == 1){
			tell_object(owner, 
 			BOLD+HIY+"Fire Djinn: "+HIW+((hit_point * 10)/max_hp)+" / 10"+NORM+"\n");
		} 
