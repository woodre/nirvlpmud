...
reset(arg)
{
	...
	set_alt_name("earth");
	set_race("djinn");
	set_alias("earth djinn");
	set_short("An "+HIW+"Ancient"+NORM+""+HIG+" Earth Djinn"+NORM+" ("+GRN+"shrouded in stone"+NORM+")" );
	set_level(0);
	set_hp(0);
	set_ac(0);
	set_wc(0);
...
long(){
	write("An ancient earth djinn that is shrouded by deep power from the element of\n");
	write("earth. It is made of nothing but complete earth and rock and just glows\n");
	write("with immense power. It appears to pay attention to only its master for which\n");
	write("it was summoned.\n");
	if(TP == owner){
...
fight(str){
	if(!str && !owner->query_attack()){
		write("The Earth Djinn blinks at you.\n");
		return 1;
	}
	
	if(!str && TO->query_attack()){ 
		write("The Earth Djinn tells you: I'm already attacking "+TO->query_attack()->query_name()+".\n");
		return 1;
	}
	
	if(!str && owner->query_attack() && !attacker_ob && !owner->query_attack()->is_player()){
		say("The Earth Djinn roars loudly and begins to attack "+owner->query_attack()->query_name()+".\n");
	...
	if(!target){
		write("The Earth Djinn tells you: I don't see any "+str+" here\n");
		return 1;
	}
	
	if(target->query_npc() == 0) {
		write("The Earth Djinn tells you: Kill the player on your own!\n");
		return 1;
	}
	say("The Earth Djinn roars loudly and begins to attack "+target->query_name()+".\n");
...
dismiss(){
	...
	say("The Earth Djinn bows to its audience and burrows underground.\n");
	ob->set_has_djinn(0);
	ob->gsave();
	destruct(TO);
	return 1;
...
feed(){
	...
	if(!ob){
		...
		owner->hit_player(30 + random(10), "other|earth");
		return 1;
	}
	say("The earth rumbles under the djinn closing up their wounds slowly as the corpse is absorbed.\n");
...
dmon(str){
	...
	if(!str){
		tell_object(owner, BOLD+HIG+"Earth Djinn: "+HIW+((hit_point * 10)/max_hp)+" / 10"+NORM+"\n");
		return 1;
	}
...
heart_beat(){
	...
	if(owner && stay != 1 && !owner->query_ghost()){
		if(ENV(TO) != ENV(owner)){
			say("The Earth Djinn follows after its owner, "+capitalize(owner->query_name())+".\n");
			move_object(this_object(), environment(owner));
			say("The Earth Djinn flows in behind "+capitalize(owner->query_name())+".\n");
			set_heart_beat(1);
		}
		if(TO->query_attack() && monitor == 1){
			tell_object(owner, 
 			BOLD+HIG+"Earth Djinn: "+HIW+((hit_point * 10)/max_hp)+" / 10"+NORM+"\n");
		} 
