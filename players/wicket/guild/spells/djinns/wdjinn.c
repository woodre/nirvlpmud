...
feed(){
	...
	say("The ancient waters whip around the djinn closing up their wounds slowly as the corpse is absorbed.\n");
...
heart_beat(){
	...
	if(owner && stay != 1 && !owner->query_ghost()){
		if(ENV(TO) != ENV(owner)){
			say("The Water Djinn follows after its owner, "+capitalize(owner->query_name())+".\n");
