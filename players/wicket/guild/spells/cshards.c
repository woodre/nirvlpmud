...
{
	int damage, damage2, cost, ranshard, chance;
	object attacker, airshroud, target, victim;
	...
	if(attacker->is_sprayed())
	{
     ...
     attacker->do_damage(({damage}), ({"other|earth"}));
	 if(victim) attacker->do_damage(({damage2}), ({"other|earth"}));
...
/*	User->spell_object(attacker, "cshards", damage, cost, "You "+msg1+".\n", User->query_name()+msg2+".\n", User->query_name()+msg3+".\n");
	User->set_spell_dtype("other|earth");
       ...
       attacker->do_damage(({damage2}), ({"other|earth"}));*/
