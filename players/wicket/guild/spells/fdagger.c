...
{
	int damage, damage2, cost, randagger, chance;
	...
	attacker->do_damage(({damage}), ({"other|fire"}));
	if(victim) attacker->do_damage(({damage2}), ({"other|fire"}));
	...
	User->set_spell_dtype("other|fire");
       ...
       attacker->do_damage(({damage2}), ({"other|fire"}));*/
