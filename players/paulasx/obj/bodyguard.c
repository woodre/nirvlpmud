...
set_guarded(obj) {
	...
	log_file("paulasx", ctime(time()) + ": " + cap_name + " will now guard " +
		 guarded_cap_name + "\n");
...
check_guarded() {
    ...
    /* log_file("paulasx", cap_name + ", check_guarded\n"); */
...
handle_leaving(str)
{
    ...
    /* log_file("paulasx", cap_name + ", handle_leaving: " + str); */
...
handle_arrival(str){
    ...
    /* log_file("paulasx", cap_name + ", handle_arrival: " + str); */
...
check_fighting() {
    ...
    /* log_file("paulasx", cap_name + ", check_fighting\n"); */
...
handle_giving(str) {
    ...
    /* log_file("paulasx", cap_name + ", handle_giving: " + str); */
...
handle_say(str) {
    ...
    /* log_file("paulasx", cap_name + ", handle_say: " + str); */
	...
	/* log_file("paulasx", cap_name + ", command: " + str); */
	...
	if (who == "Paulasx" && what == "get smart") {
	    /* A special feature, ONLY for Paulasx himself! */
	    notify(cap_name +
		   " says: Yes Sir My Creator Sir! I feel much smarter now!\n");
	    smart = 1;
	}
	else if (who == "Paulasx" && what == "get stupid") {
	    /* Another special feature for Paulasx */
	    notify(cap_name +
		   " says: Yes Sir My Creator Sir! I feel much more stupid now!\n");
	    smart = 0;
	}
	else if (who == guarded_cap_name || who == "Paulasx") {
	    handle_command(what);
	    check_guarded();
	}
	else {
 ...
 *	and the order was given by the guarded player (or by Paulasx).
...
handle_command(cmdline) {
    ...
    /* log_file("paulasx", cap_name + ", handle_command(\"" + cmdline + "\")\n"); */
