...
tell_the_guilders(str,lt,rt) {
  ...
  for(n=0;n<sizeof(people);n++)
    if(present(WYTCH,people[n]) && 
       !present(WYTCH,people[n])->query_muffled())
...
wytch_emote(str) {
  ...
  for(i=0;i<sizeof(people);i++) {
    if((present(WYTCH,people[i])
      && people[i]->QGR>lev)) {
      if(!present(WYTCH,people[i])->query_muffled())
