...
reset(arg) {
    if (arg) return;
if(!present("barkeep")) {
move_object(clone_object("/players/heroin/hunting_grounds/preds/barkeep.c"), this_object());
}
    set_light( 1);
}
...
move_object(this_player(),"/players/heroin/hunting_grounds/preds/hg6.c");
