...
reset(arg) {
  if(!arg){
    ...
    set_light(1);
    short_desc = HIY+"Hut";
    long_desc = HIY+"A small hut, it's thatched walls are surprisingly sturdy.  The\n"+
                    "interior is quite neat although it is simple and sparce.  It feels\n"+
                    "as though a person of great power and authority resides here.\n"+NORM;
    dest_dir = ({
      "/players/hogath/area/forest/PATH/path3.c","out",
      });
  }
  
  items = 
      ({
        "hut","A small hut that you are in",
