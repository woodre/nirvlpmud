...
int w;
 ...
 reset(arg) {
    ...
    set_read("Type cut <player_name> (must be female) for fun.\n");
    set_class(13);
    set_weight(1);
    set_value(200);
...
if(ob->query_level() > 101)  {
  ...
  if(ob->query_gender() == "male")  {
    write(capitalize(str)+" is male.\n");
     return 1;
        }

 write("You reach over and grab "+CAP+"'s tits and flurish your knife.\n"+
    "You lower the knife and quickly cut "+CAP+"'s tits off.\n");
 say(tp+" reaches over and grabs "+CAP+" by the tits.\n"+
    capitalize(TP->query_pronoun())+" cuts off "+CAP+"'s tits and holds them up for all to see.\n");
 tell_object(ob, "\nOuch ouch ouch ouch ouch ouch....\n");

  move_object(clone_object("/players/emerson/obj/tits.c"),TP);

  ob2 = present("tits",TP);

 call_other(ob2, "set_short","The tits of "+ob->query_name()+" <bloody>");
return 1;  }
