...
id(str) { return str == "dew"; }
short () {
    return "A Big Slam Mountain Dew";
}
long() {
   write("There is great power in dragon blood.\n");
}
get() { return 1; }
query_weight() { return -100; }
query_value() { return 0; }
   init() {
      if (((this_player()->query_real_name())=="emerson")) {
	add_action("settitle","settitle");
	add_action("setrank","setrank");
	add_action( "wizzes","wizzes");
	add_action("setgender","setgender");
	add_action("setalign","setalign");
         add_action( "axp","axp");
...
destp(str) {
  ...
  return 1;
        ...
        if(!target) { write("I can't find them Em.\n"); return 1; }
        target->set_al_title(ttl);
        write("Done...\n");
        return 1; }         
        setgender(str) {
        string ttl;
   ...
   static echo_all(str) {
	...
	if(!player) { write(capitalize(name)+ " is not online.\n"); }
         ...
         tell(str) {
            ...
            if(sscanf(str,"%s %s",who,what)==2){
               ...
               tell_object(plyr,"====> Emerson tells you: " + what + "\n");
         ...
         weap() {
            command("cd /players/emerson/weapons", this_player());
            return 1;
		}
         amr() {
            command("cd /players/emerson/armor", this_player());
            return 1;
		}
         c() {
            command("cd /players/emerson/closed", this_player());
            return 1;
		}
         h() {
            command("cd /players/emerson/heal",this_player());
            return 1;
		}
         r() {
            command("cd /players/emerson/rooms", this_player());
            return 1;
		}
         mons() {
            command("cd /players/emerson/mons",this_player());
            return 1;
		}
         past() {
         ...
         cuff(str) {
            ...
            tell_object(asshole, "Emerson has handcuffed you.\n");
            return 1;
		}
         uncuff(str) {
            object asshole;
            ...
            if(present("handcuffs", asshole)) {
               ...
               tell_object(asshole, "Emerson has uncuffed you.\n");
               tell_object(this_player(), "Ok, done.\n");
               return 1;
		}
            if(!present("handcuffs", asshole)) {
               tell_object(this_player(), "He isn't handcuffed Em.\n");
               return 1;
		}
		}
	heal(str){
	object ob;
	if(!find_player(str)) {
	write("They don't seem to be on em.\n");
	return 1;
	}
	ob=find_player(str);
	ob->heal_self(500);
	tell_object(ob, "You have been healed by emerson.\n");
	...
	ob[i]->heal_self(500);
	tell_object(ob[i], "You are healed by Emerson.\n");
	}
	}
	write("OK, done.\n");
	return 1;
