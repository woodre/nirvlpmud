...
#define BOARD_NAME "players/emerson/obj/bb"
...
long() {
	int ind;
        write("This is a board for you to give any suggestions you may have to improve\n");
write("Pizza Hut.\n");
        write("Usage: note <title>, read <message number>\n");
	if (!msg_num) {
...
short() {
    return "A bulletin board for you to express your ideas on.\n"+
   "It contains " + msg_num + " notes";
}
...
reset(arg) {
	if (arg)
		if (!random(5)) {
			say("A manager appears and secures some " +
			"notes on the board that were loose.\n");
			say("The manager leaves again.\n");
		}
}
...
if(!what_msg) return 0;
	...
	note -= 1;
	say(this_player()->query_name() + " carefully tears off an idea titled '" +
		headers[note] + "'.\n");
	ind = note;
	while (ind < msg_num - 1) {
