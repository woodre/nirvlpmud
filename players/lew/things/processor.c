...
install(str) {
    ...
    if(member <10 ) { write("You do not have enough experience to install ware.\n"); return 1; }
    if(!present(newmem, environment(this_player()))) {
      write("Install ware in who?"); return 1;}
    if(newmem->query_guild_rank()>0){ rank = 0;
      if(newmem->query_guild_rank()<20){
      if(newmem->query_guild_rank()<20){
    write(capitalize(str)+" is already in a guild.\n"); return 1; }
      }
    tell_object(newmem, capitalize(TP->NAME)+" installs a neural processor in you.\n");
    write("Installed.\n");
    move_object(processor, newmem);
    call_other(newmem, "add_guild_rank", rank);
    return 1;
}
}
