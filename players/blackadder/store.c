...
string name_of_item;
short() {
   return "store room for the boutique";
}
init() {
...
east() {
   call_other(this_player(), "move_player", "leaves#/players/blackadder/shop");
   return 1;
}
heart_beat()
{
...
reset(arg) {
   ...
   if (!present("wand")) {
      object wand;
      wand = clone_object("/players/blackadder/mi_wand");
      call_other(wand,"set_charge",5);
      move_object(wand, this_object());
   }
   if(!present("tubtrump")) {
   object tubtrump;
   tubtrump = clone_object("/players/blackadder/trumps/tub");
   move_object(tubtrump, this_object());
   }
}
long()
{
   write("All things from the boutique are stored here.\n");
}
store(item)
{
   string short_desc;
   ...
   move_object(item, this_object());
   if (call_other(item,"id","blackwand1")) {
      call_other(item,"set_charge",5);
     write("The shop gives the wand a recharge.\n");
     say("The shop recharges the wand.\n");
   }
}
