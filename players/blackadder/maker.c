...
set_file_path = "/players/" + this_player()->query_real_name()+"/";
...
set_dir_file(str) 
{
	...
	if(!str || str == "") {
		...
		dir_array[dir_cnt] = str;
          if(sscanf(str,"/%s", inp1) != 1)
             dir_array[dir_cnt] = set_file_path + str;
		dir_cnt += 2;
...
change_room(str) {
	if(str && str != "") {
		dir_array[edit_dir_num] = str;
          if (sscanf(str,"/%s",inp1) != 1)
             dir_array[edit_dir_num] = set_file_path + str;
	}
	edit_dirs("");
	return 1;
}
...
do_file()
{
	...
	add_line("");
	add_line("");
add_line("inherit \"/players/blackadder/room\";");
	add_line("");
	add_line("reset(arg) {");
