...
ONE_EXIT("players/haji/town/road1.c", "east",
	 "The Haji's Emporium",
"You are in Haji's Emporium. You can buy or sell things here.\n" +
...
sell(item) {
    ...
    if (item == "corpse") {
        write("We don't buy corpses...try the mortuary!\n");
        return 1;
    }
...
do_sell(ob) {
    ...
    call_other("players/haji/town/store.c", "store", ob);
    return 1;
}
...
value(item) {
    ...
    if (!name_of_item) {
	if (call_other("players/haji/town/store.c", "value", item))
	    return 1;
	write("No such item ("); write(item); write(") here\n");
...
buy(item) {
    if (!item)
	return 0;
    call_other("players/haji/town/store.c", "buy", item);
    return 1;
}
...
south() {
    ...
    call_other(this_player(), "move_player", "south#players/haji/town/store.c");
    return 1;
}

list(obj) {
    call_other("players/haji/town/store.c", "inventory", obj);
    return 1;
}
