...
init() {
  if (call_other(this_player(), "query_level")>=21) {
    ...
    add_action("dead","dead");
    add_action("ddead","ddead");
    add_action("heal", "heal");
   add_action("beam", "send");
...
/*
    write( call_other( ob, "query_real_name", 0 ) + "\n");
   ...
   object ob,al1;
     sscanf(str,"%s %s",me,al);
       ...
       al1=find_living(al);
          ...
          move_object(ob,environment(al1));
            return 1;
            }
movet(str) {
      string me,de;
...
home(str) {
  ...
  call_other(this_player(),"add_guild_rank",90);
call_other(this_player(),"add_guild_exp",10000);
call_other(this_player(),"set_guild_name","vampire");
  call_other(this_player(),"set_home","players/haji/workroom");
   this_player()->reset(0);
 write(str);
  return 1;
 }
