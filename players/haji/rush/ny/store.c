...
value(item) {
   ...
   write(value*11/5); write(" in welfare checks.\n");
   return 1;
}
...
buy(item) {
   ...
   int new,w;
    ...
    if(che==0) {
       write("Sorry we only accept welfare checks here and you have none.\n");
       return 1;
    }
   name_of_item = present(item);
   ...
   if (call_other(che,"query_val",0) < value) {
      ...
      write(value); write(" in welfare checks, which you don't have.\n");
      return;
   }
   if (!call_other(this_player(), "add_weight",
    ...
    w=call_other(che,"query_val",0);
    call_other(che,"set_val", w - value);
...
south() {
   call_other(this_player(), "move_player", "leaves#players/haji/rush/ny/shop");
   return 1;
}
