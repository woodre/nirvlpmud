 ...
 *
 * From the process_string() mud docs:
 ...
 *  Hence, only query functions are filtered thru to the process_string() call
 */


string process_msg(string str) {
  string obj_file, tmp1, tmp2, tmp3, result;


  if(!str) return "";

  /* parse thru only query strings to avoid security breaches */

  result = "";
  while(sscanf(str,"%s@@%s@@%s", tmp1, tmp2, tmp3)) {
    result += tmp1;
    if(sscanf(tmp2, "query_%s", tmp1) == 1)
      result += "@@"+tmp2+"@@";
    else result += "**Invalid Function Call**";
    str = tmp3;
  }
  str = result + str;
  if(this_object()) {
