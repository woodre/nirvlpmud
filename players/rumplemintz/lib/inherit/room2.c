...
#include "/players/rumplemintz/lib/include/fn_specs/room.spc"
#include "/players/rumplemintz/lib/include/fn/vowel.h"
#include "/players/rumplemintz/lib/include/fn/exit_con.h"
#include "/players/rumplemintz/lib/include/fn/weather.h"
#include "/players/rumplemintz/lib/include/fn/i_to_str.h"
#include "/players/rumplemintz/lib/include/cfg/door_mac.h"
#include "/players/rumplemintz/lib/include/fn/newline.h"

#define ALT         "external"
#define GUILD_OB    "guild_object"
#define TIME_TO_CLEAN_UP 43200 /* 12 hours */
...
string *land_development;     /* types of land development that can occur */
...
mixed *monster_files;        /* monsters who live here!   */
string *search_array;
int search_complex;
...
int no_summon, no_fight;              /* This will prevent fighting and
                                         summoning in|from this room */
int land_value;                       /* The value of the land to buy it ! */
...
#ifdef RPG
static status ooc_room;
#endif /* RPG */

static mapping ship_info;   /* only ships use this */
static int create_time;     /* time when the room was created */
...
void init() {  
  ...
  if(!create_time)   create_time = time();
...
*/
  ...
  if(query_day()) {  
    if(day_desc) {
      if(ANSI_ON) write(CYAN_F+ process_msg(day_desc) + OFF);
      else        write(process_msg(day_desc));
    }
  }
  else {
    if(night_desc) {
/*
      if(ANSI_ON) write(BLACK_F+ process_msg(night_desc) + OFF);
      else        write(process_msg(night_desc));
*/
      write(process_msg(night_desc));
    }
  }
  if(sizeof(ship_info)) {

    keys = m_indices(ship_info);
    values = m_values(ship_info);
    for(i=0; i<sizeof(keys); i++) {
      if(values[i]) write(values[i]+"\n");
    }
  }
}  
...
#ifdef RPG
status set_ship_info(string ship, string message) {
  if(!ship) return 0;
  if(!ship_info || !sizeof(ship_info)) ship_info = ([ ]);
  ship_info[ship] = message;
}

string query_ship_info(string ship) {
  if(!ship_info || !sizeof(ship_info)) ship_info = ([ ]);
  return ship_info[ship];
}

mapping query_all_ship_info() { return ship_info; }
status set_ooc_room(status arg) {
  ooc_room = arg;
  this_object()->set_no_teleport(arg);
  this_object()->set_no_teleport(1);
  return arg;
}
#endif /* RPG */
...
string *set_land_development(string *arr) {
  return (sizeof(arr)) ? land_development = arr : ({ });
}

mixed *set_monsters(mixed *arr) {
  return (sizeof(arr)) ? monster_files = arr : ({ });
}
...
int set_land_value(int i)        { return land_value = i; }
...
#ifdef RPG
status query_ooc_room()       { return ooc_room;   }
#endif /* RPG */
...
mixed *query_monsters()      { return monster_files; }
...
int query_land_value()        { return land_value;}
...
void exits(status brief) {  
  ...
  for(i = 0, size = sizeof(doors); i < size; i++) {
    if(!SECRET_DOOR_FINDERS || !sizeof(SECRET_DOOR_FINDERS))
      SECRET_DOOR_FINDERS = ({ });
    if((SECRET_DOOR || TRAIL)
...
*/
  ...
  if(ANSI_ON) {
    if(query_day()) write(BOLD+CYAN_F+ str +OFF);
    else write(BOLD+CYAN_F+ str +OFF);
    write("\n");
  }
  else
    write(str +"\n");
...
case "secret door finders":             SECRET_DOOR_FINDERS = arg[++j]; break;
  ...
  /* This is a catch to ensure the target destination exists.  If it doesn not, the
   * door will not be created in the room.  We don't bother catching any errors on the
   * compile of the target room as we want to ensure the error is seen fully.
   */

  if(DOOR_DEST && strlen(DOOR_DEST)) {

    // Make sure the other room exists.  This isn't fancy, it's just to make sure it
    // loads to avoid various problems.
    if(!(targ_obj = find_object(DOOR_DEST))) {
      call_other(DOOR_DEST, "??");
      targ_obj = find_object(DOOR_DEST);
    }
  }

  // The door hasn't passed the test, so we remove it and stop any removal of
  // other versions.
  if(!targ_obj) {
    doors = doors[0..i - 1];
    return;
...
int search_room(string str) {
  ...
  search += (int)this_player()->query_search_bonus();
  if(id(str)) {
...
status open(string str) {
  ...
  if(str == "door" || str == "gate") {
    notify_fail("Open which "+str+"?\n");
    return 0;
  }
  if(sscanf(str, "%s %s", tmp1, tmp2) == 2) {
    str = convert_exit_name(tmp1)+" "+tmp2;
  }
  for(i = 0, size = sizeof(doors); i < size; i++) {
    if(DOOR_ID == str) {
      if((SECRET_DOOR || TRAIL)
...
void clean_up() {
  ...
  tell_room(this_object(), "Gully dwarf arrives.\n"+
        "Gully dwarf cleans up a little.\n");
  if(!dest_dir || !sizeof(dest_dir)) {
    tell_room(this_object(), "Gully dwarf leaves again.\n");
  } else {
    tell_room(this_object(),
  }
  this_object()->reset(0);
