query_auto_load(){return("/players/rumplemintz/obj/enhancer.c:");}
...
id(str){ return str == "enhancer"; }
get(){ return 1;}
short(){ return "Blah"; }
long(){ write ("Emotions enhancer. 'help2'.\n");}
...
help2() {
  cat("/players/rumplemintz/obj/enhance_help");
  return 1;
}
...
cackle() {
  say(V+" throws back "+gender_pos()+
      " head and cackles maniacly. Thunder crashing in the distance.\n");
  write("You cackle maniacly.\n");
  return 1;
}
...
melt(str) {
  ...
  if (!friend || !living(friend) || friend == this_player())
  return 0;
  say(this_player()->query_name()+" melts in "+capitalize(str)+"'s arms.\n", friend);
  write("You melt in "+capitalize(str)+"'s arms.\n");
  tell_object(friend, this_player()->query_name()+" melts in your arms.\n");
  return 1;
}
...
thwap(str) {
  ...
  write("You thwap "+str+".\n");
  say(V + " thwaps "+str+".\n", victim);
  tell_object(victim, V + " thwaps you, damn that hurts.\n");
  return 1;
} 
...
noogie(str) {
  ...
  tell_object(victim, V + " gave you a noogie, tangling your hair.\n");
  return 1;
}
...
glare(str){
  ...
  victim=present(str, environment(this_player()));
  if(!victim) return 0;
  write("You glare devilishly at "+str+".\n");
  say(V + " glares "+gender_pos()+" eyes devilishly towards "+str+".\n");
  tell_object(victim, "You are struck with terror as "+ V +" glares at you.\n");
  return 1;
}
