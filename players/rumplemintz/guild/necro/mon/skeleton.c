...
object mas;
string master, mname;  /* Changed to mname - Rumplemintz */
int lev, guildlev, mlev, follow, autoattack, time;

id(str){ return str == "skeleton" || str == "dead" || str == "undead" || str == "necro_skeleton"; } 

short(){ return "The skeletal remains of "+BOLD+capitalize(mname)+OFF+" (undead)"; }
long(){  
    write("The undead skeleton of "+capitalize(mname)+".\n"+
...
set_mname(n){ mname = n; cap_name = BOLD+capitalize(mname)+"'s "+OFF+"undead skeleton"+OFF; }
set_level(l){ lev = l; }
