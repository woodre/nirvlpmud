...
int cmd(string str){
	...
	if(!random(2))
	    ...
	    whichbodypart = "right arm";
	break;
    case "leg":
	part = "leg";
	if(!random(2))
	    whichbodypart = "right leg";
	else
	    whichbodypart = "left leg";
	break;
    case "head":
	part = "head";
	whichbodypart = "head";
    ...
    bodypart->set_name(capitalize((string)corpse->query_name()));
    move_object(bodypart,this_player());
    write("You "+HIR+
      "savagely hack at the corpse as blood splatters everywhere.\n"+OFF+
      "You "+HIR+"pick up the dismembered "+what+".\n"+OFF);
    say(this_player()->query_name()+HIR+
      " savagely hacks at the corpse as blood splatters everywhere.\n"+OFF+
      this_player()->query_name()+HIR+
      " picks up the dismembered "+what+".\n"+OFF);
    corpse->set_short("The "+HIR+"mutilated"+OFF+" corpse of "+
      capitalize((string)corpse->query_name()));
    corpse->set_long("This is the mutilated corpse of "+
      capitalize((string)corpse->query_name())+".\n"+
