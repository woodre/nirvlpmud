...
int cmd(string str){
	...
	if(!ppl[i]->query_invis() || 
	  ...
	  this_player()->query_level() > 21){
		...
		msg += " ";
	    name = cutstring(capitalize((string)ppl[i]->query_real_name()), 12);
	    if(ppl[i]->query_pl_k())
		msg += HIR+name+OFF;
	    else
		msg += name;
		...
		if(creator(environment(ppl[i])))
		    ...
		    msg += "            ";
		msg += " ";
		msg += cutstring((string)environment(ppl[i])->short(),40)+" "+OFF;
	    }
	    count++;
	    write(msg+"\n");
	}
    }
    write(BOLD+BLK+"\n         There are currently "+HIR+count+OFF+BOLD+BLK+
      " players online\n"+OFF);
    write("\t<---------------------------------------------------------->\n\n"+
      NORM);
