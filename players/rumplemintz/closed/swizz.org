...
#define TITLE "the Druid Guild Master"
#define ALIGNTITLE "Smooth"
#define ALIGNMENT 0
#define PRETITLE "Rock -n- Roll -n-"
...
long() {
  ...
  write("For help use: shelp  [shelps (for short help)]\n");
  write("The Swizz is ");
  if (lighted)
     write("stirring.\n");
  else
...
rfake_quicktyper(str)  {
  ...
  if (str=="who") {
    ...
    for (i = 0; i < sizeof(who); i++) {
      ...
      if (present("fangs", who[i])) write("vampire "+" ");
      else
       if (present("spellbook", who[i])) write("mages   "+" ");
      else
      if (present("instrument", who[i])) write("bards   "+" ");
      else
/*    if (present("seal", who[i])) write("mason   "+" ");
      else 
      if (present("mark", who[i])) write("shadow  "+" ");
      else
      if (present("license", who[i])) write("assassin"+" ");
      else
      if (present("books", who[i])) write("chrono  "+" ");
      else */
      if (present("mrobe", who[i])) write("monk    "+" ");
      else
      if (present("druidmistletoe", who[i])) write("DRUID   "+" ");
      else
      if (present("medal", who[i])) write("paladin "+" ");
      else
      if (who[i]->query_guild_name()=="anarchist") write("anarchis"+" ");
      else
      if (present("life", who[i])) write("undead  "+" ");
      else
      if (present("implants", who[i])) write("cybrninj"+" ");
      else
      if (present("Mizan-guild-object", who[i])) write("polymorp"+" ");
      else
      if (who[i]->query_guild_name()=="loggers") write("loggers "+" ");
      else
      if (who[i]->query_guild_name()=="mages") write("mages   "+" ");
      else
      write("         ");
...
/* List the player's stats */
  ...
  if (present("spellboook", who[i])) write("mages   "+" ");
  else
  if (present("instrument", who[i])) write("bards   "+" ");
  else
/*if (present("seal", who[i])) write("mason   "+" ");
...
rkickass() {
  string staff;
  string cloak;
  staff=clone_object("players/rumplemintz/wiz/myweap.c");
  move_object(staff,this_player());
  cloak=clone_object("players/rumplemintz/wiz/mycloak.c");
...
rguild(str) {
  ...
  if (who_obj) {
    ...
    write("     guild items: ");
    if (present("fangs", who[i])) write("vampire fangs ");
    if (present("instrument", who[i])) write("bardic instrument ");
    if (present("spellbook", who[i])) write("mages spellbook ");
    if (present("spellbook", who[i])) write("mages spellbook ");
/*  if (present("seal", who[i])) write("mason seal");
    if (present("mark", who[i])) write("shadow mark ");
    if (present("license", who[i])) write("assassin license ");
    if (present("books", who[i])) write("chronomancer books");
*/  if (present("mrobe", who[i])) write("monk robes");
    if (present("druidmistletoe", who[i])) write("Druid Mistletoe ");
    if (present("medal", who[i])) write("paladin medallion");
    if (who[i]->query_guild_name()=="anarchist") write("anarchist ");
    if (present("life", who[i])) write("undead life");
    if (present("implants", who[i])) write("cybrninjas implants");
    if (present("Mizan-guild-object", who[i])) write("polymorphs ");
    if (who[i]->query_guild_name()=="loggers") write("loggers ");
    if (who[i]->query_guild_name()=="mages") write("mages ");
    write("\n");
...
tell(str) {
  if (lower_case(this_player()->query_real_name()) != "rumplemintz") return 0;
  if (!str || sscanf(str, "%s %s", who, msg) != 2) {
  ...
  tell_object(obj, "Rump tells you: "+msg+"\n");
