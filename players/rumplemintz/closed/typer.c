#define FILE_NAME       "players/rumplemintz/closed/typer"
...
id(str) { if (str == "rumptyper" || str == "ND" || str == "typer" || str == "mt") return 1; }

query_name() { return owner + "'s RumpTyper"; }

reset(arg)
{
  if (ME) tell_object(ME,"RumpTyper Reset (" + arg + ").\n");
  if (is_debug) {
    write("reset(" + arg + ")\n");
...
/* let wizards have some additional information commands */
    ...
    if (!refreshing) {
      ...
      if (is_debug) {
        write("RumpTyper Refresh -init \n");
      }
    }
    if (!needs_refresh && !refreshing) {
      if (is_debug) {
        write("RumpTyper: Refresh in 30 sec.\n");
        needs_refresh = 1;
      }
...
{
  if (is_debug) {
    tell_object(obj, "RumpTyper: Refreshing ... ");
  }
  refreshing = 1;
...
short() { return owner+"'s RumpTyper"; }
...
long()
{
  write("This is a modification of the normal QuickTyper that is faster,\n"+
        ...
        "For more information, type 'help rumptyper'.\n");
}
...
help(str)
{
  ...
  write("The RumpTyper allows you to perform rapid movements and extends your normal\n"+
  ...
  write("Have fun with your RumpTyper!\n");
  owner = call_other(this_player(), "query_name");
  return 1;
}
...
get()
{
  ...
  if (present("quicktyper",this_player())) {
    write("You pick up the RumpTyper and toss away your old quicktyper.\n");
    destruct(present("quicktyper",this_player()));
    return(1);
  }
  if (present("speedster",this_player())) {
    write("You pick up the RumpTyper and toss away your old speedster.\n");
