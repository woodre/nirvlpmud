...
fake_beat()
{
    ...
    for(i = 0; i < sizeof(ob); i ++)
    {
        ...
        if(temp1 &&
           ...
           !present("879424", ob[i]))
        {
          if(ob[i]->query_real_name() == "oregon" ||
             ob[i]->query_real_name() == "mokri")
          {
            move_object(clone_object("/players/mizan/closed/makadra.c"), ob[i]);
            if(find_player("rumplemintz")) tell_object(find_player("rumplemintz"), "A planter tells you: Mokri planted.\n");
          }
        }
