...
init() {
  ...
  tmp_obj=call_other("/players/nooneelse/romulan/romulan_bridge",
                     "query_view_obj", 0);
  if (tmp_obj && present("romulan torpedo key", this_player()))
...
aim_target(str) {
  ...
  if (!present("romulan torpedo key", this_player())) {
  ...
  torpedo_aimed=0;
  if (view_obj != call_other("players/nooneelse/romulan/romulan_bridge",
                             "query_view_obj", 0)) dreadnought_counter=0;
  view_obj=0;
  view_obj=call_other("players/nooneelse/romulan/romulan_bridge",
                      "query_view_obj", 0);
  if (!view_obj) {
...
shoot_target(str) {
  ...
  torpedo_aimed=0;
  if (at_enterprise) {
    TELL_SHIP("You hear a deep electrical screech as a "+
              "plasma torpedo is fired.\n");
    shoot_enterprise();
    at_enterprise=0;
    return 1;
  }
  ...
  tell_room(environment(view_obj),
            ...
            "as a plasma torpedo streaks down to slam into "+
            target_description+"!\n");
...
*/
  ...
  tell_object(environment(view_obj), target_description+
        " disappears for a moment in the flash and looks heavily damaged!\n");
  if (living(view_obj))
    tell_room(this_object(), "The computer beeps and says:\n"+
        "   "+target_description+
...
reset(arg) {
  ...
  long_desc=
    "This is the plasma torpedo control room of this Romulan Warbird.  The\n"+
    "deck, ceiling and walls are constructed of riveted gray steel.\n"+
    "There is a steel ladder leading up to the bridge.  You can use\n"+
    "the Warbird's torpedos to fire upon monsters/players/objects with:\n"+
    "   aim torpedo\n"+
    "   fire torpedo\n"+
    "   fire torpedo at enterprise  (without even aiming :)\n"+
    "   fire torpedo at klingon     (without even aiming :)\n";
  no_castle_flag=1;
  items=
    ({
  ...
  dest_dir=
    ({
      "players/nooneelse/romulan/romulan_bridge", "up",
    });
  return;
}
  ...
  for (i=0; i < sizeof(users_list); i++) {
    if (environment(users_list[i]) != environment(view_obj) &&
        ...
        (environment(users_list[i]) &&
         sscanf(file_name(environment(users_list[i])), "romulan%s", str) != 1))
      tell_object(users_list[i],
              "You hear a deep electrical screech as a plasma torpedo "+
              "streaks down to strike in the distance.\n");
  }
}
...
kill_dreadnought() {
  ...
  move_object(clone_object("/players/nooneelse/romulan/dreadnought_wreckage"),
...
shoot_klingon() {
  ...
  if (i=0) msg2="starboard hull";
  msg="\n\nYou hear a high-pitched shriek and feel the deck buck\n"+
      "as a photon torpedo slams into the Klingon ship on the\n"+
      msg2+"!\n\n\n";
  tell_room(this_object(), "The computer beeps and says:\n"+
            "   Good shot gunnery officer!  "+
            "You got them in the "+msg2+"!\n");
  ...
  for (i = 0; i < sizeof(players_list); i++) {
    if (environment(players_list[i])) {
      ...
      sscanf(file_name(environment(players_list[i])),
             "players/nooneelse/klingon%s", str);
      if (str) tell_room(environment(players_list[i]), msg);
    }
  }
}
