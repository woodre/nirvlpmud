...
transport_me(str) {
  ...
  tell_room(old_room,
      capitalize(this_player()->query_name())+
      " speaks into a small silver box,\n"+
      "saying something that sounds like:\n"+
      "   Transport me!\n"+
      "You hear a high-pitched electrical hum and see a sparkling\n"+
      "rainbow of color surround "+capitalize(this_player()->query_name())+
...
reset(arg) {
  ...
  set_short("A silver metal box");
  set_value(225);
  set_weight(1);
}

long() {
  write("A small, silver, metal box with a small built-in microphone.\n"+
        "On the side, in small letters are the words 'transport me'.\n");
  if (sscanf(file_name(environment(this_player())),
             "%sromulan%s", str1, str2)==2)
    write(
      "\nWhile on the Romulan Warbird, the following abreviations will work:\n"+
      "    forward   = fo\n"+
      "    aft       = af\n"+
      "    starboard = st\n"+
      "    port      = po\n");
...
id(str) { return (str=="romulan communicator" ||
                  str=="communicator" ||
                  str=="box"); }
