...
init() {
  ...
  tmp_obj=call_other("/players/nooneelse/romulan/romulan_bridge",
                     "query_view_obj", 0);
  if (tmp_obj && present("romulan captain key", this_player()))
...
energize_transporter() {
  ...
  if (!view_obj && present("romulan captain key", this_player()))
      view_obj=call_other("players/nooneelse/romulan/romulan_bridge",
                          "query_view_obj", 0);
  if (!view_obj) {
    tell_room(this_object(), "The computer beeps and says:\n"+
...
reset(arg) {
  ...
  long_desc=
    "This is the transporter control room of this Romulan Warbird.  The\n"+
    "deck, ceiling and walls are constructed of riveted gray steel.\n"+
    "There is a doorway leading to engineering.  You can use the Warbird's\n"+
    ...
    "                                the Enterprise or the Klingon ship\n"+
    "                                unless the Captain's key is used to\n"+
    "                                link the transporters to the Warbird's\n"+
    "                                scanners, in which case, you won't need\n"+
  ...
  dest_dir=
    ({
      "players/nooneelse/romulan/romulan_engineering", "up",
    });
  return;
}
