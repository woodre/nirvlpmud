...
init() {
  ...
  tmp_obj=call_other("/players/nooneelse/romulan/romulan_bridge",
                     "query_view_obj", 0);
  if (tmp_obj && present("romulan phaser key", this_player()))
...
aim_target(str) {
  ...
  if (!present("romulan phaser key", this_player())) {
...
reset(arg) {
  ...
  long_desc=
    "This is the phaser control room of this Romulan Warbird.  The\n"+
    "deck, ceiling and walls are constructed of riveted gray steel.\n"+
    "There is a steel ladder leading down to the bridge.  You can\n"+
    "use the Warbird's phasers to fire upon monsters/players/objects with:\n"+
    "   aim phasers\n"+
    "   fire phasers\n"+
    "   fire phasers at enterprise  (without even aiming :)\n"+
    "   fire phasers at klingon     (without even aiming :)\n";
  no_castle_flag=1;
  items=
    ({
  ...
  dest_dir=
    ({
      "players/nooneelse/romulan/romulan_bridge", "down",
    });
  return;
}
...
shoot_klingon() {
  ...
  if (i=0) msg2="starboard hull";
  msg="\n\nYou hear a high-pitched shriek and feel the deck buck\n"+
      "as a phaser bolt slams into the Klingon ship on the\n"+
      msg2+"!\n\n\n";
  tell_room(this_object(), "The computer beeps and says:\n"+
            "   Good shot gunnery officer!  "+
            "You got them in the "+msg2+"!\n");
