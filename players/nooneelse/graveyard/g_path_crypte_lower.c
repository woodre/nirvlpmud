...
reset(arg) {
  ...
  dest_dir=({
             "players/nooneelse/graveyard/g_path_crypte_stairs", "up",
           });
  if (!find_living("cabal")) {
    lich_obj=clone_object("players/nooneelse/mon/lich");
    move_object(lich_obj, this_object());
  }
}
