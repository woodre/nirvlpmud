...
sell_items(item_name) {
  ...
  if (!item_name) return;
  if (item_name=="all") {
    write("To sell all, you must use 'sell everything'.\n"+
...
east() {
  if (this_player()->query_level() < 21) {
    write("A previously invisible force screen flashes black preventing\n"+
          "you from going east.\n");
    say("As "+capitalize(this_player()->query_name())+" tries to go east\n"+
  ...
  say("As "+capitalize(this_player()->query_name())+" goes east\n"+
      "into the back room, a previously invisible force screen flashes\n"+
      "black, but allows "+this_player()->query_objective()+" to pass.\n");
  this_player()->move_player("east#"+ALCHEMIST_STOREROOM);
  return 1;
}
...
reset(arg) {
  ...
  set_light(1);
  no_castle_flag=1;
  short_desc="The Alchemist Guild's Shop\n"+
  ...
  dest_dir =
    ({
      ALCHEMIST_LOBBY, "west",
      ALCHEMIST_STOREROOM, "east",
    });
  call_other(ALCHEMIST_STOREROOM, "???", 0);
  shop_storeroom_obj = find_object(ALCHEMIST_STOREROOM);
