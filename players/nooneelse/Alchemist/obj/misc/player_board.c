/*
  alchemist_player_board.c - new version of bull_board that allows setting the
                             file name and directory of the saved file
                           - also, guild master can reply to a note without
                             adding a new note by appending the reply to the
                             original note
*/

#include "/players/nooneelse/Alchemist/lib/defs.h"

#define BOARD_NAME "players/nooneelse/Alchemist/board/player_board_save"
...
reply(str) {
  ...
  if (this_player()->query_level() < 21) {
    write("Only Wizards can use reply.\n");
    return 1;
  }
  if (i > num_messages) {
    write("Not that number of messages.\n");
...
remove(str) {
  ...
  if (this_player()->query_level() < 21) {
    write("Only Wizards can remove these notes.\n");
    return 1;
  }
  if (str == 0 || (sscanf(str, "%d", i) != 1 && sscanf(str, "note %d", i) != 1))
    return;
