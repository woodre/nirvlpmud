...
recruit(str)
{
   ...
   if(who && (env(who) == env(tp)))
      ...
      if(!who->query_guild_name())
         ...
         move_object(clone_object(PENT),who);
         f_emote(who,"has just been recruited!");
         who->add_guild_rank(-who->query_guild_rank());
...
brew(str)
{
   ...
   if(!potion->is_potion(str)) return 0;
   ...
   potion->set_type(str);
   write("You brew up a potion of "+str+".  It is a draining task.\n");
   tp->add_sp(-potion->cost());
   return 1;
}
