...
accept_invitation(str) {
  ...
  if (str2 != "players/nooneelse/black/guild_coffin") {
    write(capitalize(coffin_owner_name)+
  ...
  if (present("vampire fangs", this_player())) {
    if (str1 != "players/nooneelse/black/guild_coffin_room") {
      write("You must be in the coffin room for the invitation to work.\n");
      return 1;
    }
  }
  else {
    if (str1 != "players/nooneelse/blackentry") {
      write("You must be in the black room outside the Vampire Guild Rooms\n"+
...
long() {
  if (present("vampire fangs", this_player())) {
    ...
    write(short()+".\n"+
          ...
          capitalize(coffin_owner_name)+"'s coffin.\n"+
          "You can either use it by typing 'accept invitation' while in the\n"+
          "black room outside the Vampire Guild Rooms or get rid of it by\n"+
          "dropping it.\n");
  }
}
