...
init() {
  ...
  add_action("stuff_item",    "stuff");
  add_action("grab_item",     "grab");
...
read_label(str) {
  ...
  write("To use the coffin safe:\n\n"+
        "  read label                  - to see these instructions\n"+
        "  open safe                   - open the safe so you can use it\n"+
        "  close safe                  - close the safe\n"+
        "  stuff <item> into safe      - puts <item> into your safe\n"+
        "  grab <item #> from safe     - gets <item> from your safe\n"+
        "  reset safe                  - clears your safe\n"+
        "                                (WARNING: you will lose all items\n"+
        "                                          in the safe!!!!)\n\n\n"+
...
stuff_item(arg) {
  ...
  if (junk == " xxx" || junk != "into safe xxx") return 0;
  if (this_player()->query_real_name()!=
        environment(this_object())->query_coffin_owner_name() &&
...
if(ob->query_broke()) {
  write("You cannot store broken items.\n");
  return 1; }
  ...
  rm(tempfile);
  write("Ok, you stuff "+item+" into the safe.\n");
/*
  write_file("/players/nooneelse/black/safes/logs/",
             this_player()->query_name()+" "+obfile+" "+ctime(time())+".\n");
*/
  say(this_player()->query_name()+" stuffs "+item+" into the safe.\n");
...
grab_item(arg) {
  ...
  arg = arg + " xxx";
  sscanf(arg, "%s %s", item, junk);
  if (junk == " xxx" || junk != "from safe xxx") return 0;
  if (this_player()->query_real_name()!=
        environment(this_object())->query_coffin_owner_name() &&
      GUILD_OFFICE_FLOOR_TOP->find_officer(MY_NAME)!=GUILD_MASTER &&
      GUILD_OFFICE_FLOOR_TOP->find_officer(MY_NAME)!=GUILD_ASSISTANT &&
  ...
  if (safe_is_open!=1) {
    write("You cannot grab items unless you open the safe.\n");
    return 1;
  }
  ...
  sscanf(tempstr, "%d", numitems);
  if (x>numitems || x<1) {
    write("There is no item number '"+x+"' in the safe.\n");
    return 1;
  }
  x+=2;
  tempstr = read_file("/players/nooneelse/black/safes/"+
  ...
  write("You grab "+ob->short()+" from your safe.\n");
  say(this_player()->query_name()+" grabs "+ob->short()+" from the safe.\n");
  ...
  write_file(tempstr, NUMBER_OF_ITEMS + "\n");
...
empty_safe(arg) {
  ...
  filename = "/players/nooneelse/black/safes/"+
             environment(this_object())->query_coffin_owner_name();
  rm("/players/nooneelse/black/safes/"+
     environment(this_object())->query_coffin_owner_name());
  write_file(filename, NUMBER_OF_ITEMS + "\n");
  write_file(filename, "0\n");
  write("Your safe has been emptied. Anything that may have been in it is now gone.\n");
  return 1;
}
