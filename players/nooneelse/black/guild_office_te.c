...
port() {
  this_player()->move_player("into space#players/saber/ryllian/portal.c");
  return 1; }

church() {
  this_player()->move_player("into thin air#room/church.c");
  return 1; }
...
lamp(str) {
  ...
  if (str=="fire") {
    if (fire_is_out) {
      fire_is_out=0;
      set_light(1);
      fire_msg="There is a  roaring fire in the fireplace.\n";
      long_desc=long_desc_office+fire_msg+door_msg+command_str;
...
reset (arg) {
  ...
  no_castle_flag=1;
short_desc = (BOLD+"The "+HIR+"Vampire Underlord's"+BOLD+" Office"+NORM);
  long_desc_office=
    "This is a huge private office.  There is a large rosewood desk\n"+
    "with a crystal lamp and a brass name plaque on it and a comfortably\n"+
    "padded chair sitting in front of a wall of bookshelves.\n";
  fire_msg="There is a roaring fire in the fireplace.\n";
  door_msg="The door is locked.\n";
  ...
  items =
    ({
      "desk",
      "It is a solid, jet-black desk cut from a single stone.\n"+
      ...
      "shelves",
        "They contain books with titles like 'Disciplining Unruly Vampires',\n"+
        "'No One Else Can Top This' and 'Running A Happy Guild'",
	"players/saber/ryllian/portal.c","port",
      "shelf",
        "They contain books with titles like 'Disciplining Unruly Vampires',\n"+
        "'No One Else Can Top This' and 'Running A Happy Guild'",
      "lamp", "It is made of fine-cut crystal.  It is fixed to the desk",
      "fireplace", "It is a large open stone fireplace",
      "fire", "It is a solid stone fireplace",
  });
  dest_dir =
    ({
      "players/nooneelse/black/guild_office_floor_top", "west",
      "room/post", "post",
    });
  fire_is_out = 0;
  door_is_locked = 1;
