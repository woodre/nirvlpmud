...
init_arg() {
  ...
  write(BOLD+
    "    ~o~\n"+
    "~o~\n"+
    "   Check 'guild news' to see what's new.\n"+
    "~o~\n"+
    "    ~o~\n"+
    NORM+"\n");
  return 0;
...
short() { if(SHOWFANGS) return HIR+"bloody, dripping fangs"+NORM;
          else return; }

long() { write(
  "A pair of long, pointed, blood_stained fangs that protrude down\n"+
  "out of your upper gums.\n"); }

init() {
  int is_a_vampire;

/*  DELETE THIS OBJECT IN FAVOR OF A NEW VAMP OBJECT
if(environment(this_object())->is_player()) {
  move_object(clone_object("/players/eurale/VAMPIRES/changefangs"),this_player());
  destruct(this_object());
  return 1; }
*/
...
*/
  ...
  add_action("vamp_seconds",      "vamp");  /* lines 591-596  */
  ...
  add_action("vbow","vbow");
  add_action("nope",              "burp");
  add_action("nope",              "cackle");
  add_action("nope",              "chuckle");
  add_action("vchortle","vchortle");
  add_action("vcurtsey","vcurtsey");
  ...
  add_action("vhiss","vhiss");
  add_action("nope",              "hug");
  add_action("nope",              "kick");
  add_action("vkhand","vkhand");
...
if(!str) return 0;
if(str == "seconds") {
  cat("/players/nooneelse/black/guild_seconds");
  return 1; }
if(str == "emotes") {
  cat("/players/nooneelse/black/guild_emotes");
  return 1; }
}
...
/*---------------------------------------------------*/
alarm_bat(str) { return VAMPIRE_FANGS_BASE->alarm_bat(str); }
assess(str) { return VAMPIRE_FANGS_BASE->assess(str); }
awaken(str) { return VAMPIRE_FANGS_BASE->awaken(str); }
backhand_enemy(str) { return VAMPIRE_FANGS_BASE->backhand_enemyx(str); }
...
message_bat(str) { return VAMPIRE_FANGS_BASE->message_bat(str); }
obit_list(str) { return VAMPIRE_FANGS_BASE->obit_list(str); }
...
vbow(str) { return VAMPIRE_FANGS_BASE->vbow(str); }
vchortle(str) { return VAMPIRE_FANGS_BASE->vchortle(str); }
vcurtsey(str) { return VAMPIRE_FANGS_BASE->vcurtsey(str); }
vhiss(str) { return VAMPIRE_FANGS_BASE->vhiss(str); }
vkhand(str) { return VAMPIRE_FANGS_BASE->vkhand(str); }
...
vsmile(str) { return VAMPIRE_FANGS_BASE->vsmile(str); }
...
vyawn(str) { return VAMPIRE_FANGS_BASE->vyawn(str); }
