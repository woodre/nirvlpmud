/*
  nooneelse_bull_board.c - new version of bull_board that allows setting the
                           ...
                           <modified to be wrtiien on by guild lieutenants &
                            master only>
*/
...
new(hd) {
  ...
  if (this_player()->query_level() < 21 &&
      GUILD_OFFICE_FLOOR_TOP->find_officer(this_player()->query_real_name())!=
          GUILD_ASSISTANT &&
      GUILD_OFFICE_FLOOR_TOP->find_officer(this_player()->query_real_name())!=
          GUILD_LIEUTENANT) {
    write("Only the Guild Master, Guild Assistants, or Lieutenants "+
          "can add notes to this board.\n");
    return 1;
  }
