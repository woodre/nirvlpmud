...
string *guild_name, *guild_display, guilds_list;
string *who_array;
...
long() {
  if (this_player()->query_level()<WIZARD_LEVEL) {
    write("This is Nooneelse's toy, and you can't use it!\n");
    return;
  }
  write("For help use: rhelp  [nhelps (for short help)]\n");
...
init() {
  ...
  add_action("nhelp",            "nhelp");
  add_action("nhelps",           "nhelps");
  add_action("nalias",           "nalias");
  add_action("nfake_quicktyper", "whos");
  add_action("nfake_quicktyper", "who");
  add_action("nfake_quicktyper", "whow");
  add_action("nfake_quicktyper", "log");
  add_action("nfake_quicktyper", "elog");
  add_action("nfake_quicktyper", "l");
  add_action("nid",              "nid");
  add_action("nplayer",          "nplayer");
  add_action("nobjects",         "nobjects");
  add_action("nobjects",         "nob");
  add_action("nalign",           "nalign");
  add_action("npretitle",        "npretitle");
  add_action("ntitle",           "ntitle");
  add_action("ntrans",           "ntrans");
  add_action("nwho",             "nwho");
  add_action("non",              "non");
  add_action("noff",             "noff");
  add_action("nfrog",            "nfrog");
  add_action("ngender",          "ngender");
  add_action("nptitle",          "nptitle");
  add_action("npalign",          "npalign");
  add_action("nmail",            "nmail");
  add_action("nmail",            "npost");
  add_action("nreturn",          "nreturn");
  add_action("ncheck",           "ncheck");
  add_action("ncheck",           "nc");
  add_action("nlog",             "nlog");
  add_action("nrmlog",           "nrmlog");
  add_action("nbug",             "nbug");
  add_action("nrmbug",           "nrmbug");
  add_action("nmove",            "nmove");
  add_action("nsummon",          "nsummon");
  add_action("nsilence",         "nsilence");
  add_action("nget",             "nget");
  add_action("ndrop",            "ndrop");
  add_action("nclone",           "nclone");
  add_action("ngoto",            "ngoto");
  add_action("ngoin",            "ngoin");
  add_action("ndest",            "ndest");
  add_action("nclean",           "nclean");
  add_action("ncleanmythos",     "ncleanmythos");
  add_action("nreset",           "nreset");
  add_action("ntrace",           "ntrace");
  add_action("ntrace",           "nfind");
  add_action("nlook",            "nlook");
  add_action("npeek",            "npeek");
  add_action("nblock",           "nblock");
  add_action("nmin",             "nmin");
  add_action("nmout",            "nmout");
  add_action("nmmin",            "nmmin");
  add_action("nmmout",           "nmmout");
  add_action("nvis",             "nvis");
  add_action("ninvis",           "ninvis");
  add_action("nfollow",          "nfollow");
  add_action("nmoney",           "nmoney");
  add_action("nweight",          "nweight");
  add_action("nstop",            "nstop");
  add_action("nsnoop",           "nsnoop");
  add_action("ngnat",            "ngnat");
  add_action("nheal",            "nheal");
  add_action("nguild",           "nguild");
  add_action("nxp",              "nxp");
  add_action("nhere",            "nhere");
  add_action("ncount",           "ncount");
  add_action("nlist",            "nlist");
  add_action("nremember",        "nremember");
  add_action("nremember",        "nrem");
  add_action("npatch",           "npatch");
/* ---------  doodad directory commands  -------------- */
  add_action("nset_directory",   "nset");
  add_action("nls_directory",    "nsls");
  add_action("ncd_directory",    "nscd");
  add_action("nlog_work",        "nfile");
  add_action("nscd_log",         "ndlog");
  add_action("npwd",             "npwd");
/* ---------------------------------------------------- */
  add_action("ntell",            "ntell");
  add_action("necho",            "necho");
  add_action("necho_to",         "nechoto");
  add_action("necho_all",        "nechoall");
  add_action("nzap_them",        "nzap");
  add_action("ndemote",          "ndemote");
  add_action("npromote",         "npromote");
  add_action("nrat",             "nrat");
  add_action("nquiet",           "nquiet");
  add_action("nforce",           "nforce");
  add_action("nprison",          "nprison");
  add_action("nrelease",         "nrelease");
  add_action("nrelease",         "nfree");
  add_action("npat",             "npat");
  add_action("nfixme",           "nfixme");
}
...
nfake_quicktyper(str)  {
  ...
  /* version of abreviated look command */
  if (str=="l") {
...
nplayer(str)  {
  if (this_player()->query_level()<WIZARD_LEVEL) return;
  if (!str) { write ("Use: nplayer <player>\n"); return 1; }
  obj = find_ob(str);
...
nsummon(str) {
  if (this_player()->query_level()<WIZARD_LEVEL) return;
  if(!str) { write("Use: nsummon <object>\n"); return 1; }
  obj = find_ob(str);
...
nsilence(str) {
  if (this_player()->query_level()<WIZARD_LEVEL) return;
  if (!str) { write("Use: nsilence <who>"); return 1; }
  obj = find_player(str);
  if (!obj) {
...
nget(str) {
  if (this_player()->query_level()<WIZARD_LEVEL) return;
  if(!str) { write("Use: nget <what> [from <where>]\n"); return 1; }
...
ngender(str) {
  ...
  if (!str ||
    ...
    write("Change who's gender to what?\n"+
          "Use: ngender <who> to <male/female/creature>\n");
    return 1;
  }
  it = lower_case(who);
  obj = find_living(it);
  ...
  obj->set_gender(what);
  tell_object(obj, "You are now a "+what+"!!!\n");
...
npretitle(str) {
  if (this_player()->query_level()<WIZARD_LEVEL) return;
  if (!str) {
    write("Use: npretitle <pretitle_string>\n"); return 1;}
...
nptitle(str){
  ...
  if (!str || sscanf(str, "%s %s",who,msg) != 2){
    write("Use: nptitle <player> <title>\n");
    return 1;
  }
  it = lower_case(who);
  obj = find_living(it);
...
npalign(str){
  if (this_player()->query_level()<WIZARD_LEVEL) return;
  if (!str || sscanf(str, "%s %s",who,msg) != 2){
    write("Use: npalign <player> <alignment message>\n");
    return 1;
  }
  it = lower_case(who);
  obj = find_living(it);
...
/* ---------- Clean (destruct) objects from a location/player ---------- */
nclean(where) {
  if (this_player()->query_level()<WIZARD_LEVEL) return;
...
/* --------------- Clean (destruct) objects from mythos --------------- */
ncleanmythos() {
  if (this_player()->query_level()<WIZARD_LEVEL) return;
  where_obj=find_player("mythos");
  if (!where_obj) {
    write("I can't find it. ("+where+")\n");
    return 1;
  }
...
nreset(str) {
  if (this_player()->query_level()<WIZARD_LEVEL) return;
  if(!str) { write("Use: nreset <object>\n"); return 1; }
  obj = find_ob(str);
...
ntrace(str) {
  if (this_player()->query_level()<WIZARD_LEVEL) return;
  if(!str) { write("Use: ntrace <object>\n"); return 1; }
  obj = find_ob(str);
  if (!obj) { Fail(str); return 1; }
  write("Object   : ");
  write_ob_and_desc(obj);
...
nguild(str) {
  if (this_player()->query_level()<WIZARD_LEVEL) return;
  if (!str) {
    write("Use: nguild <who> to see <who>'s guild & guild exp\n");
    return 1;
  }
...
nlog_work(str) {
  write_file(directory+"/WORK_REPORT",
             "Nooneelse "+ctime()+" ** "+str+"\n");
  write("Logged.\n");
  return 1;
}
...
ntell(str) {
  ...
  tell_object(obj, NORM+HIB+"Nooneelse tells you:"+NORM+GRY+" "+msg+NORM+"\n");
...
npatch(str) {
  if (this_player()->query_level()<WIZARD_LEVEL) return;
  if(!str) { write("Use: npatch <object> <function> <argument>\n"); return 1; }
  ...
  while(count) {
    if(check)
      call_other(obj,do_str,iarg);
    else
      call_other(obj,do_str,arg);
    count -= 1;
  }
  return 1;
}
...
Fail(str) { write(NORM+HIR+"Couldn't find '" + str + "'."+NORM+"\n"); return 1; }

/* Display an error message */
Error(str) {
  write(NORM+HIR+"*** Error"+NORM);
  if(!str)
    write(".\n");
  else
    write(": " + str + "\n");
...
rlist() {
  ...
  if(rem_obj) write("rem: ");write_ob_and_desc(rem_obj);
  if(old_rem_obj) write("old: ");write_ob_and_desc(old_rem_obj);
  if(last_arg) write("arg: ");write(last_arg);
  return 1;
}
