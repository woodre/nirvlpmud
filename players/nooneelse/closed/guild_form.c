...
init(str) {
  ...
  add_action("im_attacking",   "kill");
  add_action("im_attacking",   "damn");
  add_action("im_attacking",   "hellfire", 4);
  add_action("im_attacking",   "paralyze", 3);
  add_action("im_attacking",   "missile");
  add_action("im_attacking",   "shock");
  add_action("im_attacking",   "fireball");
  add_action("im_attacking",   "fire");
...
dissipate_form(str) {
  if (lower_case(this_player()->query_name()) != form_owner_name) return;
  ...
  present("nooneelse@vampire@fangs", this_player())->dissipate_form();
...
shoot(str) {
  ...
  if (present("slingshot", this_player())) return;
  im_attacking(str);
  return;
}
...
catch_tell(str) {
  if (!form_owner_name || !find_player(form_owner_name)) {
    move_object(this_player(), environment(this_object()));
    ...
    write("\nEither there's something wrong with this form or the form\n"+
          ...
          "there's still a problem, mail Nooneelse.)\n\n"+
...
set_form(str) {
  string short_desc;
  sscanf(str, "%s %s", form_owner_name, form_description);
  if (form_description=="mist") {
    short_desc="A tenuous mist";
    msgin="drifts in";
    msgout="drifts";
  }
  if (form_description=="bat") {
    short_desc="A huge, black bat";
    msgin="flys in";
    msgout="flys";
  }
  if (form_description=="wolf") {
  ...
  this_object()->set_alias(form_description);
