/*
 groad31.c - good side of green road
*/
...
swim(str) {
  ...
  if (str && str=="water" || str=="pool" || str2=="water" || str2=="pool") {
    ...
    move_object(this_player(), "players/nooneelse/green/groad35");
...
reset(arg) {
  ...
  long_desc="You are standing on a path in a pretty forest.  "+
            ...
            "Next to the path is a deep pool of water.\n";
  items =
    ({
  ...
  dest_dir=
    ({
      "players/nooneelse/green/groad32", "north",
      "players/nooneelse/green/groad30", "south",
    });
  if (!present("nixie")) {
    move_object(clone_object("players/nooneelse/mon/nixie"), this_object());
  }
  return;
}
