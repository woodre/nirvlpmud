...
reset(arg) {
     ::reset(arg);
          if(!arg){
          weapon=clone_object("players/rumplemintz/weapon/slaysword");
          name="slaysword";
          move_object(weapon, this_object());
          command("wield "+name, this_object());
          set_name("demon");
          set_short("Demon Guardian of Rump");
          set_level(1);
	set_hp(1);
          set_aggressive(0);
          set_long("A massive Black Guardian Demon. He is assigned\n"+
                   "the task of making mince-meat of any opponent\n"+
                   "of Rump's.  It looks like he could probably\n"+
                   "kick your ass with that sword.\n");
          set_race("demon");
          set_ac(5);
     chat = allocate(5);
    ...
    i = this_player()->query_hit_point();
    i+=-10;
    this_player()->restore_hit_points(-i);
