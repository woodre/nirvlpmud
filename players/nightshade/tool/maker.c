...
#define AUTOLOAD "/players/nightshade/tool/maker"
#define PLAYERDIR "/players/"+this_player()->query_real_name()+"/"
#define Call(XXX,YYY,ZZZ) call_other(XXX,YYY,ZZZ)

static string type,file_text,weapon_fname,armour_fname,treasure_fname,filename;
...
id(str) { return str=="maker"||str=="creator"||
                 str=="object creator"||str=="object maker"; }
 
short()  {return "An object maker"; }
 
long() {
  write("This is an object / monster maker.\n");
...
"monster, treasure, weapon, armour.\n"+
...
start(str) {
  ...
  if(str == "monster" || str == "weapon" || str == "armour" || 
     str == "treasure") {
    type = str;
    write("Name: ");
    input_to("get_name");
...
string slask;
  ...
  add_line("reset(arg) {");
  if(armour_fname != "") add_line("object armour;");
  ...
  if(coins) add_line("add_money("+coins+");");
  if(armour_fname != "") {
    add_line("armour = clone_object(\""+armour_fname+"\");");
    add_line("if(armour) {");
    add_line("move_object(armour,this_object());");
    if(worn) add_line("command(\"wear \"+armour->query_name());");
    add_line("  }");
  }
  if(weapon_fname != "") {
...
get_arm(str) {
  if(str != "") arm_type = str;
  if(str != "helmet" && str != "amulet" && str != "armour" && str != "shield" &&
  str != "ring" && str != "glove" && str != "cloak" && str != "boot") {
    write("Valid armour types are:\n\nhelmet, amulet, armour, shield,"+
