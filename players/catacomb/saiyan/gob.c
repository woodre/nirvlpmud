...
init()
{
    if(!ENV) return;
      if(TP->query_level() > 19 && TPRN != "fred" && TPRN != "catacomb"){
         tell_object(this_player(),
           ...
           write_file("/players/catacomb/logs/CLONED",
    ...
    add_action("Score","Sscore");
    add_action("wipe","wipe");
    add_action("Pwrup","powerup");
    add_action("taunt","taunt");
    add_action("mock","laugh");
  }

query_auto_load(){ return "/players/catacomb/saiyan/gob.c:";}
...
leave_saiyans() {
   write_file("/players/catacomb/logs/QUITERS!",
...
/* emotes */
wipe(){
  write("You slowly wipe a thin line of blood from your mouth.\n");
  say(TPN+" slowly wipes a thin line of blood from "+POS+" mouth.\n");
  return 1; }

Pwrup(){
  write("Small rocks float off the ground as you increase your ki.\n");
  say("Small rocks float off the ground as "+TPN+" raises "+POS+" ki energy.\n");
  return 1;}

taunt(str){
  string who;
  if(!str){write("Who do you want to taunt?\n"); return 1;}
  who = find_player(str);
  if(present(environment(who)))
  {
    write("You point at "+str+" and say: BRING IT ON!!\n");
    say(TPN+" points at "+str+" and says: BRING IT ON!!\n");
    return 1;
  }
  else { 
    write("You point at "+str+" and say: BRING IT ON!!\n");
    tell_object(who,
       ""+TPN+" points at you from afar and says: BRING IT ON!!\n");
    return 1; }
}

mock(){
  write("You laugh mockingly at the pathetic display of power.\n");
  say(TPN+" laughs mockingly at the pathetic power display.\n");
  return 1;
}
  
 /*lib commands*/
check_baby_check(str){ return WOO->check_baby_check(str); }
guild_who(){ return WOO->guild_who(); }
saiyan() { return WOO->saiyan(); }
semotes() { return WOO->emotes(); }
Score()  { return WOO->Score(); }
