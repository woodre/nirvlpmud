...

{
  ...
  if( path[strlen(path)-1] != '/' )
    path += "/";
  header = implode(explode(pad("",width/2,'='),""),"-")+"\n";
  write( header );
  file_list = get_dir( path );
  if( file_list )
    printf("%-*#s\n", width, implode(file_list, "\n"));
  else
...
object find_living(string name)
{
    ...
    if (pointerp(a = r = name_living_m[name])) {
#if find_living_debug
