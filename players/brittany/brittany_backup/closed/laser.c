...
long() {
  ...
  write("Just a small tube, with a little orifice at one end.  It shows also\n"+
       ...
       "on it:  the "+HIG+"green"+NORM+" one is for 'power on', the "+HIR+"read"+NORM+" one is for \n'power off' "+
...
power(str) {
   ...
   if(str=="on")
   {
    ...
    say(this_player()->query_name()+" pushes the red button of the laser and it is powered on.\n");
    return 1;
   }  
   if(str=="off")
   {
   ...
   call_other(this_player(), "stop_wielding");
    ...
    say(this_player()->query_name()+" pushes the green button of the laser and it is powered off.\n");
