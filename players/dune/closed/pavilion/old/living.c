...
move_player(dir_dest)
{
  ...
  if(this_object()->query_blown_away())
    say(capitalize(this_object()->query_myname())+
        " is blasted " + dir + ".\n");
...
/* PUT a cap on damage a player can do in one hit. */
  ...
  if (hit_point<0) {
    ...
    hunter = 0;
    hunted = 0;
    say(capitalize(this_object()->query_myname())+" died.\n");
    corpse = clone_object("obj/corpse");
    call_other(corpse, "set_name", 
               capitalize(this_object()->query_myname()));
    call_other(corpse, "set_corpse_level", level);
    if(present("_cockpit_")) destruct(present("_cockpit_"));
    if (!attacker_ob)
