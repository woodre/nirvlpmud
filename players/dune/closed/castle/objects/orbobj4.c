...
id(str) { return str == "orb" || str == "glass orb" || 
...
long()
{
  ...
  write("Gazing into it, you see your reflection.\n");
  if (!name)
    write("An inner voice tells you to name the orb.\n");
  else
    write("The name of the Glass Orb is "+name+".\n");
}
...
int name (string str)
{
  ...
  if (orbType == "glass")
  {
    write("You name the Glass Orb "+orbName+".\n");
    set_name(orbName);
    return 1;
  }
}
