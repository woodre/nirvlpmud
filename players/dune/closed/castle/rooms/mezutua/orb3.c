...
"of the room rests a red pedestal.\n";

    dest_dir=({
CASTLEROOM+"/secret.c","west"  ,
    });

    items=({
"pedestal","     Perhaps you should search this",
"words",   "     You take a look at the engraved words.\n"+
           "An inner chamber of life's flow,\n"+
           "Where cares and feelings go.\n"+
           "The words lie engraved upon a field of red",
    });
  }
}
...
int remove(string str)
{
      ...
      str == "orb from the pedestal")
  {
    if (presentorb == 1)
    {
      ...
      move_object(clone_object(CASTLEOBJECT+"/orbobj3.c"),this_player());
      write_file("/players/dune/log/QUEST_OBJ", this_player()->query_real_name()+
                 " (level "+this_player()->query_level()+
                 ") took the Blood Orb. ("+ctime()+")\n");
      return 1;
    }
