...
"of the room rests a black pedestal.\n";

   dest_dir=({
CASTLEROOM+"/smoke3.c","south"  ,
   });

   items=({
"pedestal","     Perhaps you should search this",
"words",   "     You take a look at the engraved words.\n"+
           "From it many threads doth seep,\n"+
           "To support this eight-legged freak!\n"+
           "The words lie engraved upon a field of black",
   });
  }
}
...
int remove(string str)
{
      ...
      str == "orb from the pedestal")
  {
    if (presentorb == 1)
    {
      ...
      move_object(clone_object(CASTLEOBJECT+"/orbobj1.c"),this_player());
      write_file("/players/dune/log/QUEST_OBJ", this_player()->query_real_name()+
                 " (level "+this_player()->query_level()+
                 ") took the Black orb. ("+ctime()+")\n");
      return 1;
    }
