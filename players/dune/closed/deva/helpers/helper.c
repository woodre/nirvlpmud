...
heart_beat()
{
    ...
    if(attacker_ob->query_hp() < attacker_ob->query_mhp()/20) helper_leave();
    if(hit_point < max_hp/2) helper_leave();
    attack();
...
helper_leave() {
  object ob;
  ob = present(owner+"_spirit_legion", environment(this_object()));
  if(!ob) { destruct(this_object()); return 1; }
  ob->cleanup_after_fight(1, owner);
  return 1; }
