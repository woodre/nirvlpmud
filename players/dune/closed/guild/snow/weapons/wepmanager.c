...
#include "/players/dune/closed/guild/includes/glevels.h"
...
int baseChance, aura_low, aura_dmg, multi_attack_low, 
    ...
    min_bonus_low, min_bonus_high, max_bonus_low;
/* int damage; */
...
/**** ATTACK TYPES ******************************************/ 
void hit_location(string msg_private, string msg_public, int dmg)
{
  int X, num;
  X = random(10);
  num = dmg + 1;
...
void standard(string msg_private, string msg_public, int dmg)
{
...
{
  if (!less_spam)
    write(NI+"A "+BOLD+BLINK+"VORTEX"+OFF+OFF+" of death energy enshrouds you!\n"+
          "You writhe in ecstasy!\n");
  if (environment(TP))
  {
    if (!less_spam)
      write(MEAN+" is lifted, screaming, from the ground!\n"+
...
void check_extra()
{
  ...
  if (bi                                  &&
      random(15) < (int)IPTP->guild_lev() &&
      gotsp(TP, ENERGY_BIONICS))
  {
    write(REV_BLUE+"~##~  BIONICS  ~##~"+OFF+"\n");
    TP->add_spell_point(-ENERGY_BIONICS);
    amt = (int)call_other(this_object(), "checkBrandLvl", (string)IPTP->query_brand());
    mod_prof(3 + random(amt));
  }

  /* No more free extra four points of mod_prof - Dune */
  meat = (object)IPTP->query_target();
  if (IPTP->item_weplink() &&
      meat                 &&
      meat == (object)TP->query_attack())
  {
    mod_prof((int)IPTP->item_weplink() * 2);
  }
}
...
void restrain_damage(object ob)
{
  ...
  if (dmg < 0) dmg = 0;
  if (TRN == OWNER1 || TRN == OWNER2) write(RED+"..WEAPON BONUS : "+dmg+OFF+"\n");
  set_prof(dmg);
}
/************************************************************/
...
/* Lowered chance by 5 to add 5 for PK so no net gain -Snow */
  ...
  if (chance > min_bonus_low   &&
      chance  < min_bonus_high &&
      min_bonus_private)
  {
  ...
  if (TP->query_attack()->query_hp() < (int)IPTP->query_art_level())
  {
    if (name_of_weapon == WEAPON_HANDS)
      ...
      if (!less_spam)
        write("Your "+name_of_weapon+" hungers for the kill!\n");
...
void do_damage(object meat)
{
  ...
  if (TP && !TP->query_pl_k() && damage > MAX_ODAM/2) damage = MAX_ODAM/2;
  if ((int)to_hit->query_hp() - 10 < damage)
    damage = (int)to_hit->query_hp() - 10;
/* Cap added by illarion on 22 Feb 2005 to prevent ludicrous amounts of extra damage */
   if(damage > 20) damage = 20+(random(damage-10)/2);
  if(damage>0) to_hit->hit_player(damage);
  damage = 0;
}

int checkBrandLvl(string brand)
{
  int lvl;
 
  if(brand == "acme")
    lvl = 0;
  if(member_array(brand, BRAND1) > -1)
    lvl = 1;
  if(member_array(brand, BRAND2) > -1)
    lvl = 2;
  if(member_array(brand, BRAND3) > -1)
    lvl = 3;
  if(member_array(brand, BRAND4) > -1)
    lvl = 4;
  if(member_array(brand, BRAND5) > -1)
    lvl = 5;
  if(member_array(brand, BRAND6) > -1)
    lvl = 6;
  if(member_array(brand, BRAND7) > -1)
    lvl = 7;
  if(member_array(brand, BRAND8) > -1)
    lvl = 8;
  if(member_array(brand, BRAND9) > -1)
    lvl = 9;
  if(member_array(brand, BRAND10) > -1)
    lvl = 10;
