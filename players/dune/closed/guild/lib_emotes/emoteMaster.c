...
status main(string str)
{
  ...
  if (!str)
  {
    ...
    write(filterMsg(myMsgU, srcName, trgName, srcPos, trgPos, srcObj, trgObj)+ NORM + "\n"); /* Added NORM in case of color bleed - Rumplemintz */
    say(filterMsg(roomMsgU, srcName, trgName, srcPos, trgPos, srcObj, trgObj)+ NORM + "\n"); /* Added NORM in case of color bleed - Rumplemintz */
    return 1;
  }
  ...
  if (myMsgD)
    write(filterMsg(myMsgD, srcName, trgName, srcPos, trgPos, srcObj, trgObj)+ NORM + "\n"); /* Added NORM in case of color bleed - Rumplemintz */
  else
    write(filterMsg(myMsgU, srcName, trgName, srcPos, trgPos, srcObj, trgObj)+ NORM + "\n"); /* Added NORM in case of color bleed - Rumplemintz */
  ...
  if (present(ob, environment(TP)) || !farMsg)
    tell_object(ob, filterMsg(tellMsg, srcName, trgName, srcPos, trgPos, srcObj, trgObj)+ NORM + "\n");  /* Added Norm - Rumplemintz */
  else
    tell_object(ob, filterMsg(farMsg, srcName, trgName, srcPos, trgPos, srcObj, trgObj)+ NORM + "\n");  /* Added NORM - Rumplemintz */
  ...
  if (present(ob, environment(TP)))
  {
    ...
    for (i=0; i < sizeof(room); i++)
    {
      ...
      if (inv->is_player())
      {
        ...
        {
          tell_object(room[i], roomMsg+ NORM + "\n"); /* Added NORM - Rump */
        }
      }
    }
  }
