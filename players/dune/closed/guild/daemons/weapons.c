...
int findWeaponWeight(string wep) {
  if(!wep) return 0;
  switch(wep) {
  case NUNCU: return 2;
  case NINJO: return 2;
...
string name_weapon(object ob, string str)
{
  object gob;
  string name;
  if(!ob || !str) return 0;
  ...
  if(!gob->query_weapon())
  {
    tell_object(ob, "You do not have a guild weapon.\n");
    return 0;
  }
  str = (string)"/players/vertebraker/closed/std/color"->filter_msg(str);
  gob->set_weapon_name(str);
  gob->save_me();
  return str;
}
