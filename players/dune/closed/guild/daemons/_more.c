...
#define  OPEN          "players/dune/closed/guild"
...
/**** CODE FOR VIEWING GUILD DOCUMENTS ******************************/
...
/**** REST OF THE CODE FOR THE MORE COMMAND *************************/
private int
get_oid() {
  string junk;
  int oid;
...
get_more_com( string str )
{
  ...
  switch( cmd )
    ...
    case 'q':			/* quit */
      if( temp_file && sscanf( fname, OPEN+"/tmp/%s", path ) == 1 ) {
#ifdef __LDMUD__
   write("Removing file name: "+ fname+".\n");
#endif
	rm( fname );
      }
#ifndef __LDMUD__
      rm( "/"+swap_file+".o" );
#else
   write("Removing LD file name: /"+ swap_file+"_LD.o.\n");
      rm( "/"+swap_file+"_LD.o" );
#endif
      temp_file = 0;
      return;
...
more_file( string str )
{
  ...
  /**** Gnar changes for LD */
#if 1
  ob = clone_object(DAEMDIR+"/more_ob");
  ob->more(fname);
  return 1;
#else
  /*** Old code for LP ***/
