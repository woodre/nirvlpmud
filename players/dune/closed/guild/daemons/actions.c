...
void reset(int arg) {
    ...
    i = sizeof(f = get_dir(LIB_GENCMDS + "*.c"));
    genCmds = ([ ]);
    while (i--) {
	sscanf(f[i], "%s.c", cmd);
    ...
    i = sizeof(f = get_dir(LIB_ADVCMDS + "*.c"));
    advCmds = ([ ]);
    while (i--) {
	sscanf(f[i], "%s.c", cmd);
    ...
    i = sizeof(f = get_dir(LIB_ADMCMDS + "*.c"));
    admCmds = ([ ]);
    while (i--) {
	sscanf(f[i], "%s.c", cmd);
    ...
    i = sizeof(f = get_dir(LIB_TOOLS + "*.c"));
    tools = ([ ]);
    while (i--) {
	sscanf(f[i], "%s.c", cmd);
    ...
    /* emotes */
    i = sizeof(f = get_dir(LIB_EMOTES + "*.c"));
    emotes = ([ ]);
    while (i--) {
	sscanf(f[i], "%s.c", cmd);
