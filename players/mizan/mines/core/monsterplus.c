...
#include "mbv3/definitions.h"
#include "mbv3/router.h"
 ...
 */
    ...
    if(arg && arg > 60)
    {
        ...
        return ::do_damage(({60,dam2}),({type1,type2}));
    }
    else 
    {
...
/* i think its fair that each monster is worth more exp dependent on their guild level.
 * The higher the levels get the more significantly badass they are compared to normal monsters. */
calculate_worth()
{
    int temp;
    int result;

    /* figure our EXP multiplier */
    result = my_composite * (my_monster_guild_level + 1);

    /* now add the exp from what is left free in our heal quota */
    temp =  (query_int(VAL_MAX_HEAL_QUOTA) - query_count(COUNT_HEAL_QUOTA)) * (my_monster_guild_level * 10);
    if(temp < 0) temp = 0;
