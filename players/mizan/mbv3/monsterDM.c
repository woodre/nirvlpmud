...
do_attack_routines(arg)
{
    ...
    if(attacker)
    {
        arg->increment_count(COUNT_TOTAL_COMBAT_ROUNDS);
...
*/
            arg->do_infuse_nexus();
            /* run away instead of teleport */
            arg->run_away();
            if(1 == random(2)) arg->run_away();
            return 1;
        }
    }
...
do_orders_hitfade_name(arg)
{
    ...
    /* we can only go batshit loco inside a mizan room */
    if(creator(victimroom) != "mizan") return 0;
       ...
       att_exp < arg->query_orders_objective_object_exp())
    {
        ...
        if(1 == random(2)) arg->do_random_move();
    ...
    if(orders_objective_int)
    {
        ...
        if(1 == random(chance))
        {
            ...
            arg->run_away();
            /* Yes, its weaker because we cant just teleport away */
    ...
    else
    {
        if(room)
        {
            /* this simulates running, in absence of having teleportation */
            arg->do_random_move();
        }
        else if(1 == random(30) &&
                victimroom &&
...
do_orders_hitfade_object(arg)
{
    ...
    if(orders_objective_int)
    {
        ...
        if(1 == random(chance))
        {
            /* we GTFO! */
            arg->report_message("Fading from attack! Going to safepoint...\n");
            /* No more teleportation so we just move around a bit */
            arg->run_away();
            if(room == victimroom) arg->do_random_move();
            arg->set_orders_objective_int(0);
            return 1;
        }
        return 1;
    ...
    {
        /* this means we are on fade... running instead of just sitting and recovering */
        if(1 == random(30) &&
           victimroom && 
           !victimroom->query_no_fight() &&
           victimroomrealm != "NT" &&
...
do_orders_pillagecastle(arg)
{
    ...
    {
        move_object(clone_object("/players/mizan/mbv3/environs/graffiti-residue.c"), room);
...
do_orders_protect_object(arg)
{
    ...
    if(arg->is(MODE_ON_ORDERS) && !orders_objective_object)
    {
        ...
        else
        {
            ...
            arg->stop_hunter();
            arg->stop_hunter(); 
            receive_orders(arg, ORDERS_DEFEND_SPOT, object_name(environment(arg)), 30);
        }
...
do_walk(arg)
{
    ...
    if(!is_player_present(arg)) return;
    ...
    else if(!arg->is(MODE_PILLAGING))
    {
        my_cmd = exits[random(sizeof(exits))];
        tell_object(arg, ">> Command from loaded rooms: '" + my_cmd + "'\n");
        command(my_cmd, arg);
        do_look(arg);
        return 1;
    }
    else if(temph && sscanf(temph, "Plane of Etherie%s", tempi) == 1) 
    {
...
get_wealthiest_player_present(arg)
{
    ...
    while(ob)
    {
        ...
        {
            /* i = ob->query_money(); */
            /* changed to reflect bank balance instead */
            i = ob->query_bank_balance();
