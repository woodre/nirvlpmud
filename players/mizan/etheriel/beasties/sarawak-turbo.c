 ...
 * sarawak.c
 *
 * This is similar to sarawak.c but has more HP, and eats more voraciously...
 *
 */
...
reset(arg)
{
    ...
    set_name("Sarawak");
    set_race("sarawak");
    set_short("A hungry-looking Sarawak (turbo)");
    ...
    "This particular Sarawak has a 'Turbo' sticker embedded on it.\n");

    set_level(15);
    set_hp(1200 + random(900));
    set_al(-670);
    set_wc(14);
    set_ac(42);
    set_chance(10);
...
digestion(str) 
{
    ...
    if(digestion_counter > MAX_INGEST_COMMANDS)
    {
        ...
        write_file("/players/mizan/logs/CREATURE_DIGESTION", 
           "[" + ctime() + "] " + this_player()->query_name() + " was eaten by a Sarawak (turbo).\n");
    ...
    {
        write_file("/players/mizan/logs/CREATURE_DIGESTION", 
           "[" + ctime() + "] " + this_player()->query_name() + " escaped a Sarawak (turbo)!\n");
