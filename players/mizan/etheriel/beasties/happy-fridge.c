...
"and a big neon one that reads: Type 'list' for a list of stuff!\n");
   ...
   load_chat("The fridge does the disco duck.\n");
   load_chat("The fridge says: Buy a Pee ball!\n");
   load_chat("The fridge says: Pee balls only cost 200 coins.\n");
   load_chat("The fridge says: Pee balls are fun, you should buy one.\n");
   load_chat("The fridge whistles happily.\n");
...
list_items() {
  cat("/players/mizan/etheriel/environs/text/SPOOGLE_ITEM_LIST");
  return 1;
}
...
buy_item(arg) {
  ...
  if(sscanf(arg, "%d", item_no) != 1) {
    notify_fail("The fridge goes: Huh?\n");
    return 0;
  }
