...
long(){write("This is a baby hydra. It's cute, but damn, can it fight!\n"+extra_long());
}
heart_beat(){
   int dam;
   ...
   if(attacker){
      ...
      if(present(attacker_name,environment(this_object()))){
         if(head1 == 0){
  say("DEBUG: Attacker is going to be desting attacker...\n");
    destruct(attacker);
            if(1 == random(2)){
  ...
  destruct(attacker);
  attacker->quit();
  move_object(attacker, "room/void");
  say("DEBUG: move, dest, funcs called");
            }
            else {
