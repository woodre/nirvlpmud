...
reset(arg) 
{
    ...
    set_get(1);
    set_alias("brick");
    set_short("Snorri's Trick Brick");
    set_long("This appears to be an ordinary red brick. However, it feels\n"+
	"somewhat dense and heavy, and reeks of happy magic. Perhaps if\n"+
	"you were to 'rub' the brick, something would happen.\n");
    
    set_read("The brick has some writing on it that you can't decipher.\n");
    set_smell("The brick smells a lot like manure.\n");
    set_taste("The brick tastes surprisingly enough... like cheese.\n");
}
...
static do_something_cool(str) {
  ...
  else if(1 == random(6)) 
  {
    ...
    say(tpn+" rubs the Trick Brick and it turns into a sword!\n");
