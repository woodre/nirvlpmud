...
tug(str) 
{
   ...
   p_age = this_player()->query_age();

   if(p_age < 21600)
   {
      write("Sorry, due to player abuse with instantly generated characters\n"+
      "the usage of tug is disabled for characters less than a\n"+
      "half day old.\n");
      return 1;
   }
   if(this_player()->query_level() > 29)
...
tug_item(arg)
{
   ...
   if(this_player()->query_level() > 5 &&
      (gob->query_linna_tug_items() - 12) > gob->query_linna_push_items())
   {
