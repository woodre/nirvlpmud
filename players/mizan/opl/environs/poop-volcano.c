...
disable_volcanogun()
{
    ...
    while(ob)
    {
        ...
        {
            call_other(ob, "attack_object");
            call_other(ob, "stop_fight");
...
/*        poop->set_size(random(50) + 5); */
        /* poop sometimes explodes when moved into the room */
        if(poop) call_other(poop, "set_size", random(50) + 5);
        if(poop) call_other(poop, "on_explode", attacker_ob);
    }
