...
reset(arg)
{
    ...
    set_name("Gundroid Mk. IV");
    set_race("gundroid");
    set_alias("droid");
    set_short("A Gundroid, Mk. IV");
    set_long(tmp_long);
    set_level(29);
    set_hp(9900);
    set_al(-600);
    ...
    if(1 == random(4))
    {
        /* aggressive monster bonus, plus multi head attack */
        ...
        if (environment(attacker) == room)
        {
            ...
            if (head1 == 0 && environment(attacker) == room && !attacker->query_ghost())
            {
                ...
                if (1 == random(HIT_PROB))
                {
                    ...
                    tell_room(room, "The " + HIY + "12.5mm snapgun" + NORM + " turret " + GUNDROID_DM->get_gundroid_st_attack_message() + " at " + capitalize(attacker_name) + ".\n");
                    attacker->hit_player(dam, "fire");
                }
                else {
                    tell_room(room,"The " + HIY + "12.5mm snapgun" + NORM + " turret missed "+capitalize(attacker_name)+".\n");
                }

                if (hps < (HEAD_HP * 2)) {
                    head1 = 1;
                    tell_room(room,"The " + HIY + "12.5mm snapgun" + NORM + " turret is destroyed!\n");
            ...
            if (head2 == 0 && t_head2 > 3 && environment(attacker) == room && !attacker->query_ghost()) 
            {
                ...
                if (1 == random(HIT_PROB))
                {
                    ...
                    tell_room(room, HIY + "\nBLAM! BLAM!\n" + NORM);
                    tell_room(room, "The " + HIB + "20mm flexgun" + NORM + " turret " + GUNDROID_DM->get_gundroid_mt_attack_message() + " at " +  capitalize(attacker_name) + ".\n");
                    attacker->hit_player(dam, "fire");
                }
                else {
                    tell_room(room,"The " + HIB + "20mm flexgun" + NORM + " turret missed "+capitalize(attacker_name)+".\n");
                }
                if (hps < (HEAD_HP)) {
                    head2 = 1;
                    tell_room(room,"The " + HIB + "20mm flexgun" + NORM + " turret is destroyed!\n");
                    attacker->add_exp(50000);
                }
            ...
            else if (t_head2 == 2)
            {
                ...
                tell_room(room, "You see the " + HIB + "20mm flexgun" + NORM + " turret rotate and lock on to your direction...\n");
                t_head2 ++;
            }
            else t_head2 ++;
            ...
            if (head3 == 0 && t_head3 > 6 && environment(attacker) == room && !attacker->query_ghost())
            {
                ...
                dam = random(50) + 60;
                tell_room(room, "The " + HIR + "60mm scattergun" + NORM + " turret " + GUNDROID_DM->get_gundroid_lt_attack_message() + " in your general direction.\n");
                ...
                if (hps < 50) {
                    head3 = 1;
                    tell_room(room,"The " + HIR + "60mm scattergun" + NORM + " turret is destroyed!\n");
                    attacker->add_exp(50000);
                }
            ...
            {
                /* tell the room of impending doom */
                tell_room(room, "You see the " + HIR + "60mm scattergun" + NORM + " turret train and lock in your general direction...\n");
                t_head3 ++;
            }
            else t_head3 ++;
...
extra_look()
{
    ...
    return "The " + HIY + "12.5mm snapgun" + NORM + " turret is " + desc1 + "\n" +
           "The " + HIB + "20mm flexgun" + NORM + " turret is " + desc2 + "\n" +
           "The " + HIR + "60mm scattergun" + NORM + " turret is " + desc3 + "\n";
}
