...
reset(arg)
{
    ...
    my_name = GOBLIN_DM->get_knobgoblin_name();

    set_name(my_name);
    set_race("knobgoblin");
    set_alias(lower_case(my_name));
    set_short(my_name + " the " + HIG + "knobgoblin" + NORM);
    ...
    "  It seems to be a result of being treated like the red-headed stepchildren\n"+
    "  and actually managing the tough feat of being uglier than other goblins!\n");

    set_level(STATS_DM->get_level_easy());

    /* we use raw numbers because i overrode set_hp() */
    max_hp = STATS_DM->get_hp_weak() + 500;
    hit_point = max_hp;

    set_hp_bonus(2500);
    if(1 == random(2)) set_aggressive(1);
    ...
    if(1 == random(5))
    {
        heal = clone_object("/players/mizan/opl/heals/instant_hangover.c");
