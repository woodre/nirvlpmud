...
#define NEWSFILE "players/mizan/opl/data/tripod_bulletin"
...
short() {
    return "A server named (tripod)";
}

id(str) {
    return str == "tripod" || str == "server";
}
...
news_delete(str,arg)
{
    ...
    accesslevel = card->query_access_level();

    if(accesslevel < 19)
    {
        write(">> You do not have the security level to perform this operation.\n");
        return 1;
    }
