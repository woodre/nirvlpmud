...
reset(arg)
{
    ...
    set_limited_issue_count(3);
    
    set_name("remote");
    set_alias("fob");
    set_short("A brown plastic remote key fob");
    set_long("This is a brown plastic key fob, with a single large button on it.\n"+
    ...
    set_read("The label reads: 'Dactrol Corporation Volcanogun EPO - All Rights Reserved 2144'.\n");
...
activate(string str)
{
    ...
    while(ob)
    {
        ...
        if(sscanf(filename, "players/mizan/opl/environs/poop-volcano%s", temp1) == 1 &&
           living(ob)) call_other(ob, "disable_volcanogun");
        ob = oc;
    }
    return 1;
}
