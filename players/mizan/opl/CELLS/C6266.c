...
reset(int arg)
{
	...
	dest_dir =
	({
		"/players/mizan/opl/CELLS/C6265", "west",
	});

	/* Our monster list */
	if(!mob0)
	{
		mob0 = clone_object("/players/mizan/opl/beasties/easy/gummybear.c");
		move_object(mob0, this_object());
	}

	if(!mob1)
	{
		mob1 = clone_object("/players/mizan/opl/beasties/easy/neocon.c");
		move_object(mob1, this_object());
	}

	if(!mob2)
	{
		mob2 = clone_object("/players/mizan/opl/beasties/easy/zombie.c");
	...
	if (!door1)
	{
		...
		door1->set_dir("down");
		door1->set_door_room("/players/mizan/opl/CELLS/C6266");
		door1->set_type("basic-security");
		...
		door2->set_dir("up");
		door2->set_door_room("/players/mizan/opl/CELLS/I6266");
		door2->set_type("basic-security");
		...
		move_object(door2, "/players/mizan/opl/CELLS/I6266");
...
long()
{
	...
	write("\"You are a tower of strength in the office, but only so-so in bed.\"\n");
