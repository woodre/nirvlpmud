...
reset(int arg)
{
	...
	dest_dir =
	({
		"/players/mizan/opl/CELLS/C3986", "east",
	});

	/* Our monster list */
	if(!mob0)
	{
		mob0 = clone_object("/players/mizan/opl/beasties/easy/guido.c");
		move_object(mob0, this_object());
	}

	if(!mob1)
	{
		mob1 = clone_object("/players/mizan/opl/beasties/easy/gold.c");
	...
	if (!door1)
	{
		...
		door1->set_dir("down");
		door1->set_door_room("/players/mizan/opl/CELLS/C3985");
		...
		door2->set_dir("up");
		door2->set_door_room("/players/mizan/opl/CELLS/I3985");
		...
		move_object(door2, "/players/mizan/opl/CELLS/I3985");
...
long()
{
	...
	write("\"NANCY!! Why is everything RED?!\"\n");
