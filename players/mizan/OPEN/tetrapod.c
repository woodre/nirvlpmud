...
extra_look() {
  if(ENV) return (ENV->query_name()) + " has four arms and looks really damn fearsome!"+
    "\n"+
    ...
    if(attacker) {
      while(i < sizeof(weapon)) {
        if(weapon[i] && environment(weapon[i]) == this_object()) {
          ...
          if(weapon[i]->query_wear() > 1 && random(4000000/weapon[i]->query_wear()) < 10)
...
/* berserk attack- looks painful but actually can cancel other weaps */
    if(1 == random(23)) {
        ...
        break;
          ...
          if(total_damage > 50) MASTER_R->extra_hit(5, i, attacker, ENV);
             else {
      if(1 == random(6)) {
        ...
        } else MASTER_R->extra_hit(3, i, attacker, ENV);
      i++;
      }
    }
  }
