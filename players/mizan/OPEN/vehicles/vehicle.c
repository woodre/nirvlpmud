...
/* our variables */
status is_lights_on_inside;     /* whether our interior lights are on - NOT READY */
...
#else
		add_action("on_single_command", "", 3");
#endif
        add_action("on_look", "look_outside");
...
on_console()
{
    ...
    if(fuel)
        write("The fuel gauge shows [" + fuel + "] units left.\n");
    else
...
is_driver(arg)
{
    ...
    if(ob_driver && ob == ob_driver) return 1;
    else return 0;
}
...
is_passenger(arg)
{
    ...
    if(ob_passenger && ob == ob_passenger) return 1;
    else return 0;
}
...
query_vehicle_condition()
{
    if(hit_point < max_hp/10) 
      return (cap_name+" is in very bad condition.\n");
    else if(hit_point < max_hp/5)
      return (cap_name+" is in bad condition.\n");
    else if(hit_point < max_hp/2)
      return (cap_name+" is somewhat damaged.\n");
    else if(hit_point < max_hp - 20) 
      return (cap_name+" is slightly damaged.\n");
    else return (cap_name+" is like new.\n");
}
