...
#define HOME "/players/mizan/core/02h/room01"
#define SAVEFILE "players/mizan/core/test/objectDM-SAVEFILE"
...
void long()
{
    if(this_player()->query_level() < 20)
    {
        write("There appears to be writing on a sticker on the mainframe, but\n"+
              "you have no idea how yo translate it\n");
    }
    else
    {
        ...
        write("  register <plyr>    Observe the player named <plyr>.\n\n");
        write("  unregister <plyr>  Stop observing the player named <plyr>.\n\n");
...
void report_creation(object ob)
{
    ...
    if(sizeof(current_objects) == 1000)
    {
...
int on_printname(string arg)
{
    ...
    i = 1;
    /*
    i = this_object()->match_by_objectname(arg);
     */
