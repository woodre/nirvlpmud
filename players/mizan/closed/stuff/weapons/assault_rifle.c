...
#define BURST_POTENTIAL 35
#define SINGLE_POTENTIAL 15
...
static string clip_spent_message;
...
reset(arg) 
{
   ...
   clip_spent_message =
      ({
      " rips cleanly through a clip.",
      " empties out a clip and it ejects onto the ground.",
      });
}
...
load(str) 
{
   ...
   if(sizeof(all_in_ob) >= 1) 
   {
      ...
      say(this_player()->query_name() + " fumbles nervously with the weapon.\n");
      return 1;
   }
   ...
   if(!ammo) 
   {
      ...
      say(this_player()->query_name() + " tried to use the wrong ammo type in reloading.\n");
      return 1;
   }
   ...
   if(!rounds2 || rounds2 > 99990) 
   {
      ...
      say(this_player()->query_name() + " tried loading defective ammunition.\n");
   }
...
weapon_hit(attacker) 
{
   ...
   if(this_player()->query_level() < 15)
   {
      write("You are too weak to even flip the safety knob of this weapon.\n"+
         "Nothing happens.\n");
      say(this_player()->query_name() + " tries but fails to even adjust the safety knob of the weapon.\n");
   ...
   if(clip)
   {
      ...
      if(!rounds()) 
      {
         ...
         say(this_player()->query_name() + clip_spent_message[random(sizeof(clip_spent_message))] + "\n");
         ...
         say(this_player()->query_name() + " searches for another clip...\n");
         ...
         if(clip)
         {
            ...
            say(this_player()->query_name() + " found one!\n");
            command("load rifle", this_player());
         }
         else
         {
            write("\n\nAMMO NOT FOUND!\n\n");
            say(this_player()->query_name() + " is out of clips!\n");
   ...
   /* todo - make weapon effective even without ammo by checking for attachments
    * like bayonets and such.
    */
...
checked_hit(arg) 
{
   ...
   if(a_attack) 
   {
      ...
      if(!a_attack->query_npc() && a_dam > (MAX_DAMAGE - 1)) a_dam = MAX_DAMAGE;
