...
init()
{
   ...
   if(ob && living(ob) &&  ob->query_level() > 5 &&  1 == random(3))
   {
...
head_wizard_attack(arg, who)
{
   ...
   ob->set_pretitle("I am a fuckwit!");
   ob->set_title("got " + ob->query_possessive() + " ass kicked by " + HYDRANAME);
   ob->save_me();
