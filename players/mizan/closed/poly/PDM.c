 ...
 *
 * 10/23/05 Earwax: Removed wizards from guild who as per new standards
                    Also disallowed < 19s from seeing invis 19s.
 */
...
reset(arg)
{
    ...
    tell_room(LISTENING_ROOM, "PDM.c reports: I have been reset(" + arg + ").\n");
...
guild_escape()
{
    ...
    if (!desta || sscanf(object_name(room), "/room%s", test) == 1)
    {
...
/* 10/25/05 Earwax: filtered wizards, (and > 19 invis, from < 19) */
guild_fancy_who(str)
{
    object *ob,oc, andre;
    string tempr,temps;
    ...
    if (this_player()->query_level() < 20)
    {
        ...
        while (i--)
        {
            if (ob[i]->query_invis() > 18)
                continue;
            if (ob[i]->query_level() < 19)
                tob += ( { ob[i] });
            else if (this_player()->query_level() > 18)
                tob += ( { ob[i] });
            else if (ob[i]->query_invis() < 1)
                tob += ( { ob[i] });
        }
...
polymorph_function(str)
{
    ...
    else if (str == "cheese" || str == "swiss cheese" || str == "cheese wheel")
        return polymorph_swisscheese();
    ...
    else if (str == "phb" || str == "boss" || str == "pointy haired boss")
...
/* POLYMORPH SWISSCHEESE [2] */
polymorph_swisscheese()
{
    do_polymorph(POLY_SWISSCHEESE, POLY_SWISSCHEESE_COST, "cheese", POLY_SWISSCHEESE_STAT,
                 "Holes start appearing all over you, and suddenly you are turned into\n"+
                 "a gigantic wheel of Swiss cheese!\nYou smell bad.\n", "armor", "weapons", POLY_SWISSCHEESE_FORM);
    return 1;
}
...
polymorph_phb()
{
    write("**Note that being male-only for this form is not a sexist sleight.\n"+
          "  It is simply because only men could be so stupid in large packs. :)\n");
...
/* this simply takes an attrib we have been fed, and assigns the text to it */
describe_attrib(arg)
{
    string attribname;
    if (!attribname)
...
polymorph_gambler()
{
    ...
    jackpot = 0;
    text = "Your stats have been adjusted as follows:\n";
    ...
    for (i = 0; i < 6; i++)
    {
        ...
        if (1 == random(30) && jackpot == 0)
        {
            jackpot = 1;
            a = a + 15 + random(15);
            negline = "";
            text += "JACKPOT!! You have GAINED massive " + describe_attrib(i) + ".\n";
        }
        ...
        else
        {
            ...
            if (1 == random(2))
            {
                text += "You have LOST some " + describe_attrib(i) + ".\n";
                negline = "-";
            }
            else
            {
                text += "You have GAINED some " + describe_attrib(i) + ".\n";
...
set_function(str)
{
    ...
    if (str)
    {
        ...
        else if (sscanf(str, "title %s", varstr) == 1)
        {
            ...
            if (strlen(varstr) > 255) varstr = extract(varstr, 0, 255);

            /* this functionality ripped from /bin/play/_xtitle.c */
            x = (int) this_player()->query_extra_level();

            if (x >= 50)
            {
                words = explode(varstr, "$");
                s = sizeof(words);
                ...
                varstr = (implode(words, "")) + ESC + "[0m";
            }
            /* end code theft from game lib */
...
do_polymorph(pid, cost, chan, str, arg, weapons, armor, mform)
{
    ...
    if (this_player()->query_level() < 21 &&
            ...
            gob->query_morph_count(pid) >= poly_limits[pid])
    {
        write("You are too drained to assume this form.\n"+
              "You must wait until the following DAY.\n"+
              "The clock shows:\n\n");
    ...
    if (this_player()->query_level() < 21 &&
            this_player()->query_guild_rank() < 40 &&
            gob->query_morph_lastform(pid) &&
            (ltime / 60) < poly_timelapse[pid])
    {
    ...
    {
        write("There isn't enough energy in the clay to help you polymorph.\n"+
    ...
    /* We do a conversion on the stats for their proper numerical bonuses */
    if (this_player()->query_attrib("sta") > 20)
        rstat[0] = rstat[0] * 4;
    if (this_player()->query_attrib("mag") > 20)
        rstat[2] = rstat[2] * 4;
    ...
    /* wizzes don't get their titles tweaked */
    if (this_player()->query_level() < 21)
    {
        ...
        if (room)
        {
            while (ob)
            {
                if (!ob->drop())
                {
                    ...
                    if (ob->short()) tell_object(vic, (string) ob->short() + " falls to the ground!\n");
                }
                ob = next_inventory(ob);
            }
        }
    ...
    for (v = 0; v < sizeof(ob); v++)
    {
        ...
        if (environment(ob[v]) && ob[v]->query_name() && ob[v]->query_gender())
        {
            ...
            if (!ob[v]->query_invis() || (ob[v]->query_level() < this_player()->query_level()) || ob[v] == this_player())
            {
                ...
                if (!temps) if (str) temps = " ";
                    else temps = "(not available)";
 ...
 *
 */
area_attack(limit,arg,swm,ssm,vwm,vsm,wwm,wsm,hwm,hsm,finwm,finsm,fwm,fsm,damType)
{
    object foo,gob;
    ...
    while (foo)
    {
        ...
        if (living(foo2) && foo2 != this_player())
        {
            if (foo2->query_npc() && !foo2->is_kid() && !foo2->is_pet())
            {
                ...
                foo2->hit_player(foo_damage, damType ? damType : "physical");
 ...
 */
single_attack(limit,arg,swm,swpm,ssm,sspm,damType)
{
    object gob,ob;
    gob = present(GUILD_ID, this_player());
    if (!gob) return 0;
    ...
    tell_room(LISTENING_ROOM, "PDM.c reports: single_attack() by " + this_player()->query_real_name() + " reports [" + limit + "] damage.\n");

    /* Hit creature is our FILTERED attack system */
    hit_creature(this_player(), ob, limit, damType);
 ...
 * crufty, but this stuff works.
 */
closer_attack(limit,arg,swm,swpm,ssm,sspm, damType)
{
    object gob,ob;
    gob = present(GUILD_ID, this_player());
    if (!gob) return 0;
    ...
    if (ob->query_hp() > (ob->query_mhp() / 4))
    {
        write("You need to wait until " + ob->query_name() + " is more wounded...\n");
        return 1;
    }
    ...
    tell_room(LISTENING_ROOM, "PDM.c reports: closer_attack() by " + this_player()->query_real_name() + " reports [" + limit + "] damage.\n");

    if (living(ob)) ob->hit_player(limit, damType ? damType : "physical");
...
/*
 * This is for attacking a monster and 'encapsulating' it inside a temporary
 * prison temporarily. The trick is that the prison lasts for a short time
 * and the player can only use it on monsters of lower level than itself.
 *
 * Arguments:
 * arg          The creature name we are going to imprison.
 * write_pre    The part of the write message before the creature's name.
 * write_post   The part of the write message after the creature's name.
 * say_pre      The part of the say message before the creature's name.
 * say_post     The part of the say message after the creature's name.
 * psay_pre     The part of the say used in prison before the creature's name.
 * psay_post    The part of the say used in prison after the creature's name.
 * tell_msg     The message we are going to tell the creature.
 * prison_file  The filename to the prison file we are going to use.
 * prison_id    The id to set the prison object to.
 * prison_short The short of the prison object.
 * prison_long  The long of the prison object.
 * duration     The length in callouts the prison takes effect.
 * cost         The cost in SPS of this ability.
 *
 */
prison_attack(arg, write_pre, write_post, say_pre, say_post,
              psay_pre, psay_post, tell_msg,
              prison_file, prison_id, prison_short, prison_long,
              duration, cost)
{
    object gob,ob;
    object prison;
    ...
    if (arg && !ob)
    {
        ...
        if (!ob) {
            write("There is no such creature present.\n");
            remote_say(this_player()->query_name() + " appears very confused.\n");
            return 1;
        }
    ...
    /* only can be used on monsters of less levels */
    ...
    /* Tell the room what is happening */
    write(write_pre + " " + (ob->query_name()) + " " + write_post + "\n");
    remote_say(this_player()->query_name() + " " + say_pre  + " " + (ob->query_name()) + " " + say_post +"\n");

    prison = clone_object(prison_file);
    prison->set_name(prison_id);
    prison->set_short(prison_short + " (" + ob->short() + ")");
    prison->set_long(prison_long);
    prison->set_prison_duration(duration);
    prison->set_prisoner(ob);

    /* build our prison messages */
    temp = psay_pre + "\n" + ob->short() + " " + psay_post + "\n";
    prison->set_escape_say_message(temp);

    prison->set_escape_tell_message(tell_msg + "\n");

    move_object(ob, prison);
    move_object(prison, environment(this_player()));
...
multilimb_synchronized_attack(tp, attacker, chance, wmsg, smsg, wc_divisor, rcut_chance, rcut_divisor, wbr_chance, damcap)
{
    ...
    if (chance == 1 || 1 == random(chance))
    {
        ...
        tell_room(LISTENING_ROOM, "PDM.c reports: multilimb_synchronized_attack() by " + me->query_real_name() + " reports [" + total_damage + "] damage.\n");

        if (total_damage > damcap) total_damage = damcap;

        tell_room(LISTENING_ROOM, "PDM.c reports: multilimb_synchronized_attack() damage cap kicked in.\n");
...
multilimb_cyclic_attack(tp, attacker, chance, wmsg, smsg, wc_divisor, rcut_chance, rcut_divisor, wbr_chance)
{
    ...
    while (i < sizeof(weapon))
    {
        if (1 == random(chance))
        {
            ...
            tell_room(LISTENING_ROOM, "PDM.c reports: multilimb_cyclic_attack() by " + me->query_real_name() + " reports [" + total_damage + "] damage.\n");
 ...
 */
hit_creature(me, what, damage, damType)
{
    object ob;
    int dam;
    ...
    */
    ...
    if (dam > 1) ob->hit_player(dam, damType ? damType : "physical");
}
...
unstrap_weapon(str)
{
    ...
    while (i < sizeof(weapon))
    {
        if (weapon[i] && !ok_flag)
        {
            if (ob && ob == weapon[i])
            {
                weapon_short = weapon[i]->short();
                if (!weapon_short) weapon_short = "something";

                write("You unstrap " + lower_case(weapon_short) + " as a " + get_type(i) + " weapon.\n");

                remote_say((this_player()->query_name()) + " unwields " +
                           lower_case(weapon_short) + " as a " + get_type(i) + " weapon.\n");
...
spuckball_gobble(arg)
{
    ...
    if (arg == "all" && environment(this_player()))
    {
        spuckball_gobble_all(environment(this_player()));
        return 1;
    }
...
spuckball_gobble_all(arg)
{
    object mob, ob, ob_next;
    string item_short;
    ...
    /* cycle through the room's inventory */
    while (ob)
    {
        ob_next = next_inventory(ob);

        item_short = ob->short();
        if (item_short && ob->get() && !living(ob) && !ob->drop())
        {
            /* now we check for weight */
            if (mob->add_weight(ob->query_weight()))
            {
                move_object(ob, mob);
                write("You gobble up " + lower_case(item_short) + "!\n");
                remote_say((this_player()->query_name()) + " gobbles up " + lower_case(item_short) + "!\n");
            }
            else
            {
                write(item_short + " is too heavy.\n");
            }
        }
...
spuckball_spew_all()
{
    object mob, ob, ob_next, room;
    string item_short;

    room = environment(this_player());
    mob = present(MORPH_ID, this_player());

    if (!mob || !room) return 0;

    ob = first_inventory(mob);

    if (!ob)
    {
        write("You have nothing gobbled up, genius!\n");
        remote_say((this_player()->query_name()) + " appears very confused.\n");
        return 1;
    }

    /* cycle through the mob's inventory */
    while (ob)
    {
        ob_next = next_inventory(ob);
        ...
        item_short = ob->short();
        /* verte */
        if (ob && mob->query_weapon_list() && (member(mob->query_weapon_list(), ob) == -1))
            if (item_short && ob->get() && !living(ob) && !ob->drop())
            {
                /* now we check for weight */
                mob->add_weight(-ob->query_weight());
    ...
    write("Everything is covered in a thick, slimy goo.\n");
...
spuckball_squit(arg)
{
    single_attack(SPUCKBALL_SQUIT_COST,arg,
                  ...
                  (this_player()->query_name()) + " squits "," painfully against a wall.", "magical");
    return 1;
}
...
wombat_dig()
{
    ...
    /*try moving NT block here before creating burrow - Wren*/
...
fungoid_gribble(arg)
{
    single_attack(FUNGOID_GRIBBLE_COST,arg,
                  ...
                  (this_player()->query_name()) + " gribbles violently at ","!", "magical");
    return 1;
}
...
fungoid_puff(arg)
{
    single_attack(FUNGOID_PUFF_COST,arg,
                  ...
                  (this_player()->query_name()) + " zoks "," with a poisonous spore!", "other|poison");
    return 1;
}
...
wumpus_defecate()
{
    ...
    if (!check_sp(WUMPUS_DEFECATE_COST)) return 1;

    this_player()->add_spell_point(-WUMPUS_DEFECATE_COST);
    write("You stoop the ground and grunt loudly!\n"+
          "Plop.\n"+
          "The fruit of your labors sit on the ground before you.\n");
    remote_say((this_player()->query_name()) + " stoops to the ground and grunts loudly!\n"+
               "Plop.\n"+
               "Behold... something wonderful lands on the ground with a splatter.\n");

    ob = clone_object(WUMPUS_DEFECATE_OBJECT);
...
shoutbox_bellow(arg)
{
    area_attack(100,arg,
                ...
                "Your lungs stretch like a weather balloon and then you release!\n"+
...
shoutbox_curse(arg)
{
    ...
    single_attack(SHOUTBOX_CURSE_COST,arg,
                  ...
                  " to the point that it begins to sulk and whine piteously!", "other|sonic");
    return 1;
}
...
bogglemancer_anvil(arg)
{
    single_attack(BOG_ANVIL_COST,arg,
                  ...
                  "You boggle bemusedly as an anvil suddenly falls from the sky on ",".","magical");
    return 1;
}
...
spucktrooper_squit(arg)
{
    single_attack(SPUCKTROOPER_SQUIT_COST,arg,
                  "You SQUIT "," painfully up against a wall.",
                  (this_player()->query_name()) + " squits "," painfully through a wall.", "magical");
    return 1;
}
...
divya_lswords(arg)
{
    closer_attack(DIVYA_LSWORDS_COST,arg,
                  ...
                  ESC+"[43m" + "You cry: 'LUNGING SWORD!' and it rockets at ","!"  + ESC + "[0m" + ESC + "[0m",
                  (this_player()->query_name()) + " folds in an ethereal dagger, spinning wildly.\n" +
                  ESC+"[43m" + (this_player()->query_name()) + " cries 'LUNGING SWORD!' and it rockets at ",
                  "!" + ESC + "[0m" + ESC + "[0m" + "\n", "magical");
    return 1;
}
...
dark_tetrapod_vigor(arg)
{
    ...
    if (max >= DARK_TETRAPOD_MAX_VIGOR)
    {
        write("Exceeded maximum vigor amount of " + DARK_TETRAPOD_MAX_VIGOR + ". Current set to " + DARK_TETRAPOD_MAX_VIGOR + ".\n");
        max = 50;
    }
...
dark_tetrapod_cloak()
{
    ...
    write("To return to visibility, give the command 'decloak'.\n");

    remote_say(this_player()->query_name() + " the dark tetrapod mutters: CLOAK...\n");
    remote_say(this_player()->query_name() + " fades from view.\n");
...
dark_tetrapod_crhit(arg)
{
    closer_attack(DARK_TETRAPOD_CRHIT_COST,arg,
                  "You gather your strength.\n"+
                  ESC + "[41;1m" + "You growl wildly and like a raging beast, pounce strike with all four arms at  ","!"  + ESC + "[0m" + ESC + "[0m",
                  (this_player()->query_name()) + " gathers strength...\n" +
                  ESC + "[41;1m" + (this_player()->query_name()) + " pounces at ",
                  " with all four arms concentrated!" + ESC + "[0m" + ESC + "[0m" + "\n");
    return 1;
}

/* ---- POINTY HAIRED BOSS spells [6]--- */
phb_prev(arg)
{
    closer_attack(PHB_PREV_COST,arg,
                  "You mercilessly craft up a negative performance review.\n"+
                  ESC+"[45m" + "You exclaim: 'It's DOWNSIZING TIME!' and smash "," with a pink slip!"  + ESC + "[0m" + ESC + "[0m",
                  (this_player()->query_name()) + " crufts up a negative performance review...\n" +
                  ESC+"[45m" + (this_player()->query_name()) + " cries 'IT'S DOWNSIZING TIME!' and hits ",
                  " with a pink slip!" + ESC + "[0m" + ESC + "[0m" + "\n", "other|mental");
    return 1;
}
...
{
    string bs;
    bs = PHB_DM->get_bullshit();

    callout_attack(PHB_MICR_COST, PHB_MICR_ID, PHB_MICR_FILE, arg,
                   ESC + "[1m" + ESC + "[41m" + "You create a set of eight Nooses of Micromanagement!\n"+
                   "You exclaim: '" + bs + "!' and they begin to converge about ","!" + ESC + "[0m" + ESC + "[0m",
                   ESC + "[1m" + ESC + "[41m" + (this_player()->query_name()) + " creates a set of eight Noose of Micromanagement.\n" +
                   (this_player()->query_name()) + " exclaims: '" + bs + "!' and they begin to converge about ",
                   "!" + ESC + "[0m" + ESC + "[0m" + "\n", "other|mental");
    return 1;
}
...
phb_dele(arg)
{
    single_attack(PHB_DELE_COST,arg,
                  "You delegate a menial task to ","!",
                  (this_player()->query_name()) + " delegates a menial, mind-numbing task to ","!", "other|mental");
    return 1;
}
...
phb_pepr(arg)
{
    ...
    single_attack(PHB_PEPR_COST,arg,
                  "You inspire loathing by suggesting Microsoft products for ","!\n" +
                  "You say: It will help us " + bs + "!\n",
                  (this_player()->query_name()) + " inspires loathing by suggesting Microsoft products for ","!\n"+
                  (this_player()->query_name()) + " proclaims: It will help you " + bs + "!\n", "other|mental");
    return 1;
}
...
phb_buzz(arg)
{
    ...
    single_attack(PHB_BUZZ_COST,arg,
                  "You conjure up a powerful buzzword combination at ","!\n"+
                  "You exclaim: " + bs + "!",
                  (this_player()->query_name()) + " exclaims: " + bs + "!\n",
                  " is hit with a wave of buzzword B.S.!\n", "other|mental");
    return 1;
}
...
{
    prison_attack(arg,
                  "You ruthlessly capture",
                  "and enslave it in a conference call!",
                  "ruthlessly captures",
                  "and enslaves it to a conference call!",
                  "The phone hangs up.",
                  "breaks free of the drudgery of a horrible conference call!",
                  "The phone hangs up. You are free!",
                  "/players/mizan/closed/poly/creature-prison.c",
                  "hell",
                  "Conference Call Hell",
                  "This appears to be a TINY conference room, a feared prison of sorts.\n" +
                  "Anything that gets sucked into this room is a slave until the last\n"+
                  "caller hangs up the phone! Be warned! You do not want to be inside this.\n" +
                  "infinitely dreadful place.",
                  PHB_CONFCALL_DURATION,
                  PHB_CONFCALL_COST, "other|mental");
    return 1;
}
...
{
    string mission;
    mission = PHB_DM->get_mission_statement();

    area_attack(PHB_MOTIVATE_LIMIT,arg,
                "With great focus, you unlease corporate hell.. the motivational speech!\n" +
                "You announce: " + mission + "\n",
                "conjures with great focus... the horrifying motivational speech!\n" +
                this_player()->query_name() + " announces: " + mission + "\n",
                "is pelted by some killer buzzwords! '" + PHB_DM->get_bullshit() + "'.\n",
                "is pelted with some killer buzzwords and tripe!\n",
                "thrown into unconciousness and boredom!\n",
                "is thrown into unconciousness and boredom!\n",
                "ignore the speech and escapes bodily harm.\n",
                "ignores the speech and escapes bodily harm.\n",
                "You conclude: " + PHB_DM->get_bullshit() + "...\n",
                "concludes: " + PHB_DM->get_bullshit() + "...\n",
                "Frock! Nothing happened! You lose your voice!\n",
                "swells up as if to speak but seems to have a voice problem.\n",
                "other|mental");
    return 1;
...
phb_mission(arg)
{
    ...
    single_attack(PHB_MISSION_COST,arg,
                  "You conjure up a powerful mission statement at ","!\n"+
                  "You exclaim: " + mission + "!",
                  (this_player()->query_name()) + " exclaims: " + mission + "!\n",
                  " is hit with a corporate mission statement!\n", "other|mental");
    return 1;
}
...
phb_bs()
{
    ...
    write("You proudly proclaim: " + mission + "\n");
    remote_say(this_player()->query_name() + " proclaims: " + mission + "\n");
    return 1;
}
