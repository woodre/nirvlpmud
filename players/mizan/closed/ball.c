...
#define FILENAME "players/mizan/closed/ball"
...
get() {
  if(this_player()->query_real_name() == "arrina") return 1;
...
people() {
  ...
  for(i=0; i<sizeof(ob); i++) {
   ...
   if(environment(ob[i])) filename = object_name(environment(ob[i]));
...
ob->show_stats();
if(environment(ob)) write("Location: "+object_name(environment(ob))+"\n");
...
write(ob->short()+"("+object_name(ob)+") was created by "+who+".\n");
return 1; }
find_it(str) {
object ob,us,found;
int i;
...
write("Nuking "+str+" in: ");
if(environment(ob)) {
write(environment(ob)->short()+" ("+object_name(environment(ob))+")"); }
else write("(Nowhere)");
write("\n");
destruct(ob);
i += 1;
