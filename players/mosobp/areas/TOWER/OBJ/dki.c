/*                                       
 *  File:   /players/mosobp/areas/TOWER/OBJ/dki.c
 *  Function:  object                    
...
reset(arg) 
{
	set_name("ki");
	set_alias("shield");
	set_short(HIR+"An Energy Ki"+NORM);
	set_long(
	"This is the Energy Ki of an evil fighter. When the evil\n"+
	"fighter powers up to fight a battle. This ki forms around\n"+
	"them. This particular ki only forms around people of the \n"+
	"darkest intentions.\n");
...
do_special(owner)
{
  ...
  if((int)owner->query_attrib("dex") > random(100))
  {
    tell_object(owner,
  ...
  return;
   ...
   if(arg=="ki"){
      if(this_player()->query_alignment() > 100) {
         write("The Ki refuses to ready for good.\n");
         return 1;
      }
      if(::wear(arg));
