...
if(TO->query_name() == "Ueshiba"){
         ...
         if(present("mkquest",this_player())){
if(present("mkquest",this_player())->QQuestJourney(TO->query_name())){
...
present("mkquest",this_player())->AQuestJourney(TO->query_name());
            ThisKey = KEYS[i];
