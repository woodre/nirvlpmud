...
id(str) { return str=="window" || str=="ViewingWindow"; }
reset(arg) {
   if (arg)
      return;
restore_object("players/beck/DarkRangers/Save/Window");
}
short() {
   return "The Viewing Window";
}
long() {
   write("To see a list of guild members type 'Rangers'.\n");
   if(COMM){
      write(""+
         "There are currently "+member_number+" Dark Rangers.\n"+
         "The maximum number of guild members allowed is 13.\n"+
   ...
   write("This Window looks out over the Earth.\n"+
      "It has other magical capabilities too.\n"+
      "If you wish to become a Dark Ranger just type 'info' for information.\n"+
      "There are currently "+member_number+" Dark Rangers.\n"+
      "The maximum number of guild members allowed is 13.\n"+
      "If you wish to join the guild type 'join'.\n"+
      "You must be above levels 7 to join as a full member.\n"+
...
init() {
   ...
   if(COMM){
      if(member_array(CAPTPRNAME,ranger_purged) != -1){
         destruct(present("DarkRangerCommunicator",TP));
         ...
         TP->set_guild_name("none");
         rm("/players/beck/DarkRangers/Save/"+TP->query_real_name()+".o");
         remove_ranger_purged(CAPTPRNAME);
         return;
      }
...
join_honorary(){
   ...
   if(TP->query_guild_name() == "rangers" || TP->query_guild_name() == "knights templar"){
      write("Your guild will never be an Ally of the Rangers!\n");
      return 0;
   }
   if(!present("dark_honor_ranger_communicator"), this_player()){
      move_object(clone_object("/players/beck/DarkRangers/HonorRangerCommunicator.c"), this_player());
      return 1;
   }
   return 0;
}
...
if(member_number == 5 || member_number > 5){
   ...
   if(this_player()->query_level() < 7){
      write("You need to be at least level 7 to join.\n");
      return 1;
   }
   /*
   ...
   */
   communicator = clone_object("/players/beck/DarkRangers/RangerCommunicator.c");
...
communicator->ASkills("rsc");
   ...
   communicator->AUnlearnedSkills("servant");
   communicator->AUnlearnedSkills("shiho-nage");
   communicator->AUnlearnedSkills("irimi-nage");
   write("Lord Zed tells you: Welcome to the Dark Rangers!\n"+
      "I am pleased you have decided to become part of our cause.\n"+
      "For a period of time you will be an initiate\n"+
      "This will be a chance for you to prove that you are worthy\n"+
      "of the powers that the Dark Rangers hold.\n"+
      "The Dark Rangers are given their abilities in order to destroy good\n"+
      "and especially destroy the Power Rangers.  In time you learn many skills that will help\n"+
      "you to use the power you are given to the fullest.\n"+
      "\n"+"Lord Zed hands you a Communicator.\n"+
      "Lord Zed tells you:  This is the device that the Dark Rangers use to\n"+
      "communicator with others.  You must learn how to use\n"+
      "it well, for it has many beneficial functions.\n"+
      "Go now and join the other Dark Rangers, learn about your communicator,\n"+
      "and begin your martial arts training.  You must do all this as\n"+
      "well as begin to start fighting the forces of good in order\n"+
      "to become a true Dark Ranger.\n"+
   ...
   if(present("dark_honor_ranger_communicator",TP)){
      destruct(present("dark_honor_ranger_communicator",TP));
   }
   return 1;
}
restore_object(){
   ...
   if(this_player()->query_guild_name() != "dark rangers"){
      write("This is for Dark Rangers guild members only.\n");
      return 1;
   }
...
leave_power_rangers(){
if(TP->query_guild_name() != "dark rangers" || !present("RangerCommunicator",TP)){
      write("You are not a Dark Ranger\n");
      return 1;
   }
   write("Are you sure you want to leave the Dark Rangers?  Yes/No: ");
...
verify_leave_power_rangers(str){
   ...
   if(!str || (str != "yes" && str != "no")){
      write("You want to leave the Dark Rangers?  Yes/No: ");
      input_to("verify_leave_power_rangers");
      return 1;
   }
   if(str == "yes"){
...
rm("/players/beck/Rangers/Save/"+TPRNAME+".o");
      save_globe();
      return 1;
   }
   if(str == "no"){
...
save_globe(){
   save_object("players/beck/save/rangers/Window");
   save_object("players/beck/DarkRangers/Save/Window");
   return 1;
}
...
update_ranger_roster(){
   int i;
   if(member_array(CAPTPRNAME, ranger_names) == -1){
      if(TPNAME == "some mist"){ return; }
      ranger_names += ({ CAPTPRNAME });
   ...
   for(i=0; i<sizeof(ranger_names); i++){
      if(ranger_names[i] == CAPTPRNAME){
         if(TP->GLEVEL < 80) ranger_types[i] = "Dimensional";
         if(TP->GLEVEL < 70) ranger_types[i] = "Stellar";
         if(TP->GLEVEL < 60) ranger_types[i] = "Shadow";
         if(TP->GLEVEL < 50) ranger_types[i] = "General";
         if(TP->GLEVEL < 40) ranger_types[i] = "Ninja";
         if(TP->GLEVEL < 30) ranger_types[i] = "Warrior";
         if(TP->GLEVEL < 20) ranger_types[i] = "Servant";
         ...
         if(TPNAME == "Beck") ranger_levels[i] = "Gold";
