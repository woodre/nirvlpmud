...
SKarateExtraLevel(arg){ KarateExtraLevel = arg; return 1; }
QKarateExtraLevel(){ return KarateExtraLevel; }
...
NotWeapon(){
   if(NotWeapon == "notweapon"){
      NotWeapon = "someweapon";
   }
   else {
      NotWeapon = "notweapon";
   }
...
Spam(){
   if(Spam == 1){
      Spam = 0;
      write("You combat messages are ON.\n");
      return 1;
   }
   Spam = 1;
   write("Your combat messages are OFF.\n");
   return 1;
}
Muffles(){
   if(Muffles == 1){
