...
TrainRanger(teacher,skillname){
   ...
   if(skillname == "random"){
      for (i=0;i<sizeof(UnlearnedSkills);i++){
         ...
         if(SKILLNAME != "servant" && SKILLNAME != "warrior" &&
               SKILLNAME != "ninja" && SKILLNAME != "general" &&
            SKILLNAME != "shadow" && SKILLNAME != "stellar"){
            CheckCost();
...
if(SKILLNAME == "random"){ return; }
   ...
   if(SKILLNAME == "rwield"){ EXPCOST = 250000; SKILLRANK = 40;
      SKILLTYPE = "skill"; MACOST = 30000; CCOST = 100000; SCOST = 20000; }
   
   if(SKILLNAME == "rwear"){ EXPCOST = 250000; SKILLRANK = 40;
      SKILLTYPE = "skill"; MACOST = 30000; CCOST = 100000; SCOST = 20000; }
   
   if(SKILLNAME == "servant"){ EXPCOST = 1000; SKILLRANK = 1; 
      SKILLTYPE = "ranger_type"; MACOST = 0; CCOST = 25; SCOST = 25; }
   
   if(SKILLNAME == "warrior"){ EXPCOST = 10000; SKILLRANK = 13; 
      SKILLTYPE = "ranger_type"; MACOST = 0; CCOST = 25; SCOST = 25; }
   
   if(SKILLNAME == "ninja"){ EXPCOST = 200000; SKILLRANK = 25; 
      SKILLTYPE = "ranger_type"; MACOST = 0; CCOST = 25; SCOST = 25; }
   
   if(SKILLNAME == "general"){ EXPCOST = 1000000; SKILLRANK = 36; 
      SKILLTYPE = "ranger_type"; MACOST = 0; CCOST = 25; SCOST = 25; }
   
   if(SKILLNAME == "shadow"){ EXPCOST = 2000000; SKILLRANK = 49; 
      SKILLTYPE = "ranger_type"; MACOST = 0; CCOST = 25; SCOST = 25; }
   
   if(SKILLNAME == "stellar"){ EXPCOST = 4000000; SKILLRANK = 59; 
      SKILLTYPE = "ranger_type"; MACOST = 0; CCOST = 25; SCOST = 25; }
   
   if(SKILLNAME == "dimensional"){ EXPCOST = 8000000; SKILLRANK = 69; 
      SKILLTYPE = "ranger_type"; MACOST = 0; CCOST = 25; SCOST = 25; }
...
CheckProperTeacher(){ 
   ...
   if(SKILLTYPE == "karate_level"){
      ...
      if(TEACHER != "Hermit"){ InTraining = 0; return 0; }
   }
   if(SKILLTYPE == "karate_extra_level" && TEACHER != "Hermit"){ InTraining =0; return 0; }
   if(SKILLTYPE == "defense" && TEACHER != "Morihei"){ InTraining = 0; return 0; }
   if(SKILLTYPE == "offense" && TEACHER != "Goldar"){ InTraining = 0; return 0; }
...
TrainRanger2(str){
   if(str == "yes"){
      ...
      if(SKILLTYPE == "karate_level"){
         ...
         MASTER->RangerMessage("all","Hermit says: "+ENVOB->NAME+" has completed training for the "+
            capitalize(SKILLNAME)+" belt.\n");
         if(SKILLNAME == "white"){
         ...
         if(SKILLNAME == "black"){
            ...
            AUnlearnedSkills("1st degree");
         }
      }
      if(SKILLTYPE == "karate_extra_level"){
         KarateExtraLevel += 1;
         KarateDegree = SKILLNAME;
         MASTER->RangerMessage("all","Hermit says: "+ENVOB->NAME+" has completed training for the "+
            capitalize(SKILLNAME)+" Black belt.\n");
      ...
      if(SKILLTYPE == "ranger_type"){
         MASTER->RangerMessage("all","Lord Zed says: "+ENVOB->query_name()+" has become a "+
         ...
         if(SKILLNAME == "ninja"){
            ...
            AUnlearnedSkills("general");
         }
         if(SKILLNAME == "general"){
            AUnlearnedSkills("shadow");
            AUnlearnedSkills("teach");
            AUnlearnedSkills("rwear");
            AUnlearnedSkills("rwield");
         }
         if(SKILLNAME == "servant"){
            AUnlearnedSkills("warrior");
            ASkills("morph");
            ASkills("unmorph");
         ...
         if(SKILLNAME == "shadow"){
            AUnlearnedSkills("stellar");
         }
         if(SKILLNAME == "stellar"){
            AUnlearnedSkills("dimensional");
         }
      }
      if(EXPCOST > 0){
      ...
      SaveRanger();
      init();
      tell_object(ENVOB,"You have learned the "+SKILLTYPE+" "+SKILLNAME+".\n");
      write_file("/players/beck/DarkRangers/Log/TRAINING",ENVOB->query_name()+
