inherit "room/room";
reset(arg) {
object flytrap1, flytrap2, flytrap3, flytrap4;
	if(arg) return;
set_light(1);
short_desc=("???");
long_desc=
...
if(!present("flytrap3", this_object())){
move_object(clone_object("/players/beck/monster/large_flytrap.c"), this_object());
}
if(!present("flytrap4", this_object())){
move_object(clone_object("/players/beck/monster/large_flytrap.c"), this_object());
}
}
init() {
...
write("You wander down the path into the field.\n");
call_other(this_player(),"move_player","wander#/players/beck/room/venus7.c");
return 1;
}
climb(arg) {
...
call_other(this_player(),"move_player","climb#/players/beck/room/venus2.c");
return 1;
}
