...
/* New Wield Function for Damage Types */
wield(str) {
    object ob;
    if (id(str) && environment() == this_player()){
       ...
       set_params(style,class_of_weapon,special);
       wielded_hands = call_other(this_player(), "wield", this_object(),silentwield, params, hands);
       wielded = 1;
       return 1;
   }
...
wielded_hands(){
   if (wielded_hands == 3)
      return "(wielded Two-Handed)";
   else if (wielded_hands == 2)
      return "(wielded Secondary)";
   else
      return "(wielded Primary)";
}
...
set_style(a){
   string cat, subcat;
   if(sscanf(a, "%s|%s",cat,subcat) !=2){ 
      if(a != "physical" && a != "magical") style = "physical";
   }
   else if(cat != "other") style = "physical";
   else style = a;
}

query_style(){ return style; }
...
short() {
   string tmp_short;
    ...
    if (call_other(this_player(), "query_level", 0) > 30) {
       ...
       if (wielded)
          return short_desc + " "+wielded_hands();
...
*/
        call_other(wielded_by, "unwield", this_object());
        wielded = 0;
        if (!silently)
...
stopwield(str) {
   if(id(str)){
      ...
      call_other(this_player(), "unwield", this_object());
      wielded = 0;
      wielded_by = 0;
      return 1;
   }
...
weapon_breaks(){
   ...
   this_object()->set_class(class_of_weapon/3);
/*
...
add_wear(arg) {
  int ah;
/*
 too many unknowns to break down into hits and misses assign a number of hits
*/
  ah = random(arg*9/4);
  hits += ah;
...
set_params(style2,class,special2){
   string cat, subcat;
   int i;
   if(!params) params = ({ });
   if(sscanf(style2, "%s|%s",cat,subcat) !=2){ 
      if(style2 != "physical" && style2 != "magical") style2 = "physical";
   }
   else if(cat != "other") style2 = "physical";
   if(member_array(style2,params) != -1){
