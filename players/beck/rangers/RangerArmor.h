...
MetallicArmor(){
   if(!MetallicArmor){
      ...
      MetallicArmor = 1;
      tell_room(ENVTP, CAPNAME+" yells: Metallic Armor, Power Up!\n");
      return 1;
   }
   MetallicArmor = 0;
...
SHelmetLamp(){
   ...
   if(!HelmetLamp){
      HelmetLamp = 1;
      set_arm_light(1);
      tell_room(ENVTP,CAPNAME+"'s Helmet begins to emit a bright beacon of light.\n");
      return 1;
   }
   HelmetLamp = 0;
   set_arm_light(-1);
   tell_room(ENVTP,CAPNAME+"'s Helmet stops emitting light.\n");
   return 1;
}
shield_bonus(ob){
