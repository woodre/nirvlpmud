...
write("\n");
   ...
   for(j=0; j<sizeof(rangers); j++){
      ...
      if(rangers[j] != NAME){
         call_other(present(rangers[j], environment(TP)),"move_player","megazord#/players/beck/rangers/thunder_megazord.c");
       }
   }
   call_other(TP,"move_player","megazord#/players/beck/rangers/thunder_megazord.c");
