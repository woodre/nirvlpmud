...
reset(arg) {
   if (arg) return;
   ranger = ([]);
   restore_object("players/beck/Rangers/Save/Globe");
}
short() {
...
long() {
   ...
   if(TP->query_guild_name() == "rangers"){
      write(""+
         ...
         "The maximum number of guild members allowed is "+MAXGUILD+".\n"+
         "To leave the guild type 'leave_rangers', but I hope you won't.\n"+
         "");
   ...
   write("Welcome to the Power Ranger Command Center.\n"+
      ...
      "Note: You may NOT have both a ranger and dark ranger character.\n"+
...
init() {
   ...
   if(TP->query_guild_name() == "rangers"){
      ...
      add_action("Animal1","Animal");
      add_action("Weapon1","Weapon");
      if(TP->query_level() < 20 && TP->query_guild_name() == "rangers"){
         update_ranger_roster();
      }
   }
  if( (this_player()->query_real_name() == "beck") ||
      (this_player()->query_real_name() == "illarion") ||
...
test()
{
  ...
  write("\n\n");
  files = get_dir("/players/beck/Rangers/Save/*.save");
    ...
    for(r = 1; r < 7; r++) /* this is convoluted */
    {
      ...
      write(
        colour_pad(
          implode(
            explode(
              read_file("/players/beck/Rangers/Save/"+ files[s], r),
            "\n"),
          ""),
        p));
    }
...
set_ranger_inactive(str){
   ranger_inactive = ({ str });
   save_globe();
   return 1;
}
add_ranger_inactive(str){
   ranger_inactive += ({ str });
   save_globe();
   return 1;
}
remove_ranger_inactive(str){
   ranger_inactive -= ({ str });
   save_globe();
   return 1;
}
...
join_honorary(){
   ...
   if(TP->query_guild_name() == "dark rangers"){
...
return 1;
   ...
   else if(COMM->QuerySkillLevel("combat") > 69){ ranger_levels[i] = "Brown"; }
   else if(COMM->QuerySkillLevel("combat") > 59){ ranger_levels[i] = "Purple"; }
   else if(COMM->QuerySkillLevel("combat") > 49){ ranger_levels[i] = "Blue"; }
   else if(COMM->QuerySkillLevel("combat") > 39){ ranger_levels[i] = "Green"; }
   else if(COMM->QuerySkillLevel("combat") > 29){ ranger_levels[i] = "Red"; }
   else if(COMM->QuerySkillLevel("combat") > 19){ ranger_levels[i] = "Orange"; }
   else if(COMM->QuerySkillLevel("combat") > 9){  ranger_levels[i] = "Yellow"; }
   else if(COMM->QuerySkillLevel("combat") > 0){  ranger_levels[i] = "White"; }
   else { ranger_levels[i] = "None"; }
