...
reset(arg) {
  if(arg) return;
if(!present("queen")) {
  move_object(clone_object("players/eurale/Chess/NPC/w_queen"),TO); }
set_light(1);
...
East() {
  TP->move_player("into the mist#players/eurale/Chess/b1.c");
  return 1; }

South() {
  TP->move_player("into the mist#players/eurale/Chess/a2.c");
  return 1; }
