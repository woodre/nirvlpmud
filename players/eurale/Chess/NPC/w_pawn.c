...
set_name("platinum pawn");
set_alias("pawn");
set_race("undead");
	
set_short(HIY+"Platinum Pawn"+NORM);
set_long(
	"A platinum chessman footsoldier of the lowest rank.\n");
...
set_spell_mess1(
	HIY+"The pawn lands a crushing blow!\n"+NORM);
	
set_spell_mess2(
	HIY+"The pawn clubs you solidly!\n"+NORM);
        ...
        if(!attacker_ob){
          if(DAEMON->check_white(ob->query_real_name())){
            tell_room(environment(),
              HIY+"The Platinum Pawn attacks its enemy!\n"+NORM);
            attack_object(ob);
          }
        }
      }
