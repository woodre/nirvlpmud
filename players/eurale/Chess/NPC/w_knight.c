...
set_name("platinum knight");
set_alias("knight");
set_race("undead");
	
set_short(HIY+"Platinum Knight"+NORM);
set_long(
  "A platinum colored piece shaped like a horse's head.\n");
	
set_level(19);
set_hp(450);
set_al(150);
move_object(clone_object("players/eurale/Chess/OBJ/dagger"),TO);
...
set_spell_mess1(
	HIY+"The knight punches fiercely!\n"+NORM);
	
set_spell_mess2(
	HIY+"The knight punches you in the face!\n"+NORM);
        ...
        if(!attacker_ob){
          if(DAEMON->check_white(ob->query_real_name())){
            tell_room(environment(),
              HIY+"The Platinum Knight attacks its enemy!\n"+NORM);
            attack_object(ob);
          }
        }
      }
