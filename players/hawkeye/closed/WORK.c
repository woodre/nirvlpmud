...
extra_init() {
   ...
   if(this_player()->query_real_name() == "hawkeye") {
...
string intruder_name;
   if(intruder->query_real_name() == "hawkeye") { return 1; }
   if(shield_status == 1) {
 write("You try to enter Hawkeye's workroom, but\n"+
    "a large hand grabs you and chucks you in the waiting room!\n");
      write("\n");
      pavlikobj = find_player("hawkeye");
      if(pavlikobj){
...
status temp_shield;
   ...
   if(whereto == "here") {
      ...
      tell_object(target, "You have been transported to Hawkeye's workroom.\n");
...
valid_read(str) { return call_other(this_player(),"valid_read",str); }
  ...
  FOUR_EXIT("room/church.c", "church",
       "room/crop","castle",
       "players/boltar/hotel/hotel.c","hotel",
    "players/hawkeye/closed/guild/guild_room","guild",
          "St. Louis Arena",
    "St. Louis Arena.\n"+
"This is the arena where the St. Louis Blues play hockey,\n" +
"Be sure to duck the slapshots of Brett Hull, and cheer\n" +
"the glove saves of Curtis Joseph. P.S. Dont slip on the ice.\n",1)
