...
rank() {
	...
	write("Health Monitor: "+capitalize(hpmon)+" <"+how_often+">\n");
...
if(obj->query_hp() < dmg) 
	dmg = obj->query_hp();
obj->hit_player(dmg);
this_player()->add_spell_point(-(20+this_player()->query_guild_rank()));
...
if(guild_rank == 1) { title = "the Monk Initiate"; }
if(guild_rank == 2) { title = "the Novice Monk"; }
if(guild_rank == 3) { title = "the Monk"; }
if(guild_rank == 3) { title = "the Monk Superior"; }
if(guild_rank == 4) { title = "the Master Monk"; }
if(guild_rank == 5) { title = "the Grandmaster Monk"; }
if(guild_rank > 5) { title = "the Grandmaster of Shadows"; }
...
if(environment(target)->query_realm() == "NT"){
...
x = random(100) - this_player()->query_guild_rank();
