...
set_alias("mrobe");
/*
set_heart_beat(1);     switched to fake_beat  - mythos <1-25-95>
*/
stunwait = 0;
lsteal = 50;
stunwait = 0;
...
/*LEAVE THE GODDAMN MONEY THING OUT*/
...
long() {
write("Type mhelp dir for more help on guild abilities.\n");
}
...
if(this_player()->query_guild_rank() >= 5) this_player()->set_ac(7);
if(this_player()->query_guild_rank() == 5) this_player()->set_ac(7);
if(this_player()->query_guild_rank() > 8) this_player()->set_ac(9);
     return;
}
...
if(this_player()->query_guild_rank() > 19) {
  ...
  Taken out by Boltar's orders.. all guild follows are now obselete
  -mythos <12-14-95>
...
special_attack(){
  ...
  if(obj->query_attack()->query_npc() != 1)
    return 1;
/* removed by wizardchild 7/28/95. having command in a call_out
   makes it look like you code for wizpoints.
  command("sp_attack", obj);
*/
  sp_attack(obj); /* you are welcome */
...
/*
heart_beat() {     switched to fake_beat
*/
fake_beat() {
  object para, mons;
   string itname, shape;
...
if(follow == 1){
      ...
      tell_room(environment(environment(this_object())), capitalize(me)+" leaves following "+followee->query_name()+"\n");
      ...
      tell_room(environment(environment(this_object())), capitalize(me)+" arrives following "+followee->query_name()+"\n");
      }
   }
*/
call_out("fake_beat",5);
...
cure_flu() { PAIN->cure_flu(); return 1; }
...
iquit = 911;
/* added by wizardchild to prevent annoying messages */
if(!this_player()->on_channel("msg"))
