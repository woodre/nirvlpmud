  ...
  for(n=0;n<sizeof(u);n++) {
   if(u[n])
    if(u[n]->query_level() > 19) {
      if(u[n]->query_real_name() != "arrina") {
        if(member_array(u[n],wiz) == -1) { wiz += ({u[n]});
        names += ({u[n]->query_real_name()});
        if(find_player("arrina")) { tell_object(find_player("arrina"),RED+"\t");
        if(u[n]->query_invis()) tell_object(find_player("arrina"),"** ");
        else tell_object(find_player("arrina"),"   ");
...
query_names() { 
  int g;
    for(g = 0; g<sizeof(wiz); g ++) {
      ...
      if(wiz[g]) {
        tell_object(find_player("arrina"),"\t"+wiz[g]+"\n"); }
      }
  ...
  for(n=0;n<sizeof(wiz);n++) {
    if(wiz[n]) {
      if(member_array(wiz[n],u) == -1) {
        if(find_player("arrina")) tell_object(find_player("arrina"),
