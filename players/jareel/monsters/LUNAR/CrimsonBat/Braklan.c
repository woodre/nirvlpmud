...
armor=clone_object("/players/jareel/SoulboundObjects/ARMOR/SetOfShadows/MiscOfDarkness.c");
     ...
     set_short(
       "Braklan - Keeper of the "+RED+"Crimson Bat"+NORM+" [ "+RED+"Seven Mothers "+NORM+"] (lord of evil)");
     set_long(
       "Braklan has a chaotic twinkle in his eyes.  Having years of\n"+
     ...
     heart_beat(){
       object       ob;
        ...
        if(heal && (this_object()->query_hp() < 400))
        {
                ...
                tell_object(stuff[x], "An aura heals Braklan!\n");
                if(stuff[x]->is_player())
                {
                        stuff[x]->hit_player(125,"other|energy");
                        tell_object(stuff[x]," "+GRN+"Braklan "+NORM+"screams as he sends out an "+HIY+"Energy Burst"+NORM+"!\n");
                }
                heal--;
        }
}
...
/* by verte */
 ...
 monster_died() {
  if( attacker_ob && armor ) {
    ...
    tell_room( environment(), "\n"+HIW+"A flash of light flashes from "+HIR+BLINK+"Braklan's A R M O R"+NORM+HIW+" as it is soulbound to "+
        ...
        if(sscanf(str,"%s leaves %s.",name,dir) == 2){
            if(attacker_ob){
                if(name = attacker_ob->query_name()){
                    if(random(4)==1){
                        tell_object(attacker_ob,"Braklan anticipates your move.\n");
                        command(dir);
                    } } } } }
...
attackme(){
    if(!attacker_ob && random(2) == 1){
        if(this_player()->is_player()){
            tell_object(this_player(),"Braklan notices you and attacks.\n");
    ...
    return 0;
     ...
     heals(arg) {
       ...
       if(!this_player()->add_weight(1)) {
         write("Braklan says: Lighten your load.\n");
           return 1; }
       if(this_player()->query_money() < a) {
         write("Braklan says: You lack the money for the heal\n");
           return 1; }
       TP->add_money(-a);
         write("Braklan takes your coins mutters a few words, the coins turn into a heal.\n");
         say(TPN+" buys a heal from Braklan.\n");
