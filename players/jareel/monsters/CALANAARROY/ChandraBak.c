...
#include "/players/jareel/define.h"
  ...
  set_multi_cast(0);
  set_hp(200); 
  set_hp_bonus(8); /* Battle Healer Heals a lot does Damage when she heals 6000 in health
  ...
  set_ac(4);
  set_res(0);
  set_ac_bonus(0); /* Illarion Said ac bonus for Physical Res should be 1 py per percent of Res */
  set_wc(3); 
...
"+NORM+BOLD+"\n\
         ...
         "+NORM+"\n",
    2,"1-1+1","other|spirit", 1);
	
  add_spell(
    "chandra armorShield1",
"Chandra strikes #TN# with a shield spike.\n",
"Chandra strikes #TN# with a shield spike.\n",
    4,"1-1+2","other\Jareel", 1); /* Changed From Physical as not to be mitigated */
...
"Chandra strikes #TN# with a shield spike.\n",
    2,"1-11+2","other\Jareel", 1); /* Changed From Physical as not to be mitigated */

  add_spell(
    "chandra armorMisc2",
HIK+"#TN# is squeezed by Chandra's flowing Demonic Tentacles..\n"+NORM,
HIK+"#TN# is squeezed by Chandra's flowing Demonic Tentacles..\n"+NORM,
    4,"1-1+1","other|dark", 1);
...
HIK+"#TN# is squeezed by Chandra's flowing Demonic Tentacles..\n"+NORM,
    ...
    if(!attacker_ob) {
      return;
        ...
        {
                this_object()->heal_self(1); /* Will Heal 2000 */
                healdam1 = all_inventory(environment(this_object()));
                tell_object(healdam1[x], "Bitch Healed 1000!\n");
                if(healdam1[x]->is_player())
                {
                        healdam1[x]->hit_player(1,"energy");  /* Will hit for 250 */ 
        ...
        {
                this_object()->heal_self(2); /* Will Heal 4000 Hit Points */
                ...
                {
                        healdam2[y]->hit_player(1,"energy"); /* Will deal 2500 Damage in the fight */
  ...
  void corpse_loop()    {
    ...
    if((ob = present("corpse", environment())) && (int)ob->is_corpse()) {
      ...
      heal2 = (0); /* heal2 = (40);Big Fuck you if you die to this Mob Sets the Heal2 counter to 40 giving her a lot of fucking hit points */
