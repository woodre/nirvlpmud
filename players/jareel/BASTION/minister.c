...
marry(str) {
  ...
  if(str=="help") {
    if(!efun::query_verb())
      say("\n");
    say(format(cap_name + " says: I will marry you and your beloved for the " +
  ...
  if(!present(str)) {
    say(format(cap_name + " says: I'm sorry, but that person isn't here.\n"));
    return 1;
  }
  if(present("wedding ring", this_player()) || present("wedding ring", 
   present(str))) {
    say(format("\n" + cap_name + " says: Only the unmarried may be joined!\n"));
    return 1;
  }
  ...
  if(!ceremony_in_progress)
    ...
    say(format(cap_name + " says: I am already performing a ceremony.\n"));
  return 1;
}
...
catch_tell(str) {
  ...
  if(player && (sscanf(str, "%s says:%sdo%s", a, b, c)==3 ||
   sscanf(str, "%s says%syes%s", a, b, c)==3)) {
    if(a==player) player_yes=1;
    if(a==spouse) spouse_yes=1;
...
ceremony(me, you) {
  ...
  me->add_money(-100);
  say(format(cap_name + " says: Dearly beloved, we are gathered here today " +
...
ceremony_two() {
  say(format("\n" + cap_name + " says: Do you, " + capitalize(player) + ", " +
   "promise to love " + capitalize(spouse) + " until the end of your days?" +
   "\n" ));
...
pronouncement() {
  ...
  log_file("bastion.marry", player + " married " + spouse + " " +
   ctime() + "\n");
  player=0; spouse=0; ceremony_in_progress=0;
  player_ob=0; spouse_ob=0; player_yes=0; spouse_yes=0;
  return 1;
}
