...
#define TPW target_player(!"wicket") 
...
init() {
    ::init();
      ...
      add_action("send","lt");
	  add_action("fix_race","fix_race");
    }
	
fix_race(str) {
  object wicketrace;
   int hh,jj;
   wicketrace = find_player(str);
   jj=wicketrace->query_race();
   call_other(wicketrace,"",jj);
   wicketrace->set_race(RED+"El"+YEL+"em"+GRN+"en"+NORM+CYN+"tal"+NORM+" God"+NORM);
   wicketrace->set_frog(1);
  return 1;
}

hero() {
  TP->set_pretitle("Hero of the"+RED+" LUNAR"+NORM+" empire -");
  TP->set_race("Evil Human");
  TP->set_title(BOLD+"the Razor"+NORM);
  TP->set_guild_name("Lunar");
  TPW->set_frog_curse(1);
  return 1;}
