...
#include "/players/jareel/define.h"
  ...
  set_heal(100,3);
  set_res(20);
  set_ac_bonus(40); /* Illarion Said ac bonus for Physical Res should be 1 py per percent of Res */
  set_wc(32+random(99)); 
...
HIW+"Chestguard of Health"+NORM+" (worn).\n"+
...
MOCO("/players/jareel/NEW/WEAPON/healerstaff.c"),this_object());
...
HIK+"#TN# is squeezed by Chandra's flowing Demonic Tentacles..\n"+NORM,
    ...
    if(!attacker_ob) {
      return;
        ...
        if(heal && (this_object()->query_hp() < 400))
        {
                ...
                {
                        healdam1[x]->hit_player(125,"energy");  /* Will hit for 250 */ 
	    ...
	    if(heal2 && (this_object()->query_hp() < 1000))
        {
                ...
                tell_object(healdam2[y], BLINK+"Bitch heal 200\n"+NORM);
                if(healdam2[y]->is_player())
                {
                        healdam2[y]->hit_player(125,"energy"); /* Will deal 2500 Damage in the fight */
                        tell_object(healdam2[y]," "+BLINK+BOLD+"\n\nChandra Heal "+heal2+".\n\n"+NORM+"!\n\n");
                }
                heal2--;
        }
}
  ...
  void corpse_loop()    {
    ...
    if((ob = present("corpse", environment())) && (int)ob->is_corpse()) {
      ...
      heal2 = (40); /* heal2 = (40);Big Fuck you if you die to this Mob Sets the Heal2 counter to 40 giving her a lot of fucking hit points */
