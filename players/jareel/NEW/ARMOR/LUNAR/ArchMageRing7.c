...
string *Armor_pieces;
     ...
     set_weight(1);
     set_type("amulet");
     set_short(HIB+"Amulet"+NORM+" of "+HIK+"["+HIW+"Power"+NORM+HIK+"]"+NORM);
     set_long(
...
string short() {
  ...
  if (check_full_set())
    return (HIR+"Amulet"+NORM+" of "+HIK+"["+HIW+"Power"+NORM+HIK+"]"+NORM+BOLD+" ("+NORM+HIG+"Set of Power"+NORM+BOLD+")"+NORM+" (worn)"+NORM);

  if ((status)this_object()->query_worn())
    return(HIR+"Amulet"+NORM+" of "+HIK+"["+HIW+"Power"+NORM+HIK+"]"+NORM+NORM+" (worn)");

  return HIR+"Amulet"+NORM+" of "+HIK+"["+HIW+"Power"+NORM+HIK+"]"+NORM+NORM;
}
...
do_special(owner) {
  ...
  while(i--) {
    ...
    set_params("physical",0,0,"powerup");
}
