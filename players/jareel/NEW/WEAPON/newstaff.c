...
#ifndef __LDMUD__
  arm = filter_array( all_inventory(wielded_by), "filter_armor", this_object() );
#else
  arm = filter( all_inventory(wielded_by), "filter_armor", this_object() );
#endif
  siz = sizeof(arm);
