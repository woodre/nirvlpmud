...
int monsters;
     ...
     long_desc=
       ...
       ""+HIK+"@@@@@"+NORM+LINE28+"   \n"+
       "---"+HIK+"@@"+NORM+LINE29+"   \n"+
       ""+BOLD+"X"+NORM+HIK+"@"+NORM+"|-E"+LINE30+"	   \n"+
       "--|"+HIK+"@@"+NORM+LINE31+"   \n"+
       "|"+HIK+"@"+NORM+"|R"+NORM+HIK+"@"+NORM+LINE32+"\n";
     ...
     items = ({
       ...
       "west","the cavern leads off that way",
       "flower","Several small purple flower blossom in the moss",
       "blossom","The flower looks very delicate",
       "pile","A stack of leather hides forming a bed for some large creature",
       "skin","Assorted leather pelts and hides forming a bed",
       "drip","Water leaking from the ceiling",
       "puddle","A small pool of liquid on the ground",
       "floor","Smooth rock",
     });

dest_dir = ({
     "/players/jareel/areas/chaos/cave/grid1/hall1.c","north",
     });
    get_monsters();
}
...
getmonsters_00to05()
{
    move_object(clone_object(MON+"parnishdarn.c"),this_object());
...
getmonsters_default()
{
    if(random(1)==0)  move_object(clone_object(MON+"parnishdarn.c"),this_object());
