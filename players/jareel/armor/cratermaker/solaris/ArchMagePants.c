...
string *Armor_pieces;
     ...
     set_name("smarty pants");
     set_ac(0);
     set_value(4200);
     set_weight(1);
     set_type("pants");
     set_short(HIK+"Lunar pants"+NORM+" of "+HIK+"["+HIW+"Illusion"+NORM+HIK+"]"+NORM);
     set_long(
       RED+"                ["+HIG+"Set of the Veil"+NORM+RED+"]\n"+
       "Swirling red and white crystals sewn in encrusted soft golden polished\n"+
	   "rivets, pulsate with raw energy.  A fine tailor has allowed the pants \n"+
...
status id(string arg) { return ( ::id(arg) || arg == "ArchMagePants" || arg == "smarty pants"); }
...
string query_short() {
  ...
  if (check_full_set())
    return HIK+"Lunar pants"+NORM+" of "+HIK+"["+HIW+"Illusion"+NORM+HIK+"]"+NORM+BOLD+" ("+NORM+HIG+"Set of the Veil"+NORM+BOLD+")"+NORM;
  else
    return HIK+"Lunar pants"+NORM+" of "+HIK+"["+HIW+"Illusion"+NORM+HIK+"]"+NORM;
}
...
do_special(owner) {
  if( !random(5) && check_full_set() )
    set_params("physical",0,2,"powerheal");
}
