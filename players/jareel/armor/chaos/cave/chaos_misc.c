...
#define NAME HIK+"Dark "+NORM+RED+"Cha"+HIR+"os "+HIK+"Aura"+NORM
...
status id(string arg) { return ( ::id(arg) || arg == "chaos_misc" || arg == "aura" || arg == "misc" || arg == "chaos aura"); }
...
string short() {
  ...
  if (check_full_set())
    return (NAME+"      ("+NORM+HIG+"Set of Chaos"+NORM+BOLD+")"+NORM+" (Enshrouded)"+NORM);

  if ((status)this_object()->query_worn())
    return(NAME+"(Enshrouded)");
