...
reset(int arg) {
  ...
  set_weight(0);
  set_type("armor");
  set_name("earring of the underworld");
  set_alias("earring");
  set_short(HIK+"Earring of "+HIK+"Evil"+NORM+" ["+RED+"Demon Fire"+NORM+HIK+"] ["+SETNAME+"]"); 
  set_long(
    "Earring Armor forged in the demon fire forge of the Malia blacksmith Mikaeus, the Umhallowed.\n"+
    "A palpable clinging evil seeps out of the magical earring.  The encircling evil sweeps out\n"+
	"of the dark armor and empowers the earring with magical energy.\n");

                     /* PUT THE ARMOR ID FOR THE SET HERE */
  Armor_pieces = ({ "PlateOfTheUnderworld", "EarringOfTheUnderworld", "MiscOfDarkness" });
     }

status id(string arg) { return ( ::id(arg) || arg == "EarringOfTheUnderworld" || arg == "earring of the unswerworld"); }
...
string short() {
  ...
  if (check_full_set())
    return (HIK+"Earring of "+HIK+"Evil"+NORM+" ["+RED+"Demon Fire"+NORM+HIK+"] ["+FULLSETNAME+"]"+NORM+" (worn)"+NORM);

  if ((status)this_object()->query_worn())
    return(HIK+"Earring of "+HIK+"Evil"+NORM+" ["+RED+"Demon Fire"+NORM+HIK+"] ["+SETNAME+"]"+NORM+" (worn)"+NORM);

  return HIK+" Earring of "+HIK+"Evil"+NORM+" ["+RED+"Demon Fire"+NORM+HIK+"] ["+SETNAME+"]"+NORM+""+NORM+NORM;
}
