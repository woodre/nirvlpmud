...
string *Armor_pieces2;
  ...
  Armor_pieces2 = ({ "death_plate", "death_amulet", "death_helm", "death_boots", "death_gauntlets",
"death_guard", "death_shield" });
     }

status id(string arg) { return ( ::id(arg) || arg == "death_gauntlets" || arg == "ring" || arg == "gauntlets" ||
arg == "gloves"); }
...
#if 0 /* Gnar - should be using base object short, adding query_short */
string short() {
  if (check_full_set())
    return ("Iron "+HIK+"Gauntlets"+NORM+" of Death "+BOLD+"("+NORM+HIG+"Set of death"+NORM+BOLD+")"+NORM+"
(worn)"+NORM);
...
query_short() {
  if( check_full_set() )
    ...
    return "Iron "+HIK+"Gauntlets"+NORM+" of Death";

}

do_special(owner) {
  int i;
  if( !ATT || (!ATT->is_npc() && !ATT->is_player())) return;
  /* hits 15% of the time */
  if(random(100) > 85) { 
    if(ATT->is_npc())
