/* Thanks to Cosmo for colorizing this */
...
id(str) {
    return str == "board" || str == "notes" || str == "note";
}

long() {
    if (num_messages == 0) {
        write("There is no messages for cooper.\n");
        return;
    }
    write("The note pad contains " + num_messages);
    if (num_messages == 1)
        write(" message:\n");
    else
        write(" messages:\n");
    say(call_other(this_player(), "query_name") +
        " studies the notepad.\n");
    headers();
    write("\nYou can "+GRN+"<write>"+NORM+" your own graffiti message "+
          "in "+GRN+"<color>"+NORM+" with a "+GRN+"<title>"+NORM+".\n");
    write("\tExample: 'write blue Title of Message'\n");
    write("\tAvailable paints: blue, green, red, yellow, cyan, magenta\n");
...
short() {
    return ("A "+BLU+"Ranger Notepad"+NORM+"(" + num_messages + " msgs)");
}

get() {
    write("You can't take the notepad.\n");
    return 0;
}

init() {
    add_action("new"); add_verb("write");
...
reset(arg) {
    if (arg)
        return;
    restore_object("players/jareel/general_stuff/graf/coop");
}
...
string hd;
    ...
    if (who && environment(who) == environment(this_object())) {
        write(call_other(who, "query_name") + " is busy leaving Cooper a note.\n");
        return 1;
    }
    if (num_messages == 10) {
        write("The notePad is full.\n");
        return 1;
    }
    if (strlen(hd) > 50) {
        write("The note is to long for the notepad.\n");
        return 1;
    }
    if (get_color() == 1) {
       new_hd = hd;
...
get_body(str) {
    if (str == "**") {
        ...
        new_body = 0;
        new_hd = 0;
        save_object("players/jareel/general_stuff/graf/coop");
        write("Ok.\n");
        who = 0;
        return;
    }
...
remove(str) {
    ...
    while(rest != 0 && rest != "") {
        ...
        if (i == 0) {
        if(sscanf(hd, "%sJareel", jun) == 1 && tp !="jareel") {
         write("You cannot remove this message.\n");
         messages=jun2;
         return 1;
        }
        if((call_other(this_player(),"query_level") < 40) && (tp != "cooper")) {
