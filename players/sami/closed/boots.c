...
#define AMULET RED+"~ "+OFF+BLUE+"Ele"+OFF+GREEN+"ment"+OFF+BOLD+"al B"+OFF+GREEN+"oo"+OFF+BLUE+"ts"+OFF+RED+" ~"+OFF

reset(arg) {
  ::reset(arg);
  set_name("elemental boots");
  set_alias("boots");
  set_short(AMULET);
  set_long("");
  set_type("boots");
...
wear(str)
{
  ...
  if(str == "boots" || str == "elemental boots"){  
   ::wear(str);
   return 1; }
 }
}
...
void long() {
  ...
  write(
    "  You see before you a legendary pair of boots, molded from the very\n"+ 
    "elements themselves, forged by the gods of Nirvana for those mighty\n"+ 
    "and worthy.  Crafted from supple leather, they have been reinforced\n"+
    "with plates of blue-green Elementite, and wide leather straps and\n"+
    "strong metal buckles fasten them tightly in place.  Flickering white\n"+
    "specks float about them. ");
  if((a = this_player()) && (b = environment()) && (b == a)) {
    write("\n  You can sense a "+BLUE+"mys"+GREEN+"tic"+OFF+BOLD+"al "+OFF+GREEN+"ene"+BLUE+"rgy"+OFF+" coursing through the boots.");
    if(present("elemental aura",environment(this_object())))
      write("  Worn by\n"+
            "those with both valor and might, these boots will protect anyone so brave\n"+
