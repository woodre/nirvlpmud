...
#define SWORD RED+"~ "+OFF+BLUE+"Ele"+OFF+GREEN+"men"+OFF+BOLD+"tal S"+OFF+GREEN+"wo"+OFF+ BLUE+"rd"+OFF+RED+" ~"+OFF

int smack;

reset(arg) {
  ::reset(arg);
  set_name("elemental sword");
  set_alias("sword");
  set_short(SWORD);
  set_long("");
  set_class(21);
...
long(){
  ...
  write("  You see before you a legendary weapon, molded from the very elements\n"+
        ...
        "A finely crafted sword, a keen blade of the most superb steel extends\n"+ 
      ...
      if((a = this_player()) && (b = environment()) && (b == a)) {
        write("\n  You can sense a "+BLUE+"mys"+GREEN+"tic"+OFF+BOLD+"al "+OFF+GREEN+"ene"+BLUE+"rgy"+OFF+" coursing through the sword.");
