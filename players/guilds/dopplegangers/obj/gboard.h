...
id(str) {
  return str == "board" || str == "bulletin board" || str == "gboard";
}
...
long() {
  ...
  if (this_player()->query_guild_rank() > 100)
...
short() {
  if (msg_num==1) 
    return "A bulletin board [1 message]";
  else
    return "A bulletin board ["+msg_num+" messages]";
}
...
init() {
  ...
  if(this_player()->query_guild_rank() > 999) {
...
remove_msg(what_msg) {
  ...
  if ((this_player()->query_name() != player &&
    ...
    write("Only wizards may remove other player's notes.\n");
...
move_msg(what_msg) {
  ...
  if ((this_player()->query_name() != player &&
       this_player()->query_level() < 21) ||
       !query_ip_number(this_player())) {
    write("Only wizards may move other player's notes.\n");
