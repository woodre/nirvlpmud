#include "../def.h"
#include "/sys/wizlevels.h"

inherit "room/room";

void reset(status arg) {
  if (!present("board"))
    move_object(clone_object("/players/guilds/dopplegangers/obj/bboard"),
                this_object());

  ::reset(arg);

  if (arg)
    return;

  short_desc = "Barbarian's Faction Party Room";
  long_desc  = "This is the Barbarian's Party Room.\n" +
"You are obviously in the guild so this is the place for you to mingle \n" +
"with everybody and spend some time away from the scum of the mud, since \n" +
"only the chosen can enter this room.\n";
  dest_dir = ({
    GUILD_DIR + "room/barbarian_entrance", "east",
    "/room/church", "church",
    "/room/shop", "shop",
    GUILD_DIR + "room/guildhall", "doppguild",
    GUILD_DIR + "room/guildhall", "guild",
    "/room/adv_guild", "adventurers",
    "/room/pub2", "pub",
  });
}

void init() {
  object badge;

  ::init();

  badge = present("V-GUILD", this_player());

  if ((int)this_player()->query_level() < WL_WIZARD) {
    if (!badge->query_faction("barbarian")) {
      write("YOU ARE NOT A BARBARIAN!\n" +
            "Only the chosen will be allowed to enter this room!\n");
      say(this_player()->query_name() +
          " tried to enter the room but gets kicked out!\n");
      move_object(this_player(),
                  "players/guilds/dopplegangers/room/barbarian_guild_entrance");
      write("Join our guild before entering the faction's party room.\n");
    }
  }
}
