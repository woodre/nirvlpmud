...
void reset(int arg) {
  ...
  long_desc = "You have entered the Doppleganger Barbarian Faction.\n" +
              "Type 'list' for a list of commands.\n";
  dest_dir = ({
    "/players/grimm/rooms/haphaz_a13", "east",
    GUILD_DIR + "room/barbarian_guildhall", "west",
  });
}

void init() {
  ::init();
  add_action("west", "west");  /* Barbarian Faction member check */
...
int west() {
  ...
  proom = "/players/guilds/dopplegangers/room/barbarian_guildhall";

  if ((int)this_player()->query_level() < WL_WIZARD) {
    if (!badge ||
        !badge->query_faction("barbarian") ||
        (string)this_player()->query_guild_name() != "doppleganger") {
      write("You are not a member of the Barbarian Faction.\n" +
...
int join() {
  ...
  badge->set_faction("barbarian");
  badge->reset();
  badge->save_me(1);
  command("renew", this_player());
  write_file(GUILD_LOG + "join",
             capitalize((string)this_player()->query_real_name()) +
             " joined the Barbarian Faction - Time: " + ctime() + "\n" +
             "-------------------------------------------------\n");
  CHAND->channel_message("dopplegangersB",
                        this_player()->query_name() +
                        " just joined the Barbarian Faction!!!",
                        HIB + "[Grimm's Assistant]: " + NORM);
  say(this_player()->query_name() +
      " just joined the Doppleganger Barbarian Faction!!!\n");
  return 1;
}
...
void unjoin() {
  ...
  write("WARNING: This will remove all Barbarian Faction status.\n" +
...
int unjoin_decision(string str) {
  ...
  CHAND->channel_message("dopplegangersB",
                         this_player()->query_name() +
                         " just left the Barbarian Faction!!!",
                         HIB + "[Grimm's Assistant]: " + NORM);
  write_file(GUILD_LOG + "unjoin",
             capitalize((string)this_player()->query_real_name()) +
             " unjoined the Barbarian Faction - Time: " + ctime() + "\n" +
             "Guild Exp: " + this_player()->query_guild_exp() + "\n" +
             "Guild Rank: " + this_player()->query_guild_rank() + "\n" +
             "----------------------------------------------------\n");
  badge->remove_faction("barbarian");
  badge->set_guild_exp(180000);
  this_player()->set_guild_rank(9);
  badge->reset();
  badge->save_me(1);
  command("renew", this_player());
  say(this_player()->query_name() + " has left the Barbarian Faction.\n");
  return 1;
}
...
int cost() {
  ...
  badge = present("V-GUILD", this_player());
  if (!badge || !badge->query_faction("barbarian")) {
    write("You are not a member of the Barbarian Faction yet...type \"join\".\n");
    return 1;
  }
  write(
...
int advance() {
  ...
  default:
    write("You are at the highest level in the Barbarian Faction.\n");
    return 1;
    break;
  }
  ...
  CHAND->channel_message("dopplegangerB",
                         this_player()->query_name() +
