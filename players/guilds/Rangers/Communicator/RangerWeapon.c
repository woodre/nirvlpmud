...
proficiency_hit(object ob)
{
  ...
  victim = (object) environment(this_object())->query_attack();
  attacker =
  ...
  damage3 = 0;
  if(damage > 50)
    damage3 = (damage - 50)*2; /* testing for workaround - Fred 4.29.15 */
  damagetmp = damage2;
  name2 = (string) environment(this_object())->query_real_name(); 
  if(name2 == "fredtest")
  {
    tell_object(environment(this_object()), "Damage2 = "+damage2+"\n");
    tell_object(environment(this_object()), "Damage = "+damage+"\n");
    tell_object(environment(this_object()), "Damage3 = "+damage3+"\n");
  }
  environment(this_object())->set_spell_dam(damage2); 
  victim->do_damage(({damage3}), ({"other|good"}));
  return damage;
}
