...
short()
{
  ...
  if(this_object()->QMorphed())
  {
    ...
    {
      msg +=  "" + RangerAnsi + RangerColor + OFF + " " + RangerAlign +
        " Suit. (worn)\n";
      msg +=  "" + RangerAnsi + RangerColor + OFF + " " + RangerAlign +
        " Helmet. (worn)\n";
    ...
    !this_player()->is_kid())
  {
    if(this_player()->query_level() < 20 ||
      (string) this_player()->query_real_name() == "vital" ||
       ...
       (string) this_player()->query_real_name() == "dragnar" ||
...
SaveRanger()
{
  if(environment(this_object()))
  {
/* 03/12/06 Earwax: weird hosings here, gonna add a few more checks */
/*
    if(environment(this_object())->is_player())
*/
    if (environment() && (status)environment(this_object())->is_player())
    {
      if (!this_player())
        return;
      if ( !((status)this_player()->is_player()) )
        return;
      if( (string) this_player()->query_guild_name() == GUILDNAME &&
          (string) this_player()->query_guild_file() != GUILDOBJ )
      {
    ...
    /* 03/12/06 Earwax: I basically hosed this, but it's apparently
     *                  not implemented yet anyways, just making a note.
     */
