...
Vital with (serious) suggestions.\n");
  return 1;
}
...
RangerPower(string str, string type)
{
  ...
  if(str == "fred") { write("Fred is too sneaky to pinpoint his location...\n"); return 1; }
...
/*   testing teleport with locator --Vital 2007-11-05
...
*/
    else
    {
      tell_room(environment(this_player()),this_player()->query_name()+
        ...
        " out of sight.\n"+
        "");
      if(living(environment(locator)))
        if(environment(environment(locator)))
          move_object(this_player(), environment(environment(locator)));
      else
