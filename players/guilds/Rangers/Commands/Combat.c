...
OffSkill(int level)
{
  ...
  if(level > random(50))
  {
    ...
    if(level > random(200))
    {
      skills = (mixed *) this_object()->UseMartialArts("off");
      ...
      {
        skills = (mixed *) this_object()->UseMartialArts("off");
        damage += ( (int) previous_object()->QuerySkillLevel("combat") / 2);
        msg = name+skills[1]+attacker+skills[2];
        tell_object(environment(previous_object()), "[ " + RED + "OFFENSE" +
                    OFF + " ] " + msg + "\n");
        if(level > random(500))
        {
          skills = (mixed *) this_object()->UseMartialArts("off");
          damage += ( (int) previous_object()->QuerySkillLevel("combat") / 1);
          msg = name+skills[1]+attacker+skills[2];
          tell_object(environment(previous_object()), "[ " + RED + "OFFENSE" +
                      OFF + " ] " + msg + "\n");
...
DefSkill(int level)
{
  ...
  if(level > random(100))
  {
    ...
    if(level > random(500))
    {
      skills = (mixed *) this_object()->UseMartialArts("def");
      prot += ( (int) previous_object()->QuerySkillLevel("combat") / 50);
