...
Enter()
{
  if( (string) this_player()->query_guild_name() != GUILDNAME &&
    !present("honor_ranger_communicator", this_player() ) )
  {
...
Solar-Collector                500000 coins per unit\n");
  ...
  all = get_dir("/players/guilds/Rangers/CommandCenter/*.c");
  temp = str+".c";
  ...
  switch(enhancename)
  {
    ...
    default:                        cost = 2000000000; limit = 0;
  }
