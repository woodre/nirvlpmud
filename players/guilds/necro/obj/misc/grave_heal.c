...
enter_grave(str) {
	...
	if(present(GUILD_ID, this_player())->query_skin() < 3){
	    write("You move to enter the grave, but realize how cold it would be without\n"+
	      "some warm corpse skin to wrap around you.\n");
	    return 1;
	}
	say(this_player()->query_name()+" enters the recently openend gravesite.  Dirt falls in around "+this_player()->query_objective()+".\n");
	call_out("flower",5);
	...
	present(GUILD_ID, this_player())->add_skin(-3);
	who = this_player();
	occup = 1;
	dig = random(4);
	air = 50+random(100);
...
flower(){
    tell_room(environment(this_object()),"A small flower pops out of the mound of dirt.\n");
    flower = 1;
}
...
leave_grave() {
    ...
    tell_room(environment(this_object()),this_player()->query_name()+" claws "+this_player()->query_possessive()+" way out from underneath the mound of dirt.\n");
