...
string long_desc, from_where;

inherit "/obj/treasure.c";
reset(int arg) {
   if(arg) return;
   set_id("clan_room_long_builder");
   set_long("There is no clan_room_long_builder here.\n");
}
...
int build_long(string str)
{
   ...
   if(str == "**")
      ...
      long_desc = (string)call_other(TOP_DIR + "funcs","modify_spell_message",long_desc, this_player());
      write("Setting long description to:\n" + long_desc + "\n");
from_where->set_long(long_desc);
      from_where->save_me();
      destruct(this_object());
      return 1;
   }
...
start_long()
{
   ...
   long_desc = "";
   from_where = "/" + file_name(previous_object());
