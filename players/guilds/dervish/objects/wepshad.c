...
dual_message(num) {
  ...
  if(ran == 1) {
    TE(wielder, color+"The Sands bless your "+weaponname+".\n"+OFF);
   return 1; }
  if(ran == 2) {
    TE(environment(weapon), color+"You twirl your "+weaponname+OFF+color+" with deadly skill.\n"+OFF);
   return 1; }
  if(ran == 3) {
    TE(environment(weapon), color+"Your "+weaponname+OFF+color+" glows with the power of the Sands.\n"+OFF);
   return 1; }
  if(ran == 4) {
    TE(environment(weapon), color+"You feel at one with the Sands.\n"+OFF);
   return 1; }
  if(ran == 5) {
    TE(environment(weapon), color+"Chaotic forces burn inside you.\n"+OFF);
   return 1; }
  if(ran == 6) {
    TE(environment(weapon), color+"Your "+weaponname+OFF+color+" twirls with the force of the Sands.\n"+OFF);
   return 1; }
  if(ran == 7) {
    TE(environment(weapon), color+"The Sands swirl in the ecstasy of Death.\n"+OFF);
   return 1; }
  if(ran == 8) {
    TE(environment(weapon), color+"Your weapons spin in twin dances of Death.\n"+OFF);
   return 1; }
  if(ran == 9) {
    TE(environment(weapon), color+"Your "+weaponname+OFF+color+" merges in the chaos of Sand.\n"+OFF);
   return 1; }
  return 1; }
