...
drain_spell(string str){
  ...
  if(!GOB->query_skills("drain")) return 0;
  ...
  if(!GOB->spell_failure(4)) return 1;
  if(GOB->casting_check()) return 1;
  gxp = (int)GOB->query_guild_exp();
  if((string)item->query_info() == "csw")
  {
    ...
    a = GOB->query_endurance();
    b = GOB->query_max_endurance();
    GOB->add_endurance(b-a);
    destruct(item);
    User->recalc_carry();
    return 1;
  }
  ...
  if(val > 50)
    val = 50;
  if(GOB->query_guild_exp() >= 8000000)
    guild_bonus = 40;
  else if(!GOB->query_guild_exp() <= 0)
    guild_bonus = (int)GOB->query_guild_exp() / 200000;
  else
      guild_bonus = 0;
  if(gxp < 650000)
    GOB->add_endurance(3);
