...
shadowblast(str){
  ...
  if(!GOB->query_skills("blast")) return 0;
  ...
  if(!random(3)){  if(!GOB->spell_failure(2)) return 1; }
  ...
  dmgx = (int)GOB->query_guild_exp() / 250000;
  temp = 0;
...
/*      msg2 = "A blast of dark flames engulfs you.\n"; */
  ...
  if(sps < (target->query_mhp() / 15) && (sps < 50) && !random(2) &&
    GOB->query_guild_exp() > 4000000 && !target->is_player())
  {
