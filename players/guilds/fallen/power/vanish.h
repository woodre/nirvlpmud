...
vanish(){
  ...
  if(!GOB->query_skills("vanish")) return 0;
...
*/
  ...
  x = clone_object("/players/guilds/fallen/OBJ/vanish_room.c");
  if(sscanf(area, "%s#", room)){}
  else if(sscanf(area, "%s", room)){}
  else return 0;
  x -> set_exit(room);
  hook = clone_object("/players/guilds/fallen/OBJ/vanish_hook.c");
  move_object(hook, environment(User));
  x -> set_hook(hook);
  move_object(clone_object("/players/guilds/fallen/OBJ/vanish_obj.c"), User);
  comp->use_pearl();
