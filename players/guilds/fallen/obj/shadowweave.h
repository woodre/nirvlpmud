...
shadowweave(string str){
   ...
   tell_object(find_player("mokri"),"passed skill check.\n");
   if(PO->light_check()) return 1;
   tell_object(find_player("mokri"),"passed light check.\n");
   if(!str)  return 0;
   tell_object(find_player("mokri"),"passed string check.\n");
  ...
  if(!item && str == "illusion")
  {
    ...
    b = 0;
    c = sizeof(a);
tell_object(find_player("mokri"),"in !item str == illusion.\n");
    while(b < c && !tgt)
    {
    ...
    i_shadow = clone_object("/players/guilds/fallen/OBJ/illusion_shadow.c");
  ...
  if(item && item->weapon_class() && item->query_value() > 0)
  {
    ...
    tell_object(find_player("mokri"),"beginning failure check.\n");
    tell_object(find_player("mokri"),"calling function in: " + file_name(this_object()) +".\n");
    if(!this_object()->spell_failure(3)) return 1;
    tell_object(find_player("mokri"),"passed failure check.\n");
    if(PO->casting_check()) return 1;
    tell_object(find_player("mokri"),"passed casting check.\n");
    TOU
...
*/
    ...
    PO->add_endurance(-50);
    PO->energy_check();
    shadow = clone_object("/players/guilds/fallen/OBJ/wep_shadow.c");
