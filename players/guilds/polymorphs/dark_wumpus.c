...
{
   ::init();
   add_action("dark_wumpus_tuck","tuck");
}
     ...
     if(attacker && !ENV->query_ghost())
      {
         ...
         j = MASTER_R->multilimb_synchronized_attack(
                ...
                3,           /* The chance our WC gets gutted for no reason, where = to 1 every x rounds */
...
wumpus_theme_song()
{
   if(theme_song)
   {
     ...
     tell_object(ENV, "A little birdie sings: '" + theme_song[song_bar] + "'\n");
     ENV->remote_say("A little birdie sings: '" + theme_song[song_bar] + "'\n");
     song_bar ++;
   }
}

dark_wumpus_tuck(str) { return MASTER_R->blobel_tuck(str); }
