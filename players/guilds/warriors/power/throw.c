/*  
    ...
    Skill Range: 1-5
    Cost: 20 sp
    Damage: r(wc of wep) + 0-9 + R(str/3) 
    Drawback: Requires a free wep
*/
...
main(string str, object gob, object player)
{
 ...
 if(call_other(wep,"id", "dagger") || call_other(wep, "id", "knife") ||
   ...
   if(!ok){ tell_object(USER, "You cannot throw that!\n"); return 1; }
	...
	switch((random(GOB->get_skill(15)) * 20) + 1){
		case   95..1000: where = "head"; bonus = 9; break;
		case   85..94: where = "neck"; bonus = 8; break;
		case   75..84: where = "stomach"; bonus = 6; break;
		case   60..74: where = "chest"; bonus = 4; break;
		case   45..59: where = "thigh"; bonus = 3; break;
		case   30..44: where = "shoulder"; bonus = 2; break;
		case   15..29: where = "arm"; bonus = 0; break;
		case    1..14: where = "leg"; bonus = 0; break;
   ...
   spamount = 20;
if(!ks){
	damage = random(wep->weapon_class()) + bonus + random(STR/2);
   if(bonus2) damage += random(bonus2);

	tell_object(USER,
	BOLD+"<"+NORM+RED+"<"+HIR+"<>"+NORM+RED+">"+NORM+BOLD+">"+NORM+"\n"+
   ...
   if(GOB->get_skill(6)) bonus2 += GOB->get_skill(6);
   damage = random(14) + bonus + random(STR/3);
   if(bonus2) damage += random(bonus2);
   ...
   GOB->ast(15);
   GOB->ast(15);
	USER->spell_object(ATT, "thrown weapon", damage, spamount,
	msg, msg2, msg3);
	return 1;
}
