...
main(string str, object gob, object player)
{
	...
	tell_object(USER, "    Level "+pad(USER->query_level()+"+"+USER->query_extra_level(), 30)+"");
        ...
        tell_object(USER, "   Food ["+BOLD+USER->query_stuffed()*100/(USER->query_max_stuff())+NORM+"%]\n");
	tell_object(USER, "    Exp   "+pad(USER->query_exp(), 30)+"");
        tell_object(USER, "   Soak ["+BOLD+USER->query_soaked()*100/(USER->query_max_soak())+NORM+"%]\n");
	tell_object(USER, "    Coins "+pad(USER->query_money(), 30)+"");
        tell_object(USER, "   Intox["+BOLD+USER->query_intoxination()*100/(USER->query_max_intox())+NORM+"%]\n\n");
   ...
   else if(GOB->query_glvl() < 60){
   tell_object(USER, "Prof Exp ["+GOB->query_gxp()+"/1000000]\t");
   }
   else{
   tell_object(USER, "Prof Exp ["+HIR+"MAX"+NORM+"]\t");
	...
	tell_object(USER, HIW+"   Type 'skills' for a listing.\n");   
        tell_object(USER, HIW+"   Carrying "+this_player()->query_pct_weight()+"% of max.\n");
   ...
   if(GOB->query_paladin() && GOB->query_chosen_align() == "good")
   tell_object(USER, HIB+"   Type 'paladin' for help on the Good Paladin Profession.\n"+NORM); 
   if(GOB->query_paladin() && GOB->query_chosen_align() == "evil")
   tell_object(USER, HIR+"   Type 'paladin' for help on the Evil Paladin Profession.\n"+NORM); 
   if(GOB->query_knight())
   tell_object(USER, HIB+"   Type 'knight' for help on the Knight Profession.\n"+NORM);
   if(GOB->query_berserker())
   tell_object(USER, HIB+"   Type 'berserker' for help on the Berserker Profession.\n"+NORM);
   if(GOB->query_assassin()) 
   tell_object(USER, HIB+"   Type 'assassin' for help on the Assassin Profession.\n"+NORM);
   tell_object(USER, "\n\n\n"+NORM);
