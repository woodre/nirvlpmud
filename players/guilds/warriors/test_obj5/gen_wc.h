...
gen_wc_bonus(){
   ...
   if(!USER) return;
	...
	if(assassin){
		wep_bonus += "/players/guilds/warriors/test_obj5/gen_wc.c"->main(0, this_object(), USER);
	}
   if(monk && mtime < time() && skills[80] > 0 && !ob){
		if(USER->query_local_weight() < 25 &&
			random(1050) < ((skills[80] * 18) - USER->query_local_weight())){
          mtime = time();
          	tmp_bonus = random(monk_bonus());
          	mess = USER->query_message_hit(tmp_bonus);
       tell_object(USER,
	...
	if(monk)
        {
          wep_bonus += monk_bonus();
          dmg = 0;
          dmg = monk_dmg;
          dmg_type = monk_dmg_type;
        }          
