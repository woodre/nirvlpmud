...
*/
   ...
   if(touch){ tell_object(USER, HIW+"  ["+NORM+BOLD+"Touch"+HIW+"]"+NORM); }
   if(regen_aura){ tell_object(USER, HIW+"  ["+NORM+GRN+"Regen Aura"+HIW+"]"+NORM); }
   if(good_aura){ tell_object(USER, HIW+"  ["+HIR+"Prot from Good"+HIW+"]"+NORM); }
   if(evil_aura){ tell_object(USER, HIW+"  ["+HIY+"Prot from Evil"+HIW+"]"+NORM); }
   if(riposte){   tell_object(USER, HIW+"  ["+HIR+"Riposte"+HIW+"]"+NORM); }
...
query_max_end(){ return ((glvl - 40) * 20) + 10 + USER->query_attrib("wil"); }
