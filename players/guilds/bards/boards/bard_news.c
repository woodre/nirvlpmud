...
#define ERROR_LOG "/players/guilds/bards/log/BOARD"
#define BOARD_NAME "players/guilds/bards/boards/bardnotes"
...
id(str)
{
	return str == "book" || str == "large book";
}
...
new_msg(msg_head)
{
	...
	say(capitalize(curr_writer->query_name()) +
		" starts writing a note in the book.\n");
	new_head = msg_head;
	new_text = "";
	input_to("get_msg");
...
object inst;
  ...
  inst=present("bard_obj",this_player());
  if(!inst || !inst->query_is_gc()) {
    write("Only Guild Commanders may remove other player's notes.\n");
    return 1;
  }
/*
