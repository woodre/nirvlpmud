...
shape(str)
{
	...
	if (!str || sscanf(str, "%s %s %s", name, stuff, detail) != 3) {
	...
	if (stuff == "shine") {
		...
		write("You shape the " + OBJ +
			  " so that it shines like a torch.\n");
	...
	if (stuff == "xname") {
		...
		write("You have given " + OBJ + " the additional name of " +
			  detail + ".\n");
	...
	if (stuff == "name") {
		if (sscanf(str, "%s %s %s | %s | %s", name, stuff, AA, BB, CC) !=
			5) {
			write
				("Name - S strand name <name> | <appearance> | <common name>\n"
