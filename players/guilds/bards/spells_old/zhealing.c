...
bury_corpse(str)
{
	...
	corpse = clone_object("/players/guilds/bards/marker");
...
object woo;
	...
	move_object(clone_object("/players/guilds/bards/regen.c"),
				this_player());
...
if(!living(woo) || !woo->is_player()){
	...
	move_object(clone_object("/players/guilds/bards/regen2.c"),
				woo);
	call_other(TP, "add_spell_point", -100);
	return 1;
    }
