...
status is_bard(object ob) {
  if(present("bard_obj",ob)) return 1;
  return 0;
}
bard_kin() {
    ...
    ob = sort_list(users());
    ob = filter_array(ob,"is_bard",this_object());


    write("\n");
    write(HIM+
        "()-<>-()-<>-()-<>-()-<>-()-<>-()-<>-()-<>-()-<>-()-<>-()-<>-()-<>-()\n"+
        "()"+NORM+BOLD+"  Name        Bard Lvl    Gender     Title                      "+NORM+HIM+"()\n"+
        NORM);
	...
	if (present("bard_obj", ob[b])) {
		...
		if(ob[b]->query_invis()) name+="*";
		if(!ob2->query_on()) name+="#";
		write(pad(name, 15));
		write(pad(ob2->query_bard_level(), 11));
		write(pad(ob[b]->query_gender(), 9));
		write(ansi_pad(ob[b]->query_title(),27));
		write(HIM+" ()\n");
