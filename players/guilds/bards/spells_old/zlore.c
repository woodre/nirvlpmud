...
true_name(str)
{
   ...
   if(time_delay >= time()) return 1;
	...
	if (ccc > ob->query_hp())
		ccc = ob->query_hp() - 1;
	ob->hit_player(ccc,"magical");
	if (living(ob))
		ob->attack_object(TP);
 	TP->attack_object(ob);

 if(!present("acwc_modifier", ob)){
   mod = clone_object("/players/guilds/bards/obj/mob_mod.c");
   move_object(mod, ob);
   mod->originals();
   numero = random(5) + 3;
while(numero){
...
reflect()
{
	...
	move_object(clone_object("/players/guilds/bards/pool.c"),
				environment(TP));
	call_other(TP, "add_spell_point", -100);
	return 1;
}
...
wander_music(str)
{
	...
	if (str == "music") {
		if (spell(-195, 5, 25) == -1)
			return 0;
		move_object(clone_object("/players/guilds/bards/music.c"),
					environment(TP));
	...
	if (spell(-111, 5, 25) == -1)
		return 0;
	move_object(clone_object("/players/guilds/bards/music2.c"),
				environment(TP));
