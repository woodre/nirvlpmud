...
create()
{
	if (!root()) {
		object mark;
		mark = clone_object("/players/guilds/bards/bard_mark");
...
monster_died()
{
	...
	if (ob = query_master()) {
		ob->hit_player(random(self_damage));
...
heart_beat(){
	...
	if(query_hp() < query_mhp()) heal_self(2);
}
...
follow_sound()
{
	return (THEPET + " flies from the room after its master.\n");
}
