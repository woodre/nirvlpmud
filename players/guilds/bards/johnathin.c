...
new_sp_say(str)
{
	...
	if (sscanf(str, "%sscroll%s", temp, temp) ) {
                if (1) { /* untangle later - Ill */
			COUNT = 0;
...
/* 12/14/06 Earwax: changed this cuz it's screwy, from 6 to 7 */
			...
			if (COUNT < 1) {
				...
				move_object(clone_object
							("/players/guilds/bards/scroll6.c"),
							this_player());
				return 0;
			}
		}
...
finish_heynonny(object play) {
  object gob;
  gob=present("bard_obj",play);
  if(!gob || gob->query_bard_level()!=1)
    return;
  tell_object(play,
"Johnathin looks over at you and smiles.  He says, \"That was very well\n"+
"done.  I think you deserve a promotion.\"\n");
  tell_room(environment(),format(
"Johnathin looks over at "+play->query_name()+" and smiles.  He says, "+
"\"That was very well done.  I think you deserve a promotion.\"\n"),
    ({this_object(),play}));
  gob->set_bard_level("2");
  gob->set_bard_xp(1000);
  return;
}
