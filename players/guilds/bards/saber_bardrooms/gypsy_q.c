...
reset(int arg){
if(count2 > 3) count = count2 = count3 = 0;
if(!present("children"))  {
move_object(clone_object("/players/saber/monsters/gypsy_kids.c"),this_object());  }
if(!present("group"))  {
move_object(clone_object("/players/saber/monsters/gypsy_p.c"),this_object());  }
 if(!arg){
  set_light(1);
...
items=({
 ...
 "ring","The camp is encircled by a small ring of brightly colored wagons",
 "men","There are many dark skinned gypsy men here",
 "women","There are many dark skinned gypsy women here",
 "children","There are many dark skinned gypsy children here",
 "wagons","There are 7 brightly colored wagons here",
 "tents","There are a few tents in the camp sight",
 "wagon","There is a single wagon decorated with silver runes and symbols here",
 });
...
SIB = "sister";
if(this_player()->query_gender() == "male") { SIB = "brother"; }
if(count < 1)  {
count = count + 1;
call_out("intro",3);
return 1;
...
begin1()  {
 ...
 call_out("begin2",7);  return 1;  }

begin2()  {
 ROM \"\Look!\"\n"+
   "Crystal points across the clearing.\n"+
   "\nA massive Troll bursts through a wagon into the clearing!\n\n");
move_object("/players/saber/monsters/gypsy_troll.c","/players/saber/closed/bards/bardrooms/gypsy_q.c");
 call_out("begin3",6);  return 1;  }
...
move_object(clone_object("/players/saber/mini/gypsy.c"),environment(OB));
  return 1;  }
...
if(temp == "my own" || temp == "mine" || temp == "myself" || temp == "none" || temp == "nobody" || temp == "nobodies")  {
if(count2 == 0)  { 
  correct();
  call_out("begin1",2);
  return 1;
