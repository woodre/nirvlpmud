...
#define BOARD_NAME "players/saber/closed/bards/bardpoetry"
   ...
   if(arg)
     if(!random(5)) {
       say("A small gnome appears and secures some "+
           "poetrys on the board that were loose.\n");
...
get_msg(str) {
   if(str == "~q") {
     say(capitalize(curr_writer->query_name())+" aborts writing a some poetry.\n");
     write("Note aborted.\n");
     curr_writer = 0;
     new_head = "";
     new_text = "";
...
delete_msg(what_msg) {
   ...
   say(capitalize(this_player()->query_name())+" removes some poetry titled '"+
       headers[poetry]+"'.\n");
   ind = poetry;
   while(ind < msg_num - 1) {
