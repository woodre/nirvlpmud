...
move_object(mark,this_object());
   ...
   if (!arg){
     ...
     set_chat_chance(3);
     set_a_chat_chance(2);
     load_chat("The hawk ruffles its feathers.\n");
     load_chat("The hawk looks at you.\n");
     load_chat("The hawk soars high overhead.\n");
     load_chat("The hawk flies high into the sky and circles.\n");
     load_chat("The hawk gazes at something.\n");
     load_a_chat("The hawk circles its foe.\n");
      load_a_chat("The hawk screams as it rakes its foe.\n");
     set_spell_mess1("The hawk rakes the face of its foe with razor sharp claws.");
     set_spell_mess2("The hawk rakes your face with razor sharp claws.");
     set_chance(15);
...
say("The hawk flies off into the sky.\n");
...
if(!ob) { return 1; }
say("The hawk screams and flies towards "+capitalize(str)+".\n");
...
tell_room(environment(this_object()),
   "The hawk falls to the ground with a thud.\n\n");
tell_room(environment(this_object()),
...
if(what == "instrument" || what == "base_obj" || what == "rock" || what == "soul")  { return 1; }
  if(flag)  {
  say("The hawk flies off into the sky.\n\n"+
     "The hawk returns and nods to "+this_object()->query_master()+".\n");
  } else {
  say("The hawk takes an item from "+this_object()->query_master()+" and flies off into the sky.\n\n"+
     "The hawk returns and nods to "+this_object()->query_master()+".\n");
         }
  tell_room(environment(target), "A hawk flies in and gives a "+what+" to "+who+".\n\nThe hawk flies off into the sky.\n");
...
object corpse;
 ...
 if(!corpse)  { notify_fail("There is no corpse here.\n"); return 0; }
  say("The hawk descends upon the corpse and plucks out its eyeballs.\n");
  destruct(corpse);
  heal_self(20);
  return 1;
         }
...
switch(word)  {
  ...
  default: notify_fail("Command the hawk to do what?\n"); return 0; break;
  }
       }

talk()  {
this_object()->set_chat_chance(5);
write("You look over at your hawk and smile.\n");
return 1;
        }

silence()  {
this_object()->set_chat_chance(0);
write("You look over at your hawk and shake your head.\n");
  return 1;
        }
...
if(!present(master,environment(this_object())))  {
  say(
   "The hawk flies from the room after its master.\n");
  move_object(this_object(),environment(there));
  say(
   "A large hawk flies into the room.\n");
        }
call_out("pet_follow",6);
  return 1;
        }
...
string who_to,what;
object target, item;
if(!str)  { notify_fail("Have the hawk give you what?\n"); return 0; }
if(sscanf(str,"%s to %s",what,who_to) < 2)  {
    notify_fail("Have the hawk give you what?\n");  return 0;  }
target = present(who_to,environment(this_object()));
item = present(what,this_object());
if(!target || !living(target))  {
  notify_fail("Have the hawk give the "+what+" to who?\n"); return 0; }
if(!item)  {
  notify_fail("The hawk does not have a "+what+".\n"); return 0; }
if(what == "instrument" || what == "base_obj" || what == "rock" || what == "soul")  { return 1; }
tell_room(environment(this_object()),
  "The hawk drops a "+what+" into "+capitalize(who_to)+"'s hand.\n");
move_object(item,target);
  return 1;
        }
...
tell_room(environment(this_object()),
  ...
  "\n       \"\This hawk is under my protection.\"\n\n"+
...
tell_room(environment(this_object()),
  ...
  write("Your hawk is not under attack by a player or pet or kid.\n");
  return 1;
        }
