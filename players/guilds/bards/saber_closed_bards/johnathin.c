...
if (!arg){
     ...
     set_race( "human");
     set_long("Johnathin de Oro is a young swashbuckler dressed in fine clothing who wears\n"+
     ...
     load_chat("Johnathin de Oro whispers something to a passing barmaid.\n");
     load_chat("Johnathin de Oro waves to somebody in the common room.\n");
     load_chat("Johnathin de Oro says: Don't I know you?\n");
     load_chat("Johnathin de Oro says: I know almost everything about Ryllian.\n");
     load_chat("Johnathin de Oro says: You should ask me a question.\n");
     load_a_chat("Johnathis de Oro smiles wickedly.\n");
     load_a_chat("Johnathin de Oro parries a blow.\n");
     load_a_chat("Johnathin de Oro punches you in the face with the pommel of his sword.");
     load_a_chat("Johnathin de Oro says: You cannot be this stupid!  Give up now!\n");
...
if(temp2 == "task")  {
if(number == 1)  {
  tell_room(environment(this_object()), capitalize(this_player()->query_name())+" says: "+str+"\n");
...
if(!present("instrument",this_player()))  { return 1; }
target = present("instrument",this_player());
if((target->query_bard_level()) !=7)  { return 1; }
if(!present("pearl",this_player()))  { NOPE(); }
if(!present("wine",this_player()) || !present("bottle",this_player()))  { NOPE(); }
if(!present("bloodstone",this_player()))  { NOPE(); }
if(!present("medallion",this_player())) { NOPE(); }
  if(COUNT < 1)  {
say("Johnathin says \"\Here you go "+this_player()->query_name()+".\"\n"+
   "Johnathin hands "+this_player()->query_name()+" a scroll.\n\n");
move_object(clone_object("/players/saber/closed/bards/scroll6.c"),this_player());
  return 1;
        }
        }
        }
...
NOPE()  {
write("Johnathin says \"\You must have all 4 items to make the scroll.\"\n");
COUNT = COUNT + 1;
  return 1;
        }
...
if(verse == "1")  {
if(count == 0)  { count = count + 1; } else { wrong(); }
        }
if(verse == "2")  {
if(count == 1 || count == 3)  { count = count + 1; }
    else { wrong(); }
if(count == 4)  { finish(); }
        }
if(verse == "3")  {
if(count == 2)  { count = count + 1; } else { wrong(); }
        }
...
finish()  {
object obj;
say("Johnathin looks over at "+this_player()->query_name()+" and smiles.\n");
