...
reset(arg)
{
	...
	ob2 = clone_object("/players/guilds/bards/bard_mark.c");
...
catch_tell(str) {
   object ob, target;
   int temp, number, b;
   int xxx; /* temp by mizan */
   string a,c,d;
   string x1, x2;
   string say_temp; /* temp output from "mizan says: say_temp"

   say("DEBUG: catch_tell() tripped.\n");
/*   xxx = sscanf(str, "%s"+NORM+"%s"+HIM+"%s", a,c,d); */
   xxx = sscanf(str, "%s %s \"%s\"", a,c,d);
   say("DEBUG: a = " + a + ", c = " + c + ", d = " + d +"\n");

   if(xxx !=3)
   {
       say("DEBUG: sscanf failed.. arg of " + xxx + " reported.\n");
       return 0;
   }
   
   say("DEBUG: arg process is ok.\n");
   say_temp = d;
   ob = present("instrument", this_player());

   if (ob)
   {
       say("DEBUG: User has instrument.\n"); 
       if (sscanf(say_temp, "%s %d %s", a,b,c)==3 ||
            sscanf(say_temp, "%s %d", a,b) == 2 ||
            sscanf(say_temp, "%d %s", b,c) == 2)
        {
            say("Star says \"\If you want to say a number to me, say only the number.\"\n");
            return 0;
        }

        say("DEBUG: number testing passed.\n");
 	



/* >>>>>>>>>>>>>> Shit is breaking RIGHT HERE */
/*
xxx = sscanf(say_temp, "%s%d%s", x1, temp, x2);
*/
say_temp = delete_colour(say_temp);
        xxx = sscanf(say_temp, "%d", temp);
say("XXX IS: "+xxx+"\n");
say("TYPEIS: "+typeof(say_temp)+"\n");
/* does not convert from string to int */

        number =(TP->query_attrib("int")) + (TP->query_attrib("mag")) +
                (TP->query_attrib("pie")) + (TP->query_attrib("wil"));

        say("DEBUG: sscanf to temp is " + temp + ", initial say_temp = " + say_temp + ", strlen(say_temp) = " + strlen(say_temp) + "\n");
        
	if(temp != 0)
        {

           say("DEBUG: Innerloop entry - number parsed to int is: " + temp + "\n");

           if (ob->query_bard_level() == 2 && ob->query_approval() == 3)
           {
               if (temp == number)
               {
                   say("DEBUG: Success. Proceed to advance() function.\n");
                   advance();
                   return 0;
               }
               ...
               if (temp != number) 
               {
                   say("Star says \"\ "+TP->query_name()+" that is not the number.\"\n");
                   say("DEBUG: Failure. The number is incorrect.\n");
               }
	    }

	    if (ob->query_bard_level() < 2) 
               say("Star says \"\ "+TP->query_name()+" has a Bard level is too low.\"\n");

            if (ob->query_bard_level() > 2)
               say("Star says \"\ "+TP->query_name()+" has a Bard level is too high.\"\n");

            if (ob->query_approval() != 3)
               say("Star says \"\ "+TP->query_name()+" has not been approved.\"\n"); 
        }    
   }

   say("DEBUG: The magic number is " + number + "\n");
   say("DEBUG: return 0 passalong... end of function.\n");
   return 0;
}  
