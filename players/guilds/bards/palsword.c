...
reset(arg)
{
	...
	if (arg)
		return;
	set_name("holysinger");
	set_alt_name("holy blade");
	set_short("A blessed blade named 'Holysinger'");
	...
	set_read("This blade was forged by Saber for the Paladin's Guild\n");
	set_class(18);
	set_weight(2);
	set_value(2000);
...
weapon_hit(attacker)
{
	...
	if (w < 36) {
		...
		if (w < 2) {
			write
				("Holysinger gleams with a pure white fire as it cleaves through your foe.\n");
			write(capitalize(attacker->query_real_name()) +
				  " staggers.\n");
			say(tp +
				"'s Holysinger blade gleams with a pure white fire as " +
		...
		if (w == 2) {
			ROM(" " + tp +
				"'s sword sings \"The Ride of the Valkyries\"\n\n");
		}
		if (w == 3) {
			ROM("  " + tp +
		...
		if (w == 6) {
			ROM("  " + tp +
				"'s sword sings \"Allegro Molto Vivace\" from Symphony No.6 \"Pathetique\"\n\n");
		}
		if (w == 7) {
			ROM("  " + tp +
		...
		if (w == 12) {
			ROM("  " + tp +
				"'s sword sings \"Sturmisch Bewegt\" from \"Titan\"\n\n");
		}
		if (w == 13) {
			ROM("  " + tp +
		...
		if (w == 32) {
			ROM("  " + tp +
				"'s sword sings the Allegretto from \"Faust\"\n\n");
		}
		if (w == 33) {
			ROM("  " + tp +
...
new_wield(str)
{
	if (str == "holysinger") {
		...
		say("\n" + tp + " is surrounded by a blue aura as " +
			this_player()->query_pronoun() + " wields HOLYSINGER.\n\n");
		return 0;
	}
}
