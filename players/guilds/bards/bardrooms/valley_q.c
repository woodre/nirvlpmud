...
reset(int arg)
{
	if (!arg) {
		...
		dest_dir = ({ "/players/saber/ryllian/mountain2.c", "east", });
...
begin()
{
	...
	if (count < 1) {
		count = count + 1;
		move_object(clone_object("/players/saber/monsters/giant_q.c"),
...
quest6()
{
	tell_room(environment(ACE), "The giant begins to dance.\n");
	call_out("quest7", 2);
	return 1;
}
...
quest7()
{
	tell_room(environment(ACE), "The giant dances.\n");
	call_out("quest8", 2);
	return 1;
}
...
quest21()
{
	tell_room(environment(ACE), "The giant begins to cry.\n");
	call_out("quest22", 2);
	return 1;
}
