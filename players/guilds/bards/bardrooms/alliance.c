...
int t;
reset(int arg){
if(!present("templar_guard")){
move_object(clone_object("players/maledicta/templar/mobs/high_templar_bard.c"), this_object());
}
if(!present("fireplace")){
move_object(clone_object("players/saber/fur/fireplace.c"),this_object());
}
if(!present("laura")){
move_object(clone_object("players/guilds/bards/laura.c"),this_object());
}
 if(!arg){
  set_light(1);
...
items=({
 ...
 "portal","A portal leading to the realm of dreams",
  ...
  dest_dir=({
 "/players/guilds/bards/bardrooms/enterance.c","down",
           });
  }   }
init(){
 ::init();
