...
reset(int arg)
{
	if (!arg) {
		...
		items = ({
			...
			"pentagram",
			"There is an inverted black pentagram is the middle of the chamber", });

		dest_dir = ({
			"/players/guilds/bards/bardrooms/teleport.c", "out", });
...
begin()
{
	...
	if (count < 1) {
		count = count + 1;
		move_object(clone_object("/players/saber/monsters/demon_q.c"),
...
a1()
{
   ROM(" \"Star is not in right now mortal, so it looks like you're going\n" +
...
a8()
{
	ROM(" \"Fair?  Fair?  This contest will be no such thing.\n" +
		"   I am going to hurt " + NAME + " here.  I am going to make " +
		GENDER + "\n" + "   scream for mercy.\"\n");
	call_out("a9", 7);
	return 1;
}
...
a9()
{
	...
	ROM2(" \"Well, I am here to make sure that you do not kill " +
		 GENDER + ".\n" +
...
{
	tell_room(environment(ACE),
			  "The Demon binds you in Crimson Magic.\n\n");
...
c2()
{
	...
	ROM2(" \"That is not within your rules.  You have no claim to " +
		 NAME + "'s soul.\"\n");
	call_out("c3", 6);
	return 1;
}
...
c3()
{
	ROM(" \"You're in my realm.  I MAKE the rules.  If " + NAME +
		" here says\n" + "   stop, I release " + GENDER +
		" unharmed.  But I get to keep " + GENDER + " soul.\"\n");
...
{
	tell_room(environment(ACE),
			  "The SOULBLADE bounces off a mystic blue shild inches before you...\n\n");
	call_out("e2", 3);
	return 1;
}
...
bad()
{
	...
	move_object(environment(ob2),
				"/players/guilds/bards/bardrooms/teleport.c");
	count = 0;
	destruct(GAG);
	return 1;
}
...
end_it_it()
{
	...
	ob2->set_bard_xp("128000");
	environment(ob2)->heal_self(350);
	command("true_name demon", environment(ob2));
	destruct(ACE);
	destruct(GAG);
	call_out("finish", 5);
	remove_call_out("e3");
...
oh_oh()
{
	...
	environment(ob2)->raise_magic_aptitude(2);
