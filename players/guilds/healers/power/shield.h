/*** Changes made on 4.23.01  Resistance = 24% max ***/
/*** Maledicta with Boltar's approval              ***/
/*** Do NOT change this back without Boltar's      ***/
/*** approval!                                     ***/
...
*/
    if(acer = present("shield_spell",targ)) {
       ...
       acer->charge(lvl*7); /* Increases duration */
     }
     else {
       acer = clone_object(root+"obj/shield_spell.c");
/****** Change made here. From 90% resistance at max, to 24% at max *****/
/****** Maledicta 4/01 with Boltars Permission *************/
      acer->start(targ, ((lvl*3)/5), lvl*10); /* target, resistance, duration */
       write("You touch "+capitalize(targ->query_real_name())+" and intone.\n"+
