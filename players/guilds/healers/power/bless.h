 ...
 bless(int lvl,int pi, string who) {
  object acer, targ; 
   ...
   if(targ->query_real_name() == "guest") { write("Guest may not be healed.\n"); return 1; }
     if(bless_var[1] > tsp) { write("You are low on energy.\n"); return 1; }
     if(bless_var[2] > thp) { write("You are low on physical energy.\n"); return 1; }
     if(bless_var[3] > tp->query_exp()) { write("You are low on exp.\n"); return 1; }
     if(bless_var[4] > tp->query_money()) { write("You are low on money.\n"); return 1; }
     tp->add_spell_point(-bless_var[1]);
     tp->add_hit_point(-bless_var[2]);
     if(hob) tp->add_xp(-bless_var[3]); else tp->add_exp(-bless_var[3]);
     tp->add_money(-bless_var[4]);  
     if(!check(lvl,pi,bless_var[6],bless_var[7],bless_var[8])) { 
