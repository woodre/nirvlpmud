...
check_heal(object ob) { 
   if(read_file("/players/guilds/healers/member/"+ob->query_real_name()+".o")) return 1;
   return 0;
}
...
int k;
  k = h = start = 0;
  meow = get_dir("/players/guilds/healers/member/");
  while(start < sizeof(meow)) {
    if(meow[start] != "left" && !sscanf(meow[start], "%s_LD.o", who)) { 
