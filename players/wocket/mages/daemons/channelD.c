...
viewhistory(){
  int i,a;
  string msg;
  if(this_player()->query_real_name() != "feldegast" && this_player()->query_real_name() != "wocket"){
    create_emote(this_player()," just tried to access the channel history");
    write("You are not allowed to access this history.\n");
    return 1;
  }
  msg = "\n";
