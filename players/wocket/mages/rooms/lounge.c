...
int cmd_polish(string str) {
  ...
  if(present("rag",this_player())) {
    ...
    say(this_player()->query_name()+" disappears into the lamp.\n");
    move_object(this_player(),ROOMDIR+"bottle.c");
  ...
  say(this_player()->query_name()+" tries to polish the lamp.\n");
  return 1;
}
