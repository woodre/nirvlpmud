#include "../def.h"
#define FILE OBJDIR+"/misc/postnotes"
...
id(str) {
  return str == "book" || str == "pedistal";
}

long() {
    write("A heavy book bound in leather and clasped in steel.  It rests\n"+
          "open on a pedistal next to a skull filled with inkish blood.  A quill\n"+
          "lays next to that.  The pedistal is carved delicately out of a solid\n"+
          "piece of oak, and the skull shine aside from a few drips of spilled\n"+
          "blood.  You may leave a note by 'post'ing it or 'read' the\n"+
          "notes that have been left there by others.\n");
    if (num_messages == 0) {
	write("It is empty.\n");
	return;
    }
...
short() {
  return("A heavy book upon a pedistal "+NORM+"("+BOLD+num_messages+" msgs"+NORM+")");
}

get() {
    write("It is secured to the pedistal.\n");
    return 0;
}
