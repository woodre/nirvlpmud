...
inherit "/players/vertebraker/closed/std/m_efuns.c";
/* conflicts with ansi.h no good for ld, sorted it out -Bp
...
#define BEGINING BLU+"( "+OFF+BOLD+"?"+OFF+BLU+" ) "+OFF
#define POETLOG "/players/wocket/junk/POETLOG"
#define POETENTER "/players/wocket/junk/POETENTER"
#define NATZI ({"feldegast","wocket","pan","jabberwocky","zoltan","adrianna", })
#define BANISHED ({ "pkme", "illarion" })
...
if( member_array(tpn,NATZI) != -1 ){
    ...
    if(member_array(tpn,BANISHED) != -1){
...
drop(){ return 1; }
get(){ return 0; }
query_auto_load(){ return "/players/wocket/junk/quill.c:"+MODE; }
...
*/
cuttitle = colour_pad((string)mem->query_title(),33);
...
if(member){
    if( (int) member->query_muffle() != 1){
    tell_object(everyone[i],str+OFF+"\n");
  }
}
}
    write_file(POETLOG,str+OFF+"\n");
}
...
int i,a;
string *temp;
  temp = read_file(POETLOG);
  temp = explode(temp,"\n");
  if(sizeof(temp) > 300){
    temp -= temp[0..250];
    rm(POETLOG);
    temp = implode(temp,"\n")+"\n";
    temp = implode(explode(temp,RED),"");
    temp = implode(explode(temp,BLU),"");
    temp = implode(explode(temp,GRN),"");
    temp = implode(explode(temp,CYN),"");
    temp = implode(explode(temp,MAG),"");
    temp = implode(explode(temp,YEL),"");
    temp = implode(explode(temp,BLK),"");
    temp = implode(explode(temp," [1;32m"),"");
    temp = implode(explode(temp," [0m"),"");
    temp = implode(explode(temp," [1m"),"");
    write_file(POETLOG,temp);
  }
...
string EXTRA;
  ...
  msg = BEGINING+this_player()->query_name()+EXTRA+MODE+", \""+str+"\"";
  if(MODE == "says"){
    if(str[i] == '?'){
      msg = BEGINING+this_player()->query_name()+EXTRA+"asks, \""+str+"\"";
      if(str[i-2..i] == "???"){
        msg = BEGINING+this_player()->query_name()+EXTRA+"wonders, \""+str+"\"";
      }
    } 
    if(str[i] == '!'){
      msg = BEGINING+this_player()->query_name()+EXTRA+"exclaims, \""+str+"\"";
      if(str[i-2..i] == "!!!"){
        msg = BEGINING+this_player()->query_name()+EXTRA+"yells, \""+str+"\"";
      }
    }
    if(str[i-2..i] == "..."){
      msg = BEGINING+this_player()->query_name()+EXTRA+"ponders, \""+str+"\"";
    }
  }
  if( str[0] == ':'){
    str = str[1..i];
    msg = BEGINING+this_player()->query_name()+" "+str;
  }
  channelsay(msg+" "+OFF);
  return 1;
}
