...
short() {
  return(YEL+"A cork bulletin board "+NORM+"("+HIG+num_messages+" msgs"+NORM+")");
}
...
reset(arg) {
    if (arg)
	return;
    restore_object("players/wocket/turtleville/OBJ/postnotes");
}
...
get_body(str) {
    if (str == "**") {
new_hd = HIG+cutstr(new_hd)+NORM+" ( "+this_player()->query_name()+" )";
	    ")";
	messages = messages + new_hd + ":\n**\n" + new_body + "\n**\n";
	num_messages += 1;
	new_body = 0;
	new_hd = 0;
    save_object("players/wocket/turtleville/OBJ/postnotes");
	write("Ok.\n");
	who = 0;
	return;
    }
