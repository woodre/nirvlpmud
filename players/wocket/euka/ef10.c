...
object tree;
  ...
  if(!present("tree",this_object())){
     ...
     tree->set_dir(EPATH+"eftd.c");
     move_object(tree,this_object());
  }  
  short_desc = GRN+"The Euka Forest"+OFF;
  long_desc = "   "+
  "The fine worn path splits here.  Leading off to the east a small\n"+
  "clearing can been seen.  The leaves above rustle with the gentle\n"+
  "winds as birds chirp in harmony.  Bushes, vines, and ferns litter\n"+
  "the surrounding areas blocking travel in those directions.\n";
  items = ({
  ...
  dest_dir = ({
  EPATH+"ef12.c","west",
  EPATH+"ef11.c","northwest",
  EPATH+"ef8.c","southwest",
  });
  set_light(1);
  get_monsters();
 }
