...
short() {
    return ("A stone tablet (" + num_messages+" msgs)");
}
...
reset(arg) {
    if (arg)
	return;
    restore_object("players/wocket/closed/guild/guildmsgs");
}
...
get_body(str) {
    if (str == "**") {
new_hd = CYN+new_hd+NORM+" ("+this_player()->query_real_name()+", "+ctime()+")";
	    ")";
	messages = messages + new_hd + ":\n**\n" + new_body + "\n**\n";
	num_messages += 1;
	new_body = 0;
	new_hd = 0;
  save_object("players/wocket/closed/guild/guildmsgs");
	write("Ok.\n");
	who = 0;
	return;
    }
