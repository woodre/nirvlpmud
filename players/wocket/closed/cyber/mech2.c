...
get_blunt_msg(amt) {
  string msg;
   if(amt > 69)  msg = "annihilates "+MEAN+" to smithereens";
   if(amt < 70)  msg = "crushes "+MEAN+" to small fragments";
   if(amt < 35)  msg = "smashes "+MEAN+" with a bonecrushing sound";
   if(amt < 25)  msg = "hits "+MEAN+" very hard";
   if(amt < 15)  msg = "hits "+MEAN+" hard";
   if(amt < 7)   msg = "leaves deep bruises on "+MEAN;
   if(amt == 0)
     return cap_name+" missed "+MEAN+".\n";
   else
     return cap_name+" "+msg+"!\n";
}
...
get_sharp_msg(amt) {
  string msg;
   if(amt > 69)  msg = "sends gobbets of "+MEAN+"'s flesh flying around";
   if(amt < 70)  msg = "chops "+MEAN+" to small fragments";
   if(amt < 35)  msg = "severely slices through "+MEAN;
   if(amt < 25)  msg = "cuts deeply into "+MEAN;
   if(amt < 15)  msg = "cuts into "+MEAN;
   if(amt < 7)   msg = "leaves deep scratches on "+MEAN;
   if(amt == 0)
     return cap_name+" missed "+MEAN+".\n";
   else
     return cap_name+" "+msg+"!\n";
...
get_laser_msg(amt) {
  string msg;
   if(amt > 69)  msg = "atomizes "+MEAN;
   if(amt < 70)  msg = "dices "+MEAN+" with laserfire";
   if(amt < 35)  msg = "slashes through "+MEAN+" with tangible light";
   if(amt < 25)  msg = "severely burns "+MEAN;
   if(amt < 15)  msg = "burns "+MEAN;
   if(amt < 7)   msg = "traces "+MEAN+" with burn lines";
   if(amt == 0)
     return cap_name+" missed "+MEAN+".\n";
   else
     return cap_name+" "+msg+"!\n";
