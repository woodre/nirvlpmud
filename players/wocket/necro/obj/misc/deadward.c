...
init(){
  ...
  if(call_other(tp,"id","undead") || call_other(tp,"id","zombie") ||
     call_other(tp,"id","undead") || call_other(tp,"id","ghost") ){
    write(BOLD+"\nThe ward flares in color.\n"+OFF);
    write("\nYou are pushed back against the room.\n");
    say(BOLD+"The ward flares against the presence of "+tp->query_name()+".\n"+OFF);
   if(direction)
