...
int monstercomp,mycomp;
  ...
  monstercomp = monster->query_composite();
  mycomp = tp->query_composite();
  write("You look over "+monster->query_name()+".\n  ");
  write(get_comp_diff(monstercomp,mycomp,monster->query_name()));
  return 1; 
}

get_comp_diff(int m,int p,string mon){
int dif;
string msg;
  dif = m - p;
  if(dif > 0){
    switch(dif){
      ...
      case 101..400:      msg = mon+" looks a little bigger then yourself.\n"+OFF;        break;
