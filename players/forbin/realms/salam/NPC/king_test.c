...
int FireBall_Damage, FireBall_Hits;
...
reset(arg)  {
  ...
  set_long(
    "  This dark red giant dwarfs you.  Dark, obsidian armor covers most\n"+
...
fireball() {
  ...
  ob = first_inventory(environment(this_object()));  
  while(ob) {
    if (ob->query_attack() == this_object() && environment(ob) == environment()) {
      target = ob;
      ob = next_inventory(ob);
      tell_room(environment(),
 ...
 *                           attacking hit poitnts in the room, it is
...
calculate_worth(){
  ...
  if(FireBall_Hits > 0) { 
    fireball_mod = (((((FireBall_Damage / FireBall_Hits) / 100) * 16) * 125) / 100);
    wc_bonus += fireball_mod;
  }                     /* else wc_bonus to prevent division by zero               */
  else wc_bonus += 20;  /* WC_bonus: lowest it can be is 10                        */                
...
monster_died() {
  ...
  string lev, xlev;
#ifdef DEBUG
  mdmsg("WC Bonus: "+wc_bonus+" AC Bonus: "+ac_bonus+" HP Bonus: "+hp_bonus+" Regular Round: "+Regular_Round+" Bezerk Round: "+Bezerk_Round);
#endif
#ifdef BIGKILLLOG
  who = this_player()->query_real_name();
  lev = this_player()->query_level(); xlev = this_player()->query_extra_level();
  log_it(BIGKILLLOG, 
    capitalize(who)+" ("+lev+"+"+xlev+") killed:\t"+this_object()->query_name());
