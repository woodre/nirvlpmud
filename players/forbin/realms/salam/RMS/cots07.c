...
reset(arg) {
  if(!present("salamite")&& ! random(2)) {  
    switch(random(3)) {
      case 0: add_object(NPC+"salamite1.c"); break;
      case 1: add_object(NPC+"salamite2.c"); break;
      case 2: add_object(NPC+"salamite3.c"); break;
    }
  }  
  if(arg) return;
  short_desc = COTS;
  long_desc = 
    "  The pitch darkness seems to close in behind you.  The swirling,\n"+
    "sinister darkness seem to part, giving way to the faint glow of light\n"+
    "up ahead.  Other than that, is is barely possible to see more than\n"+
  ...
  add_item(({"light","glow","faint glow"}),
    "Up ahead, the cave seems to lighten up\n");
  set_chance(random(5));
  ...
  add_exit(RMS+"cots10","east");
  add_exit(RMS+"cots04","west");
}
