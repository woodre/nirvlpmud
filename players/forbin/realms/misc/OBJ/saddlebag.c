/* 10/12/05 Earwax: Fixed a couple minor bugs. */
...
status query_dont_clean_obs_here() { return 1; }
...
status recalculate_local_weight() {
  object *items;
  int x;  
  LWeight = 1; /* weight of bag itself */
...
void init() {
  ...
  add_action("cmd_bag_list", "baglist"); 
...
status cmd_bag_inventory() {
  ...
  write("\n");
  if(!item) {  
    write(YEL+"There isn't anything in the saddlebag.\n"+NORM);
    return 1;
  }
    things = ([ ]);
    while(item) {
...
status cmd_bag_list() {
  ...
  while(item) {
    nextitem = next_inventory(item);
    if(sh = (mixed)item->short()) {
      things += ({ (string)item->short() });
      bling ++;
    }
    item = nextitem;
  }
...
status cmd_bag_superget(string str) {
  ...
  if(sscanf(str, "%d", inum) == 1) {
    ...
    for(x = 0; x < sizeof(things); x++) {
      if(things[x]->id(str)) { 
        if(!call_other(this_player(), "add_weight", call_other(things[x], "query_weight"))) 
          break;  
...
string short() { return YEL+"A leather saddlebag"+NORM+(Worn ? " (worn)" : ""); }                         
