...
//  -18 WC, weight 5 weapon (WC not static, tied to wear)
//  -weapon cannot be repaired if broken
//  -38% chance (w/ maxed piety) of special attack, 0-1 other|light and
//     0-1 other|good damage to evil aligned, or 1-4 other|light and
//     1-4 other|good damage to undead creatures, 
//  -smaller returns of 2 (13%), 1 (13%), random(2) (6%) if the piety special isn't called
*/
  ...
  if(this_player()->query_attrib("pie") > random(80)) {
    ...
    if(undead_check(att)) {
      if(a > 2) { /* 98% of time this attack is called on undead */
        ...
        att->hit_player(1+random(4), "other|light");
        att->hit_player(1+random(4), "other|good");
        return 1; 
      }
      return 0;
    }
    if(att->query_alignment() < 0) {
      att->hit_player(random(2), "other|light");
      att->hit_player(random(2), "other|good");
    }
    return 1;
  }
