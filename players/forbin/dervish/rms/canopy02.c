...
reset(arg) {
  ...
  long_desc = GRN+HBGRN+
    "  You stand upon a wooden platform the encircles a massive tree.\n"+
  ...
  add_item("tree",
    ...
    "niche carved into its side");
  add_item("niche",
    "A small niche has been fashioned into the side of the tree");
  ...
  set_light(1);
  no_exit_display = 1;
  add_exit(ROOT+"rms/canopy01.c","down");
}
...
cmd_search(string str) {
  ...
  int x;
  object moss;
  if(!str || str != "niche") return 0;
  if(mfixed == 0 && mrandom == 0) {
    write("You carefully search the niche, but find nothing of interest.\n");
    say(this_player()->query_name()+" searches a niche in a tree.\n");
      return 1;
  }
  totalmoss = mfixed + random(1+mrandom));
  say(this_player()->query_name()+" searches a niche in a tree.\n");
  write("You carefully search the niche, and find some moss growing within.\n");
