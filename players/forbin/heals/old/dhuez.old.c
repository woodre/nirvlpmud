 ...
 *                    (18*3)/(50*3) = 0.36->0.35 = 10/pt. = 150*10 = 1500
 *  Notable Changes:            
 */
...
id(str) { return (str == "dhuez" || str == "leaves" || str == "dhuez leaves" 
               || str == "d'huez" || str == "d'huez leaves"); }

short() { return "d"+C+"'"+N+"Huez "+C+"Leaves "+N+HIW+">"+N+C+Uses+N+HIW+"<"+N; }

long() { 
  write("  A small clutch of long thin leaves.  Marked with colorful\n"+
        "stripes of varying width, they hardly look edible - but\n"+
        "the leaves of the d'Huez tree are tasty none the less.\n");
  if(Uses == 1) {
    write("There appears to be "+C+Uses+N+" more bite left.\n"+
          "To heal yourself: 'use' or 'eat' the leaves.\n");
  }
  else write("There appears to be "+C+Uses+N+" more bites left.\n"+
             "To heal yourself: 'use' or 'eat' the leaves.\n");
}
...
use_heal(str) {
  ...
  if(USER->query_stuffed() + 9 > USER->query_level() * 8) {
    write(C+"You are too stuffed to use it!\n"+N);
      return 1;
  }
  if(USER->query_soaked() + 9 > USER->query_level() * 8) {
    write(C+"You are too soaked to use it!\n"+N);
      return 1;
  }
  if(!call_other(TP, "eat_food", 9) || !call_other(TP, "drink_soft", 9)) return 0;
  write("You greedily stuff a handful of leaves into your mouth.\n");
  say(USERN+" eats a few bits of d'Huez Leaves.\n");
  USER->heal_self(50);
  USER->recalc_carry();
