...
string Scope;     /* Area(s) to reset automatically from daemon */
...
*/
int cycle_reset(string file);

id(str) { return (str == "reset_cycle_daemon" || str == "rcd"); }
...
void long() { 
  if(this_player() && this_player()->query_level() > 99) {
    tell_object(this_player(),
      ...
      "  well enough alone.  Yeah, I mean YOU!!!                  \n"+
      ...
      "      '"+HBRED+HIW+"set_length <#>"+HBRED+HIR+"'    sets TimerLength to #            \n"+
      ...
      "      '"+HBRED+HIW+"set_scope <arg>"+HBRED+HIR+"' sets the autocycle scope           \n"+
      "         valid args:  "+HBRED+HIW+"forest"+HBRED+HIR+", "+HBRED+HIW+"salamite"+
      HBRED+HIR+", "+HBRED+HIW+"ruins "+HBRED+HIR+", "+HBRED+HIW+"main"+
      HBRED+HIR+", "+HBRED+HIW+"all  \n"+
      HBRED+HIR+"                                                           \n"+NORM);
  }
}
...
init() {
  if(!query_allowed((string)this_player()->query_real_name())) return 1;
  ...
  add_action("cmd_set_time_length","set_length");
  add_action("cmd_check_time","check_time");
  add_action("cmd_set_report","set_report");
  add_action("cmd_set_scope","set_scope");  
}
...
void reset(int arg) { 
  ...
  if(arg) return;
  Timer = 0;
  TimerLength = 700; /* defaults to about 47 minutes */
  Allowed = ({ "forbin", "boltar", "fred", "rumplemintz"});
  Scope = "all";
  Report = "short";
}
...
/*get around ld limitation on castle preloading and call_outs -Bp */
heart_beat() {
  while(remove_call_out("fake_hb") != -1) ;
#ifndef __LDMUD__
  call_out("fake_hb", 3);
#endif
/* 08/17/06 Earwax: Note: ldmud has strict limits on numbers of 
 * callouts, and this can be done entirely without using a repeating
 * callout, if it needs to be done at all.  It's going to be a 
 * major use of resources, and these functions in mizan's, mythos'
 * areas also need to be yanked or SEVERELY limited.
 */
  set_heart_beat(0);
}
...
status cmd_set_scope(string arg) {
  if(arg == "all")           Scope = "all";
  else if(arg == "main")     Scope = "main";
  else if(arg == "forest")   Scope = "forest";
  else if(arg == "salamite") Scope = "salamite";
  else if(arg == "ruins")    Scope = "ruins";      
  else {
    tell_room(environment(this_object()),
      "Valid scopes are:  all, main, forest, salamite, ruins\n"+
      "Scope set to: 'all' by default.\n");
    Scope = "all";
    return 1;
  }
  tell_room(environment(this_object()),
...
status cmd_check_time() {
  tell_object(this_player(),
    ...
    if(Timer == 1) 
      tell_object(this_player(),
        "    "+Timer+" increment has gone by.\n");
    else    
      tell_object(this_player(),
        "    "+Timer+" increments have gone by.\n");
    if((TimerLength - Timer) == 1)
      tell_object(this_player(),        
        "    "+(TimerLength - Timer)+" increment left to go before next cycle.\n");
    else
      tell_object(this_player(),        
...
status cmd_cycle(string arg) {
  ...
  if(arg == "main") {
    while(n < 2) {
      switch(n) {    
        case 0:  File = "/players/forbin/realms/entrance"; break; 
        case 1:  File = "/players/forbin/realms/shack"; break; 
    }  
    cycle_reset(File);
    n++;
    }
  ...
  if(arg == "salamite") {
    while(n < 11) {
      switch(n) {    
        case 0:   File = SALAM+"cots01"; break; 
        case 1:   File = SALAM+"cots02"; break; 
        case 2:   File = SALAM+"cots03"; break;
        case 3:   File = SALAM+"cots04"; break;
        case 4:   File = SALAM+"cots05"; break;
        case 5:   File = SALAM+"cots06"; break;
        case 6:   File = SALAM+"cots07"; break;
        case 7:   File = SALAM+"cots08"; break;
        case 8:   File = SALAM+"cots09"; break;
        case 9:   File = SALAM+"cots10"; break;
        case 10:  File = SALAM+"cots11"; break; 
      }  
    cycle_reset(File);
    n++;
    }
  ...
  if(arg == "ruins") {
    while(n < 23) {
      switch(n) {    
        case 0:  File = RUINS+"r01"; break; 
        case 1:  File = RUINS+"r02"; break; 
        case 2:  File = RUINS+"r03"; break; 
        case 3:  File = RUINS+"r04"; break; 
        case 4:  File = RUINS+"r05"; break; 
        case 5:  File = RUINS+"r06"; break; 
        case 6:  File = RUINS+"r07"; break; 
        case 7:  File = RUINS+"r08"; break; 
        case 8:  File = RUINS+"r09"; break; 
        case 9:  File = RUINS+"r10"; break; 
        case 10: File = RUINS+"r11"; break; 
        case 11: File = RUINS+"r12"; break; 
        case 12: File = RUINS+"r13"; break; 
        case 13: File = RUINS+"r14"; break; 
        case 14: File = RUINS+"r15"; break; 
        case 15: File = RUINS+"r16"; break; 
        case 16: File = RUINS+"r17"; break; 
        case 17: File = RUINS+"r18"; break; 
        case 18: File = RUINS+"r19"; break; 
        case 19: File = RUINS+"r20"; break; 
        case 20: File = RUINS+"r21"; break; 
        case 21: File = RUINS+"r22"; break; 
        case 22: File = RUINS+"r23"; break;                                                                                                                                                                         
    }  
    cycle_reset(File);
    n++;
    }
...
status cycle_reset(string file) {
  ...
  for(i=0; i < sizeof(inv); i++) {
    ...
    if(inv[i]->is_npc() && 
      (int)inv[i]->query_hp() < (int)inv[i]->query_mhp()) hcount++;
...
void fake_hb() {
  ...
  if(Timer > TimerLength) {
    cmd_cycle(Scope);
    tell_room(environment(this_object()),
      HBRED+HIR+"Reset Cycle Daemon:"+HBRED+HIW+"  Cycle called on "+Scope+".\n"+NORM);
    if(find_player("forbin") && 
       environment(find_player("forbin")) != environment(this_object())) {
