...
#define HOME "/players/trix/workroom"
...
id( str ) {
        if(str=="flirter-daemon") return 1;
        return 0;
}

long() {  write("This is the flirter's daemon.\n"); }
...
fli(arg) {
  ...
  if(present("flirter",this_player())->query_muffled())  {
...
roomtell(str) {
    ...
    if (this_player()->query_ghost()) {
        say(this_player()->short() + " "+verb+"s: " + str + ".\n");
    ...
    say(this_player()->query_name() + " "+verb+"s: " + str + "\n");
    write("You "+verb+": " + str + "\n");
    return 1;
}
