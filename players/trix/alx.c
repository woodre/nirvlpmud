...
extra_init() {
   ...
   if(this_player()->query_real_name() == "aleena") {
...
if(intruder->query_real_name() == "aleena") {return 1;}
   if(shield_status == 1) {
 write("You try to enter Aleena's workroom, but\n"+
    "a large hand grabs you and chucks you in the waiting room!\n");
      write("\n");
      pavlikobj = find_player("aleena");
      if(pavlikobj){
...
status temp_shield;
   ...
   if(whereto == "here") {
      ...
      tell_object(target, "You have been transported to Aleena's workroom.\n");
...
valid_read(str) { return call_other(this_player(),"valid_read",str); }
  ...
  FOUR_EXIT("room/church.c", "church",
    ...
    "players/hawkeye/closed/guild/guild_room","guild",
          "The Swamp",
    "Aleena's Workroom.\n"+
"This is my workroom, deal with it.\n"+
"CMON BOUNCE!\n",1)
