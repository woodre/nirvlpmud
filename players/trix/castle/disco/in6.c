...
"before entering the disco.  After all, such fun deserves a little waiting.\n";
   ...
   dest_dir = ({ "/players/trix/castle/disco/in5.c","west",
                 "/players/trix/castle/disco/in9.c","southwest"});
   cont=3+random(2);
   for(i=1;i<=cont;i++) {
...
init()
{ ::init();
  add_action("sowe","southwest");
  add_action("we","west");
}
sowe()
{  if(present("bouncer",this_object()))
      write("You struggle among the crowd to go southwest.\n");
}
we()
{  if(present("bouncer",this_object()))
