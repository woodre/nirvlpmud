...
"before entering the disco.  After all, such fun deserves a little waiting.\n";
   ...
   dest_dir = ({ "/players/trix/castle/disco/in8.c","west",
                 "/players/trix/castle/disco/in6.c","northeast"});
   cont=3+random(2);
   for(i=1;i<=cont;i++) {
...
init()
{ ::init();
  add_action("noea","northeast");
  add_action("we","west");
}
noea()
{  if(present("bouncer",this_object()))
      write("You struggle among the crowd to go northeast.\n");
}
we()
{  if(present("bouncer",this_object()))
