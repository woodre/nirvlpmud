...
reset(arg) {
   ...
   dest_dir = ({ "/players/trix/castle/disco/in3.c","east",
                 "/players/trix/castle/disco/in7.c","southeast"});
   cont=3+random(2);
   for(i=1;i<=cont;i++) {
...
init()
{ ::init();
  add_action("soea","southeast");
  add_action("ea","east");
}
soea()
{  if(present("bouncer",this_object()))
      write("You struggle among the crowd to go southeast.\n");
}
ea()
{  if(present("bouncer",this_object()))
