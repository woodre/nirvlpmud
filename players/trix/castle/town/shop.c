...
ONE_EXIT("players/trix/castle/town/mainroad2", "south",
	 "The shop",
...
"blue light in the doorway.\nTo the south you see Trixtown main road.\n", 1)
...
do_sell(ob) {
    ...
    value = 1000+random(100); /* changed by wizardchild from +random(1000); */
    }
   ...
   /* add_worth(value, ob);  */
    ...
    call_other("players/trix/castle/town/store", "store", ob);
    return 1;
}
...
value(item) {
    ...
    if (!name_of_item) {
	if (call_other("players/trix/castle/town/store", "value", item))
	    return 1;
	write("No such item ("); write(item); write(") here\n");
...
buy(item) {
    if (!item)
	return 0;
    call_other("players/trix/castle/town/store", "buy", item);
    return 1;
}
...
north() {
    ...
    call_other(this_player(), "move_player", "north#players/trix/castle/town/store");
    return 1;
}

list(obj) {
    call_other("players/trix/castle/town/store", "inventory", obj);
    return 1;
}
