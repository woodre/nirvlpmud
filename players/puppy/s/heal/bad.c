...
long() {
   if (potionnr == 0) 
      ...
      write("It's a bad potion.\n"); return 1;
}

short() {
   if (potionnr == 0) return "empty bottle" ;
   else  return "A potion of evilness";
}
...
query_info() {
   return "A potion that will change you into an evil monster.\n";}
query_value() {
...
drink(str) {
   if (str == "potion" || str == "bottle") {
      ...
      if (potionnr < 1)  {
         ...
         write ("Swallowing the concoction makes you feel very evil.\n");
         query_value(); 
         return 1;}
   }return 0;}
