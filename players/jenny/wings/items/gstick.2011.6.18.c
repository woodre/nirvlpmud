/* 07/19/07 Earwax: modified to make this weapon useful again */
...
weapon_hit(attacker) {
  ...
  if(TP && TPRN != "Gordie Howe") {
    if(random(100) > 75) hithim();
      ...
      /* END */
    for (i = 0, times = 1+random(2); i < times; i++) {
      if (attacker) {
...
#ifdef 0
Earwax: yanking this shit, it's been nerfed to death.
   if(heh) return;
...
#ifdef 0
Earwax again, sheesh show some consideration, it's a puck
