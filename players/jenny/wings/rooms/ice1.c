...
reset(arg) {
   ...
   if(!present("player"))  {
      if(whichplayer < 40) {
      MOCO("/players/jenny/wings/mobs/players/fischer.c"),TO);}

      if(whichplayer > 59) {
      MOCO("/players/jenny/wings/mobs/players/legace.c"),TO);}}
    if(arg) return;
    set_light(1);
...
"goal at the south end of the rink.  A blue line stretches across\n"+
...
dest_dir = ({
"/players/jenny/wings/rooms/ice2","north",
...
TP->move_player("onto the bench#players/jenny/wings/rooms/bench1.c");
return 1; }}
box(arg) {
if(!arg) { return 0; }
if(arg == "box" | arg == "penalty box") {
TP->move_player("into the penalty box#players/jenny/wings/rooms/box1.c");
return 1;}
}
north() {
