...
whichplayer = random(100);
   if(!present("player"))  {
      if(whichplayer < 37) {
      MOCO("/players/jenny/wings/mobs/players/gill.c"),TO);}

      if(whichplayer > 84) {
      MOCO("/players/jenny/wings/mobs/players/kuznetsov.c"),TO);}}
    if(arg) return;
    set_light(1);
...
"sign","The sign reads: EQUIPMENT ROOM",
...
dest_dir = ({
 "/players/jenny/wings/rooms/locker3","south",
 "/players/jenny/wings/rooms/locker2","west",
...
TP->move_player("through the door#players/jenny/wings/rooms/eqroom.c");
return 1; }}}
open(arg) { 
if(!arg) { return 0; }
if(arg == "door" | arg == "the door")  {
