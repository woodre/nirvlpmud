...
reset(arg) {
    if(arg) return;
    set_light(1);
     set_search(BOLD+"You notice some bubble gum stuck to the floor."+NORM+"\n");
long_desc=
...
"Red Wings local television announcers is to the west.\n";
...
"booths","Small booths that provide a view of the game, and a place for\n"+
               "the media to work",
"gum","Some thoughtless reporter must have spit this out on the floor",
});

dest_dir = ({
"/players/jenny/wings/rooms/2h2","south",
 "/players/jenny/wings/rooms/wingstv","west",
"/players/jenny/wings/rooms/2h8","northwest",
});
}
short() {
