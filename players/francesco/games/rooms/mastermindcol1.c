...
long_desc =
     ...
     "on a stand.  There is also a small sign on the left wall.\n",
...
items = ({
   ...
   "sign","It reads:  'This is the second Mastermind color room'",
});
...
if (!present("brownie",this_object())) {
  move_object(clone_object("/players/francesco/games/mobs/brownie.c"),
	this_object());  }
if (!present("table",this_object())) {
  move_object(clone_object("/players/francesco/games/items/mastermindcol.c"),this_object());}
::init();
return 1;
}
