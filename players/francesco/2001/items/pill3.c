...
long() { write("A blue pill, must be something like dehydrated food.\n"+
               "You can eat it for healing purposes\n"); }

short() { return HIB+"A blue pill "+counter+NORM; }
...
eat(arg) {
      if (present(arg)==this_object()) /*if (arg=="blue pill") */   
   {
    if(   this_player()->query_stuffed() + 8 < this_player()->query_level()*8)
        { 
         call_other(this_player(), "eat_food", 8);
         call_other(this_player(), "heal_self",19);
         counter--;
         say(call_other(this_player(),"query_name") + " eats one third of a blue pill.\n");
         write("You eat one third of a blue pill.\n");
