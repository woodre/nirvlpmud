...
reset(arg){
  if(arg) return;
   ...
   typ = random(5) +1;
   if(typ == 1) 
    { load_chat("A recorded voice says:  What is the english translation for 'disegno'?\n"); }
   if(typ == 2) 
    { load_chat("A recorded voice says:  What is the english translation for 'rappresentazione'?\n"); } 
   if(typ == 3)
    { load_chat("A recorded voice says:  What is the english translation for 'mezzi'?\n"); }
   if(typ==4)
    { load_chat("A recorded voice says:  What is the english translation for 'righe'?\n"); }
   if(typ==5)
    { load_chat("A recorded voice says:  What is the english translation for 'squadre'?\n"); }
}
