...
reset(arg){
  if(arg) return;
   ...
   typ = random(5) +1;
   if(typ == 1) 
    { load_chat("A recorded voice says:  What is the english translation for 'geometria'?\n"); }
   if(typ == 2) 
    { load_chat("A recorded voice says:  What is the english translation for 'studio'?\n"); } 
   if(typ == 3)
    { load_chat("A recorded voice says:  What is the english translation for 'forme'?\n"); }
   if(typ==4)
    { load_chat("A recorded voice says:  What is the english translation for 'equazioni'?\n"); }
   if(typ==5)
    { load_chat("A recorded voice says:  What is the english translation for 'formule'?\n"); }
}
...
answer(str) {
   ...
   if(str=="geometry" && typ==1 || str=="study" && typ==2 || str=="shapes" && typ==3 || str=="equations" && typ == 4 || str=="formulas" && typ == 5)
{
    if(contatore == 1){
