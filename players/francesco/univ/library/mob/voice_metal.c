...
reset(arg){
  if(arg) return;
   ...
   typ = random(5) +1;
   if(typ == 1) 
    { load_chat("A recorded voice says:  What is the english translation for 'metallurgia'?\n"); }
   if(typ == 2) 
    { load_chat("A recorded voice says:  What is the english translation for 'scienza'?\n"); } 
   if(typ == 3)
    { load_chat("A recorded voice says:  What is the english translation for 'tecnologia'?\n"); }
   if(typ==4)
    { load_chat("A recorded voice says:  What is the english translation for 'metalli'?\n"); }
   if(typ==5)
    { load_chat("A recorded voice says:  What is the english translation for 'leghe'?\n"); }
}
...
answer(str) {
   ...
   if(str=="metallurgy" && typ==1 || str=="science" && typ==2 || str=="technology" && typ==3 || str=="metals" && typ == 4 || str=="alloys" && typ == 5)
{
    if(contatore == 1){
