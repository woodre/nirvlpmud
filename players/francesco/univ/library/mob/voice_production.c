...
reset(arg){
  if(arg) return;
   ...
   typ = random(5) +1;
   if(typ == 1) 
    { load_chat("A recorded voice says:  What is the english translation for 'sistemi di produzione'?\n"); }
   if(typ == 2) 
    { load_chat("A recorded voice says:  What is the english translation for 'avanzato'?\n"); } 
   if(typ == 3)
    { load_chat("A recorded voice says:  What is the english translation for 'processi'?\n"); }
   if(typ==4)
    { load_chat("A recorded voice says:  What is the english translation for 'integrazione'?\n"); }
   if(typ==5)
    { load_chat("A recorded voice says:  What is the english translation for 'fabbrica'?\n"); }
}
