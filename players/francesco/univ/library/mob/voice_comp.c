...
reset(arg){
  if(arg) return;
   ...
   typ = random(6) +1;
   if(typ == 1) 
    { load_chat("A recorded voice says:  What is the english translation for 'informatica'?\n"); }
   if(typ == 2) 
    { load_chat("A recorded voice says:  What is the english translation for 'calcolatori'?\n"); } 
   if(typ == 3)
    { load_chat("A recorded voice says:  What is the english translation for 'dati'?\n"); }
   if(typ==4)
    { load_chat("A recorded voice says:  What is the english translation for 'acquisizione'?\n"); }
   if(typ==5)
    { load_chat("A recorded voice says:  What is the english translation for 'analisi'?\n"); }
   if(typ==6)
    { load_chat("A recorded voice says:  What is the english translation for 'presentazione'?\n"); }
}
