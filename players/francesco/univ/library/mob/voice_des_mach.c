...
reset(arg){
  if(arg) return;
   ...
   typ = random(5) +1;
   if(typ == 1) 
    { load_chat("A recorded voice says:  What is the english translation for 'pianificare'?\n"); }
   if(typ == 2) 
    { load_chat("A recorded voice says:  What is the english translation for 'struttura'?\n"); } 
   if(typ == 3)
    { load_chat("A recorded voice says:  What is the english translation for 'componente'?\n"); }
   if(typ==4)
    { load_chat("A recorded voice says:  What is the english translation for 'prestazioni'?\n"); }
   if(typ==5)
    { load_chat("A recorded voice says:  What is the english translation for 'ottenere'?\n"); }
}
...
answer(str) {
   ...
   if(str=="planning" && typ==1 || str=="structure" && typ==2 || str=="component" && typ==3 || str=="performances" && typ == 4 || str=="achieve" && typ == 5)
{
    if(contatore == 1){
