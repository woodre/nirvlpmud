...
reset(arg){
  if(arg) return;
   ...
   typ = random(6) +1;
   if(typ == 1) 
    { load_chat("A recorded voice says:  What is the english translation for 'gestione'?\n"); }
   if(typ == 2) 
    { load_chat("A recorded voice says:  What is the english translation for 'progettazione'?\n"); } 
   if(typ == 3)
    { load_chat("A recorded voice says:  What is the english translation for 'sviluppo'?\n"); }
   if(typ==4)
    { load_chat("A recorded voice says:  What is the english translation for 'installazione'?\n"); }
   if(typ==5)
    { load_chat("A recorded voice says:  What is the english translation for 'personale'?\n"); }
   if(typ==6)
    { load_chat("A recorded voice says:  What is the english translation for 'attrezzature'?\n"); }
}
...
answer(str) {
   ...
   if(str=="management" && typ==1 || str=="design" && typ==2 || str=="development" && typ==3 || str=="installation" && typ == 4 || str=="personnel" && typ == 5 || str=="equipments" && typ == 6 )
{
    if(contatore == 1){
