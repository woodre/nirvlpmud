...
reset(arg){
  if(arg) return;
   ...
   typ = random(5) +1;
   if(typ == 1) 
    { load_chat("A recorded voice says:  What is the english translation for 'motori'?\n"); }
   if(typ == 2) 
    { load_chat("A recorded voice says:  What is the english translation for 'potenza'?\n"); } 
   if(typ == 3)
    { load_chat("A recorded voice says:  What is the english translation for 'lavoro'?\n"); }
   if(typ==4)
    { load_chat("A recorded voice says:  What is the english translation for 'conversione'?\n"); }
   if(typ==5)
...
answer(str) {
   ...
   if(str=="engine" && typ==1 || str=="power" && typ==2 || str=="work" && typ==3 || str=="conversion" && typ == 4 || str=="energy" && typ == 5)
{
    if(contatore == 1){
