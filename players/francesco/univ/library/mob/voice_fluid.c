...
reset(arg){
  if(arg) return;
   ...
   typ = random(5) +1;
   if(typ == 1) 
    { load_chat("A recorded voice says:  What is the english translation for 'fluidodinamica'?\n"); }
   if(typ == 2) 
    { load_chat("A recorded voice says:  What is the english translation for 'gassosi'?\n"); } 
   if(typ == 3)
    { load_chat("A recorded voice says:  What is the english translation for 'liquidi'?\n"); }
   if(typ==4)
    { load_chat("A recorded voice says:  What is the english translation for 'circostante'?\n"); }
   if(typ==5)
    { load_chat("A recorded voice says:  What is the english translation for 'ambiente'?\n"); }
}
...
answer(str) {
   ...
   if(str=="fluid dynamics" && typ==1 || str=="gaseous" && typ==2 || str=="liquid" && typ==3 || str=="surrounding" && typ == 4 || str=="environment" && typ == 5)
{
    if(contatore == 1){
