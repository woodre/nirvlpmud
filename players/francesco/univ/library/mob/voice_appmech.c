...
reset(arg){
  if(arg) return;
   ...
   typ = random(5) +1;
   if(typ == 1) 
    { load_chat("A recorded voice says:  What is the english translation for 'meccanica applicata'?\n"); }
   if(typ == 2) 
    { load_chat("A recorded voice says:  What is the english translation for 'movimenti'?\n"); } 
   if(typ == 3)
    { load_chat("A recorded voice says:  What is the english translation for 'forze'?\n"); }
   if(typ==4)
    { load_chat("A recorded voice says:  What is the english translation for 'macchina'?\n"); }
   if(typ==5)
    { load_chat("A recorded voice says:  What is the english translation for 'trasmettere'?\n"); }
}
...
answer(str) {
   ...
   if(str=="applied mechanics" && typ==1 || str=="movements" && typ==2 || str=="forces" && typ==3 || str=="machine" && typ == 4 || str=="transmit" && typ == 5)
{
    if(contatore == 1){
