...
reset(arg){
  if(arg) return;
   ...
   typ = random(4) +1;
   if(typ == 1) 
    { load_chat("A recorded voice says:  What is the english translation for 'elettrotecnica'?\n"); }
   if(typ == 2) 
    { load_chat("A recorded voice says:  What is the english translation for 'elettricita''?\n"); } 
   if(typ == 3)
    { load_chat("A recorded voice says:  What is the english translation for 'scienza'?\n"); }
   if(typ==4)
    { load_chat("A recorded voice says:  What is the english translation for 'metodi'?\n"); }
}
...
answer(str) {
   ...
   if(str=="electrotechnics" && typ==1 || str=="electricity" && typ==2 || str=="science" && typ==3 || str=="methods" && typ == 4 )
{
    if(contatore == 1){
