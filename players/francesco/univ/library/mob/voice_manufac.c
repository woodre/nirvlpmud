...
reset(arg){
  if(arg) return;
   ...
   typ = random(6) +1;
   if(typ == 1) 
    { load_chat("A recorded voice says:  What is the english translation for 'tecnologia meccanica'?\n"); }
   if(typ == 2) 
    { load_chat("A recorded voice says:  What is the english translation for 'processi'?\n"); } 
   if(typ == 3)
    { load_chat("A recorded voice says:  What is the english translation for 'sistemi'?\n"); }
   if(typ==4)
    { load_chat("A recorded voice says:  What is the english translation for 'grezzi'?\n"); }
   if(typ==5)
    { load_chat("A recorded voice says:  What is the english translation for 'semilavorati'?\n"); }
   if(typ==6)
    { load_chat("A recorded voice says:  What is the english translation for 'finiti'?\n"); }
}
...
answer(str) {
   ...
   if(str=="manufacturing engineering" && typ==1 || str=="processes" && typ==2 || str=="systems" && typ==3 || str=="rough" && typ == 4 || str=="semifinished" && typ == 5 || str== "final" && typ == 6 )
{
    if(contatore == 1){
