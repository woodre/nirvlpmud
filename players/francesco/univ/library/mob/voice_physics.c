...
reset(arg){
  if(arg) return;
   ...
   typ = random(5) +1;
   if(typ == 1) 
    { load_chat("A recorded voice says:  What is the english translation for 'physics'?\n"); }
   if(typ == 2) 
    { load_chat("A recorded voice says:  What is the english translation for 'natura'?\n"); } 
   if(typ == 3)
    { load_chat("A recorded voice says:  What is the english translation for 'elementare'?\n"); }
   if(typ==4)
    { load_chat("A recorded voice says:  What is the english translation for 'principi'?\n"); }
   if(typ==5)
    { load_chat("A recorded voice says:  What is the english translation for 'leggi'?\n"); }
}
...
answer(str) {
   ...
   if(str=="physics" && typ==1 || str=="nature" && typ==2 || str=="elementary" && typ==3 || str=="principles" && typ == 4 || str=="laws" && typ == 5)
{
    if(contatore == 1){
