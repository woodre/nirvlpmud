...
reset(arg){
  if(arg) return;
   ...
   typ = random(5) +1;
   if(typ == 1) 
    { load_chat("A recorded voice says:  What is the english translation for 'meccanica dei materiali'?\n"); }
   if(typ == 2) 
    { load_chat("A recorded voice says:  What is the english translation for 'resistenza'?\n"); } 
   if(typ == 3)
    { load_chat("A recorded voice says:  What is the english translation for 'materiali'?\n"); }
   if(typ==4)
    { load_chat("A recorded voice says:  What is the english translation for 'azione'?\n"); }
   if(typ==5)
    { load_chat("A recorded voice says:  What is the english translation for 'esterne'?\n"); }
}
...
answer(str) {
   ...
   if(str=="mechanics of materials" && typ==1 || str=="resistance" && typ==2 || str=="materials" && typ==3 || str=="action" && typ == 4 || str=="external" && typ == 5)
{
    if(contatore == 1){
