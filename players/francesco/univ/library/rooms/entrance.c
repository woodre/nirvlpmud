...
"is possible to wander around the shelves.  A doorway will lead you\n"+
...
dest_dir = ({
  "/players/francesco/univ/library/library.c","back",
});
...
"   Computer Science     Electrothecnics          Robotics\n"+
...
wander_around(str){
 ...
 if(str=="corrosion") {
   ...
   return 1;}
 if(str=="electrothecnics") {
   for(i=0; i < sizeof(entered13); i++){
   if(entered13[i] == this_player()->query_real_name()){
   write("You have already done this subject.\n");
   return 1;}}   
   entered13 += ({ this_player()->query_real_name() });
   write("You go wandering through the Electrothecnics books.\n\n");
   tp->move_player("blah#players/francesco/univ/library/rooms/electrothecnics.c");
   return 1;}
 if(str=="metallurgy") {
