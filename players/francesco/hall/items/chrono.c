...
short() { return "An analogic chronograph"; }

long(str) {
    write("This is an analogic chronograph. It has two hands and three buttons.\n"+
          "The first button is used to start and to stop the time.  The second\n"+
          "one serves for split times while the last one resets the whole thing.\n"+
          "It also has a digital window where the current time is displayed.\n"+
          "A small tag is tied to the chain of the chrono. It can be read.\n"+
...
alarm(str) {
if(environment() != this_player()) {notify_fail("You must get the chrono first.\n"); return 0;}
  if(!str) { notify_fail("What do you want to alarm?\n"); return 0; }
  if(str=="chrono") {write("The alarm can be set at every minute of the hour.\n"+
...
if(environment() != this_player()) {notify_fail("You must get the watch first.\n"); return 0;}
  ...
  if(str=="watch") {
    write("The chrono can be tuned to EST, CST, MST, PST.\n"+
          "Syntax is:  tune est, tune cst, tune mst, tune pst.\n");
    return 1;}
  if(str=="est") {a2=e; tu = 1; write("Ok, the chrono is tuned to east standard time.\n");}
  if(str=="cst") {a2=c; tu = 2; write("Ok, the chrono is tuned to central standard time.\n");}
  if(str=="mst") {a2=m; tu = 3; write("Ok, the chrono is tuned to mountain standard time.\n");}
  if(str=="pst") {a2=p; tu = 4; write("Ok, the chrono is tuned to pacific standard time.\n");}
return 1;
}
  ...
  if(!str) { notify_fail("What do you want to trade?\n"); return 0; }
    if(str=="chrono"){
      ...
      write("The narrator replies:  'Thank you, thank you, I lost\n"+
            "this wonderful chrono ages ago, and now I am very happy\n"+
...
if(environment() != this_player()) {notify_fail("You must get the chrono first.\n"); return 0;}
   ...
   if(str=="the back")  {write("On the back of the chrono there are some engravings to read.\n"); return 1;}
...
if(environment() != this_player()) {notify_fail("You must get the chrono first.\n"); return 0;}
  if(!str) { notify_fail("What do you want to read?\n"); return 0; }
   ...
   if(str=="tag") {
      write("On the tag are written the instructions for properly using the chrono.\n");
      return 1;}
   if(str=="instructions") { 
...
if(environment() != this_player()) {notify_fail("You must get the chrono first.\n"); return 0;}
  ...
  if(str=="time") {
    write("The digits of the chrono can be set to 'r'ed, 'b'lue and 'w'hite.\n");
    return 1;}
  if(str=="alarm") {
    write("The alarm of the chrono can be set to 'y'ellow, 'c'yan and 'g'reen\n");
    return 1;}
  if(str=="r") { color1=HIR; write("Ok, the digits of the chrono will show in red.\n"); return 1;}
  if(str=="w") { color1=HIW; write("Ok, the digits of the chrono will show in white.\n"); return 1;}
  if(str=="b") { color1=HIB; write("Ok, the digits of the chrono will show in blue.\n"); return 1;}
...
id(str) { return str == "chrono" || str=="chrono" ; }
