...
long() { write("This kaleidoscope is just a toy which both adults and childrens\n"+
               ...
               "end, and is all "+HIW+BLINK+HIR+"Ka"+YEL+"le"+HIY+"id"+HIG+"os"+HIB+"co"+HIM+"pi"+YEL+"cly"+NORM+" colored. By returning it to the proper\n"+
               "person, it is possible to 'obtain money' or, alternatively, \n"+
               "'obtain heals'.\n"); }

short() { return ""+HIW+BLINK+HIR+"Ka"+YEL+"le"+HIY+"id"+HIG+"os"+HIB+"co"+HIM+"pic  "+NORM+""+HIR+"Ka"+YEL+"le"+HIY+"id"+HIG+"os"+HIB+"co"+HIM+"pe"+NORM+"";}
    ...
    if(arg=="heals")
     {
      ...
      item = clone_object("/players/francesco/univ/items/tea.c");      
      move_object(item,this_player());
write("Hope says: Well done, "+call_other(this_player(),"query_name")+", enjoy these TWO full set of heals.\n");
      write_file("/players/francesco/kale/log/"+this_player()->query_name()+"",ctime(time())+" kaleidoscopicplatinum  3\n");
    ...
    if (arg=="money") 
     { 
      call_other(this_player(),"add_money",(6015+random(9000))); 
      write("Hope says: Well done, "+call_other(this_player(),"query_name")+", these coins will help you in your further mud life.\n");
      write_file("/players/francesco/kale/log/"+this_player()->query_name()+"",ctime(time())+" kaleidoscopic  coins\n");
      destruct(this_object());
      this_player()-> recalc_carry();
      return 1; 
     }
