...
short() { return ""+HIW+"PLATINUM "+HIR+"Ka"+NORM+YEL+"le"+HIY+"id"+HIG+"os"+HIB+"co"+HIM+"pe"+NORM+""; }
...
write("Hope says: Well done, "+call_other(this_player(),"query_name")+", enjoy these TWO full set of heals.\n");
      write_file("/players/francesco/kale/log/"+this_player()->query_name()+"",ctime(time())+" platinum  3\n");
    ...
    if (arg=="money") 
     { 
      ...
      write_file("/players/francesco/kale/log/"+this_player()->query_name()+"",ctime(time())+" platinum  coins\n");
      destruct(this_object());
      this_player()-> recalc_carry();
      return 1; 
     }
