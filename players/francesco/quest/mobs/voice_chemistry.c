  ...
  if(arg) return;
   ...
   if(typ == 1) 
    { load_chat("You notice the librarian which comes close to you and whispers:\n"+
                tpn+", help me please:  what is the english translation for 'chimica'?\n"); }
   if(typ == 2) 
    { load_chat("You notice the librarian which comes close to you and whispers:\n"+
                tpn+", help me please:  what is the english translation for 'materia'?\n"); } 
   if(typ == 3)
    { load_chat("You notice the librarian which comes close to you and whispers:\n"+
                tpn+", help me please:  what is the english translation for 'trasfomazioni'?\n"); }
   if(typ==4)
    { load_chat("You notice the librarian which comes close to you and whispers:\n"+
                tpn+", help me please:  what is the english translation for 'struttura'?\n"); }
   if(typ==5)
    { load_chat("You notice the librarian which comes close to you and whispers:\n"+
...
whisper(str) {
   ...
   if(str=="chemistry" && typ==1 || str=="matter" && typ==2 || str=="transformations" && typ==3 || str=="structure" && typ == 4 || str=="energy" && typ == 5)
{
    if(contatore == 1){
    ...
    write("The librarian whispers to you:  hmmmm, no, "+tpn+", your answer did not work.\n"); 
    return 1; 
}