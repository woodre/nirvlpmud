...
if (!present("voice")) {move_object(clone_object("/players/francesco/quest/mobs/voice_draw.c"),this_object()); }
if (!present("silent")) {move_object(clone_object("/players/francesco/quest/items/silent.c"),this_object()); }

set_light(1);

long_desc =
 "   You are in the drawing area of the library.  This is an aisle\n"+
...
items = ({
     ...
     "books","Most of them are a collection of sheets",
