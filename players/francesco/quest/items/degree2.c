...
/*  dude = tpn;*/
   write_file("/players/francesco/log/graduation",ctime(time())+"  "+tpn+"    [Level  "+tp->query_level()+"+"+tp->query_extra_level()+"]\n");  
               this_player()->set_quest("graduation");
               write_file("/players/francesco/quest/log/graduation.log",ctime(time())+"  "+tpn+
               " solved graduation quest\n"); 
  play = 1; 
  call_out("narrate",1);
return 1; }	
...
narrate_it(){
   ...
   break;
   case 7: text = "\n'Moreover, take this pin and be happy of wearing it'\n";
   break;
   case 8: text = "\nThe Dean of the Department of Mechanical Engineering hands "+call_other(dude,"query_name")+" a small pin.\n";
   break;
   case 9: text = "\nThe Dean of the Department of Mechanical Engineering hugs "+call_other(dude,"query_name")+" warmly.\n\n";
   break;
...
/* 
   ...
   tell_room(environment(dude),WHT+"\n'Congratulations, "+call_other(dude,"query_name")+"'\n\n"+NORM);
