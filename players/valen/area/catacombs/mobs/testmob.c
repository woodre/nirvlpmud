...
reset(arg)
{
  ...
  ::reset(arg);
  if(arg) return;
  set_name(RED+"Imp"+NORM);
  set_alt_name("imp");
  set_race(RED+"imp"+NORM);
  set_alias("demon");
  set_short(RED+"Imp <Demonic>"+NORM);
  ...
  "burning red skin with the glowing red eyes. The "+RED+"Imp"+NORM+" has two little horns\n"+
  ...
  add_spell("mana_leech",
  "#MN# $HR$LEECHS$N$ magical energy from the room!\n",
