...
weapon_hit(attacker) {
  i=random(10);
  if (i>2) {
  if(call_other(attacker,"id","critter")) {
    say(this_player()->query_name()+"'s blade rips deeply into the critter.\n");
    write("Your blade seems to lust for the critter's blood!\n");
  return 25;
  }
