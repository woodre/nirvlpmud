...
reset(arg) {
  ...
  set_race("critter");
  set_alias("elk");
  set_short(YEL+"Elk"+NORM);
    if(random(100)>40) { set_aggressive(1); }
    if(random(100)>60) { set_whimpy(1); }
  set_long(HIR+
  "A very large elk.  He looks mean!\n"+NORM);
  set_level(13);
  set_hp(195);
  set_al(650-random(1300));
  set_dead_ob(this_object());
/* I may want to increase chat numbers later, after testing */
  set_chat_chance(2);
  set_a_chat_chance(5);
  load_chat("The elk snorts.\n");
  load_a_chat("The elk charges!\n");
  set_chance(40+random(20));
  set_spell_mess1("The elk gores his victim!\n");
  set_spell_mess2("The elk gores you!\n");
...
monster_died() {
  ...
  write_file("/players/traff/delnoch/forest/CRITTERS",ctime()+" Elk killed\n");
