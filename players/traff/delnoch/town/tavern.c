/*  TAVERN.C - Part of Delnoch Town.  Drinks are sold here, they are
    heals and use stuff, the most potent heals for 45.  The Drinks are
    cloned from /obj/drink and moved directly to the player, therefore
    they can not be individually marked as for potency.
...
reset(arg) {
  ...
  if(!present("soldier"))  {
  move_object(clone_object("/players/traff/delnoch/mon/soldier.c"),this_object());  }

  if(!present("farmer"))  {
  move_object(clone_object("/players/traff/delnoch/mon/farmer.c"),this_object());  }

  if(!present("drunk"))  {
  move_object(clone_object("/players/traff/delnoch/mon/drunk.c"),this_object());  }

  if(!present("barmaid"))  {
  move_object(clone_object("/players/traff/delnoch/mon/barmaid.c"),this_object());  }

  if(!present("barkeep"))  {
  move_object(clone_object("/players/traff/delnoch/mon/barkeep.c"),this_object());  }
...
"walls","The walls are rough cut timbers, chinked with clay",
    ...
    dest_dir=({
    "/players/traff/delnoch/town/dmain.c","east",
      });
  }   }
/* From here down should problably be re-written!  
I'll work on that soon.  Traff */
...
short() {return BLU+"Delnoch Tavern"+NORM;}
...
#endif
     call_other(this_player(), "move_player", "east" + "#" +"/players/traff/delnoch/town/dmain.c");
     return 1;
}
