...
rip_it() {
string *exits, nexit, y;
int x;
nexit = "";
exits = query_dest_dir();
for (x=1;x<sizeof(exits);x+=2) {
if (exits[x] == "north") nexit += "n ";
if (exits[x] == "south") nexit += "s ";
if (exits[x] == "east") nexit += "e ";
if (exits[x] == "west") nexit += "w ";
if (exits[x] == "northeast") nexit += "ne ";
if (exits[x] == "northwest") nexit += "nw ";
if (exits[x] == "southeast") nexit += "se ";
if (exits[x] == "southwest") nexit += "sw ";
if (exits[x] == "up") nexit += "u ";
if (exits[x] == "down") nexit += "d ";
y = exits[x];
if (y != "north" && y != "south" && y != "west" && y != "east" && y != "up" && y != "down" && y != "northeast" && y != "northwest" && y != "southeast" && y != "southwest") nexit += y+" ";
}
return nexit;
}
id(str) {
...
long(str) {
    int i;
    if (set_light(0) == 0){
       write("You cant see a damn thing.\n");
       return;
    }
    if (!str) {
	write(long_desc);
if(!dest_dir || no_exits)
	    write("    You see no way out.\n");
	else {
	    i = 1;
