...
clean_up() {
  ...
  */
  ...
  if(LOGCLEAN == 3) {
    if(thi_ob) {
      if(env_ob)
        log_file("CLEAN",ctime(time())+" "+object_name(thi_ob)+
        " clean CALLED environ:"+object_name(env_ob)+"\n");
      else
        log_file("CLEAN",ctime(time())+" "+object_name(thi_ob)+
  ...
  if(env_ob) {
    if(extract(object_name(env_ob),0,19)=="players/boltar/hotel") return 1;
    if(sscanf(object_name(env_ob),"%shotel%s",a,b)==2) return 1;
    if(env_ob->query_dont_clean_obs_here()) return 1;
    if(env_ob2) {
      if(sscanf(object_name(env_ob2),"%shotel%s",a,b)==2) return 1;
...
/* 08/21/06 Earwax: added this */
   ...
   if(LOGCLEAN) {
     log_file("CLEAN",object_name(thi_ob)+" Cleaned.");
     if(env_ob) log_file("CLEAN","  Environment: "+object_name(env_ob));
     log_file("CLEAN","\n");
   }
