...
descend() {
  this_player()->move_player("descend#/players/fakir/fcastle/room57.c");
  return 1;
}

climb() {
  if(!present("bloodguard")) {
    this_player()->move_player("climb#/players/fakir/fcastle/room86.c");
    return 1;
  } 
...
door5() {
  if(!present("bloodguard")) {
    this_player()->move_player("climb#/players/fakir/fcastle/room59.c");
    return 1;
  } 
  write("The Bloodguard moves in one quick motion to block your way.\n");
  say("The Bloodguard quickly moves to block "+capitalize(this_player()->query_name())+"'s path!\n");
...
door6() {
  if(!present("bloodguard")) {
    this_player()->move_player("climb#/players/fakir/fcastle/room62.c");
    return 1;
  } 
  write("The Bloodguard moves in one quick motion to block your way.\n");
  say("The Bloodguard quickly moves to block "+capitalize(this_player()->query_name())+"'s path!\n");
