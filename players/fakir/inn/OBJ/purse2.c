...
reset(arg) {
  if(arg) return;
  set_id("purse");
  set_short("A small coin purse");
  set_long(
      "A small leather coin purse with rawhide drawstrings.  If\n"+
      "you open the purse you can 'snatch' the coins from inside!\n");
  set_value(25);
  set_weight(1);
  }
...
cmd_open(arg) {
  ...
  if(arg == "purse") {
    if(environment(this_object()) != this_player()) {
      ...
      return 0; }
    if(open) {
      notify_fail("The coinpurse is already open!\n");
      return 0; }
