...
id(str) { return str == "christmas dust" || str == "dust"; }
short() { return RED+"Christmas Dust"+OFF; }
long() {
  write(
"A small pile of "+RED+"red christmas dust"+OFF+" which when sniffed,\n"+
"renews ones spiritual, mental, and physical health.\n");
return 1;
}
...
call_out("decay",150);
  ...
  say(
capitalize(this_player()->query_name())+" sniffs some christmas dust.\n");
...
decay() {
  ...
  ob = environment();
  if(!ob) return;
  tell_object(ob, "The "+RED+"red christmas dust"+OFF+" fades away...\n");
