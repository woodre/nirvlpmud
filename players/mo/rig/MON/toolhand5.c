...
hair_index=random(4);
set_name( ({"skip","vertebaker","steve","arnold","john","justin"})[random_index] );
set_alt_name("worker");
set_race("human");
set_alias("toolhand");
set_short( ({"Skip"+BOLD+" |Well 5 Toolhand|"+NORM+"","VerteBaker"+BOLD+" |Well 5 Toolhand|"+NORM+"","Steve"+BOLD+" |Well 5 Toolhand|"+NORM+"","Arnold"+BOLD+" |Well 5 Toolhand|"+NORM+"","John"+BOLD+" |Well 5 Toolhand|"+NORM+"","Justin"+BOLD+" |Well 5 Toolhand|"+NORM+""})[random_index]);
set_long(
query_name() + " has " + (({"blonde","brown","red","black"})[hair_index])+" hair but it is mostly covered by his\n"+
"hard hat.  He has a name tag on his chest that says "+query_name()+"\n"+
"|Well #5 Toolhand| that is slightly covered by reflective gear.\n"+
