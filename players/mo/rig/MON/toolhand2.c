...
hair_index=random(4);
set_name( ({"skip","snip","steve","arnold","verte","justin"})[random_index] );
set_alt_name("worker");
set_race("human");
set_alias("toolhand");
set_gender("male");
set_short( ({"Skip"+BOLD+" |Well 2 Toolhand|"+NORM+"","Snip"+BOLD+" |Well 2 Toolhand|"+NORM+"","Steve"+BOLD+" |Well 2 Toolhand|"+NORM+"","Arnold"+BOLD+" |Well 2 Toolhand|"+NORM+"","Verte"+BOLD+" |Well 2 Toolhand|"+NORM+"","Justin"+BOLD+" |Well 2 Toolhand|"+NORM+""})[random_index]);
set_long(
query_name() + " has " + (({"blonde","brown","red","black"})[hair_index])+" hair but it hair is mostly covered by his\n"+
"hard hat.  He has a name tag on his chest that says "+query_name()+"\n"+
"|Well #2 Toolhand| and it is slightly covered by reflective gear.\n"+
