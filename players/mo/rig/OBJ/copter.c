...
id(str){
   ...
   return str == "helicopter" || str == "copter" || str == "heli";
}
realm() { return "NT" || "LA"; }
...
init(){
   ...
   add_action("fly_away","fly_away");
}

fly_away() {
   object pilot;
   object occupied;
   occupied = present("occupied",environment(this_object()));

   if(!present("pilot",environment(this_object()))) {
      say("a Pilot walks in and climbs the ladder of the helicopter..\n");
      move_object(clone_object("/players/mo/rig/OBJ/occupied_heli.c"),environment(this_object()));
      command("fly_away",occupied);
      destruct(this_object());
      return 1; }
   if(present("pilot",environment(this_object()))) {
      say("the Pilot climbs the ladder of the helicopter.\n");
      move_object(clone_object("/players/mo/rig/OBJ/occupied_heli.c"),environment(this_object()));
      command("fly_away",occupied);
      destruct(present("pilot",environment(this_object())));
      destruct(this_object());
      return 1; }
   return 0;
}
...
if(present("helicopter",environment(this_object()))) {
write("You have to wait for the other Helicopter to leave the platform!\n");
return 1; }
