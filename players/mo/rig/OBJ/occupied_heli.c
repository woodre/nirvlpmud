...
init(){
   ...
   add_action("fly_away_b","fly_away_b");
   add_action("fly_away_2","fly_away_2");
   add_action("fly_away_3","fly_away_3");
}

fly_away() {
   call_out("fly_away_2", 3);
   return 1; }
fly_away_2() {
   object hover;
   hover = present("hover",environment(this_object()));
   say("the Helicopter takes off...\n");
   move_object(clone_object("/players/mo/rig/OBJ/hover.c"),environment(this_object()));
   command("fly_away_3",hover);
