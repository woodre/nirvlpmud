...
void destruct_environment_of(object ob) {
    ...
    if (error = catch(ob->move_player("abruptly#room/void"))) {
	write(error);
...
#define SIMUL_EFUN_FILE "obj/simul_efun32"
#endif
...
valid_exec(string name, object ob, object obfrom)
{
    switch(name) {
      ...
      case "obj/master.c":
#ifdef MUDWHO
        if (!interactive(ob)) if (interactive(obfrom)) {
...
{
    string a,b,user,tmpa,tmpb,ficre,ficre2;

/*
    write("valid_write case:"+call_fun+" path:"+path+"\n");
*/
    switch ( call_fun ) {
        case "save_object":
	      ...
	      strlen(path) > 35
	        ...
	        return path;
              ...
              if(previous_object(1)) 
                 ficre=creator(previous_object(1));
             if(previous_object()) ficre2=creator(previous_object());
               if(!ficre && !ficre2 && path=="/WIZLIST")
                   return path;
            if(sscanf(path,"/players/%s/%s",tmpa,tmpb) == 2 && tmpa == ficre)
               return path;
	    break;
	case "ed_start":
	    path = "/"+path;
...
{
    string user,ficre,tmpa,tmpb;

/*
    write("valid_read case:"+call_fun+" path:"+path+"\n");
*/
    switch ( call_fun ) {
...
int valid_query_snoop(object wiz) {
    return this_player()->query_level() >= 600;
}
