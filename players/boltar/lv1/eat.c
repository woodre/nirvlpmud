...
starta_goldeat() {
        goldeat = this_object();
	call_other(goldeat, "set_name", "barry");
	call_other(goldeat, "set_short", "Barry the taxman");
	call_other(goldeat, "set_long",
  "This is Barry, he raises and collects taxes as a hobby.\n");
...
call_other(goldeat, "set_match", "Barry");
call_other(goldeat, "set_function", "eatgold");
...
eatgold(str) {
   ...
   if (lostgold->query_level() < 3) mon = 0 - random(3) - 1;
    ...
    if (lostgold->query_money() > 45000) mon = 0 - random(1000) - 1;
     ...
     if (mon < plymon) {
        ...
        eatmoney = 0 - mon/3;
       if(mon == 0) {
   tell_object(lostgold, "Barry finds you have no money and smacks you across the face.\n");
   ...
   return 1;
    ...
    call_other(goldeat, "set_level",12);
     tell_object(lostgold, "Barry takes your money!!\n");
    do_something();
   return 1;
  }
do_something(){
object cash;
   ...
   ran = random(25);
   if (ran == 1) {
     say("Barry opens his mouth.\n");
     random_move();
    return 1;
     }
  if (ran == 11)
  say("Barry says: I want to cut spending.\n");
  if (ran == 3) {
     say("Barry says: We must all sacrifice to change things.\n");
    random_move();
      }
  if (ran == 4)
   {
     say("Barry looks around.\n");
    random_move();
   }
  if (ran == 5)
     say("Barry says: Most people will not see their income taxes go up under my plan.\n");
  if (ran == 6)
     say("Barry smiles.\n");
  if (ran == 10 || ran == 0)
      say("Barry looks at you.\n");
   if (ran == 7)
      say("Barry says: The economy will grow under my plan.\n");
   if (ran == 15)
     {
      expl = random(100);
      if (expl > 13)
            return 1;
       say("Suddenly, Barry explodes.\n");
   ...
   if (ran == 8)
     say("Barry starts jogging in place.\n");
   if (ran == 9)
     say("Barry looks around for a place to eat.\n");
     if (ran == 2) {
       repro = random(100);
     if (repro > 20)
 {
