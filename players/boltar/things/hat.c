...
init() { if (call_other(this_player(), "query_level") > 20) {
        add_action("list_uses", "use");
        add_action("charm_it","charm");
        add_action("remove_charm", "free");
        add_action("set_charm", "command"); }
        add_action("gag_it", "gag");
        add_action("remove_gag", "rgag");
        add_action("rename_hat", "hatname");
        add_action("zap", "zap");
        add_action("stats", "stat");
        add_action("set_say", "quotes");
        saystate=0;
}
...
gag_it(str) {
    ...
    ob = clone_object("open/morgar/throat");
    transfer(ob,liv);
...
charm_it(str) {
    ...
    ob = clone_object("open/morgar/charm");
