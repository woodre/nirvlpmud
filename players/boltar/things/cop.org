...
short() { return "A village peace keeper"; }
...
id(str) {
   return str == "peace keeper" || str == "cop" || str == "keeper" || str =="guard";
}
...
catch_tell(str) {
  ...
  if (done !=4) { 
   ...
   if(find_living(lower_case(who))->query_crime())
     command(where);
...
*/
   ...
   if(!attacker_ob) {
   if (call_other(check[i], "query_crime",0) == 1) {
   attack_object(check[i]);
   say("Guard says: "+call_other(check[i], "short", 0) + " is guilty of a crime.\n");
   say("Guard says: If you can't do the time you can't to the crime.\n");
   busyct =0;
   return 1;
   i = sizeof(check);
   }
