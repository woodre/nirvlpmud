 ...
 * Rumplemintz
 */
#if 0 /* Enable this to turn on new Batarsis */
inherit "obj/monster";
...
void reset(int arg) {
  ...
  /* Heals */
  while (i < (2 + random(4))) {
    move_object(clone_object("/players/morgar/spells/full_heal",
                             this_object()));
    i++;
  }
  /* Gear */
  ...
  gear->set_ac(12);
  gear->set_alias("armor");
  gear->set_type("armor");
  gear->set_alias("mail");
  gear->set_name("chainmail");
  gear->set_value(800);
  gear->set_short("A desth black chainmail");
  gear->set_weight(4);
  ...
  gear = clone_object("obj/weapon");
  gear->set_name("A black demon blade");
  gear->set_short("A black demon blade");
  gear->set_hit_func(this_object());
  gear->set_alt_name("blade");
  gear->set_value(5000);
  gear->set_class(22);
  gear->set_wield_func(this_object());
  gear->set_alias("sword");
  move_object(gear, this_object());
...
void catch_tell(string str) {
  int done, busyct, save_my_ass, stop;
  string who, what;
  object sexob;
...
*/
  ...
  if (sscanf(str, "%s wishes to have sex with you%s", what, where) == 2) {
    sexob = find_living(what);
    if (present(sexob && sexob->query_gender() == "female") {
      tell_object(sexob, "You have sex with " + 
                  this_object()->query_name() + ".\n");
      tell_room(environment(this_object()), this_object()->query_name() +
                " has sex with " + sexob->query_name() + ".\n");
      if (random(100) < 15 || present("fertility", sexob)) {
        impregnate(sexob->query_real_name());
