...
catch_tell(str) {
 string bb,blah;
 if(extract(str,0,5)=="In the") return 0;
 if(extract(str,0,5)=="Outsid") return 0;
 if(busy) return;
 busy = 1;
 blah=file_name(environment(this_object()));
 if(sscanf(blah,"%sprison",bb)==1) {
   tell_room("room/police","In the prison cell, "+str);
   busy = 0;
   return 1;
 }
 if(sscanf(blah, "%spolice", bb)==1) {
   tell_room("room/prison","Outside the cell, "+str);
   busy = 0;
   return 1;
 }
 busy = 0;
