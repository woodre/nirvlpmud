...
make_stuff() {
  object emoter;
   object watch;
  if(!present("emoter",this_object())) {
 emoter = clone_object("players/boltar/things/emoter");
 move_object(emoter, this_object());
}
  if(!present("watch",this_object())) {
   watch = clone_object("players/boltar/things/watch");
   move_object(watch, this_object());
  }
    if(!present("shotgun",this_object())){
     watch =clone_object("players/boltar/things/nerfgun");
      move_object(watch,this_object());
   }
    if(!present("pill",this_object())){
     watch =clone_object("players/boltar/things/pill");
      move_object(watch,this_object());
   }
    if(!present("kit",this_object())){
     watch =clone_object("players/boltar/sprooms/clinic/test");
      move_object(watch,this_object());
     }
     if(!present("condom",this_object())){
     watch =clone_object("players/boltar/sprooms/clinic/condom");
     move_object(watch,this_object());
   }
}
