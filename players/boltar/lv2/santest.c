...
starta_sandra() {
	...
	call_other(sandra, "set_long",
		   "She looks like everthing you have ever dreamed of and more. She looks           expensive, but worth every penny. She looks like she has alot of money.\n");
	call_other(sandra, "set_ac", 14);
	call_other(sandra, "set_al",125);
	call_other(sandra, "set_hp",920);
	call_other(sandra, "set_wc",15);
	call_other(sandra, "set_aggressive", 0);
call_other(sandra, "set_level", 19);
	move_object(sandra, "players/boltar/lv2/hookerroom");
	money = clone_object("obj/money");
        call_other(money, "set_money", random(1700));
        move_object(money, sandra);
teddy = clone_object("obj/armor");
...
call_other(teddy, "set_type", "armor");
call_other(teddy, "set_ac", 4);
...
call_other(sandra, "set_chance", 60);
call_other(sandra, "set_spell_dam", 50);
...
call_other(find_living(lower_case(who)), "heal_self", 25);
call_other(sandra, "heal_self", 920);
   } else if (what == "1000 coins") {
   } else if (spent > 1000) {
...
call_other(find_living(lower_case(who)), "heal_self", 35);
call_other(sandra, "heal_self", 920);
    } else {
