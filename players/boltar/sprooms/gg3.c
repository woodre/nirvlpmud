...
"with in the next room.\nThe exits are north (gg4) and south (gg2).\n");
   write(" "+
 ...
 set_light(1);
  ...
  parents = allocate(CAP);
   genders =allocate(CAP);
     restore_object("players/boltar/sprooms/gg3");
   while (i<CAP) {
     inout[i] = 1;
     i += 1;
    }
...
move_object(clone_object("obj/kid_soul"), kid);
       ...
       say(kid->query_name()+" runs out of the playroom to "+this_player()->query_name()+"\n");
        reset_count();
        save_object("players/boltar/sprooms/gg3");
   return 1;
 }
leave(str) {
int par_too_flag;
...
south() {
    call_other(this_player(), "move_player", "south#players/boltar/sprooms/gg2.c");
    return 1;
  }
north() {
    call_other(this_player(), "move_player", "north#players/boltar/sprooms/gg4.c");
    return 1;
  }
reset_count(){
  int k;
