...
string parent_too,filee;
int exps,hitpoints;
#include "/players/boltar/sprooms/dcbak/ct.c"
...
reset(arg) {
  ...
  nparents=allocate(CAP+1);
     ptoo = allocate(CAP+1);
   preg = allocate(CAP+1);
   if(!filee) filee = "daycare";
  restore_object("players/boltar/sprooms/dcbak/"+filee);
  while (i<CAP+1) {
    inout[i] = 1;
    whoout[i] = 0;
    i += 1;
...
move_object(clone_object("obj/kid_soul"), kid);
      ...
      /*  save_object("players/boltar/sprooms/daycareXX");*/
   return 1;
 }
leave(str) {
int par_too_flag;
...
north() {
    call_other(this_player(), "move_player", "north#players/boltar/sprooms/daycare2.c");
    return 1;
  }
reset_count(){
  int k;
