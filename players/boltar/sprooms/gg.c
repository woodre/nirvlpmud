...
string genders,names,parents;
string parent_too;
int exps,hitpoints;
#include "/players/boltar/sprooms/count.c"
short() {return "The gg center"; }
long() {
    write("You have entered the gg center, where you can leave your\n"+
   "child and pick him up later. Commands are 'leave name' and 'pickup name'.\n"+
   "The gg center appears to be well staffed and well maintained.\n"+
   "Through the window you can see many toys for the children to play\n"+
   "with in the next room.\nThe exits are east and north (gg 2 ).\n");
   write(" "+
   "Type 'buy' to buy your kid some munchies, only 50 coins a bag!\n");
write("To set a second parent: the regsistered parent must type:\n"+
"'other <kid's name> <2nd parent's name>' 'list' will list your kids.\n");
   return 1;
 }
init() {
...
reset(arg) {
  ...
  names = allocate(CAP);
 set_light(1);
  parent_too = allocate(CAP);
  levels = allocate(CAP);
  hitpoints = allocate(CAP);
  exps = allocate(CAP);
  timeleave = allocate(CAP);
  kidages = allocate(CAP);
   inout = allocate(CAP);
  parents = allocate(CAP);
   genders =allocate(CAP);
     restore_object("players/boltar/sprooms/gg");
   while (i<CAP) {
     inout[i] = 1;
     i += 1;
    }
...
move_object(clone_object("obj/kid_soul"), kid);
       ...
       say(kid->query_name()+" runs out of the playroom to "+this_player()->query_name()+"\n");
        reset_count();
        save_object("players/boltar/sprooms/gg");
   return 1;
 }
leave(str) {
int par_too_flag;
int yyu;
 ...
 object kidob;
     ...
     if(call_other("players/boltar/sprooms/gg2", "check_names", str)) yyu =1;
     if(call_other("players/boltar/sprooms/gg3", "check_names", str)) yyu =1;
     if(call_other("players/boltar/sprooms/gg4", "check_names", str)) yyu =1;
     if(yyu==1) {
     write("Your child is registered at another gg center.\n");
    return 1;
   }
     i=0;
      if (vac != 1) {
...
north() {
    call_other(this_player(), "move_player", "north#players/boltar/sprooms/gg2.c");
    return 1;
  }
reset_count(){
  int k;
