#define CAP 110
#define vac 0 
int levels,timeleave,kidages,inout,whoout,count;
string genders,names,parents,nparents;
string parent_too;
int exps,hitpoints;
#include "/players/boltar/sprooms/count.c"

short() {return "The daycare center"; }
id(str) {
...
long(str) {
   ...
   if(str) return 0;
   write("You walk through the brick entry way into the first section\n"+
     "of the daycare center. There is a hallway to the north leading\n"+
     "to section number two. To the east is the exit to the Northroad\n"+
     "section of the village. The daycare center is well appointed and\n"+
...
reset(arg) {
  ...
  restore_object("players/boltar/sprooms/dayc_test");
  while (i<CAP+1) {
    inout[i] = 1;
    whoout[i] = 0;
    i += 1;
...
move_object(clone_object("obj/kid_soul"), kid);
       ...
       say(kid->query_name()+" runs out of the playroom to "+this_player()->query_name()+"\n");
        reset_count();
        save_object("players/boltar/sprooms/dayc_test");
   return 1;
 }
leave(str) {
int par_too_flag;
...
if(str=="baby") { write("You must name your child.\n"); return 1; }
     ...
     if(call_other("players/boltar/sprooms/dayc_test2", "check_names", str)) yyu =1;
     if(call_other("players/boltar/sprooms/dayc_test3", "check_names", str)) yyu =1;
     if(call_other("players/boltar/sprooms/dayc_test4", "check_names", str)) yyu =1;
     if(call_other("players/boltar/sprooms/dayc_test5", "check_names", str)) yyu =1;
     if(yyu==1) {
     write("Your child is registered at another daycare center.\n");
    return 1;
   }
     i=0;
      if (vac != 1) {
...
north() {
    call_other(this_player(), "move_player", "north#players/boltar/sprooms/dayc_test2.c");
    return 1;
  }
reset_count(){
  int k;
