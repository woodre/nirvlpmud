...
long(str) {
   ...
   if(str) return 0;
   write("You walk through the brick entry way into the second section\n"+
     "of the daycare center. There is a hallway to the north leading\n"+
     "to section number three. To the south is the exit to the first\n"+
...
init() {
 ...
 add_action("set_nparent","setnparents");
...
move_object(clone_object("obj/kid_soul"), kid);
       ...
       say(kid->query_name()+" runs out of the playroom to "+this_player()->query_name()+"\n");
        reset_count();
        save_object("players/boltar/sprooms/daycare2");
   return 1;
 }
leave(str) {
int par_too_flag;
...
south() {
    call_other(this_player(), "move_player", "south#players/boltar/sprooms/daycare.c");
    return 1;
  }
north() {
    call_other(this_player(), "move_player", "north#players/boltar/sprooms/daycare3.c");
    return 1;
  }
reset_count(){
  int k;
