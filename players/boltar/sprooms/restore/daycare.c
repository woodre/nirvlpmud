...
long() {
    write("You have entered the daycare center, where you can leave your\n"+
   "child and pick him up later. Commands are 'leave name' and 'pickup name'.\n"+
   "The daycare center appears to be well staffed and well maintained.\n"+
   "Through the window you can see many toys for the children to play\n"+
   "with in the next room.\nThe exits are east and north (daycare 2 ).\n");
   write(" "+
...
add_action("set_nparents","setnparent");
 ...
 set_light(1);
  ...
  parents = allocate(CAP+1);
   ...
   nparents=allocate(CAP+1);
     restore_object("players/boltar/sprooms/restore/daycare");
   while (i<CAP+1) {
     inout[i] = 1;
     i += 1;
    }
...
move_object(clone_object("obj/kid_soul"), kid);
       ...
       say(kid->query_name()+" runs out of the playroom to "+this_player()->query_name()+"\n");
        reset_count();
        save_object("players/boltar/sprooms/restore/daycare");
   return 1;
 }
leave(str) {
int par_too_flag;
...
if(str=="baby") { write("You must name your child.\n"); return 1; }
     ...
     if(call_other("players/boltar/sprooms/restore/daycare4", "check_names", str)) yyu =1;
     if(yyu==1) {
...
north() {
    call_other(this_player(), "move_player", "north#players/boltar/sprooms/restore/daycare2.c");
    return 1;
  }
reset_count(){
  int k;
