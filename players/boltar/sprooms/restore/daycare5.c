 ...
 set_light(1);
  ...
  nparents=allocate(CAP+1);
   genders =allocate(CAP+1);
     restore_object("players/boltar/sprooms/restore/daycare5");
    if(!restore_object("players/boltar/sprooms/restore/daycare5"))
      save_object("players/boltar/sprooms/restore/daycare5");
   while (i<=CAP) {
     inout[i] = 1;
     i += 1;
    }
