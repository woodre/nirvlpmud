...
long(str) {
   ...
   write("You walk through the brick entry way into the forth section\n"+
     ...
     "section of the daycare. To the north is the exit to the fifth\n"+
 ...
 dcnumberid = 4;
  ...
  nparents=allocate(CAP+1);
   ...
   preg = allocate(CAP+1);
    ...
    if(!restore_object("players/boltar/sprooms/daycare4"))
      save_object("players/boltar/sprooms/daycare4");
   while (i<=CAP) {
     inout[i] = 1;
     whoout[i] = 0;
     i += 1;
...
south() {
    call_other(this_player(), "move_player", "south#players/boltar/sprooms/daycare3.c");
    return 1;
  }
north() {
    call_other(this_player(), "move_player", "north#players/boltar/sprooms/daycare5.c");
    return 1;
  }
reset_count(){
  int k;
