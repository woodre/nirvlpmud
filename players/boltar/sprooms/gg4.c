...
short() {return "The gg annex center"; }
long() {
   ...
   "The gg annex center appears to be well staffed and well maintained.\n"+
   "Through the window you can see many toys for the children to play\n"+
   "with in the next room.\nThe exit is south (gg 3).\n");
   write(" "+
 ...
 set_light(1);
  ...
  parents = allocate(CAP);
   genders =allocate(CAP);
     restore_object("players/boltar/sprooms/gg4");
    if(!restore_object("players/boltar/sprooms/gg4"))
      save_object("players/boltar/sprooms/gg4");
   while (i<CAP) {
     inout[i] = 1;
     i += 1;
    }
...
int yyu;
 ...
 object kidob;
     ...
     if(call_other("players/boltar/sprooms/gg", "check_names", str)) yyu =1;
     if(yyu==1) {
     write("Your child is registered at the other gg center.\n");
    return 1;
   }
     i=0;
      if (vac != 1) {
...
south() {
    call_other(this_player(), "move_player", "south#players/boltar/sprooms/gg3.c");
    return 1;
  }
north() {
    call_other(this_player(), "move_player", "north#players/boltar/sprooms/gg22.c");
    return 1;
  }
reset_count(){
  int k;
