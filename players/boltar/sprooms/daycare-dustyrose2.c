...
long(str) {
   ...
   if(str) return 0;
   write("This is Dustyrose's daycare #2. It contains children that\n"+
         "were once purged but now restored. They only exist here\n"+
         "and are not connected to the main dc system.\n"+ 
         "There is a large unbreakable window exposing\n"+
...
reset(arg) {
  ...
  nparents=allocate(CAP+1);
     ptoo = allocate(CAP+1);
   preg = allocate(CAP+1);
  restore_object("players/boltar/sprooms/daycare-dustyrose2");
  while (i<CAP+1) {
    inout[i] = 1;
    whoout[i] = 0;
    i += 1;
...
move_object(clone_object("obj/kid_soul"), kid);
       ...
       say(kid->query_name()+" runs out of the playroom to "+this_player()->query_name()+"\n");
        reset_count();
        save_object("players/boltar/sprooms/daycare-dustyrose2");
   return 1;
 }
leave(str) {
int par_too_flag;
...
south() {
    call_other(this_player(), "move_player", "north#players/boltar/sprooms/daycare-dustyrose.c");
    return 1;
  }
reset_count(){
  int k;
