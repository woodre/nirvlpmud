...
south() {
  call_other(this_player(),"move_player","south#"+CASINO+"hall4x4");
  return 1;
}
west() { 
  call_other(this_player(),"move_player","west#"+CASINO+"hall4x2");
  return 1;
}
enter(str) {
...
id(str) {
    return str == "door" || str ==  "plate" || str == "plaque" || 
           ...
           str=="button" || str=="sky light" || str== "hallway" ||
...
east() {
    if (call_other(CASINO+"elevator", "query_door", 0) ||
	call_other(CASINO+"elevator", "query_level", 0) !=4) {
...
open(str)
{
    if (str != "door")
	return 0;
    if (call_other(CASINO+"elevator", "query_level", 0) !=4) {
...
push(str)
{
    ...
    if (call_other(CASINO+"elevator", "call_elevator", 4))
	lamp_is_lit = 1;
    return 1;
}
