...
init() {
  ...
  blahhh = this_player()->query_real_name();
  if (blahhh == "dustyrose" || blahhh == "boltar")
  add_action("north","north");
  if (blahhh != "boltar")
  if (blahhh != owner_name && shield == 1) {
...
long() {
  ...
  blahhh = this_player()->query_real_name();
  if (blahhh == "dustyrose" || blahhh == "boltar") {
    write("There is a door to the north and a lobby exit to the south.\n");
  } else
...
south() {
  call_other(this_player(), "move_player", "south#players/boltar/hotel/hotel.c");
   say(this_player()->query_name()+" leaves through the door to the lobby.\n");
  return 1;
}
set_owner(str){
  lighton = 1;
      ...
      if (first != "players") {
        if (creator_name && creator_name !="boltar") {
          ...
          if(call_other(all_stuff[count], 
            "save_thing","players/boltar/hotel/room/q"
            +owner_name + inv_no)){
    ...
    */
    ...
    if(thing) {
      ...
      if(thing->generic_object() && !thing->locker_arg())
        first = "OK";
      if (first != "players") {
         call_other(thing, "restore_thing",
            "players/boltar/hotel/room/q"+owner_name + count);
