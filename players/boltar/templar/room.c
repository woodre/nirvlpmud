...
short() {
  return HIW+ capitalize(owner_name) + "'s Templar room ["+NORM+GRN+"Fortress Stone"+NORM+"]";
}
long() {
  if (!long_dsc) {
    write("You are in " + capitalize(owner_name) +"'s Templar room.\n");
    write ("Use '"+GRN+"clear"+NORM+"' to clear room description.\n");
    write ("Use '"+GRN+"@desc"+NORM+"' to add a line to room description.\n");
  }
...
store(){
  ...
  count = 0;
  if (inv_no > 0) {
    write ("Please do a '"+GRN+"restore"+NORM+"' first.\n");
    return 1;
  }
  inv_no = 0;
...
object blah;
  ...
  "private room in Templar Castle. If you\n"+
  "wish to accept, type "+GRN+"accept"+NORM+". Simply drop the summons to dispose of it.\n");
  blah=clone_object("/players/boltar/templar/summons");
  blah->set_room(this_object());
  move_object(blah,guest);
  write("You invite "+str+" to your room.\n");
   return 1;
}
light_switch() {
...
look(str) {
  ...
  if (str == "instructions" || str == "at instructions") {
    ...
    write("replace #\t Replaces item # in storage with a cookie.\n");
