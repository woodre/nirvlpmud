short() { return "The Doppleganger Ninja's Faction"; }
 
long() {
  write("You have entered the Doppleganger Ninja's Faction.\n"+
...
east() {
  this_player()->move_player("east#/players/grimm/rooms/hapahz_a13");
  return 1;
}
 
west() {
  if(!(present("Dopple-Ninja",this_player()))) {
    write("You are not a member of the Ninja's Faction. You cannot enter that room.\n");
    return 1;
  }
  this_player()->move_player("west#/players/grimm/closed/obj/badge_stuff/n_pr");
  return 1;
}
...
join() {
   ...
   write("You have just joined the Ninja's Faction!!!\n");
   badge = clone_object("/players/grimm/closed/obj/ninja");
   move_object(badge,this_player());
   write_file("/players/grimm/closed/LOGS/ninj",capitalize(this_player()->query_real_name())+" just joined the guild.\nThe time is: "+ctime(time())+".\n-------------------\n");
...
tell_em("unjoined");
write_file("/players/grimm/closed/LOGS/unninj",capitalize(this_player()->query_real_name())+" had "+this_player()->query_guild_exp()+" guild exp when he unjoined.\nThe time is: "+ctime(time())+"\n------------------------------\n");
badge = present("Dopple-Ninja",this_player());
destruct(badge);
...
write("You have just unjoined the Ninja's Faction.\n");
return 1;
}
...
for(n=0;n<sizeof(people);n++) {
  if(present("Dopple-Ninja",people[n])) {
...
write_file("/players/grimm/closed/LOGS/ninjswaps",capitalize(this_player()->query_real_name())+" just swapped "+x+" exp.\nThe time is: "+ctime(time())+"\n-----------------------\n");
...
if(x>z) {
  if(y<15) {
    write("You are now level "+(y+1)+" in the faction!\n");
     write_file("/players/grimm/closed/LOGS/ninjadvanced",capitalize(this_player()->query_real_name())+" just made level "+(y+1)+" in the faction.\nThe time is: "+ctime(time())+"\n--------------------------------------\n");
    tell_em("advanced in");
    this_player()->add_guild_rank(1);
  } else {
