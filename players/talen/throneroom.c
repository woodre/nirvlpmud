...
reset(arg) {
  ...
  set_light(1);
  long_desc=
"  This is "+capitalize(OWNER)+"'s Throne Room.  It is lined with\n"+
...
cmd_listen(str) {
  write("The room is completely silent.\n");
  return 1;
}
...
short() {
  ...
  if(cloak) {
    ...
    return str+"("+NORM+capitalize(OWNER)+"'s Throne Room"+str+")"+NORM;
  }
}
...
"A giant eagle swoops down and snatches "+capitalize(str)+" from the ground.\n"
  ...
  tell_object(person,"You have been taken to "+capitalize(OWNER)+"'s Throne Room.\n");
