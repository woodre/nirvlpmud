...
ONE_EXIT("players/deathmonger/hall2","east",
         "Isle of Death Bazaar",
          "You are in the Isle of Death Bazaar.  Lots of neat stuff here.\n"+
...
/* Changed from random 800 to random 50 -snow 2/00 */
...
/*  add_worth(value, ob):  */
	...
	return 1;
     ...
     call_other("players/deathmonger/storage","store",ob);
    return 1;
}
...
value(item) {
    ...
    if (!name_of_item) {
        if(call_other("players/deathmonger/storage","value",item))
	    return 1;
	write("No such item ("); write(item); write(") here\n");
...
buy(item) {
    if (!item)
	return 0;
    call_other("players/deathmonger/storage","buy",item);
    return 1;
}
...
north() {
    ...
    this_player()->move_player("north#players/deathmonger/storage");
    return 1;
}

list(obj) {
     call_other("players/deathmonger/storage","inventory",obj);
    return 1;
}
