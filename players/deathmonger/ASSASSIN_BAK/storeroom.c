...
int value;
string name_of_item;
object sootmine, quitmine, modamine, shoutmine;
...
{
    if (!present("sootmine")) {
       sootmine = clone_object("players/deathmonger/ASSASSIN/MINES/soot_mine");
       move_object(sootmine, this_object());
    }
    if (!present("shoutmine")) {
     shoutmine = clone_object("players/deathmonger/ASSASSIN/MINES/shout_mine");
     move_object(shoutmine, this_object());
}
    if (!present("modamine")) {
     modamine = clone_object("playesr/deathmonger/ASSASSIN/MINES/mod_dam_mine");
     move_object(modamine, this_object());
    }
    if (!present("quitmine")) {
    quitmine = clone_object("players/deathmonger/ASSASSIN/MINES/fs_quit_mine");
    move_object(quitmine, this_object());
    }
}
