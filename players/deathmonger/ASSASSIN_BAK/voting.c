...
my_reset(arg) {
   ...
   if(arg) return;
   if(!present("ballot")) {
      board = clone_object("players/deathmonger/ASSASSIN/ballot");
...
south(){
     this_player()->move_player("south#players/deathmonger/ASSASSIN/cenguild");
     return 1;
}
vote(str) {
    ...
    if(sscanf(str,"%d %s -- %s",issue, vote, comments) == 3) {
     write_file("/players/deathmonger/ASSASSIN/tally",
