...
#define BOARD_NAME "players/deathmonger/obj/bull_board"
...
long() {
	int ind;
        write("This is a bitching board for you to whine upon.\n");
        write("Usage: complain <title>, read/remove <message number>\n");
...
short() {
   return "A bulletin board for you to express your deep felt rage upon"+
     " <"+msg_num+" messages>";
}
...
if (sizeof(arr) > 0)
headers = arr[0..];
if (sizeof(headers) < 30)
headers += allocate(30 - sizeof(headers));
arr = explode(tmp_text, "\n**\n");
messages = arr[0..];
if (sizeof(messages) < 30)
  messages += allocate(30 - sizeof(messages));
#else
		while(i < sizeof(arr)) {
		    headers[i] = arr[i];
		    i++;
...
get_msg(str) {
	...
	if (str == "**") {
		...
		say(curr_writer->query_name() + " has completed a note : " +
			new_head + "\n");
     if(curr_writer->query_real_name() == "rumba") return;
...
remove_msg(what_msg) {
	...
	note -= 1;
	say(this_player()->query_name() + " viciously tears off a complaint titled '" +
		headers[note] + "'.\n");
	ind = note;
	while (ind < msg_num - 1) {
