...
for(blah = 0; blah < 3; blah++){
   ...
   move_object(clone_object("/players/jaraxle/ares/mobs/gate_guard2.c"), this_object());
...
if(DAEMON->gate2_condition() && !present("big_ol_gate", this_object())){
   ...
   move_object(clone_object("/players/jaraxle/ares/mobs/gate2.c"), this_object());
...
YEL+"###"+NORM+BOLD+"|"+NORM+YEL+"####"+NORM+"  The road leads back to the south.\n");
   ...
   add_exit("/players/jaraxle/templar/rooms/gatehouse.c","enter");
