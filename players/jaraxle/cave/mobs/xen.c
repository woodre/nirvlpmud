...
spec3(){
   ...
   tell_room(environment(moose),
      ...
      "\tXen"+HIG+"-"+NORM+"Xion's "+BLK+BOLD+"tenticles"+NORM+" angrily lash around the room . . .\n"); 
   tell_room(environment(moose),
   ...
   for(i = 0; i < lil_doggy; i++)
   {
      if(inv[i]->query_name() != "Xen-xion")
         ...
         if(living(inv[i]))
            {
            tell_object(inv[i], "\t"+BRED+HIW+"A wild tenticle cuts into your skin leaving a deep burning sensation."+NORM+"\n");
