...
reset(arg) {
   ...
   set_weight(1);
   set_value(1000+random(5000));
}

short(){ return ""+HIW+"Nature's "+HIG+"Blessing"+NORM+""+(worn_by?" ("+HIG+"floating nearby"+NORM+")":""); }

id(str) {
   return str == "natures blessing" || str == "orb" || str == "nature's blessing";
}
   ...
   if(id(str)){
      ...
      do_special(owner) {
         ...
         if(i==1){
          ...
          else ATT->hit_player(4+random(3));
            tell_room(environment(USER),
               capitalize(ATT->query_name())+" is "+damage+" with a "+cast+" from "+HIW+"Natures "+HIG+"Blessing"+NORM+"!\n");
            return 0;}
         return 0; }
