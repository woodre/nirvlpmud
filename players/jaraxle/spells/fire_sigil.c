...
if(USER->query_extra_level() > 49){
    write(
    "      "+HIR+"enflame "+HIW+"<"+HIR+"weapon"+HIW+">"+NORM+"......adds the fire element to any weapon.\n"+
    "                            (Cost is "+HIR+"high"+NORM+", be careful of overuseage)\n\n");
}

if(USER->query_extra_level() > 74){
    write(
    "      "+HIR+"pho"+HIY+"e"+HIR+"nix"+NORM+"..............Summons the Mighty Phoenix to\n"+
    "                                      bring life to the dead, and death to the living.\n");
}
write(
...
query_auto_load(){ 
	return "/players/jaraxle/spells/fire_sigil.c:"+location+"#"+powerlevel;
 }
...
return 1;
    ...
    add_action("summon_phoenix","phoenix");
...
if(AT)
   ...
   /*******************************************************************************/
  /******* WC Bonus SPell - Adds +1 WC per casting.  Max wc possible 19 **********/
 /******* ***********************************************************************/
...
static enflame_it(str){
    ...
    object a;
      ...
      if(USER->query_ghost()) return 0;
        ...
        if(USER->query_spell_dam()) return 1;
	    if(USER->query_extra_level() < 49) return 0;
        if(!AT){
			tell_object(USER,
				"You can only use this while in combat!\n");
			return 1;
    ...
    if((wc = (int)a->weapon_class()) == 20)
    {
		...
		/* Weapon breaks if you try to make it higher than wc19 */
   ...
   /******************************************************************/
	                        /* */
/*
   PHOENIX spell : Takes any and every corpse in the room and makes a
   'zombie' monster out of it. (dir/spells/zombie.c). Also causes 
   damage to every npc in the room at a cost of 25hp/25sp per 20hp
   per monster.
*/
                               /* */
static summon_phoenix(){

object corpse, ob, zombie;
string picture;
ob = first_inventory(environment(this_player()));
corpse = first_inventory(environment(USER));
zombie = clone_object("/players/jaraxle/spells/zombie");
picture = HIR+"\t    _,=\"( _  )\"=,_\n\t _,'    \\_>\\_/    ',_\n\t .7,     {  }     ,\\.\n\t  '/:,  .m  m.  ,:\\'\n\t    ')\",(/  \\),\"('\n\t       '{'!!'}'\n"+NORM;
      ...
      if(USER->query_ghost()) return 0;
	    ...
	    if(USER->query_extra_level() < 74) return 0;
/*
        if(!AT){
			tell_object(USER,
				"You can only use this while in combat!\n");
...
*/
    write("You raise your hands high into the air and yell, 'PHOENIX!'\n");
    say(this_player()->query_name()+" raises "+this_player()->query_possessive()+" hands high into the air and yells,\n"+
       "            P H O E N I X !\n");

    write("A blaze of fire erupts from the ground...\n");
    cat("/players/jaraxle/spells/PHOENIX");
    say(picture);
    write("A giant phoenix rises from the ground\n"+
	  "and bathes the room in a wave of heat!\n");

    say("A blaze of fire erupts from the ground...\n"+
	"A giant phoenix rises from the ground\n"+
	"and bathes the room in a wave of heat!\n");

if(!corpse && !ob){ return 1; }
  
while(corpse){
if(corpse->query_corpse_level()) {
say("The Phoenix engulfs the corpse in flames.\nThe lost soul of the fallen levitates from the ashes.\n");
write("The Phoenix engulfs the corpse in flames.\nThe lost soul of the fallen levitates from the ashes.\n");
     move_object(zombie, environment(USER));
     destruct(corpse); 
}
corpse = next_inventory(corpse); }

while(ob){
if(ob->is_npc()){
  ob->attack_object(this_player()); 
  ob->hit_player(10); }
    say(HIR+"Flames"+NORM+" shroud over "+ob->query_name()+".\n");
    write(HIR+"Flames"+NORM+" shroud over "+ob->query_name()+".\n");
  if(ob->is_player()){ write(ob->query_name()+" is unaffected by the Phoenix.\n"); }
