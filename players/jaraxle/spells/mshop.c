...
if(!present("sigil_mage")){
	owner = clone_object("/players/maledicta/closed/sigils/sigil_mage.c");
	move_object(owner, this_object());
	}

  if(arg) return;
set_light(1);
short_desc = "Qual'tor sigil shop";
long_desc =
"  This is a small shop. Several shelves line the stone walls, each\n"+
...
items = ({
  "walls",
  "Made from stone, these walls seem very sturdy",
  "shelves",
...
dest_dir = ({
  "/players/daranath/qualtor/east_road4.c", "out"
});
}
...
if(busy){ write("The mage says, 'Sorry, one person at a time.'\n");
      ...
      busy = 1;
	if(!purchase){ write("Usage: purchase fire, ice or electricity.\n"); busy = 0; return 1; }
	if(purchase == "fire"){
	  item = clone_object("/players/maledicta/closed/sigils/fire_sigil.c");
	  what = "Fire Sigil";
	  }
	else if(purchase == "ice"){
	  item = clone_object("/players/maledicta/closed/sigils/ice_sigil.c");
	  what = "Ice Sigil";
	  }
	else if(purchase == "electricity"){
	  item = clone_object("/players/maledicta/closed/sigils/elec_sigil.c");
...
/* Copied from Zeus' Code - Thanks BUDDY! */
 ...
 tp->add_money(-10000);
 write_file("/players/maledicta/log/sigils",
