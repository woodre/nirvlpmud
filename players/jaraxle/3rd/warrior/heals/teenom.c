...
reset(arg){
   ...
   set_short(HIB+"Teen"+HIR+"om"+NORM+" fruit "+HIB+"("+HIR+NM+HIB+")"+NORM);
   set_weight(1);
   set_long(
...
squeeze_cmd(str){
   if(NM == 2){
      ...
      write(HIB+"\tHP "+HIR+"("+HIW+""+TP->query_hp()+""+HIB+"/"+HIW+""+TP->query_mhp()+""+HIR+")  "+HIB+"SP "+HIR+"("+HIW+""+TP->query_sp()+""+HIB+"/"+HIW+""+TP->query_msp()+""+HIR+")\n"+NORM);
      say(TP->QN+" squeezes some juice from a "+HIB+"Te"+HIR+"enom"+NORM+" fruit.\n");
      
      this_player()->recalc_carry();
      NM -= 1;
      write("You can sqeeze "+HIB+"("+HIR+NM+HIB+")"+NORM+" more use out of the fruit.\n");
      return 1; }
