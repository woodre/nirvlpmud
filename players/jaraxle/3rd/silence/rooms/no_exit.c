...
check_light() { tell_object(find_player("vertebraker"), "\n\tLight check " + set_light(0) + ".\n\n"); }
...
if(!dest_dir || no_exits)
            ...
            if (sizeof(dest_dir) == 2)
                write("    "+GRN+"There is one obvious exit:"+NORM);
            else
               write("    "+GRN+"There are " + convert_number(sizeof(dest_dir)/2) +
                     " obvious exits:"+NORM);
            while(i < sizeof(dest_dir)) {
                write(" " + dest_dir[i]);
                i += 2;
