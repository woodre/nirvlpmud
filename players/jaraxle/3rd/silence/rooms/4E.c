...
reset(arg) {
  ...
  ""+GRN+"\t@"+CYN+"#"+GRN+"@@@"+YEL+"#####"+GRN+"@"+NORM+"\n"+
   ""+GRN+"\t@@@@"+YEL+"#"+GRN+"@@@@@@\n"+NORM+""+
   ""+RED+"\t$"+NORM+YEL+"#"+GRN+"@@"+BLU+"X"+YEL+"#####"+HIM+"~"+NORM+"\n"+
...
south(){
   ...
   this_player()->move_player("south#/players/jaraxle/3rd/silence/rooms/5E");
   return 1; }

east(){
   write("You follow the path to the east.\n");
   this_player()->move_player("east#/players/jaraxle/3rd/silence/rooms/4F");
   return 1; }

north(){
   write("You follow the path to the north.\n");
   this_player()->move_player("north#/players/jaraxle/3rd/silence/rooms/3E");
   return 1; }


search_me(str){
   if(!str){
      write("You come to a perpendicular crossing at the edge of\n"+
         "the path.  Large trees have sprouted up to the west\n"+
         "blocking any further travel in that direction.\n");
      return 1; }
