...
short() { return ""+YEL+"-"+HIY+"Pack of Heals"+NORM+YEL+"-"+NORM+""; }
long() { price();
   write("This is a small little computer box.  It has several differen't\n"+
      ...
      ""+HIY+"["+HIW+" "+tox+" "+HIY+"]"+NORM+" Tox Rate.\n"+
      ""+HIY+"["+HIW+" "+no+""+HIY+" ]"+NORM+" Number of heals.\n"+
      ""+HIY+"["+HIW+" "+price+" "+HIY+"]"+NORM+" Price to expand.\n"+
...
makeheals(arg) {
   write("You may set the following attributes:\n"+
      "1. Tox "+HIY+"<"+HIC+"Low"+NORM+", "+HIB+"Medium"+NORM+","+HIR+" High"+HIY+">"+NORM+"\n"+
      "2. Healnumber "+HIY+"<"+NORM+"Up to 15"+HIY+">"+NORM+"\n"+
      "3. Expand Heals\n"+
...
tox(arg) {
   ...
   write("Your heals will now expand to be "+BLU+BLINK+tox+NORM+" toxing.\n");
   return 1; }

healno(arg) {
   if(!arg) { write("The pack may be expanded into a maximum of 15 groups of\n"+
        "heals.  Each group consits of (3) of each heal.  Choose\n"+
       "how many heals you would like using this command:\n"+
         "Healnumber <Up to 15>\n");
      return 1; 
   }
...
expand(arg) { price();
   ...
   if(no == "UnSet" || tox == "UnSet" || price == 0) { write("The pack is not ready to expand yet.\n"); return 1; }
   write("\nYou press the expand button on the pack and it vaporizes into "+HIB+BLINK+""+no+""+NORM+" heals!\n");
   TP->add_money(-price);
   ex();
   return 1;
}
...
MOCO("/players/jaraxle/3rd/alt/wastes/heal.c"),TP);
