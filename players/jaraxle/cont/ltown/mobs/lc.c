...
vlvl = 19;
switch(random(4)){
case 0: my_name = "luckycurse";
break;
case 1: my_name = "luckycurse";
break;
case 2: my_name = "luckycurse";
break;
case 3: my_name = "luckycurse";
break;
}
        ::reset(arg);
        if(arg) return;
    set_name(RED+"Luckycurse"+NORM);
set_alt_name(my_name);
    ...
    set_mp(700);
   set_sp(700);
set_symb_name(my_name);
        set_hp(450);
        set_wc(3);
        set_ac(3);
   set_wander(20, 0, ({ "leave" }));
        set_heart_beat(1);
...
int x, z;
      ...
      x = sizeof(obs);
      for(z = 0; z < x; z++){
if(obs[z]->query_level() < 20 && obs[z]->query_level() > 10 && call_other(obs[z], "query_pl_k",0) == 1) {
attacked_by(obs[z]);
          }
      }
   }

if(environment() == environment(attacker_ob))
