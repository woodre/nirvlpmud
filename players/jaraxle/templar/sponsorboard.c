...
short() { return "Templar Sponsor Board"; }
...
long() {
int i;
  write("Templar Knight Prospective Members\n\n");
  write("Use add <name> to add prospective members.\n");
  write("Use remove <name> to remove entries.\n");
  write("To sponsor a Prospect, you must be at least\n"+
        "guild rank 6: sponsor <name>\n");
  write("To Veto a prospect: veto <name>\n");
...
while(i < CAP){
  if(who[i] == str){
     if(sp1[i] && sp2[i] && sp3[i]){
        write("That prospect already has 3 sponsors.\n");
        return 1;
        }
...
write("You sponsor "+str+" as a member of the knights.\n");
save_object("players/boltar/templar/items/sponsorboard");
return 1;
}
...
while(i < CAP){
  if(who[i] == str){
     if(vt1[i] && vt2[i]){
      ...
      vt3[i]=0;
        write("Done.\n"+
              "Please notify Jaraxle or Fakir of this occurence.\n");        
       write("You Veto "+str+" as a member of the knights.\n");
       save_object("players/boltar/templar/items/sponsorboard");
       return 1;
