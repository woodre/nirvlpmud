...
reset(arg) {
	if(!present("templar_trainer")){
		owner = clone_object("/players/jaraxle/templar/mobs/trainer.c");
		move_object(owner, this_object());
	}
	if (arg) return;
	skill_setup();
	set_light(1);
    short_desc = HIW+"Training Hall ["+NORM+GRN+"Fortress Stone"+HIW+"]"+NORM+"";
	long_desc =
    "  The main training hall of the castle has a circular dirt area for\n"+
    "combat training. There are numerous types of weapons held on racks\n"+
    "beside the western wall, and a door leads to the north where personal\n"+
    "lockers are visible within the well lit room. A second doorway exits\n"+
    "the room to the east, and a magical portal leads south to the Arena.\n"+
    "There is a "+BLINK+GRN+"sign"+NORM+" here.\n";
	skill_setup();
	items = ({
			"door",
			"A heavy wooden door inlaid with solid strips of iron",
                   ...
                   "The sign reads:\n"+
                   "  \n"+
                   "  Going south through the portal will send you to Feldegast's Arena",
	});
	
	dest_dir = ({
			"/players/jaraxle/templar/rooms/equipment.c","east",
                   "/players/feldegast/arena/rooms/foyer.c","south",
			"/players/jaraxle/templar/rooms/templar_locker.c","north",
	});
