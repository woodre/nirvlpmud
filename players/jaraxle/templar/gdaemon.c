...
reset(arg){
   ...
   castle_donation = 50000;   
	mobs = allocate(4);
	mobs = ({0,50,100});
	restore_object("players/jaraxle/ares/guards");
}
...
sub_guards(int i){
	mobs[i] -= 1;
	save_object("players/jaraxle/ares/guards");
}
...
set_war(int i){ war = i; save_object("players/jaraxle/ares/guards"); }

query_war2(){ return war2; }
set_war2(int i){ war2 = i; save_object("players/jaraxle/ares/guards"); }
