...
reset(arg) {
	...
	skill_setup();
	set_light(1);
	short_desc = "Templar training hall";
	long_desc =
	...
	dest_dir = ({
			"/players/jaraxle/templar/rooms/hallway2.c","west",
	});
...
init(){
	...
	if(!present("KnightInvite", this_player()) && this_player()->query_level() < 20){
		if(this_player()->query_level() < 15 || !this_player()->query_pl_k()){
			tell_object(this_player(),
			"You do not have the courage to invade the templar castle!\n");
			move_object(this_player(), "room/south/sshore1");
			return 1;
		}
	}
