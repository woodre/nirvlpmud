...
heart_beat(){
  ...
  my_att = USER->query_attack();

  if (!my_att)
    return;

  enemy_att = my_att->query_attack();


  if ((my_att && enemy_att) && (enemy_att == USER) && (other_knights_atking(USER, my_att))) {
   ...
   /* Check if other Knights are present and attacking the mob */
