...
enchant_it(str) {
   ...
   if(!ob->is_armor()) { write("You don't think that is an armor.\n"); return 1; }
   ...
   arm = clone_object("/players/jaraxle/templar/items/armshad.c");
   arm->add_shadow(ob);
   enc_obj = clone_object("/players/jaraxle/templar/items/arm_enchant_obj");
   enc_obj->set_shadow(arm);
   enc_obj->start(this_player(), 0);
   move_object(enc_obj, this_player());
   arm->set_enc_obj(enc_obj);
