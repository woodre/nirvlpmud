...
id(str) { return str == "foot" || str == "champion foot" || str == "right foot"; }
short(){ return "A "+GRN+"foot"+NORM+""; }
long() {
   write(
      "This is the right foot of the Coorquak Champion. Its green skin, and scaly feel\n"+
...
consume_foot(str){
   ...
   if(present("jar_ste2",TP)){ write("You do not think you could bare to eat another foot.\n"); return 1; }
   
   if(TP->query_real_name() == "guest") { write("You may not be healed.\n"); return 1; }
   typ = "ste";
   spell = clone_object("/players/jaraxle/closed/forbin/ste2.c");
   spell->start(typ);
