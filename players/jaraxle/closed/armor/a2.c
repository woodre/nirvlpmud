...
#define PATH "/players/jaraxle/closed/armor/powers/"
...
get_armor(object ob, int bonus){
   ...
   if(random(100) < abonus){
      switch(random(abonus) + random(20) + 2){
         ...
         case 68..72:   pre = "Brutal"; wc += 1; vl += 300; break;
         ...
         case 51..54:   pre = "Massive"; wc += 1; wgt += 3; vl += 400; break;
   ...
   /* Little Bonus Ac to go with the special */
   if(random(100) < abonus * 2){
      switch(random(abonus) + 1){
         case 95..100:  rwc = 1;  rchance = 30; vl += 1000; break; 
   ...
   wep = clone_object("/players/jaraxle/closed/weapon/weapon2.c");
   wep->set_name(nn);
   wep->set_short(short);
   wep->set_long(long);
   wep->set_type(type);
