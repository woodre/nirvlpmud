...
catch_tell(string txt) {
   if (listen && owner) tell_object(owner, "Undead> " + txt);
}
#endif
...
heart_beat()
{
   object att, weap;
      ...
      if (att) tell_object(att, "corpse::heart_beat() in root()\n");
   }
#endif
...
init()
{
   if (objectp(owner) && this_player() == owner) {
      ...
      if ((int) owner->query_alignment() > -40
            ||  random(query_wc()) > (int) owner->query_attrib("wil"))
      kill_ob = owner;
   }
...
if ((string) armour->query_type() != "shield" && (string) armour->query_type() != "armor") {
