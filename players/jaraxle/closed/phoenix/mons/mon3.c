...
switch(random(4)){
case 0..1:
   MOCO("/players/jaraxle/closed/phoenix/weaps/strike.c"), environment(this_object()));
break;
case 2..3:
MOCO("/players/jaraxle/closed/phoenix/amulet.c"),environment(this_object()));
break;
}
write_file("/players/jaraxle/closed/phoenix/phoenix.log",ctime(time())+" "+HIY+""+this_player()->query_name()+""+NORM+" killed Phoenix.\n");
}
   return 0; }     
...
spec1(){
  tell_room(environment(this_object()),
    "The "+RED+"Phoenix"+NORM+" lets out a loud shrill.\n"+
"\tThe flames over the "+RED+"Phoenix"+NORM+" glow brighter.\n");
  weapon_class += 1+random(2);
}
