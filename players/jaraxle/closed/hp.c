...
if(!present("metalbox",ENV(TP))) { write("You need to attach the pack to the metal box!\n"); return 1; }
...
write("You attach the pack to the metal box.\nThe box hums and the pack vaporizes!\n");
TP->add_money(-price);
write_file("/players/jaraxle/logs/healpack",""+CAP(TPRN)+" expanded "+POS+" heals: [ "+no+" ] [ "+tox+" ] [ "+price+" ].\n");
ex();
return 1;
}
...
write(""+YEL+"A heal pops out of the metal box and lands in your inventory!"+NORM+"\n");
MOCO("/players/kazer/specialty/OBJ/heal.c"),TP);
