...
wield(str) {
	...
	if(sscanf(str, "%s with both hands", woot) == 1){
		...
		if(!message_hit)
		message_hit=({""+HIR+"massacre"+NORM+""," to small fragments",""+RED+"smashed"+NORM+""," with a bone crushing sound","hit"," very hard","hit"," hard","hit","","grazed","","tickled"," in the stomach"});
...
short() {
	string tmp_short;
   if(!environment()) return "Special Weapon";
	if(this_player())
...
id(str) {
	...
	return str == type || str == name_of_weapon ||
	str == alt_name || str == alias_name ||
	str == "weapon" || str == "generic_wc_bonus" ||
	(pet_allow ? (str == "pet_allow_wep") : (str == "weapon"));
...
/* 02/23/06 Earwax: Changed to just doing locker storage */
/*
...
/* 03/13/06 Earwax: making it so his misc weapons don't save */
int query_save_flag() { return Save_Flag; }
void set_save_flag(int arg) { Save_Flag = arg; }
