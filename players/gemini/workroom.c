...
/* Changed by Rumplemintz
pwdwho = find_player(str); */
  if (pwdwho->query_level() > this_player()->query_level())
    write(str+" is in /players/"+pwdwho->query_real_name()+"/\n");
  else
...
memory(str) {
  ...
  if (!str) {
    command("tail /log/WR" + this_player()->query_real_name() + "_memory",
            this_player());
    return 1;
  }
  if (sscanf(str, "%s#%s", action, reason) != 2) {
    ...
    write_file("/log/WR/" + this_player()->query_real_name() + "_memory",
               ctime() + "--IP: " + reason + " Player: " + action + "\n");
    write("Wrote: IP: " + reason + " Player: " + action + ", to /log/WR/" +
          this_player()->query_real_name() + "_memory.\n");
    return 1;
  }
}
...
dungeon_player(str) {
#if 1
  /* Changed by Rumplemintz - use imprison command instead */
  write("Use the imprison command instead....\n");
  command("wizhelp imprison", this_player());
#else
   object fag;
   string bye, there;
     if(!str) return 0;
...
harry_tell(str) {
  ...
  if (sscanf(str,"%s %s",who,what)==2) {
    ...
    if (!plyr || (plyr->query_invis() > this_player()->query_level())) {
      write(capitalize(who) + " is not online now.\n");
      return 1;
    }
#if 0 /* Basically an echoto - not legal until wiz level 40 - Rump */
...
someone_tell(str) {
	...
	if(!plyr || (plyr->query_invis() > this_player()->query_level())) {
          write(capitalize(who) + " is not online now.\n"); return 1;}
#if 0 /* Not legal to impersonate 'Someone' until wiz level 200 - Rump */
...
extra_tell(str) {
#if 0 /* Why bypass the regular tell command? - Rump */
      object plyr;
	string who;
	string what;
	if(!str) {write("Tell what?\n"); return 1;}
	...
	return 1;
#else
  return (int)"/bin/play/_tell"->cmd_tell(str);
#endif
   }
...
force (str) {
  ...
  if (!player || player->query_invis() > this_player()->query_level()) {
    write ( capitalize(name) + "is not online. \n");
  }
#if 0 /* Rumplemintz - level 50 wizard command */
  command(cmd,player);
#endif
  return 1;
   }
...
#if 0 /* Rumplemintz - Level 40 wizard command */
     shout(str + "\n");
#endif
    write("You echo: " + str + "\n");
   return 1;
...
iwho(str) {
  ...
  for(x=0;x<sizeof(us);x++) {
    ...
    y++;
#endif
    if(is_invis==0 && us[x]->query_invis() < this_player()->query_level()){
...
church() {
#if 0 /* No need for this, just use wizard command - Rump */
    this_player()->move_player("away#room/church");
    return 1;
#else
  return (int)"/bin/wiz/_church"->cmd_church();
#endif
}
