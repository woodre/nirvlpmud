...
/* will have to be updated year by year */
      ...
      while(sscanf(deathlog,"%s\n%s",worktext,deathlog)==2) {
         ...
         string playcolor;
         spar = 0;
          if(sscanf(worktext,"%s 2009 %s %d exp %s gold killed by %s.",deathtime,playname,playexp,playgold,playkill)==5
          ||sscanf(worktext,"%s 2010 %s %d exp %s gold killed by %s.",deathtime,playname,playexp,playgold,playkill)==5
          ) {
