/*   This weapon is very very hard to get.  It is on a huge monster in 
   ...
   if they don't have them.  It also sucks 2 hitpoints every round, hit
...
/*  Note this 2 hitpoint reduction every round.  */
   if(wielded_by)
   wielded_by->add_hit_point(-2);
   if(whack > 95) {
      ...
      if(wielded_by->query_sp() < 15) {
         ...
         wielded_by->add_hit_point(-random(20));
         write(NORM+"\n");
   ...
   if(whack > 85) {
      ...
      if(wielded_by->query_sp() < 10) {
         ...
         wielded_by->add_hit_point(-random(15));
         return 0;
      }
