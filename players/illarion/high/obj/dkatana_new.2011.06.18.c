...
init() {
  ...
  if(this_player() == environment()) {
    write("This weapon has been updated:\n"+
          " - Higher level wielders will find it is much more effective.\n"+
          " - Very skilled wielders will find they can wield it in one hand.\n"+
          " - Persistent wielders will get a nice surprise.\n");
  }
}
...
status count_hit_made() {
  ...
  else if(align_hit && deathblows>10 && random(deathblows) > random(40+deathblows))
    alignment_hit();
  return 1;
}
...
int weapon_hit(object att) {
  ...
  if(wil>6)
      wil=6;
 align_hit=0;
  if(att->query_hp() < ((int)att->query_mhp())/12) {
  ...
  if(random((int)WIELDER->query_extra_level()/2) + deathblows/10 > random(20)) {
    if(wil) {
      
      if(WIELDER->is_npc()) return 0;
      write("You focus your mind on your sword.\n"+
...
add_deathblow() {
  if(ETO) tell_object(ETO,"Your katana grows stronger!\n");
  deathblows++;
}
