...
void long(string str) {
  string desc;
  if(set_light(0) < 1 && !present("dark_sight_object", this_player())) {
    write("It is dark.\n");
    return;
  }
  if(!str) {
    ...
    if(this_player() && (int)this_player()->query_level()>=21) {
      write("Level: "+CONTROLLER->query_level_number()+"\n");
    }
...
void create(int c, int r, string *exits) {
  ...
  row=r;
  if(!random(4)) {
    long_desc+="  A torch attached to the wall provides a little light.";
    add_item("torch",
"The torch is firmly attached to the wall, and gives off light, but no heat or "+
"smoke.");
    set_light(1);
  }
}
void set_special_exit(string dir,string dest) {
  ...
  spec_dest=dest;
  if(spec_dir=="up") {
    long_desc+="  A stone staircase leads up.";
  } else if(spec_dir=="down") {
    long_desc+="  A stone staircase leads down.";
  }
}
...
status has_exit(string str) {
   return member_array(str, dest_dir) != -1;
}
