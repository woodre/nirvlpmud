...
static int pow, lt, fortify_power;
...
need_monitor() {
    ...
    int itx, f,s;
    int nip;
    if(reduced) NEEDMON = "hp/sp/t  ";
    else NEEDMON = "hp/sp/i/f/s/t  ";
	...
	if(need_mon[j]) if(needer = present(need_mon[j],environment(ep))) if(needer->is_player()) {
			...
			if(reduced) NEEDMON += need_mon[j]+" : "+nhp+"/"+nsp+"/"+nip+"  ";
			else NEEDMON += need_mon[j]+" : "+nhp+"/"+nsp+"/"+itx+"/"+f+"/"+s+"/"+nip+"  ";
		    }
		}
    }
...
add_fortify(int a) { fortify_power += a; }
query_fortify() { return fortify_power; }
consume_fortify() { fortify_power = 0; }
...
heal(str) {
  ...
  if(query_verb() == "hp" || query_verb() == "sp") {
    call_other(POWER,"heal",lev,pow,mwho,query_verb(),mmuch);
  } else {
    call_other(POWER,"heal",lev,pow,mwho,"heal",mmuch);
  }
  return 1; 
}
mass_heal(str) {
  ...
  call_other(POWER,"mass_heal",lev,pow,mass_p,mtype,mmuch);
  return 1;
}
...
defend(str) {
    ...
    call_other(POWER,"defend",lev,pow,str);
    return 1;
}
/* Changed for a cap on level - Maledicta */
shield(str) {
    if(shield_var[0] > lev) return 0;
    call_other(POWER,"shield",lev,pow,str);
    return 1;
}
