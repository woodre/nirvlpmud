...
status main(string str, object PO, object User)
{
  ...
  write(pad("Hit Points: "+(int)User->query_hp()+"/"+(int)User->query_mhp(), 32));
  write("Energy: "+(int)User->query_sp()+"/"+(int)User->query_msp()+"\n");
  ...
  if((int)User->query_level() > 50) PO->show_weapons();
  return 1;
}