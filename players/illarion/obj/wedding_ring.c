...
#define PATH "players/illarion/obj/savefiles/"
...
string extra_look () {
  return own + " is wearing a "+HIY+"golden "+NORM+"wedding band inscribed: "+inscript;
}

string query_auto_load() { return "/players/illarion/obj/wedding_ring:";
}
...
init_arg(){
	if(restore_object(PATH+tp->query_real_name())) {
		...
		if(!fsp) {
           ...
           int x;
           for(x=0;x<3;x++) {
              tmp=find_player(({"lyric","alexis","lizard"})[x]);
              if(tmp) {
                 this_object()->set_spouse(({"lyric","alexis","lizard"})[x]);
              }
           }
        }	   
	}
...
string query_spouse() {
  ...
  tell_object(eo,"query_spouse done by "+name+"\n");
  if(name == "lyric" || name == "alexis" || name == "lizard")
    set_spouse(name);
  return spouse;
}
...
int renew_my_ring() {
   object ob;
   ob = clone_object("/players/illarion/obj/wedding_ring.c");
   move_object(ob, eo);
   if( (int)ob->fix_ring() ) {
...
sex( str ){  
   ...
   if( str == "yes" && (int)find_a_player()  ||  present(str, environment(tp)) ){
     ++ring_color;
     if( ring_color > 4 ){ ring_color = 4; }
     ring_save();
   }
