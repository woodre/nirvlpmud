...
advance() {
  ...
  int player_xlvl;
  int player_qp;
  int player_approvals;

  /* Convenience variables to make conditionals more readable. */
  player_xlvl = (int)this_player()->query_extra_level();
  player_qp = (int)this_player()->query_quest_point();
  player_approvals = (int)this_player()->query_aprv();

  player_ob = this_player();
  if ((string)this_player()->query_guild_name() == "paladin") {
  ...
  if (next_level == 20) {
    /* Handle potential wiz advancement failure cases. */  
    if (player_xlvl < 16) {
      /* Must be at least +16 */
      notify_fail("You must go upstairs to advance your level.\n");
      return 0;
    } else {
      if (player_xlvl < 100 && player_qp < NEEDED_QP) {
        /* If less than +100 they must have enough quest points. */
        notify_fail("You have " + player_qp + " quest points"
                  + " and you need " + NEEDED_QP + " of "
                  +   TOTAL_QP + " to become a wizard.\n");
        return 0;
      } else if (player_approvals < 3) {
        /* Check approvals. */
        notify_fail("You need the approval of three senior wizards to advance " +
                    "to apprentice wizard.\n");
        notify_fail("You have " + player_approvals + " approvals.\n");
        return 0;
      }
    }
  }
