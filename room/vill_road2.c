...
FOUR_EXIT("room/vill_road1","west",
 ...
 "constant travel of people back and forth. There is a small "+BOLD+"shop"+NORM+" to\n"+
 "the north and the "+BOLD+"Adventurer's guild"+NORM+" to the south.\n"+
...
look(str) {
  ...
  if (str == "at shop") {
    ...
    "The entrance is on the north side of the main road.\n");
    return 1;
  }
...
gives(str) {
...
#else
        funcall(symbol_function('transfer), obj, environment(harry));
#endif
...
#else
      funcall(symbol_function('transfer), obj, find_living(lower_case(who)));
#endif
      notify("Harry returned the " + what + " to " + who + ".\n");
  }
    } else if(what == "corpse") {
  notify("Harry says: HEY, burry your corpses yourself, asshole.\n");
  obj = first_inventory(harry);
  while(!call_other(obj, "id", what))
      obj = next_inventory(obj);
#ifndef __LDMUD__
  transfer(obj, find_living(lower_case(who)));
#else
  funcall(symbol_function('transfer), obj, find_living(lowercase(who)));
#endif
  notify("Harry returned the " + what + " to " + who + ".\n");
    } else {
