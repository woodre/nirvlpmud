...
RestoreGuild( string gname ) {
  ...
  if( siz==1 && gname ) {
    write("Type 'restore' to restore your guild object.\n");
    return 1;
  }
  ...
  if( siz > 1 && member(gnames, gname) < 0 ) {
    write("'"+gname+"' is not a valid option.\nType 'restore' for a list of valid options.\n");
    return 1;
  }
...
Advance_To( string str ) {
  ...
  if( this_player()->query_quest_point() > random( 700 ) ) {
    this_player()->add_prestige_skillpt( 1 );
    write(HIBLK+"Your honor gained while questing has granted you a bonus prestige skill point!\n"+NORM);
    return 1;
  }
  ...
  if( !this_player()->is_testchar() ) {
    call_other("/room/exlv_guild", "nirvana_msg", 
      "The clouds part as an "+YEL+"ancient light"+NORM+" floods into "+this_player()->query_name()+" and\n\
