...
extra_reset() {
/*
    if(!present("sign", this_object())) move_object(clone_object("obj/depository-sign"), this_object());
*/
    if (!guard || !living(guard)) {
  object key, weapon;
        guard = clone_object("obj/monster");
  call_other(guard, "set_name", "guard");
        call_other(guard, "set_alias", "villagebankguard");
...
TWO_EXIT("room/northroad1","west",
/*
         "room/depository","north",
*/
         "players/earwax/bank/bank", "north",
   "The bank",
   "You are in the bank.\n" +
...
unlock(str) {
  ...
  return 1;
    ...
    write("ok.\n");
/*
    if(this_player()->query_alias() != "villagebankguard") {
*/
   write("As you unlock the door, your key snaps off.\n");
    destruct(present("bank key",this_player()));
...
north()
{
 this_player()->move_player("north#room/depository");
}
