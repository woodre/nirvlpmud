...
"Note that players never can come here. set_quest(str) will return 1 is\n"+
    ...
    if (!silently) {
        if (i == 0)
            ...
            if (i - FREE_QUESTS <= 0)
               ...
               write("You must solve quests worth " +(NEEDED_QP - this_player()->query_quest_point()) + " quest points.\n");
/*
    ...
    while(ob) {
        if (call_other(ob, "id", "quest")) {
            ...
            if (i == 0) {
                ...
                write("Physical challenge: " + this_player()->quest_pt_physical(str) + "\t"
                  + "Mental challenge: " + this_player()->quest_pt_mental(str) + "\n");
                return;
            }
        }
        ob = next_inventory(ob);
