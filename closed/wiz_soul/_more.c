...
*/
/*
// Snatched from TMI by Shadowhawk for Nirvana 9/1
*/
...
get_oid() {
  ...
  sscanf( object_name( this_player() ), "%s#%d", junk, oid );
  return oid;
}
...
private void
swap_in() {
  swap_file = "/open/moreswap" + get_oid();
...
log_file("WSOUL", ctime()+": /closed/wiz_soul/_more.c called (player: "+(string)this_player()->query_real_name()+") by "+object_name(previous_object())+".\n");
  data = read_file( str );
  if( ! data ) return 0;
  swap_file = "/open/moreswap" + get_oid();
  fname = str;
