...
*/
  ...
  if(str) {
    if(level < ARCH && !("/pa/admin/pa-daemon"->query_name(name)) &&
...
string check;
...
#if 1
    log_file("WSOUL",
               "ED called from wiz_soul\n" +
               ctime() + "\n" +
               object_name(myself) + " : " +
               capitalize((string)myself->query_real_name()) + "\n" +
    ...
    if (find_player("rumplemintz")) {
      tell_object(find_player("rumplemintz"),
                  "ed() called from wiz_soul \n" +
                  "myself = " + object_name(myself) +"\n" +
                  "myself real_name = " + myself->query_real_name() + "\n" +
                  "handshake = " + handshake + "\n" +
                  "calling remote_ed from wiz_soul\n");
    }
#endif
    return 1;
}
...
#ifndef __LDMUD__ /* Changing this here as well as in file_auth - not
                     sure which one is being called right now
                     Rumplemintz */
check_me(str,check){
...
int remove_file(string str) {
  ...
  if(this_object()->check_me(str, check))
    return 1;
#if 1 /* Rumplemintz */
  write_file("/closed/log/RM",
             ctime() + "\n" +
             "RM " + str + " called by " +
             this_player()->query_real_name() + "\n" +
             "PWD is " + this_player()->query_pwd() + "\n" +
             "-----------------------------------------------------------\n");
#endif
  rm(str);
  return 1;
