...
status prevent_prepare_action_event(mapping e) {
  ...
  // MUST come first because of impossibility check!
  if (h = BODYPART::prevent_prepare_action_event(e))
    return h;
  if (e[E_TOOL] == this_object()) {
    switch (e[E_ACTION]) {
      case "punch":
        if (sizeof(h = get_held()))
          return preschedule_next_action(build_action("let_go"));
        if (!query_clenched())
          return preschedule_next_action(build_action("clench"));
        break;
    }
  }
  return
