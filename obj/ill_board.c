...
#if 0 /* Rumplemintz - changing this to setable variable... */
...
/* saved vars */
mixed *message_data;         /* contains content of messages mapping for
                               saving purposes */
mixed *reader_data;          /* contents of readers mapping for saving
                               purposes */
...
/* non-saved vars */
static mapping messages;     /* ([int message_number:({string header,
                                                      string message,
                                                      mapping comments]) */
                             /* comments is: ([int comment_number:
                                               ({string header,
                                                 string message})]); */
static mapping readers;      /* ([string reader_name:({message_nums_read,
                                                      comment_nums_read})]) */
static int num_msgs;         /* number of messages on the board */
static string name;          /* the name of the board (should be unique from
                               other boards with same DATA_DIR */

/* set these vars up when cloning the board */
static string data_dir;      /* Directory for saving .o files - Rumplemintz */
...
static object post_ob;       /* use the set functions for these objects to
                               control special cases like guilds, etc */
...
static mapping lusers;       /* keeps track of data from people writing
                               messages to allow multiple writers */
...
#else
convert_comments_to_map(mixed *comments) {
#endif
  int s,x;
  mapping ret;
  ret=([]);
...
/* Added next two functions - Rumplemintz */
string query_data_dir() { return data_dir; }
void set_data_dir(string str) { data_dir = str; }
...
/* Make sure the special can handle a post number of -1 for a general check,
   and a specific number for reading a certain message */
...
set_name(str) {
  ...
  for(x=0; x<s; x++)
    if(name[x..x]==" ")
      ...
      fname+=name[x..x];  
  restore_board();
  num_msgs=sizeof(m_indices(messages));
  lusers=([]);
}
...
get_all_unread() {
  ...
  while(s--) {
    if(member_array(mess_nums[s],readers[name][0])==-1)
      unread_msgs++;
    comm_nums = m_indices(messages[mess_nums[s]][2]);
    s2=sizeof(comm_nums);
    while(s2--) {
      total_coms++;
  ...
  return " (" + num_msgs + " message" + (num_msgs==1 ? "" : "s") + ", " +
            unread_msgs + " unread, " + total_coms + " comment" +
            (total_coms==1 ? "" : "s") + ", " + unread_coms + " unread)";
}
...
comment_check(int msg_num) {
  ...
  if(!readers[name])
    readers[name]=({({}),({})});
  comments = m_indices(messages[msg_num][2]);
  s=sizeof(comments);
  for(x=0;x<s;x++)
...
int cmd_list(string str) {
  ...
  comments = messages[m_indices(messages)[num-1]][2];
  comms = m_indices(comments);
  s=sizeof(comms);
  if(!s) {
...
int cmd_read(string str) {
  ...
  string name,junk;
  if(str) {
    msg_nums = m_indices(messages);
    name=(string)this_player()->query_real_name();
    if(sscanf(str,"message %d",mnum) ||
       (sscanf(str,"%d%s",mnum,junk)==2 && junk=="")) {
...
void remove_read_nums(int msg_id) {
  ...
  int s2;
  string *reader_names;
/* Changed keys to m_indices next two lines - Rumplemintz */
  comment_nums = m_indices(messages[msg_id][2]);
  reader_names = m_indices(readers);
...
int cmd_remove(string str) {
  ...
  if(str) {
    ...
    msg_nums = m_indices(messages); /* keys->m_indices Rumplemintz */
    ...
    if (sscanf(str, "message %d", mnum) ||
      ...
      sscanf(messages[msg_nums[mnum-1]][0], "Poster: %s Subject: %s",
             postername, junk);
      ...
      comments = messages[msg_nums[mnum-1]][2];
      com_nums = m_indices(comments); /* keys->m_indices Rumplemintz */
      s = sizeof(com_nums);
      if(cnum < 0 || cnum > s) {
        notify_fail("There is no comment " + cnum + " for message " + mnum +
                    "\n");
      ...
      sscanf(comments[com_nums[cnum-1]][0], "Poster: %s Subject: %s",
             postername,junk);
      if (lev < remove_level && lower_case(postername) != name) {
      ...
      messages[msg_nums[mnum-1]][2] = m_delete(messages[msg_nums[mnum-1]][2],
                                               com_nums[cnum-1]);
...
int cmd_post(string str) {
  ...
  if (!str || str=="message") {
    ...
    if (sizeof(messages[m_indices(messages)[mnum-1]][2]) > MAX_COMMENT) {
      write("No more comments can be posted for message " + mnum + ".\n");
      return 1;
    }
    lusers[me] = ({"comment",0,"", m_indices(messages)[mnum-1]});
...
#if 1 /* Rumplemintz */
  TEXT_EDIT->start_edit(this_object(), "end_write", MAX_LINES);
#else
  call_other(TEXT_EDIT,"start_edit",this_object(),"end_write",MAX_LINES);
#endif
}

void end_write(string final) {
  object me;
  string name,header;
  ...
  if (!final) {
...
#endif
             header[10..15];
    header = "Poster: " + capitalize(name) + " Subject: " + lusers[me][1] +
             " Time: " + header;    
    if (!readers[name])
      readers[name]=({({}),({})});
    if (lusers[me][0]=="message") {
      ...
      if (post_announce)
        emit_channel(post_announce, "(" + post_announce +
                     ") A new message has been posted to the " + query_name() +
                     " board.\n");
    } else {
      int msg_num;
      next_comment_number++;
      ...
      if(comm_announce)
        emit_channel(comm_announce, "(" + comm_announce +
                     ") A new comment has been posted to the " + query_name() +
                     " board.\n");
    }
...
void set_post_level(int x) { post_level=x; }
...
void set_comment_level(int x) { comment_level = x; }
...
void set_read_level(int x) { read_level = x; }
...
void set_remove_level(int x) { remove_level = x; }
...
void set_post_announce(string x) { post_announce = x; }
void set_comm_announce(string x) { comm_announce = x; }
void set_short_color(string x) { short_color = x; }
