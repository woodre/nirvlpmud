...
#else
#define TIME    ctime(time())[4..9] + ", " + ctime(time())[20..24]
#endif
#define EDITOR  "obj/editor"
#define PAGER   "obj/pager"
#define PASTE   "/open/paste/"+ REAL
...
void long(status wiz) { 
    ...
    write("'remove num'.  You can clip a note to your clipboard with\n"+
...
status remove(string str) { 
  ...
  say(NAME +" removed a note titled '"+ bulletins[i][HEADER] +"'.\n"); 
  write("You remove the note "+ bulletins[i][HEADER] +"...\n");
  bulletins = bulletins[0..i-1]+bulletins[i+1..sizeof(bulletins)-1];
  if(save_file) save_object(save_file);
  return 1;
} 
