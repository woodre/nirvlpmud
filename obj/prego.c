#define CAP_NAME environment()->query_name()
int pregnancy;
string pregtoo;
#ifndef __LDMUD__
string name, gender, current_room;
int age;
#endif
...
#else
  if ((string)this_player()->query_gender() != "female") return 0;
#endif
    if(pregnancy) return 0;
     ...
     if(this_object()->query_phys_at(3) > 1200)  {
      ...
      pregnancy = age;
#else
      pregnancy = (int)this_player()->query_age();
#endif
