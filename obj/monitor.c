...
align_bar() {
  ...
  align = (int) this_object()->query_alignment();
...
replace_read(string reading)
{
    ...
    switch(reading)
    {
      ...
      case "mhp": 
        ...
        break;
      case "g1":
        if(gob = present("GI", this_object()))
        {
	  z = (string )gob->query_gmon1();
	  if( !z )
	    z = "g1";
         break;
        }
...
print_monitor(object who)
{
    ...
    if( o = present("GI", this_object()) ) {
      guild_monitor = (string) o->query_guild_monitor();
      if( guild_monitor ) {
        call_other( o, guild_monitor, this_object() );
	return;
      }
    }
