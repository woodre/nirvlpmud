...
static int hitTime; /* v */
string owner; /* for 1-owner weapons */
string *ids;

/* 08/16/07 Earwax: Added these: */
static int MinXlevel;           /* Minimum XLevel to use */
static string MinXlevelMessage; /* Message given if below level */
...
/* added for donated items at the green. Fred [7.28.06] */
...
long() {
   int wear;
    write( call_other( this_object(), "query_long") );
    enchant_long();
    write("It is ");
#if 0 /** Modified by Gnar in case weapon is shadowed. Forces shadow obj to be called **/
    wear=query_wear();
#endif
    wear = call_other( this_object(), "query_wear" );
    ...
    if (owner) write("This weapon is owned by "+capitalize(owner)+".\n");
    if(donated_item) write("This weapon has been donated.\n");
    if(!this_object()->query_save_flag())
    {
      write("This weapon is Saveable.\n");
    }
    else
    {
      write("This weapon is Non-Saveable.\n");
    }
...
offwield(object ob_offwielding) {
  ...
  {
    notify_fail("You are already "+
      (offwielded?"offwielding":"wielding")+" it!\n");
    return 0;
  }
  if(wield_func)
...
wield(str) {
    ...
    /* Earwax - for one-owner weapons */
    if (owner && (string)this_player()->query_real_name() != owner)
    {
      notify_fail("Only "+capitalize(owner)+" can use this weapon!\n");
      return 0;
    }
    if (!id(str))
    {
      notify_fail("You may only wield \""+name_of_weapon+"\".\n");
      return 0;
    }
    ...
    /* 08/17/07 Earwax: added this */
    if (MinXlevel && (int)this_player()->query_extra_level() < MinXlevel)
    {
      notify_fail((MinXlevelMessage? MinXlevelMessage
        : "You aren't experienced enough to use a weapon of this quality.\n"));
      return 0;
    }
    if(wield_func)
        if(!call_other(wield_func,"wield",this_object())) 
...
short_old() {
  ...
  if(this_player()->query_level() > 30) {
      tmp_short = query_short() + "  < wc " + class_of_weapon + ","+eff_wc + " >";
   if (broke) tmp_short = query_short() + " [BROKEN]";
      if (wielded)
         return tmp_short + " (wielded)";
      return tmp_short;
     }
 if(wielded && broke) return query_short() + " [BROKEN] (wielded)";
 if(broke) return query_short() + " [BROKEN]";
    if (wielded)
        if(query_short())
            return query_short() + " (wielded)";
    return query_short();
}


/* 03/17/06 Earwax - Ownership code */
set_owner(string arg) { owner = arg; }
...
id(str) {
   ...
   return str == type || str == name_of_weapon || str == alt_name || str == alias_name || str == "weapon" || (multipleIds && member_array(str, multipleIds) > -1)  || (ids && member_array(str, ids) > -1);
}

void add_id(string arg) {
  if (!ids) ids = ({ arg });
  else if (member_array(arg, ids) < 0) ids += ({ arg });
}
string *query_ids() { if (!ids) return 0; else return ids; }
...
status remove_object(object caller) { 
  drop(1);
...
hit(attacker)
{
    if (wielded_by && hit_func && !present("no_weapon_attack", wielded_by) )
...
string junka,junkk;
  hit_func = ob;
  if(sscanf(object_name(ob),"%s#%s",junka,junkk) == 2)
    hit_func_fn = "/"+junka;
  else
    hit_func_fn = "/"+object_name(ob);
}
...
string junka,junkk;
  ...
  if(sscanf(object_name(ob),"%s#%s",junka,junkk) == 2)
    ...
    wield_func_fn = "/"+object_name(ob);
}

set_alias(n) { alias_name = n; }

set_short(sh) { short_desc = sh; if(!long_desc) long_desc = short_desc + "\n"; }
...
stopwield(str) {
  ...
  if(str && this_player() && present(str, this_player()) != this_object())
    return 0;
  
  if(!this_player() && (wielded || offwielded))
    ob = wielded_by;
...
if(broke) return 1;
/* dont save donated weapons. */
if(donated_item) return 1;
/* don't save enchanted weapons */
/* yes save enchanted weps. enchantments don't save anyway */
/*
...
/*   this stuff commented out cuz living uses count_hits/misses
  if(time() < hitTime) return;
  hitTime = time();
*/
  ah = random(arg*9/4);
...
fix_weapon() {
   ...
   if(broke == 7 && this_player() && this_player()->query_guild_name() != "samurai") {
...
/* 08/16/07 Earwax: added these */
void set_min_xlevel(int num) { MinXlevel = num; }
int query_min_xlevel() { return MinXlevel; }
void set_min_xlevel_message(string msg) { MinXlevelMessage = msg; }
string query_min_xlevel_message() { return MinXlevelMessage; }
