    ...
    if(!is_npc)
    {
      ...
      else if(!str)
      {
        ...
        if(this_player())
        {
          ...
          log_file("DEATHS","previous object is "+file_name(previous_object())+"\n");
        }
...
to the ground to die.\n");
    ...
    if(!is_npc)
    {
     ...
     /* Added this to clear crime if it exists - Rumplemintz */
      if(this_player()->query_crime()) this_player()->clear_crime();
    ...
    else if(attacker_ob && is_npc)
    {
      ...
      if(present("double_xp_object", attacker_ob))
        exp_worth *= 2;
        ...
        if(attacker_ob && !weps[s]->query_silent_damage() && 
          ...
          if(sizeof(dmess)==3) {
            ...
            if(query_invis() < INVIS_ACTION)
              tell_room(environment(), resolve_emote(dmess[1], this_object(), attacker_ob), ({ this_object(), attacker_ob }));
          } else {
  ...
  if(attacker_ob && call_other(attacker_ob, "query_name", 0) != NAME_OF_GHOST) {
    ...
    if(this_object() && !present("no_msg_hit", this_object())) {
      ...
      if(sizeof(mess)==3) {
        ...
        if(query_invis() < INVIS_ACTION)
          tell_room(environment(), resolve_emote(mess[1], this_object(), attacker_ob), ({ this_object(), attacker_ob }));
      } else {
...
drop_all_money(verbose) {
   ...
   if(!is_npc)
  {
      save_me();
#ifndef __LDMUD__ /* Rumplemintz */
  log_file("ADD_MONEY", ctime() +" "+ file_name(environment(this_object())) +" "+ name +" dropped "+ tmp +" coins\n");
#else
    log_file("ADD_MONEY",
...
#else
        funcall(symbol_function('log_file), "ADD_MONEY", object_name(environment(this_object())));
        funcall(symbol_function('log_file), "ADD_MONEY", ctime() +" "+ name +"("+ level +
#endif
