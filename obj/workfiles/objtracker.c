...
void objtracker_init_check()
{
 ...
 /* 5-12-07 added testchar check ~verte */
  if ( environment(this_object()) != this_player()
  || (status)this_player()->is_npc()
  || (status)this_player()->is_testchar() )
  {
    return;
  }
  ...
  log_file(OBJECT_VIOLATIONS_LOG, "["+ctime()[4..15]+"]: "+basename(environment(find_player(pname)))+" "+capitalize(AllHostName[hname][0])+": "+file_name(this_object())+" now carried by "+capitalize(pname)+" ("+hname+").\n");
  ...
  emit_channel("admin", "(admin) "+capitalize(AllHostName[hname][0])+" passed equipment to "+capitalize(pname)+"!\n");
  emit_channel("admin", "(admin) They were in: "+basename(environment(find_player(pname)))+".\n");
}
