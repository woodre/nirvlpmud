...
nomask string valid_read(string arg)
{
  ...
  play_pwd   = (string)this_object()->query_pwd();
  ...
  if (arg == "~" || arg == "~/") 
	  arg = "/players/" + play_name;
  else if (sscanf(arg,"~/%s",temp))
	  arg = "/players/" + play_name + "/" + temp;
  else if (sscanf(arg,"~%s",temp))
	  arg = "/players/" + temp;
  else if (arg[0] != '/')
	  arg = play_pwd + arg;
  ...
  {
    /* temporary ? free access for all 21+ */
    if(file[0..6]=="closed/")
    {
      if(play_level < SENIOR)
      {
        return 0;
      }
    }
    /* add guilds role hook */
    if(play_level < READ_OTHERS && creator(c) != who) 
    {
      return 0;
    }
  }
  
  else if(sscanf(str, "/pa/%s",file))
  {
    if("/pa/admin/pa-daemon"->query_name(play_name)) /* replace with role hook */
    {
      return str[1..strlen(str)-1];
    }
    else if(play_level < ELDER)
    {
      return 0;
    }
  }
...
nomask varargs string valid_write(string arg, object c)
{
  ...
  if(this_object()->query_inact())
  {
    ...
    {
      if(play_level < ELDER && who != play_name[0..0]+"/"+play_name)
      {
        return 0;
      }
    }
    ...
    {
      if(play_level < ELDER && who != play_name)
      {
        return 0;
      }
    }
  ...
  {
    if(play_level < ELDER && play_level > EXPLORE)
    {
  ...
  else if(sscanf(str, "/pa/%s/%s",dir,file))
  {
    if(dir == "log")
    {
      if(c_name=="/obj/soul" ||
      ...
      if(str[0..20]=="/pa/log/ip_data/alts/" ||
         str[0..21]=="/pa/log/ip_data/names/" ||
         str[0..20]=="/pa/log/ip_data/nums/")
      {
        return str[1..strlen(str)-1];
      }
    }
