...
reset(arg) {
	...
	call_other(money, "set_money", random(10000));
...
open(str) {
    ...
    if (find_living("villagebankguard")) {
       object guard;
       write("There is a loud clank.\n");
       safe_is_unlocked = 0;
       safe_is_open = 0;
       num_turn = 0;
       listen_ob = 0;
       write("The safe is locked.\n");
       guard=find_living("villagebankguard");
       guard->set_aggressive(1);
       command("unlock door",guard);
       command("open door",guard);
       command("east",guard);
       return 1;
    }
    safe_is_open = 1;
