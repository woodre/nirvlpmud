...
/*
This is a simple editor for making strings.

Example on how to invoke the editor.
...
#define PASTE "/open/paste/"+(string)this_player()->query_real_name()


static object caller;
static string caller_fn;

static status mark_paste;
...
command_parser(string str)
{
  ...
  if (!sizeof(new_message) && str == "**")
    str = "~q";
  if (str == "~q")
  {
    write("You quit...\n");
    if (caller)
      call_other(caller, "editor_quit", caller_fn);
      destruct(this_object());
      return 1;
  }
  ...
  if (sscanf(str,"~d%d,%d",from, to) || sscanf(str,"~d%d",from))
  {
    ...
    new_message = new_message[0..from-2]+new_message[to..sizeof(new_message)-1];
  ...
  if (sscanf(str, "~c%d,%d", from, to) ||
      sscanf(str, "~c%d", from))
  {
    if (!to)
      to = from;
    if (from < 1 ||
        from > sizeof(new_message) ||
        to < from ||
...
varargs void
edit(string return_fn, string init_message, status mark, mixed *args)
{
  extra_args = args;
