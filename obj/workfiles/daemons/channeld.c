...
add_channel(string channel_name, object player)
{
  ...
  else
  {
    ...
    if (player->query_channel_names())
    {
      channel_list = (string*) player->query_channel_names();
      if (member_array(channel_name, channel_list) == -1)
      {
        channel_list = channel_list + ({ channel_name });
        player->set_channel_names( (channel_list) );
      }
    }
  }
}
...
remove_channel(string channel_name, object player)
{
  ...
  if(players = ChannelsMap[channel_name])
  {
    ...
    if (player->query_channel_names())
    {
      channel_list = (mixed *) player->query_channel_names();
      if (member_array(channel_name, channel_list) == -1)
      {
        channel_list = channel_list - ({ channel_name });
        player->set_channel_names( (channel_list) );
...
channel_message(string channel, string msg)
{
  ...
  object *targets;
  string ansi;
  /* 07/10/07 Earwax: hehe, it annoys me and it'll be funny :) */
  if (msg == "..." || lower_case(msg) == "dot dot dot")
  {
    write("That is so annoying, the mud rebels against you!\n");
  ...
  for(i = 0; i < sizeof(targets); i++)
  {
    if(targets[i])
    {
      if (objectp(targets[i]) && environment(targets[i]))
      {
        if(member_array(targets[i], on_channel(channel)) != -1)
        {
          ...
          {
            if(!(ansi =
              (string) environment(targets[i])->get_ansi_pref(channel)))
                ansi = "";
