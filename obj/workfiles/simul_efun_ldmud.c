...
private inherit "/kernel/support/name"; /* Rumplemintz */
inherit "/obj/simul_efun/compatible";  /* Rumplemintz */
...
void log_file(string file,string str)
{
    ...
    set_this_object(previous_object());
  write_file(file_name, sprintf("%s%s%s", ANSI_BOLD, str, ANSI_NORMAL));
}
...
/* uncommented by mizan, at mokri's request, via root on MUD server. */
#include "/obj/simul_efun/mysql.h"
/* end changes */
...
#if 0 // Rumplemintz - renamed to filter()
  return filter_array(list, str, ob, extra);
#else
  return filter(list, str, ob, extra);
#endif
}
...
varargs object   /* varargs added by Rumplemintz */
*all_inventory(mixed ob)
{
  object obj;
/* Added by Rumplemintz to pass with no arg as per original efun
 * functionality
 */
