...
#else
      write_file("/OBJDUMP", sprintf("%-40s %s\n", object_name(ob),
                (environment(ob) ? object_name(environment(ob))
                                : "None" )));
#endif
    write("Done.\n");
}
...
varargs mixed query_attribute(string m, object ob) {
    mapping attributes;
    attributes = get_extra_wizinfo(0)[ATTRIBUTE_INDEX][ob || previous_object()];
...
#if 0 /* Rumplemintz */
  return (sscanf(file_name(ob||previous_object()), "%s#%d", j, j2) != 2);
#else
