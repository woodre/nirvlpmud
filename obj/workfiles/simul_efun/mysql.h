...
#else
    calling_from = object_name(previous_object());
#endif
...
*/
    handle_num = efun::db_connect("mud_data_base", "petebre");
...
{
    string calling_from, tmp_str, *tmp;
    int handle_return;
    ...
    calling_from = object_name(previous_object());
#endif
    if(sscanf(calling_from,"%s#%s",calling_from,tmp_str));
    if (calling_from != "object/mail_reader" && calling_from != "room/post")
    {
	tmp = explode(command,"mail");
	command = implode(tmp,"mail2");
    }
    if(valid_connector())
