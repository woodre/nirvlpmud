/* Not sure what happened to this, so putting it back in.
 * Mud complaining on startup about it missing. -- Rump
 */
 ...
 *    object, and make an inheritance of thie objet on the first line.
...
/* optional */
void set_ac(int ac) { armor_class = ac; }
/* optional */
...
/*
 * Force the monster to do a command. The force_us() function ins't
...
void remove_chat(string str) {
    chat_nr -= 1;
    chat_head = (object)chat_head->remove_chat(str);
}
...
void remove_a_chat(string str) {
    a_chat_nr -= 1;
    head = (object)a_chat_head->remove_chat(str);
}
...
void remove_match(string match) {
    head = (object)head->remove_match(match);
}
