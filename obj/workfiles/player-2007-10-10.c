...
static logon() {
  ...
  /*
  if (sscanf(query_ip_number(), "66.189%s", blockmsg))
  {
    write("No.\n");
...
set_wiz_level(string key){
    ...
    */
    ...
    log_file("PROMOTIONS", "["+ctime()[4..15]+"] "+capitalize(name)+" was "+verb+" to Level " + key +
...
#ifdef __LDMUD__
if (name == "sparrow") level = save_level = 100;
#endif
    no_give = 0;
  ...
  if(where=="room/adv_guild") {
    ...
    write(HIW+"The New Adventurer's Hall is located n,3w,enter hall from here.\n\n"+NORM);
  }
  ...
  if(!present("GI", this_object()) && !gob){
    if (guild_file && guild_file !="" && guild_file != 0)
    {
...
#include "/obj/player/dr/closed_guild_check.c"
  ...
  if (treasure > 0) {
    ...
    if (treasure > 1000) log_file("earwax.reimburse", ctime()+": "+name+" " + treasure+"\n");
   if (treasure > 200000) {
      treasure = 200000;
    }
	  money += treasure;
          if(money > 200000) money = 200000;
	  treasure = 0;
  }
  qc = 0;
...
string query_pwd() {
  ...
  if (this_player() == this_object())
    return pwd;
  if ((int)this_player()->query_level() > level) return pwd;
  return "";
}
