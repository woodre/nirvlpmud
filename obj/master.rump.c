...
#if 1
void set_this_object(object arg);
void raise_error(string arg);
string *get_error_file(string arg);
#endif
...
/* Could bypass all shutdown() security with MASTER->flag("shutdown")
  if (str == "shutdown") {
    shutdown();
    return;
  }
...
string get_wiz_name(string file) {
  ...
  if (sscanf(file, "d/%s/w/%s/%s", domain, name, rest) == 3) {
    return name;
  }
  return 0;
}
...
void log_error(string file, string message) {
  ...
  timestamp = ctime();
  if (!name) name = "log";
  write_file("/log/" + name, timestamp + message);
}
...
int save_ed_setup(object who, int code) {
  ...
  file = "/players/" + (string)who->query_name(1) + "/.edrc";
  rm(file);
