...
int size, save_flag, worn, ac, res, siz;
status silently;
object worn_by;
object next;
string info;
...
#else
    enchants=m_indices(enchantments);
#endif
    s=sizeof(enchants);
    while(s--)
 ...
 */
  ...
  if (MinXlevel && (int)this_player()->query_extra_level() < MinXlevel)
  {
    notify_fail((MinXlevelMessage ? MinXlevelMessage 
      : "You aren't experienced enough to use armor of this quality.\n"));
    return 0;
  }
  if (worn)
  {
...
/* END PALADIN ADDITION */
  ...
  /* Added to support multi-slot armors.
     Start changes.  Fred [03.28.07] */
  ...
  if(siz)
  {
    ...
    if(siz > 1)
    {
      while(z)
      {
        ob = call_other(this_player(), "wear", this_object(), silently, Types[z-1], params);
    ...
    write("You are already wearing an armor of type " +Types[z-1]+ ".\n");
  }
  /* End Changes - Fred */
...
remove(str)
{
  ...
  if(siz > 1)
  {
    while(y)
    {
      call_other(worn_by, "stop_wearing", this_object(), Types[y-1], silently);
...
del_param(ptype) {
  int i;
   if(!params) params = ({});
   if((i = member_array(ptype, params)) != -1) {
     params -= params[i+3];
     params -= params[i+2];
     params -= params[i+1];
     params -= params[i];
