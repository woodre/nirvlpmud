...
#define VERSION   "2.06-B"
/* game crash and object clone fixed by boltar (petebre@elof.iit.edu) */
/* Also 3.0.44-DR conversion */
#define VERSION_DATE  "911214"
...
{
   object tmpo;
   tmpo = environment(this_object());
   if(this_player() && tmpo) {
     if(this_player() != tmpo && this_player()->query_level() < 60)
       return 1;
     if(this_player() != tmpo && this_player()->query_level() < tmpo->query_level())
       return 1;
     }
...
id(str) 
{
    if(str && (str == "quicktyper" || str == "ND" || str == owner + "'s quicktyper" || str == "tech_quicktyper" || str == "qt")) {
  return 1;
    }
    return 0;
}
...
/*
The simul-efun replacement for root, this, and LD's preload don't play
nice together. the QT is obsolete, this appears to be a housekeeping 
line, so it should work fine commented out. -Bp
    if(!root()) call_out("destroy_me", 6);
*/
}
...
*/
  ...
  write("environment(this_player())="); write(environment(this_player())); write("\n");
    ...
    if(this_player()) {
        ...
        if(!owner || !environment(this_object())->is_player()) 
  owner = call_other(this_player(), "query_name");
  ...
  while(i < sizeof(list_ab)) {
      if(list_ab[i] && list_ab[i] != "" && list_cmd[i] && list_cmd[i] != "") {
            bxb=list_ab[i];
   if(bxb=="n"||bxb=="e"||bxb=="w"||bxb=="s"||bxb=="ne"||bxb=="nw"||bxb=="sw"||bxb=="se"||bxb=="u"||bxb=="d") {
          list_ab[i]=bxb+"q";
         }
   if(bxb=="north"||bxb=="east"||bxb=="west"||bxb=="south"||bxb=="northeast"||bxb=="northwest"||bxb=="southwest"||bxb=="southeast"||bxb=="up"||bxb=="down") {
          list_ab[i]=bxb+"q";
         }
...
refresh(obj) {
  ...
  owner = call_other(obj, "query_name");
  ...
  tell_object(obj, "back again\n");
    ...
    if(may_need_warning && obj->query_level() > 400000)  {
...
short()
{
    int temp;
    string jjh;
if(environment(this_object())) jjh = environment(this_object())->query_name();
if(jjh) return jjh + "'s quicktyper";  
...
alias(str) {
  ...
  return 1;
   ...
   if(ab=="n"||ab=="e"||ab=="w"||ab=="s"||ab=="ne"||ab=="nw"||ab=="sw"||ab=="se"||ab=="u"||ab=="d") {
    write("NOT ALLOWED.\n");
    return 1;
    }
   if(ab=="north"||ab=="east"||ab=="west"||ab=="south"||ab=="northeast"||ab=="northwest"||ab=="southwest"||ab=="southeast"||ab=="up"||ab=="down") {
    write("NOT ALLOWED.\n");
    return 1;
    }
    i=0;
...
/*
     log_file("BAD_PLAYER", ctime(time()) + " " + this_player()->query_real_name() + " with quicktyper recursion. "+ list_ab[i] + " " + list_cmd[i] + "\n");
*/
      return 1;
      }
      i = i +1;
