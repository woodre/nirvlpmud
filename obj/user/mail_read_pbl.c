...
load_player() {
   ...
   if (!restore_object(NAME + this_player()->query_real_name()) ||
      ...
      arr_messages = 0;
      messages = "";
   if(channel_block) this_player()->set_mail_channel_block(1);
      return;
   }
   if (new_mail) {
      new_mail = 0;
...
loop() {
   ...
   write("[1 - " + (sizeof(arr_messages)/2) + " h l d r x q b ?]" + tmp);
   input_to("get_cmd");
}
...
log_file("BAD_PLAYER", ctime(time()) + " " + this_player()->query_real_name() + " with mail_reader, file " + cc + "\n");
   ...
   if (cc && cc != "") {
      ...
      while (cc != "") {
         ...
         if(restore_object("players/"+next_pern))
...
*/
too_many() {
 if(sizeof(arr_messages)/2 > 6 && this_player()->query_level() < 60){
...
chan_block() {
  if(!channel_block) {
      channel_block = 1;
      this_player()->set_mail_channel_block(1);
  ...
  if(channel_block) {
     channel_block = 0;
     this_player()->set_mail_channel_block(0);
...
remove_object() {
   if(channel_block) environment(this_object())->set_mail_channel_block(0);
}
