...
void flag(string str) {
    ...
    if(this_player()) {
       if(this_player()->query_level() < 1001) 
       return;
    }
...
string master_create_wizard(string owner, string domain, object caller) {
    ...
    string dest, castle, wizard, workroom;
    object player;
...
#ifdef PLAYER_DIR
    wizard = "/" + PLAYER_DIR + "/" + owner;
    castle = "/" + PLAYER_DIR + "/" + owner + "/castle.c";
#else
    wizard = "/players/" + owner;
    castle = "/players/" + owner + "/castle.c";
#endif
#ifdef DEFAULT_WORKROOM
#ifdef PLAYER_DIR
    workroom = "/" + PLAYER_DIR + "/" + owner + "/workroom.c";
#else
...
#endif
    def_castle = "#define NAME \"" + capitalize(owner) + "\"\n#define DEST \"" +
#ifdef DEFAULT_CASTLE
...
#ifdef INIT_FILE
    	if (!write_file(INIT_FILE, "#"+extract(castle, 1) + "\n"))
#else
	...
	tell_object(player, "You already had a workroom!\n");
        ...
        if(!cp("/" + DEFAULT_WORKROOM, workroom))
 ...
 * hasn't denied it with a query_prevent_shadow() returning 1, and
 * as long as the shadow doesn't redefine valid_read or valid_write.
 */
/*
...
/***************************************************************************/
/* file_size for player object */

int master_file_size(string file) {
  object caller;
  string tmp;

  caller = (previous_object()) ? previous_object() : this_player();
  /* this bit is used by monster, it is mainly here for 312 */
  if (sscanf(file, "/skills/%s", tmp) == 1) {
    caller = this_object();
  }
  return (valid_player_call(caller)) ? file_size(file) : 0;
}


shut_down_game()
{
   if (previous_object() != find_object("obj/shut"))
      return;
   shutdown();
}

/*
 * returns true if arg can receive a shout.
 */
valid_shout(arg) { return !arg->query_muffled(); }
...
/***************************************************************************/
/* A number of functions in master will accept calls only from specific
 * objects.  They form the basis of the security for the mud */

status valid_master_call(object ob) {
  string file;

  if (!ob) { return 0; }
  if (file_name(ob) == SIMUL_EFUN ||
      file_name(ob) == SPARE_SIMUL_EFUN_FILE ||
      sscanf(file_name(ob), "players/rumplemintz/private/%s", file) == 1 ||
      ob == this_object()) {
    return 1;
  }
  return 0;
...
status valid_player_call(object ob) {
  ...
  if (sscanf(file_name(ob), "obj/player" + "#%d", i) ||
      sscanf(file_name(ob), "obj/debug" + "#%d", i) ||
      sscanf(file_name(ob), "obj/wizard" + "#%d", i) ||
      file_name(ob) == SIMUL_EFUN ||
      file_name(ob) == SPARE_SIMUL_EFUN_FILE ||
      sscanf(file, "secure/%s", file) == 1 ||
      ob == this_object()) {
    return 1;
  }
  return 0;
...
int query_allow_shadow(object ob) {
     ...
     */
      ...
      */
       tmp = (int)call_other("/obj/shadow_master","query_shadows",file);
       if(tmp == -1)
	return 0;
      who = (string)ob->short();
      if(!who)
...
*/
 ...
 {
     return (name == "obj/player.c");
 }

#else  /*  Using LDMud Driver - use code below */
...
#define INIT_FILE                 "/room/init_file_ld"
...

{
    ...
    */
    if (object_name(previous_object()) == get_simul_efun())
      ...
      return this_player()->query_level() >= 60;
}
...

{
  ...
  if (caller == this_object()) {
     ...
     object *pobjs;
     pobjs = caller_stack();
     write_file("/log/valid_read_gnar", ctime()+" path1: "+path+" depth: "+caller_stack_depth()+".\n"); 
     for(x=0; x < sizeof(pobjs); x++)
       write_file("/log/valid_read_gnar", "\t\t["+x+"]"+ object_name(pobjs[x])+".\n"); 
#endif
     return 1;
  }
#ifdef __COMPAT_MODE__
  if (object_name(caller) == "obj/simul_efun") {
#else
  if (object_name(caller) == "/obj/simul_efun") {
#endif
    return 1;
...
#if 0
   write_file("/log/valid_read_gnar", ctime()+" path3: "+path+" fun: "+fun+"\n"); 
#endif
    switch ( fun ) {
        ...
        case "make_path_absolute": /* internal use, see below */
            ...
            if( this_player() && query_ip_number(this_player()) ) {
                ...
                if (!stringp(path)) {
#ifdef __LDMUD__
                    write("make_path_absolute called\n");
#endif
                    write("Bad file name.\n");
                    return 0;
                }
...

{
...
#else
        if (sscanf(path,"/log/%s", user)) return path[1..];
#endif
        break;
    case "ed_start":
        if (path[0] != '/')
    ...
    case "remove_file":
        ...
        if (this_player() && query_ip_number(this_player())) {
          ...
          if (!stringp(path)) {
#ifdef __LDMUD__
            write(fun + " function called in valid_write\n");
#endif
            write("Bad file name: " + path + "\n");
            return 0;
          }
#if 0 /* Rumplemintz */
          tell_object(this_player(),
                      "MASTER: returning ADD_SLASH("+path+")\n");
#endif
          return ADD_SLASH(path);
        }
#endif
