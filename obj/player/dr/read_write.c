...
tell_object(find_player("earwax"), HIR+"VALIDWRITE TEST: "+NORM+(string)this_player()->query_real_name()+" ... "+arg+" ... "+object_name(prev_caller)+"\n");
write("THIS HIT\n");
*/
  ...
  if ((sscanf(str, "/room/%s", file) == 1) && level >= EDIT_STANDARD)
    ...
    if(sscanf(str,"/pfiles/%s",file))
      if(level >= ELDER || (prev_caller &&
                            ...
                            (object_name(prev_caller) == "obj/simul_efun_new" ||
                             object_name(prev_caller) == "obj/simul_efun")))
#else
                            object_name(prev_caller) == "obj/simul_efun"))
#endif
