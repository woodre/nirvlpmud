...
#include <security.h> /* Old security stuff..... */
...
#include "/include/fn_specs/living.spc" /* Prototypes */
...
int hit_point;               /* Legacy variable for compatibility */
...
int strength;                /* How much you can carry */
...
#if 0 /* This are defined in base_obj.c now */
string name;                 /* name */
#endif

/* Variables from original Nirvana living.c */
int time_to_heal;            /* Count down variable. */
int hcnum;
int crime;                   /* have we been bad.  players */
static int crimetime;
int luck2,lucky;
static int already_fight;    /* see if more than one attack per hb,
...
#if 0 /* Changed back to int and fixed code below - Rumplemintz */
string gender;
#endif
...
#endif /* LDMUD query_name changes */
...
void long(status wiz) {
  ...
  if (query_disguise_on() &&
    ...
    write(str + " vaguely looks like " +
          capitalize((string)query_name(1)) + ".\n");
  }
  ...
  str += (hp < max_hp/12)
      ...
      : "is " + (hp == max_hp ? "unscathed and " : "") + "looking string.\n";
  write(str);
}
