...
mixed *set_top_kill( int exp, object corpse ) {
  string mname;
  object rm;
  ...
  if( !top_kill || top_kill[0] < exp ) {
    ...
    if( !is_testchar() && query_level() <= EXPORE && (!top_kill || exp - top_kill[0] > 1000) )
      chan_msg( NORM+"\n(junk) " +HIY+cap_name +NORM+ " just crushed their top kill record!\n", "junk" );
