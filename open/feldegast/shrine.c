...
void reset(int arg) {
  ...
  items=({
    ...
    "word","Each archway has a different word over it:\n"+
           "\tarena, island, desert, tower, dark, angel, dreams, and arch"
  });
  dest_dir=({
    "/room/plane12","out",
  });
...
void create_portal(string destination) {
  if(!present("azure portal",find_object(destination))) {
    object gate;
...
int arch(string str) {
  create_portal("/room/south/sforst1");
  write("You step through the archway and find yourself in the forest\n"+
        "to the southwest of the village.\n");
  this_player()->move_player("through the 'arch' portal#room/south/sforst1");
...
int reflect(string str) {
  ...
  switch(str) {
    ...
    case "tower" : 
      ...
      break;
    case "arch" : 
      write("You see a misty archway leading to the sanctum of the Dark Gentleman.\n");
      break;
