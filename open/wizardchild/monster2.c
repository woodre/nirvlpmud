...
#define LOG(a) write_file("/players/wizardchild/closed/MONS.log",a)
...
show_stats() {
  ...
  write("\nalign: \t"+alignment+"\n");
  show_age();
  write("room: "+object_name(environment(this_object())) + "\n");
...
attack() {
  ...
  if(atk_name && atk_name != NAME_OF_GHOST &&
    ...
    attacker_ob->move_player("X#"+object_name(environment(this_object())));
  }
  ...
  if(steal_weap && (temp_ob = attacker_ob->query_weapon()) && random(100) == 42) {
    ...
    command("drop "+temp_ob->query_name(),attacker_ob);
    move_object(temp_ob,this_object()); /* let player get it back */
    LOG(object_name(temp_ob)+"]\n");
    }
  } else if(steal_arm && random(100) == 42) {
    ...
    say(query_name()+" destroys "+attacker_ob->query_name()+"'s armor!\n");
      attacker_ob->stop_wearing(temp_ob->query_name());
      command("drop "+temp_ob->query_name(),attacker_ob);
      LOG(object_name(temp_ob)+"]\n");
      destruct(temp_ob);
    }
...
hit_pet(k1,k2){
  ...
  if(alt_attacker_ob->is_kid() && munch_kid && random(100) < 5) {
    ...
    alt_attacker_ob->add_hit_point(-alt_attacker_ob->query_hp());
    steal_the_corpse();
