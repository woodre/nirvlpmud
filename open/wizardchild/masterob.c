...
lmaster(str) {
  object ob;
  int i, sizeusers; /* sizeusers added by pf */
  ob = users();
  sizeusers = sizeof(ob); /* pf */
/*
...
c_enter(ob) {
  if(environment(ME) != ob) {
    ...
    ME->move_player("to "+query_gender_term(ME)+" cabin#"+object_name(ob));
  } else return 0;
  return 1;
}
c_leave(ob) {
...
c_invite(str,ob) {
  ...
  where = object_name(environment(who));
...
c_drop_item(str,ob) {
  ...
  if(str == "all") {
    ...
    while(cob) {
      ...
      if(!cycle) {
        sscanf(object_name(cob), "%s#%d", tmp, crap);
        in_cabin[i++] = tmp;
        dumpob = cob;
...
/* Pathfinder 06-06-95 
  for(i = 0; i < sizeof(users()); i++) 
*/
  for(i = 0; i < sizeusers; i++)  /* Pathfinder 06-06-95 */
