...
reset(arg){
  ...
  dest_dir = ({
  "open/wocket/cavetest/mainpath5.c","north",
  "open/wocket/cavetest/mainpath3.c","southeast",
  });
  set_light(1);
}
...
init(){
 ...
 if(!alt_path){
    alt_path = random(2)+1;
    "/open/wocket/cavetest/mainpath2.c"->set_alt_path(alt_path);
...
get_alt_path(i){
  switch(i){
  case 1: dest_dir += ({ "/open/wocket/cavetest/apath4.c", "east" });
          break;
  case 2: dest_dir += ({ "/open/wocket/cavetest/bpath4.c","northeast" });
          break;
  }
}
