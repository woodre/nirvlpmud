...
int cmd_play(string str)
{
  ...
  write("You climb the ladder and start down the slide.\n");
  say(TPN+" climbs the ladder and starts down the slide.\n");

  if(!random(10))
    {
      write("You fall and bump your nose. OUCH!\n");
      say(pro()+" falls off the slide!\n");
      TP->add_hp(-3);
    }

  write("You are now at the bottom of the slide.  Now wasn't that fun?\n");
  say(TPN+" ends up at the bottom of the slide.  Doesn't that look fun?\n");
