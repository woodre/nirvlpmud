...
reset(arg)
{
   ...
   /* On reset, see if there are any players in room.  If none, relock. */
      ...
      */
      ...
      Locked = 1;
      remove_exit("east"); /* Remove exit from short desc */
      return;
   }
   ...
   dest_dir =
   ({
         "/players/star/rooms/hallway1.c",     "west",
    });
   Locked = 1;
}
...
status do_lock(string arg)
{
   ...
   write("You lock the door with your "+ob->short()+".\n");
...
status do_unlock(string arg)
{
   ...
   */
   ...
   write("You unlock the door with your "+ob->short()+".\n");
   say(this_player()->query_name()+" unlocks the door.\n", this_player());
   Locked = 0;
   /* This lets it show up in room short. */
   add_exit("/players/star/rooms/live_room.c", "east");
   return 1;
}
...
status do_east(string arg)
{
   ...
   this_player()->move_player("east through the door#/players/star/rooms/live_room");
   return 1;
}
