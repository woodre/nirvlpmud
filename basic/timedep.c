...
/*
 * set_timedep
 * Set time dependant references
 * Ex:
 *  set_timedep(({ 5*3600,     20*3600}),
 *              ({"daylight",  "moonshine"}),
 *              ({"light"}));
 *  to have "dayling" from 5:00 to 20:00 and moonshine from 20:00 to 5:00.
 *  Times must be sorted in ascending order.
 */
varargs string *set_timedep(int *when, mixed *what, string *space,
                            string which) {
  if (!space)
...
varargs mixed query_timedep(string *space) {
  int now, time, *when;
  ...
  if (which = get_namespace(space + ({ "which" })))
    now = call_other(SE_TIME, which);
  else
    now = SE_TIME->squery_day_time();
...
varargs string *add_timedep(int when, mixed what, string *space) {
  ...
  if (!old_when)
    return set_timedep(({when}), ({what}), space);
  else {
    for (i=0; i<sizeof(old_when) && old_when[i] < when; i++);
    new_when = old_when[0..i-1] + ({when}) + old_when[i..sizeof(old_when)];
    new_what = old_what[0..i-1] + ({what}) + old_what[i..sizeof(old_what)];
    ...
    if (i+1 < sizeof(new_when) && (new_when[i+1] == when ||
                                   new_what[i+1] == what)) {
...
varargs mixed *query_timedep_table(string *space) {
  int *when;
  mixed *what;
...
void set_timedep_space(string *space) {
  timedep_space = space;
}
